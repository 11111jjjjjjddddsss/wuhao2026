...
写作要求：
- 标题短而具体；摘要目标 90-130 个中文字符左右，写 2-3 句完整短讯，信息量要够，一般别低于 80 个中文字符。summary 字段可以使用 JSON 转义换行 \n，把摘要拆成 2-3 个短段，别写成一整块长段。摘要要像正常新闻短讯，不写空话套话；能具体到地区、作物、措施、价格、面积、补贴金额或进度时就写清楚，来源不确定的数字宁可省略；不要写成一句话压缩稿、薄通知、套话、推荐理由或“根据搜索结果”等元表达。
...
输出前自检：是否正好 3 条；是否新、真、具体；是否都是种植侧；是否避开养殖水产、广告软文、传言、旧闻和编造数字；摘要是否像正常新闻短讯而不是一句薄通知；summary 是否不是一整块长段；三条是否尽量不是同一件事换标题。`, day, recentHistory, attemptGuidance),
		},
	}
}
...
func formatDailyAgriAttemptGuidance(attempt int) string {
	...
	return "\n本次是补救生成：请换地区、作物和生产主题继续找近 7 天真实种植侧公开材料，仍必须 3 条；优先找有生产价值或技术含量的种植新闻，不要继续围着水稻、蔬菜、夏收夏种或天气提醒打转，除非有明显新进展；不用天气预报、养殖水产、旧闻、软文或重复事件凑数。"
}
