...
const (
	...
	dailyAgriPromptVersion      = "2026-06-11-v52"
...
核心目标：
- 给普通种植户、农资门店和基层农技人员看；内容要像正式资讯卡片，短、真、新、具体、能看懂，不写官样话、行业口号或模型自我说明。
- 只输出正好 3 条；每条只写标题、摘要、来源名和可选日期，不输出 URL，不输出解释。
...
- 只取种植侧，按材料主体判断，不按单个词机械判断。本卡服务粮油、蔬菜、果品、棉花、茶桑、种子种业、植保、农药肥料、农机农时、农田水利、农业气象灾害、种植侧价格流通、补贴保险、技术推广等种植生产和种植侧流通场景。下面这些通常属于养殖或水产主体：畜牧、水产、动物疫病、生猪、猪肉、猪价、家禽、禽蛋、牛羊、奶业、饲料、饲用原料、兽药、渔业、鱼虾等；这些例子只是帮助判断边界，不是让你做关键词过滤。如果材料只是顺带提到这些词，但主体仍是作物、种植、农资或农时，可以只摘种植侧事实；如果材料主体是养殖、水产、饲料或饲用原料，就换成种植侧材料。不要用“间接影响种植户”作为保留理由。
- 小类不要卡死。天气、农时、价格、政策、补贴、平台建设、技术推广都可以选；这些只是选题方向，不是硬配额。关键是材料真实具体，并且能说明对种植生产或种植侧流通有什么影响；不要为了凑不同类别去选旧闻、弱材料或泛泛动态。
...
- 摘要 80-110 个中文字符左右，手机卡片约 3-4 行体量；只写事实和直接农业影响，交代“哪里、什么作物/品类、发生了什么、影响什么”。尽量用普通话表达，必要术语可以保留但不要堆术语；不要写推荐理由、读者价值判断、泛泛建议或“根据搜索结果/检索显示”等元表达。
...
func formatDailyAgriAttemptGuidance(attempt int) string {
	...
	return "\n本次是失败后的补救检索：请换一组检索词全网宽搜，优先找近 7 天、种植侧、具体事实清楚的材料。不要限定固定网站；不要被泛农业、养殖水产主体、饲料行情、广告软文、空泛动态、旧闻或重复事件带偏，也不要机械按单个词判断，核心看材料主体是否服务种植生产或种植侧流通。小类不要卡死，天气、农时、价格、政策、补贴、技术推广都可以用；关键是材料真实、新、和种植生产或种植侧流通有关。用户端不点击链接，优先保证三条内容像新闻、事实清楚、来源清楚、短而好读。"
}
