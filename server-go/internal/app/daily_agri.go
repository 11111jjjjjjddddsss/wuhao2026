...
const (
	...
	dailyAgriPromptVersion      = "2026-06-12-v62"
...
- 优先输出 3 条；每条只写标题、摘要、来源名和可选日期，不输出 URL，不输出解释。质量优先，不为了凑够 3 条去选养殖、水产、旧闻、广告软文、弱材料或泛泛动态；确实只有 2 条可靠种植侧材料时，可以只输出 2 条。
...
- 每条摘要至少包含三个具体事实要素，例如地区、作物/品类、时间/进度、农时窗口、风险、价格、面积、补贴、农资供应、流通影响或技术措施。摘要大致不到 85 个中文字符时，先补足来源里能支撑的事实再输出 JSON；不要用空泛建议凑字。
...
输出前自检：是否优先给足 3 条且没有为了凑数牺牲质量；每条摘要是否至少两句且不低于 85 个中文字符，不足就先重写该条；如果某条还是 70 多字的薄摘要，就继续补足具体事实和直接影响后再输出；标题是否仍用“调研推动”“场景上新”“即将投用”“成果展示”这类过程词掩盖空泛材料，是就改成具体种植事实或换条；是否尽量新、真、具体；每条标题和摘要的主体是否都是种植侧；若出现猪肉、生猪、猪价、家禽、禽蛋、鸡肉、鸡蛋、牛羊、奶业、奶价、水产、饲料、饲用原料、兽药或鱼虾作为主体，必须重写该条；是否有养殖水产、动物类价格、广告软文、传言、旧闻或编造数字；三条是否尽量不是同一件事换标题。`, day, recentHistory, attemptGuidance),
		},
	}
}
...
func formatDailyAgriAttemptGuidance(attempt int) string {
	...
	return "\n本次是失败后的补救检索：请换一组检索词全网宽搜，优先找近 7 天、种植侧、具体事实清楚的材料。不要限定固定网站；不要被泛农业、养殖水产主体、饲料行情、广告软文、空泛动态、旧闻或重复事件带偏，也不要机械按单个词判断，核心看材料主体是否服务种植生产或种植侧流通。大类边界要清楚，小类不要抠太细，天气、农时、价格、政策、补贴、技术推广都可以用；关键是材料真实、新、和种植生产或种植侧流通有关。用户端不点击链接，优先保证三条内容像新闻、事实清楚、来源清楚、短而好读。"
}
