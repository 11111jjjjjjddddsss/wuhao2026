...
const (
	...
	dailyAgriPromptVersion      = "2026-07-01-v80"
...
- 面向普通大众用户，写成手机资讯卡片：先说清楚发生了什么，再说明它对农事安排、防灾减损、田间管理、农资选择、产地销售或政策申报有什么参考。语言自然、具体、正式，不写成文件通报，也不要过度口语化。
- 把它当成给农户、合作社和农资店老板挑三件今天值得知道的事。每条最好有清楚的选题钩子：病虫或灾害风险窗口、作物管理关键期、价格 / 供应 / 库存变化、补贴 / 申报 / 登记等政策节点、新品种 / 新技术 / 农机农资、可复制的地方田管案例。三条尽量不是同一种钩子。
- 会议、调研、发布会、意见、通知、推进工作、总体适宜、稳中向好、加强指导这类行政材料本身不优先；只有能翻译成“哪个地区、哪类作物、哪类人现在该注意什么或能用什么”时才写，否则换题。不要用“总体适宜”“创新高”“加强推进”这类泛标题遮盖空内容。
- 每条至少落到地区、作物、风险 / 窗口 / 措施 / 价格 / 补贴 / 进度 / 技术点中的几个具体要素；搜不到具体要素时换新闻，不要把官方句子换个说法。
- 优先近 7 天公开来源，今天或昨天的新进展更优；三条尽量分散地区、作物和主题。最近 7 天已推送过的同地区、同作物、同一事件尽量避开；如果确实继续写同类主题，必须有明显新进展，不能换标题重复写。
- 选题以种植方面的新闻为主，不限制具体作物；大田作物、经济作物、设施农业、果树、蔬菜、茶叶、特色作物等都可以。不要总集中在水稻、蔬菜、夏收夏种和天气提醒。
- 更优先选择对生产或经营有实际参考价值的内容，例如栽培管理、植保病虫、种子种苗、农资农机、技术推广、土壤墒情、节水灌溉、产地流通 / 价格、仓储保鲜、政策补贴、地方农技推广等真实进展。
...
写作要求：
- 标题短而具体，像新闻标题，不像公文小标题；摘要目标 90-130 个中文字符左右，写 2-3 句完整短讯，信息量要够，一般别低于 80 个中文字符。summary 字段可以使用 JSON 转义换行 \n，把摘要拆成 2-3 个短段，别写成一整块长段。摘要要像正常新闻短讯，不写空话套话；第一句讲清真实新进展，第二句给出用户能用的判断、提醒或参考，避免“指导各地做好”“加强推进”“持续关注”这类空泛句；能具体到地区、作物、措施、价格、面积、补贴金额或进度时就写清楚，来源不确定的数字宁可省略；不要写成一句话压缩稿、薄通知、套话、推荐理由或“根据搜索结果”等元表达。
...
输出前自检：是否正好 3 条；是否新、真、具体；是否都有用户愿意点开的选题钩子和实际参考；是否都是种植侧；是否避开养殖水产、广告软文、传言、旧闻和编造数字；摘要是否像正常新闻短讯而不是一句薄通知或行政流水账；summary 是否不是一整块长段；三条是否尽量不是同一件事换标题。`, day, recentHistory, attemptGuidance),
		},
	}
}
...
func formatDailyAgriAttemptGuidance(attempt int) string {
	...
	return "\n本次是补救生成：请换地区、作物、信息源和选题钩子继续找近 7 天真实种植侧公开材料，仍必须 3 条；优先找有生产价值、经营参考或技术含量的种植新闻，不要继续围着水稻、蔬菜、夏收夏种、天气提醒或行政通知打转，除非有明显新进展和可用参考；不用天气预报、养殖水产、旧闻、软文、空泛政务稿或重复事件凑数。"
}
