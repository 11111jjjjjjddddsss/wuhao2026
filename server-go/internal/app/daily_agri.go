...
const (
	...
	dailyAgriPromptVersion      = "2026-06-12-v63"
...
核心目标：
- 给普通种植户、农资门店和基层农技人员看；内容要像正式资讯卡片，短、真、新、具体、能看懂，让普通人知道这条新闻和种植生产、农资、农时、防灾或流通有什么关系。不写官样话、行业口号或模型自我说明。
...
- 只取种植侧，按材料主体判断，不按单个词机械判断。本卡服务粮油、蔬菜、果品、棉花、茶桑、种子种业、植保、农药肥料、农机农时、农田水利、农业气象灾害、种植侧价格流通、补贴保险、技术推广等种植生产和种植侧流通场景。下面这些通常属于养殖或水产主体：畜牧、水产、动物疫病、生猪、猪肉、猪价、家禽、禽蛋、鸡肉、鸡蛋、牛羊、奶业、奶价、饲料、饲用原料、兽药、渔业、鱼虾等；这些例子只是帮助判断边界，不是让你做关键词过滤。如果材料只是顺带提到这些词，但主体仍是作物、种植、农资或农时，可以只摘种植侧事实；如果材料主体是养殖、水产、饲料、饲用原料或动物类价格，就换成种植侧材料。不要用“间接影响种植户”作为保留理由。
- 大类边界要清楚，小类不要抠太细。天气、农时、价格、政策、补贴、平台建设、技术推广都可以选；这些只是选题方向，不是硬配额。关键是材料真实具体，并且能说明对种植生产或种植侧流通有什么影响；不要为了凑不同类别去选旧闻、弱材料或泛泛动态。
...
- 综合指数、市场综述、批发价格指数、菜篮子指数只能作为背景，不能单独成条；价格类必须聚焦明确的种植侧单品、品类、产区、批发市场或农资变化，标题和摘要都要写出具体对象，不要用“农产品价格”“批发价微降”“市场价格变化”这类泛泛标题独立成条。综合农业材料只摘种植侧事实，不夹带养殖水产主体内容；如果材料主体是养殖、水产、饲料、饲用原料或动物类价格，就整条换掉，不要只把摘要改成“间接影响种植”。最终成稿标题或摘要中，如果主体是猪肉、生猪、猪价、家禽、禽蛋、鸡肉、鸡蛋、牛羊、奶业、奶价、水产、饲料、饲用原料、兽药或鱼虾，就说明选题错了，必须换成种植侧材料。
...
输出格式：
items 必须正好 3 个对象；如果一次检索不够，就继续扩大到政策、气象、农时、农资、农机、种业、植保、地方农业部门、市场流通等种植侧方向，不要输出 2 条或空对象。
{
  "card_name": "今日农情",
  "items": [
    {
...
- 摘要必须像一条可读新闻短讯，目标 90-130 个中文字符左右，手机卡片约 3-4 行体量；至少两句，第一句交代“哪里、什么作物/品类、发生了什么”，第二句补“影响什么、农时窗口、风险点、供应变化、农资/流通影响或基层可留意的事项”。宁可写到 100 字左右，也不要收成 70 多字的薄摘要；第二句不要只写“需关注”“可留意”这类很短的收尾。只写来源里能支撑的事实和直接农业影响，不要用套话凑字。尽量用普通话表达，必要术语可以保留但不要堆术语；不要写推荐理由、读者价值判断、泛泛建议或“根据搜索结果/检索显示”等元表达。
...
输出前自检：是否给足 3 条且没有为了凑数牺牲质量；每条摘要是否至少两句、是否明显偏薄，偏薄就补足具体事实和直接影响后再输出；标题是否仍用“调研推动”“场景上新”“即将投用”“成果展示”这类过程词掩盖空泛材料，是就改成具体种植事实或换条；是否尽量新、真、具体；每条标题和摘要的主体是否都是种植侧；若出现猪肉、生猪、猪价、家禽、禽蛋、鸡肉、鸡蛋、牛羊、奶业、奶价、水产、饲料、饲用原料、兽药或鱼虾作为主体，必须重写该条；是否有养殖水产、动物类价格、广告软文、传言、旧闻或编造数字；三条是否尽量不是同一件事换标题。`, day, recentHistory, attemptGuidance),
		},
	}
}
...
func formatDailyAgriAttemptGuidance(attempt int) string {
	...
	return "\n本次是失败后的补救检索：请换一组检索词全网宽搜，必须补齐 3 条可读新闻短讯。优先找近 7 天、种植侧、具体事实清楚的材料；不要限定固定网站；不要被泛农业、养殖水产主体、饲料行情、广告软文、空泛动态、旧闻或重复事件带偏，也不要机械按单个词判断，核心看材料主体是否服务种植生产或种植侧流通。大类边界要清楚，小类不要抠太细，天气、农时、价格、政策、补贴、技术推广都可以用；关键是材料真实、新、和种植生产或种植侧流通有关。用户端不点击链接，三条都要像新闻、事实清楚、来源清楚、短而好读。"
}
