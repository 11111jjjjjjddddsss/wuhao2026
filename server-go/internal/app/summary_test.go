...
func TestSummaryExtractionPromptKeepsMemorySafeAndUseful(t *testing.T) {
	...
	for _, want := range []string{
		...
		"边界用于减少混写，不用于把可用信息卡掉",
		"短、准、能接住",
		"不要写成流水账、审计报告、病例定论库、客服工单流水、开发任务清单或知识库条目",
		"非农业事务如果会影响下一轮对话，也可以简要保留在短期承接",
		"账号登录、验证码、会员权益、支付或订单、资料填写、页面设置、功能使用、当前试用或用户刚提出的回答方式要求",
		"这类内容用普通用户口径概括，不写成内部配置或排障清单",
		"不要因为内容不是农事问题就整段删除",
		"也不要把一次性使用问题、当前试用、临时页面或账号操作、费用咨询、一次回答格式要求写成长期背景或用户画像",
		"除非输入明确说明它是长期事实或稳定偏好",
