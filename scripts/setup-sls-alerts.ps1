...
$alerts = @(
    ...
    @{
        ...
        Description = "server-go Logstore detected model key or DYPNS critical config errors in 5 minutes.";
        Logstore = "server-go";
        Query = "missing_key OR MODEL_BACKEND_NOT_CONFIGURED OR fusion_auth_not_configured OR sms_auth_not_configured OR sms_send_not_configured OR sms_provider_config_invalid OR sms_cache_not_configured | select count(1) as cnt";
        Severity = 8;
        RepeatInterval = "60m";
