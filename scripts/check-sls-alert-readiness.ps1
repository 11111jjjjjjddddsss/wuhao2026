...
$expectedAlerts = @(
    ...
    @{ Name = "nongji-quota-outbox-needs-ops"; Logstore = "server-go"; Severity = 6; Query = "quota consume outbox needs ops automatic retry scheduled OR quota consume outbox auto marked uncollectable OR quota consume outbox mark needs ops failed OR quota consume outbox mark uncollectable failed | select count(1) as cnt"; Condition = "cnt > 0"; RepeatInterval = "6h" },
