...
expectAdminPattern("launch readiness cards show item owner type", /class="launch-kind \$\{row\.launch_only \? "info" : row\.manual \? "manual" : "program"\}"/);
expectAdminPattern("launch readiness cards distinguish launch-only and manual items", /row\.launch_only \? "上线准备" : row\.manual \? "人工确认" : "程序处理"/);
...
expectServerPattern("phone login remains manual after dependencies are healthy", /Manual:\s+loginDepsOK/);
