...
- 摘要模型：默认 `qwen3.5-flash`，用于 B 层通用短期记忆和 C 层长期通用记忆；摘要请求走 OpenAI 兼容模式 HTTP 原始接口，`enable_thinking=false` 必须放在请求顶层。后端支持 `B_SUMMARY_MODEL`、`C_SUMMARY_MODEL` 和兜底 `SUMMARY_EXTRACTION_MODEL` 分层灰度；默认不变，若真实摘要质量仍不稳，优先考虑只把低频 C 层临时切到 `qwen-plus`，B 层继续 flash 控成本。`qwen-plus` 资源包只适合非思考实时推理且单次输入不超过 128K 的请求；摘要通常输入大、输出短，资源包是否省钱取决于输出占比，不能因为有资源包就全量切 plus。2026-06-10 远端实测发现放在 `extra_body` 内不会真正关闭思考，会产生大量 `reasoning_tokens` 和明显更高延迟；顶层关闭后同类摘要请求回到正常 token 量级
...
- C 层长期通用记忆：默认≤650字，复杂场景最多≤850字；每 20 轮更新一次，提取输入为旧 C 层长期通用记忆 + `session_round_archive` 最近 20 轮完整问答，不使用 A 层 6/9 轮窗口替代 20 轮归档；如果归档不足 20 轮，则保持 `pending_retry_c`，后续轮次完成后继续补提取。C 层当前仍写入单个 `c_summary` 文本字段，但提示词要求分成“长期通用记忆 / 用户画像 / 农业相关重点事件记忆”三块；它不是通用知识库，也不是病例流水账。C 层不得把“倾向于 / 更像 / 可能 / 不能排除 / 未见明显...”升级成“确诊 / 确认为 / 已排除 / 已证实”等长期结论，除非历史对话里明确有用户复查、检测或人工确认
...
- B 层通用短期记忆 / C 层长期通用记忆默认由后端 `qwen3.5-flash` 异步处理，摘要请求在 OpenAI 兼容模式请求顶层显式设置 `enable_thinking=false`，单次提取请求有 60 秒超时保护；可通过 `B_SUMMARY_MODEL` / `C_SUMMARY_MODEL` 分层灰度模型，当前推荐只在 C 层质量确有必要时试 `qwen-plus`，不把 B/C 全量切 plus，也不把中转站作为生产默认。B 层使用当前 A 层 6/9 轮窗口更新，C 层使用 `session_round_archive` 最近 20 轮完整问答更新且仍为每 20 轮一次；同一用户同一层有本进程运行中保护；摘要写回必须匹配触发时的 `round_total`，旧快照结果不能覆盖更新轮次。提取失败或超时会保持对应 `pending_retry_b / pending_retry_c`，后续轮次完成后继续补提取
...
- 生成链路默认使用 `qwen-plus` 的 DashScope `text-generation/generation` 非流式接口，显式设置 `temperature=0.8`、`enable_search=true`、`search_options.search_strategy=turbo`、`search_options.enable_source=true`、`freshness=7` 和 `prompt_intervene`；这是当前生产默认链。`DAILY_AGRI_MODEL=qwen3.5-flash` 仅作为手动实验 / 降本候选，会走 DashScope `multimodal-generation/generation` 流式接口并开启 `enable_thinking=true`，HTTP 头必须带 `X-DashScope-SSE: enable`；它不能直接套旧 text-generation 联网链，否则会复现 `400 InvalidParameter / url error`，且质量 / 来源稳定性不作为当前生产默认。用户已明确 `qwen-flash` 质量不行、`qwen-turbo` 即将下线，二者都不作为今日农情生产候选
- 今日农情默认不需要结构化 URL 来源列表下发给用户，`enable_source=true` 主要用于事实核对和辅助模型在正文 JSON 里写短来源名称；公开接口只返回标题、摘要和短来源名称，不下发 URL、source_index 或 published_date，Android 单条农情不可点击跳浏览器。默认 qwen-plus 链保留 `prompt_intervene + freshness=7`，不使用 `assigned_site_list`，继续全网宽搜；手动 3.5flash 实验链则主要靠主提示词约束近 7 天、种植侧、宽检索、来源名和 JSON 格式。`agent / agent_max` 属于多轮检索整合且通常带来更多输入 token 和延迟，今日农情默认不使用
