...
- 2026-04-19 发送起步锁值的当前唯一口径：普通发送且会收口 composer 时，`conversationBottomPaddingLockPx` 不再吃当前多行输入框量出来的 `stableComposerBottomBarHeightPx`，而是改吃现有 `STARTUP_BOTTOM_BAR_HEIGHT_ESTIMATE` 这份固定 collapsed 总高度基准，专门避免长文本发送把工作线抬高。只有 `collapseComposer = false` 这类不收口发送才继续退回当前快照
