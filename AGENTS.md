...
- streaming 行级 reveal 当前又收成了一层更窄的“隐藏行高度预留门闩”：当 `buildStableStreamingLineBuffer(...)` 首次测出“上一行升格为 stable、下一行开始成为 activeLine”时，渲染层不会立刻把这条新行文字见光，而是继续保留上一拍已经显示出来的旧可见行，同时先把隐藏新行对应的真实高度预留出来；只有后续再次观察到 activeLine 真实继续吐字时，才放行新的整组 `stableLines + activeLine`。这样 follow 仍能及时感知到高度变化，但工作线下面不会先露出真实新行文字；同时也不能把它回退成旧 fresh-line preview 那种“锁空串 / 锁预览串”的实现
...
- 当前修法：先在 `ChatStreamingRenderer.kt` 显示层加“隐藏行高度预留门闩”：换行首拍继续保留上一拍的旧可见行，但会先预留隐藏新行对应的真实高度，等后续再次观察到 activeLine 真实继续吐字时，再放行新的整组 `stableLines + activeLine`；再把 streaming follow 的 gate 从 `isNearStreamingWorkline()` 里拆出来，改用更严格的 `isAtStreamingWorklineStrict()`，把上容差从一整行收回 `BOTTOM_POSITION_TOLERANCE`
