...
- 2026-04-19 发送起步 offset 的当前唯一口径：一旦 `commitSendMessage()` 已经选定这次要锁给 `LazyColumn` 的 `conversationBottomPaddingLockPx`，同一发送事务里的 `requestScrollToItem(index, offset)` 必须直接从这份锁值反推工作线并生成最终 offset；不要再让列表实际吃进去的是“锁定 padding”，而发送起步 offset 还继续吃另一条预先派生的工作线快照。现有 `pendingStartAnchorScrollOffsetPx` 只保留为锁值拿不到时的兜底，不得再反过来主导正常发送
