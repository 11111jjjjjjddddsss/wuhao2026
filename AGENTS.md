...
- Android 已移除 `SESSION_API_TOKEN` 静态共享 token 运行时绕过，不再用测试 ID 作为登录桥接；正式登录必须使用后端按真实手机号账号动态签发的 per-user session token。本机旧 `user_id` 到账号身份的数据迁移已经有首版后端事务骨架；Redis-backed 手机号认证限流已部署到 ECS 并健康，当前仍缺阿里云融合认证 Android SDK、`DYPNS_FUSION_SCHEME_CODE`、短信签名 / 模板和真机登录联调
