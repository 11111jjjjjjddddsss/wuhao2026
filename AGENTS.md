...
- 后端只发布可解析 JSON、正好 3 条有效 item、标题摘要完整且同批标题不重复的结果；主题方向、广告软文、假新闻、前端元表达、推荐理由和标题党类内容主要靠提示词控制，后端不再用过严硬过滤把来源卡死。公开响应只包含 `title / summary`，不包含 URL、来源或条目日期
- Android 只把今日农情作为 `ChatTimelineItem.TodayAgriCard` 追加在真实消息后方，作为靠近输入框的聊天列表尾部 UI-only 卡片；真实 `messages` 仍只包含用户 / assistant 对话。同一用户当天只要还没有发送过文字 / 图片 / 失败态消息，今日农情可以展示；当天一旦发送，Android 会按今日农情 `date_cn` 的 `yyyyMMdd` 日期键记录并让卡片当天隐藏。发送入口会先让卡片用约 180ms 淡出 / 垂直收起过渡在原位置退出，再插入用户消息和 assistant 占位，避免卡片在新消息后方抢尾部；删除历史也不恢复当天卡片，第二天有新日期卡片再重新出现。聊天页按上海日期定期检查跨天，跨天后先清空旧日期卡片，只接受 `date_cn` 等于当前日期的 ready 卡片，避免长时间前台或后台恢复时继续显示昨天内容。卡片不参与本地聊天快照、发送、重试、复制或后端上下文，不能作为最新真实消息锚点，空闲态只允许作为视觉尾部贴近输入框；卡片右上角不放关闭叉号，不提供手动关闭 / 手动隐藏入口，单条农情不可点击跳外部链接。设置页新增“今日农情”入口，展示最近 30 天已 ready 的标题 + 摘要记录
...
- `ChatRecyclerViewHost.kt` 当前使用：
  ...
  - `messages` 仍按 oldest -> newest 存储；视觉底部最新真实消息通过 `ChatTimelineItem.Message` 反查 index，不能把今日农情卡片当成最新真实消息锚点；今日农情只在空闲态作为列表尾部 UI-only 资讯卡贴近输入框
