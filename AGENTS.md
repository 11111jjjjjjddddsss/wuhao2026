...
- 如果本轮结束时正文还没吃完冻结底线，应把剩余空白转成静态保留量 `retainedBottomGapPx`；这段空白继续保留到下一轮发送前，不允许本轮刚结束就往下掉
- 下一轮发送开始前，必须先清上一轮 `retainedBottomGapPx`，再插入新消息、再上新锚点、再记录新 `frozenBottomPx`；顺序不能反，否则会出现旧空白和新空白交替顶一下
...
- 当前滚动链口径更新为“发送锚点冻结底线 + 普通工作线 + 独立按钮脉冲”：
  ...
  - 本轮短文本/失败态如果还有剩余空白，转成 `retainedBottomGapPx` 静态保留到下一轮发送前再清
