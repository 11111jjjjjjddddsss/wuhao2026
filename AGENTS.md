...
部署与基础设施：
- 部署：首版线上后端路线已从“SAE 镜像托管优先”转向“ECS 传统部署优先”。当前已购买阿里云 ECS `i-2ze5nrem0jrchln4f0eh`，位于 `华北2（北京）/ cn-beijing` 可用区 L，规格 `ecs.u1-c1m2.large`（2 vCPU / 4 GiB），Ubuntu 22.04 64 位，包年包月到期 `2027-05-24`，公网 IP `39.106.1.151`，私网 IP `192.168.1.237`，VPC `vpc-2zeax2zowza2398b9dzot`，交换机 `vsw-2zemsq82lj2kp8za90aky`，安全组 `sg-2ze4tilwxw1h5w77lwl1`，固定公网带宽 5 Mbps。ECS 已通过 Cloud Assistant 初始化 `nongji` 系统用户、`/opt/nongjiqiancha/server`、`/etc/nongjiqiancha/server.env`、`nongji-server.service`、Nginx 反向代理、基础限流、fail2ban 和 logrotate；`server-go` 已部署并运行在 `127.0.0.1:3000`，Nginx 按 `api.nongjiqiancha.cn` 代理，Go 进程默认监听 `127.0.0.1:${PORT:-3000}`，只有显式配置 `LISTEN_ADDR` 或 `LISTEN_HOST` 才会改为其他监听地址，避免绕过 Nginx 直接暴露业务端口；当前健康检查返回 `ok=true / auth_strict=true / bailian=missing_key / dev_order_endpoints=false`。阿里云 DNS 已创建 A 记录 `api.nongjiqiancha.cn -> 39.106.1.151`，在 ECS 内解析和 HTTP healthz 已生效；因尚未配置 DashScope 模型 Key，聊天接口会返回 `MODEL_BACKEND_NOT_CONFIGURED`，不会真实开流或烧模型；公网正式可用仍等 ICP / App 备案、HTTPS 证书和模型 Key。此前曾在 `华北2（北京）/ cn-beijing` 创建标准版 SAE 应用 `nongjiqiancha`，AppId `366147d5-3760-4548-bd68-f38debbc5f23`，规格 `0.5 核 / 1GB / 单实例`，自动弹性未开启；该 SAE 应用只是默认 demo 镜像，未部署 `server-go` 真实后端，已于 2026-05-24 21:50 左右先停止，并于 2026-05-24 21:51 左右通过阿里云 CLI 删除，删除变更单 `14a360d3-e2b4-4b93-9701-b76dfcc7bfd9` 已提交成功；删除后 `ListApplications` 返回空列表，`TotalSize=0`。运维侧本机已安装阿里云 CLI，默认 Region 为 `cn-beijing`；真实 AccessKey 只允许保存在本机 CLI 配置或云端密钥管理中，不允许写入仓库、聊天记忆或文档
- 域名：`nongjiqiancha.cn` 已在阿里云购买，用户口头确认域名实名认证 / 模板审核已通过；后续用于 `api.nongjiqiancha.cn`、下载域名和管理后台域名。当前已创建 `api` A 记录指向 ECS 公网 IP `39.106.1.151`，仍需完成 ICP / App 备案、HTTPS 证书和正式后端入口绑定（优先按 ECS + 反向代理规划，若后续回到 SAE 再同步改本文和 runbook）后才能对外正式服务
- 数据库：首版使用阿里云 RDS MySQL，当前实例 `rm-2zes3vmj76p85n8g1` 已创建并运行，MySQL 8.0、基础版、1 核 2GB、50GB、内网地址 `rm-2zes3vmj76p85n8g1.mysql.rds.aliyuncs.com:3306`、VPC `vpc-2zeax2zowza2398b9dzot`、交换机 `vsw-2zemsq82lj2kp8za90aky`、到期时间 2027-05-24；当前自动备份保留 7 天，默认每周二 / 四 / 六 17:00-18:00 北京时间左右执行；已创建数据库 `nongjiqiancha`、应用账号 `nongji_app` 并授予该库读写权限，RDS 白名单当前为 `127.0.0.1,192.168.1.237`，后端环境变量已写入 ECS `/etc/nongjiqiancha/server.env`。数据库密码、`APP_SECRET`、内部 job / support secret 只保存在本机 `%USERPROFILE%\.nongjiqiancha\prod-secrets.json` 和 ECS 环境文件中，不进仓库。PolarDB 暂作为后续高并发 / 更高规格升级选项，不再作为个人创业首版默认采购项
...
- OSS：已购买阿里云 OSS 标准-本地冗余存储包（华北2 / 北京）100GB，资源包实例 `OSSBAG-cn-mqq4sqfvr001`，有效期 `2026-05-24T15:00:00Z` 至 `2027-05-24T15:00:00Z`，用于抵扣标准存储容量费用；当前 OSS Bucket 数量仍为 0，CLI 创建私有北京 Bucket 时返回 `UserDisable`，需要到 OSS 控制台确认服务开通 / 账号状态后再重试；`server-go` 图片上传链仍是 ECS 本机 `/var/lib/nongjiqiancha/uploads`，尚未接 OSS，因此首版保持单台 ECS，不能加第二台或自动扩容。后续 Bucket 应建在 `cn-beijing`，标准存储、本地冗余、私有读写，配生命周期短周期清理旧问诊图片；下行流量包、CDN、SLS 资源包、Redis 暂未购买
- SLS：阿里云日志服务本体已开通，`GetSlsService` 返回 `Opened`；当前未购买 SLS 节省计划 / 资源包，未创建农技千查专用 Project / Logstore，也未接入 `server-go` 日志采集。北京区已有阿里云系统托管 Project / Logstore（例如 CMS / XTrace / 产品数据），不等于农技千查业务日志已接入。App 侧当前只新增最小自动日志接收骨架：Android 在关键失败点自动调用后端 `POST /api/app/logs`，后端写入 `client_app_logs` 表并打结构化服务日志，后续可接管理后台或 SLS；不做用户手动上传日志，不上传聊天正文、图片内容、token、手机号或模型 Key
...
- C 层用户长期记忆：默认≤260字，复杂场景最多≤320字；每 20 轮更新一次，提取输入为旧 C 层用户长期记忆 + `session_round_archive` 最近 20 轮完整问答，不使用 A 层 6/9 轮窗口替代 20 轮归档；如果归档不足 20 轮，则保持 `pending_retry_c`，后续轮次完成后继续补提取。C 层定位是农业长期档案 + 通用用户画像 / 偏好，不是通用知识库或病例流水账
...
- clean-state / 删除所有历史后的稀疏首屏是当前 96dp 工作线规则的唯一例外：只要当前会话还没有成功落地过非失败 assistant 回复，`ChatScreen.kt` 就允许临时启用 `cleanStateSparseLayoutActive`；单纯的本地失败用户消息、图片上传 pending、图片上传失败、assistant 失败态或重试态，不算“已有普通历史”。`ChatRecyclerViewHost.kt` 仍全程保持 `Arrangement.Bottom`，不再切到 `Arrangement.Top`。稀疏态不再依赖 `contentPadding.bottom` 定位，而是在 `ChatTimelineItem` 末尾插入 UI-only `SparseBottomSpacer`，让 spacer 作为真实 LazyColumn item 参与布局，把早期文字 / 图片从顶部栏下方约 `32dp` 处自然往下长；随着真实内容高度增加，spacer 高度缩小并在恢复正常 96dp 工作线前移除，避免 `Arrangement.Top -> Arrangement.Bottom` 切换造成 layout jump，也避免 padding 改动被 LazyListState 已有锚点抵消。稀疏态 streaming 中会按当前 assistant 文本、实际 item 宽度和 renderer 同款段落 / 标题 / 列表 / 编号 / 引用样式预估高度，再与非 spacer item 的真实 bounds 单调高度取最大值，避免新行先进树、spacer 下一帧才补偿造成吐字跳闪；streaming 结束后回到真实 bounds 优先。稀疏首发不启用普通发送起步的 bottom-lock / `sendStartAnchorActive`，正常已有成功 assistant 历史后发送仍保留发送起步锁。只有最新真实消息的实测底边已经到达或超过正常 96dp 工作线且 spacer 已缩回容差内时，才退出稀疏模式；不能用 `canScrollForward` 作为稀疏退出条件，因为 `SparseBottomSpacer` 自己是真实列表 item，可能制造“可滚动”的假信号。退出时若用户没有拖动 / 浏览且列表滚动已停，会同拍请求正向底部锚点，如果用户正在拖动、浏览或滚动仍未停止，不主动请求正向底部锚点。稀疏态或图片上传等待态的真实拖动也会标记 `UserBrowsing`，上传完成进入 streaming 时会保留该浏览意图。退出判断必须使用真实布局坐标，文字、图片、图片与文字间距、失败态 footer 等都算，不允许按第几轮、字数或有没有图片做估算；有图片的用户消息统一上报整条消息 Column bounds，不依赖当前图片和文字的视觉顺序；`canScrollBackward` 不作为稀疏退出条件
- 发送起步当前保留的旧保护只有两样：
