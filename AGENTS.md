...
- 2026-06-01 已将后端 commit `065a3f73` 部署到 ECS，随后已通过 CLI 创建阿里云融合认证 Android 方案并写入 DYPNS AccessKey / Secret、`DYPNS_FUSION_SCHEME_CODE`、包名和签名；同日短信资质、签名和验证码模板已通过并写入 ECS。生产健康检查为 `ok=true / auth_strict=true / bailian=missing_key / dypns=ok / dypns_fusion=ok / dypns_sms=ok / dev_order_endpoints=false / redis=ok / upload_storage=oss`
