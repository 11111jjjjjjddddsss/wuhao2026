...
- 今日农情提示词版本当前为 `2026-06-12-v62`：检索阶段最大限度全网宽搜、不锁固定网站，质量优先级是“近 7 天真实具体材料 > 种植侧大类正确 > 信息对生产或流通有直接意义 > 内容尽量不重复 > 手机卡片好读”。提示词优先让模型输出 3 条，但质量优先，确实只有 2 条可靠种植侧材料时可以只输出 2 条，并明确 JSON 示例里的 3 个对象只代表 3 条时的格式，不为了凑数去选养殖、水产、旧闻、广告软文、弱材料或泛泛动态；发布和 Android 展示仍只要求至少 2 条标题 / 摘要完整 item，不增加后端内容过滤。大类边界要清楚：本卡只取种植侧，覆盖粮油、蔬菜、果品、棉花、茶桑、种子种业、植保病虫草害、农药肥料农资、农机农时、农田水利、明确影响作物生产的农业气象 / 灾害预警 / 田管措施、种植侧产地 / 批发 / 农资价格、补贴保险和技术推广；养殖、水产、畜牧、动物疫病、生猪、猪肉、猪价、家禽、禽蛋、鸡肉、鸡蛋、牛羊、奶业、奶价、饲料、饲用原料、兽药、渔业、鱼虾和动物类价格等不要。即使来源把这类内容说成影响农户收入、消费预期、农资购买力或农业整体行情，也不作为本卡条目；不要用“间接影响种植户”作为保留理由。标题约 10-14 个中文字符、最多不超过 16 个中文字符；摘要约 90-130 个中文字符、尽量不低于 85 个中文字符，至少两句，宁可写到 100 字左右也不要收成 70 多字的薄摘要；调研、会议、平台上线和成果展示类材料必须有具体种植事实和直接影响，否则换更具体材料；标题不要用“调研推动”“场景上新”“即将投用”“成果展示”这类过程词掩盖空泛材料。必须是自然资讯口吻，禁止“值得看 / 参考意义 / 对农户有用 / 根据搜索结果 / 检索显示 / 可关注”等元表达、推荐理由和标题党话术；这仍是提示词层面的方向控制，不增加后端内容过滤或字数拦截；养殖水产和动物类价格边界继续靠提示词、探针和后台复核观察控制
