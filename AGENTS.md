...
- UI 监护默认优先：
  - `Logcat` / 本地调试日志
  - 必要时再补临时 debug 面板
- 如果 Android 设备已通过调试连接，优先直接读取 `adb logcat` 或同级后台日志，不要先要求用户盯着屏幕面板手抄状态
- UI 调试日志不应依赖手动清理旧数据；应优先采用“最近 N 条自动覆盖”的环形日志，默认至少保留最近 20 条关键记录
- 调试日志至少覆盖三类状态：
...
- Compose 滚动手势链如果使用 `NestedScrollSource`，实现时要跟当前库版本对齐；像 `Drag` 这类已废弃 source 不允许继续当真源判断，否则会出现“代码看着在拦，实际手势根本没进来”
- 如果需要 debug 面板：
  - 只能作为调试辅助，不是主链
  - 默认不应长期遮挡用户界面
  - 必须可收起，且优先只在 debug 构建显示
- release / 正式上架包不允许暴露调试监护面板、调试按钮、调试文案或调试浮层
- 调试日志能力可以长期保留在代码里，但必须受 debug 开关控制；不能影响正式用户体验、性能和界面
