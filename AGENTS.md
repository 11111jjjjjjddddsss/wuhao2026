...
- 时间 / 地点：后端每轮主对话必须注入当前时间、用户地点和地点可信度；历史轮次如果有后端 `created_at / region / region_source / region_reliability`，进入模型上下文时也要带轻量时间 / 地点前缀。时间以后端服务器时间为准，不用前端手机时间当业务真相；前端暂不显示每条消息时间戳或地点条。Android 已接入可选定位权限 `ACCESS_COARSE_LOCATION / ACCESS_FINE_LOCATION`：进入聊天页后请求一次权限，授权后每次发送问诊前尽量短窗口刷新系统定位，并只用系统 Geocoder 反查出的省 / 市 / 区县等地区文本，通过 `X-User-Region`、`X-Region-Source=gps`、`X-Region-Reliability=reliable` 传给后端；不上传经纬度，不保存轨迹。未授权、定位失败或未传 `X-User-Region` 时，当前 Nginx 会透传真实客户端 IP 给 Go 服务，`ResolveRegionByIP` 会优先使用 ECS 本地免费离线 `ip2region` xdb 文件（`IP2REGION_V4_XDB_PATH` / 兼容 `IP2REGION_XDB_PATH`）把公网 IP 粗定位到省 / 市级，结果只作为 `region_source=ip`、`region_reliability=unreliable` 的低可信参考，不走收费 API、不放 RDS、不把完整 IP 注入模型；库未配置、私网 IP、代理漂移或查询失败时仍兜底 `未知 / unreliable`
