...
- Android 已移除 `SESSION_API_TOKEN` 静态共享 token 运行时绕过，不再用测试 ID 作为登录桥接；正式登录必须使用后端按真实手机号账号动态签发的 per-user session token。本机旧 `user_id` 到账号身份的数据迁移已经有首版后端事务骨架；Redis-backed 融合认证 token / 手机号短信认证限流已部署到 ECS 并健康，阿里云融合认证 Android 方案已通过 CLI 创建并把 `DYPNS_FUSION_SCHEME_CODE`、包名和签名写入本机密钥文件与 ECS 环境；当前仍缺阿里云融合认证 Android SDK、DYPNS AccessKey / Secret、短信签名 / 模板和真机登录联调
...
- 2026-06-01 已将后端 commit `065a3f73` 部署到 ECS，随后已通过 CLI 创建阿里云融合认证 Android 方案并写入 `DYPNS_FUSION_SCHEME_CODE`、包名和签名；生产健康检查仍为 `ok=true / auth_strict=true / bailian=missing_key / dypns=missing_key / dypns_fusion=missing_config / dypns_sms=missing_config / dev_order_endpoints=false / redis=ok / upload_storage=oss`，其中 `dypns_fusion` 仍未 ok 的直接原因是 DYPNS AccessKey / Secret 尚未配置
