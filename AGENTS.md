...
- 流式阶段如果用户先上滑看历史、再快速下滑回到底部，一旦已经回到返回线或真实底部，除了恢复 `StreamAnchorFollow`，还应立即走一次“不包含锚点 spacer”的程序滚动，把尾部空白直接吃掉，不要只切状态不补滚
...
- 流式阶段如果用户只是在当前生成线附近轻微上滑又回到原位，`AnchorUser -> StreamAnchorFollow` 的重新接管应留一个很短的 settle 窗口，不要在刚松手那一拍立刻抢回去，否则容易出现被往下拖和正文发颤
- 当前流式滚动链基线：
