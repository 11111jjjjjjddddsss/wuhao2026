...
- 流式阶段底部可见 spacer 不允许通过“正文长高就同步缩短 spacer”来消空白，这会把正文带成一行一行上蹦；消空白应通过手势边界和恢复跟随判定收掉，而不是改正文几何
- 流式阶段用户从历史位置再向下滑时，恢复自动跟随的判定应以“是否回到当前生成行”为准，而不是等到底部 spacer 全部消耗完；一旦回到当前生成行，应立即恢复跟随，并阻止继续向下拖出空白
- 流式阶段上滑应始终允许用户自由浏览历史；只对“继续向下拖过当前生成行”做限制，不再通过上滑预滚动或预甩动去抢手
- 流式阶段用户一旦明确上滑，应立即切到手动浏览态并隐藏可见 spacer，不要等列表先滚一拍再把 `userDetachedFromBottom` 置真，否则会出现“抢手”和先露一截空白
- 不要再乱调无关参数
- 不要顺手改输入区
