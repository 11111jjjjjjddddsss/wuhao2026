...
模型：
- 主聊天优先模型：配置中转联盟后优先走 `gpt-5.5 /v1/responses`，用于农业问诊分析、图片理解、推理判断；原 `qwen3.5-plus` 保留为千问回落链路和未启用中转站时的原主链路
...
- 原千问主对话回落链路、记忆文档摘要和今日农情生成统一显式设置 `temperature=0.8`；主对话优先中转站 Responses 链路不显式设置 `temperature / top_p / max_tokens / max_output_tokens`，输出长度继续靠提示词控制
- 2026-06-26 起，主对话支持可选的后端优先中转站链路：配置 `CHAT_PRIMARY_ENABLED=true`、`CHAT_PRIMARY_API_MODE=responses`、`CHAT_PRIMARY_BASE_URL` 和 `CHAT_PRIMARY_API_KEY` / `CHAT_PRIMARY_API_KEYS` / `CHAT_PRIMARY_API_KEY_1...50` 后，主聊天会先把同一套后端组装好的上下文发给中转站 Responses 流式模型，当前备注“中转联盟”，默认模型名 `gpt-5.5`。Responses 请求使用 `tools=[web_search]`、`tool_choice=auto`、`search_context_size=low`、`reasoning.effort=high`，并只追加一条联网工具规则：仅当本轮问题涉及最新信息、价格行情、政策公告、购买渠道、天气、灾害预警或其他时效性判断时，以最快速度联网搜索；拿到足够信息后立刻回答，不解释搜索过程。普通农技知识、图片可见信息和非时效性问题直接回答。不追加“简洁回答”、字数限制或质量测试提示。后端按用户可见正文首字计算 `CHAT_PRIMARY_FIRST_VISIBLE_TIMEOUT_SECONDS`，当前生产统一配置 15 秒；连接失败、超时、非 2xx、非 SSE 或 15 秒无可见正文时快速回落原 `qwen3.5-plus` 主备 Key 池，若原请求是明确实时 / 价格 / 行情意图，回落千问仍保留原 `ForceSearch` 搜索口径。中转站 Key 池按请求轮询，开流前遇到连接错误、`401 / 403 / 429` 或 `5xx` 会短暂冷却当前 Key 并尝试下一把；一旦已经向用户吐出可见正文，不在同一条回复中途切模型，避免半条回答和重复成本。Android 仍不保存、不注入、不直连任何模型 Key；中转站密钥只能在服务器环境变量 / 本机私密配置里保存，不进仓库、文档、日志或聊天复述。该链路会把用户文本和普通问诊图片一起发给第三方中转服务，属于数据处理和可用性风险，正式长期使用前需继续观察质量、首字延迟、图片能力、联网稳定性、成本和隐私责任
...
- 后端千问 / 百炼模型 Key 池支持 `DASHSCOPE_API_KEY_1/2/3`、旧 `DASHSCOPE_API_KEY` 和 `DASHSCOPE_API_KEYS` 逗号 / 分号 / 换行列表，自动去重；当前生产按用户拍板使用 `DASHSCOPE_KEY_SELECTION_MODE=fallback`，也就是 `DASHSCOPE_API_KEY_1` 主钥匙优先吃满，`DASHSCOPE_API_KEY_2` 只做副钥匙失败兜底，不再因短窗口请求量或 token 用量阈值主动轮询分流。原千问主对话回落链路、记忆文档摘要和今日农情共用该池。若千问模型请求打开阶段遇到 `401 / 403 / 429` 或带限流 / quota 语义的 `400`，后端会在流开始前切到下一把 Key，并把触发限流的 Key 短暂冷却，默认 1 秒，可用 `DASHSCOPE_KEY_COOLDOWN_SECONDS` 调整；一旦 SSE 流已经成功开始，不在同一条回复中途切 Key。后续如果真实流量长期打满主钥匙，再评估改回 `auto` 或 `round_robin` 做主副轮换。扩真实并发必须使用不同阿里云主账号的 Key；同一主账号下多个 API Key 共享该账号 RPM / TPM 限流，只适合轮换或应急，不算扩容
- 联网搜索另有阿里云官方 15 RPS 主账号级限制，按该主账号下所有 API Key 的联网搜索请求总和统计，不区分模型；超过时 API 不报错但搜索链路不触发。当前中转联盟 Responses 主链由模型按中性工具规则自行判断是否联网；原千问回落链路默认 `forced_search=false`，若原请求是明确实时 / 价格 / 行情等意图，回落千问保留 `ForceSearch=true`；今日农情强制联网生成需单独观察生成任务成功率
