...
- 当前单宿主路径下，已停用的发送期 overlay 预热/取样链不应继续在首次聚焦和 IME 切换时白跑、白等帧
- 流式阶段如果用户明确向上拖动浏览历史，应尽快脱离底部跟随，不要用滚动锁把整段拖动硬吞掉造成“卡住”的体感
- 流式阶段用户再向下拖回底部时，只要已经接近真实底部，应立即切回 `StreamAnchorFollow`，不要只挂一个延迟恢复状态让底部继续空白
- 流式阶段底部 spacer 的可见高度应以真实正文溢出为上限；当底部没有真实正文可显示时，不应继续保留可被拖出的大片空白
