...
- 生成期底部护栏当前继续收口为更简单的统一规则：整个生成阶段都允许用户上下滑，但往底部方向不允许越过当前工作线；完成态直接退出这套特殊规则，恢复普通静态滚动
- 也就是说，生成阶段不再同时长期维护“锚点空白边界”和“生成行边界”两套主规则；工作线是唯一底部边界，正文把锚点空白吃满后，生成行会自然贴近工作线
- 实现上应继续只消费“本次下滑会超过工作线的那部分 delta / velocity”，不要锁死下滑手势本身；按钮和状态机也不要再反向干预这条几何护栏
