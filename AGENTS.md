...
- Android 当前一键登录使用阿里云融合认证 `AlicomFusionBusiness` / `fusionauth-1.2.15-online-release.aar` 链路，不是普通 `PhoneNumberAuthHelper` 三 AAR 直连链路；该 AAR 已包含普通网关认证、日志类、native so 和授权页 Activity，debug / release merged manifest 已合入 `FusionNumberAuthActivity`、`FusionSmsActivity`、`AlicomFusionUpSmsActivity`、`FusionGraphAuthActivity`、`LoginAuthActivity` 和 `PrivacyDialogActivity`。2026-06-09 已补 `FusionAuthProtocolActivity` 承接包名专属协议页 action `com.nongjiqiancha.FUSION_AUTH_PROTOCOL`，授权页 UI model 同步设置 `protocolAction` 和 `setPackageName(BuildConfig.APPLICATION_ID)`，并给 `FusionOneLoginClient` 增加请求代际、重复拉起锁、Activity `finish/destroy` 兜底、登录页销毁主动停止当前 SDK 场景、SDK token 鉴权失败收口、`onGetPhoneNumberForVerification` 非空兜底、`onVerifyInterrupt` 不强行继续、服务端登录中忽略后到模板结束 / verify failed 回调、重复 `onVerifySuccess` 保护和 100001 意外半程回调直接失败回验证码登录。后续登录排障不要把普通号码认证“三个 AAR”清单直接当成当前仓库缺包结论；若仍真机闪退，优先看后台 `auth.app_crash` / `auth.*` 和 logcat `AndroidRuntime / FusionAuth / AlicomFusion / PhoneNumberAuth / DEFENDER / jaffer`
