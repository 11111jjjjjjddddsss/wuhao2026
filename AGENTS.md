...
- 生成期底部护栏应正式拆成三段语义：第一段是“锚点空白保护态”，第二段是“生成行边界态”，第三段是“完成态直接放开”；不要再把三种语义混成一条统一规则
- 第一段的判定不能直接用“当前视口里的 `contentBottom < legalBottom`”反复计算；用户上滑浏览历史时这个量也会变小，容易把浏览态误判回早期锁死态。应改成“本轮流式是否已经真正吃到锚点空白边界”的阶段标记，并冻结当前自然底边作为第一段的底部边界：未吃满前，用户仍可自由上下滑，但往底部方向只允许回到这个第一段边界，不能继续越过；一旦本轮已吃满，就永久切到第二段，不再因为当前视口位置回退
- 第二段只认当前生成行 / 工作线边界，按 `projectedBottom / overflow` 只拦越界量；完成态则无论第一段是否吃满，都直接退出这套生成期限制，恢复普通静态滚动
- 如果这条“保留空白是否吃满”的规则继续导致体感复杂或不稳，允许进一步收紧成更简单的统一规则：整个生成阶段只要用户往底部方向 drag / fling，就不允许超过当前生成行 / 工作线；文本全部生成完成后，再恢复普通自由上下滑
