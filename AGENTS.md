...
- 2026-03-31 生成期 `UserBrowsing -> AutoFollow` 继续增加浏览态粘性时间门槛；用户一旦进入浏览态，不允许因小幅上滑后马上停稳就秒切回 follow，至少要经过短时间停留后，才允许按“停稳 + 生成消息重新可见 + 回到生成工作线/返回线附近”恢复
- 2026-03-31 底部空白 drag / fling 保护继续统一使用同一份 `safeDistance`；如果拿不到最后内容 item 的稳定测量，保守按 `0` 处理，不再把“拿不到测量”当成“安全距离无限大”
- 2026-03-31 生成阶段 `回到底部` 按钮除 `scrollMode == UserBrowsing` 外，还必须满足“底部空白未暴露”；一旦已经滑进空白区，按钮不允许显示
