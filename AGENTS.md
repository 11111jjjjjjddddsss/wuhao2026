...
- 发送后插入 streaming 占位的第一拍，不允许立刻 `AutoFollow + 补滚` 同时接管；应先让新消息插入和 streaming 占位稳定 1–2 帧，再进入正常自动跟随，避免整个消息区上下弹一下
- streaming 等待小球只允许作为轻壳存在，不允许再把 waiting indicator 的 bounds 当成正文底边真相；follow 也不允许在首字刚出现的那一拍立刻接管，应等真实正文尾部连续稳定 1–2 帧后再进入自动跟随
...
- 生成期底部空白护栏必须只认一套纯几何真相：最后一个真实内容底边 vs 合法视口底边；护栏不读 `scrollMode`，不写按钮状态，也不参与恢复 follow 判定
- 生成期 drag 护栏要保证“远离底部不拦、接近底部才拦、只吃越界量”；如果仍有极端 fling 或测量晚到漏空白，允许在列表停稳后按同一几何边界做一次小幅纠偏
...
- 第一阶段边界不是“空白面积”，而是“发送后第一次程序性定锚完成并停稳时记录下来的冻结底线 `frozenBottomPx`”；正文尾部碰到这条固定底线之前，往底部方向只允许回到这条线，不能越过它把初始预留空白继续放大
- `frozenBottomPx` 的记录时机必须在“assistant streaming 容器已插入 + 小球锚点已上抬 + 第一次定锚已完成 + 列表停稳”之后；实现上优先取发送后稳定下来的真实内容自然底边，拿不到时再退回当时工作线，不要把工作线直接当成第一阶段唯一真相源
- 当前生成期特殊护栏只保留这一段第一阶段冻结底线；正文尾部 `tailBottomPx >= frozenBottomPx` 后，直接退出第一阶段并回普通工作线逻辑，不再额外保留第二套特殊边界系统
- 完成态或失败态直接退出这套生成期特殊护栏，恢复普通静态滚动；短文本允许从第一阶段直接进入完成态，不要求必须经过额外阶段，也不要求强制收口
- 如果本轮结束时正文还没吃完冻结底线，应把剩余空白转成静态保留量 `retainedBottomGapPx`；这段空白继续保留，确保本轮短文本/失败态结束后页面不往下掉
- 下一轮发送开始时，不允许先把上一轮 `retainedBottomGapPx` 当成可见空白做退场处理；新一轮发送锚点应直接以“上一轮最后一条真实正文尾部”附近接上，旧 `retainedBottomGapPx` 只作为内部保底状态失效，视觉上必须像正常消息接力，不允许出现先清空白再上新锚点的上下抖动
...
- 当前工作线应比输入框上边缘留出更明显一点的安全距离，不要贴得过近；后续若再调距离，优先只调这条统一 gap，不再额外造第二条底部线
- 生成阶段用户允许正常上滑下滑；系统只在“继续往底部方向移动会越过当前工作线”时消费越界量，不再整段锁死手势
- `回到底部` 按钮必须与滚动链写状态解耦：
  - 动态/静态都只按“当前是否已经离开底部”派生
...
- 2026-03-31 生成期最终方案继续收口：恢复 follow 不再靠时间粘性或额外 boundary 门槛，主判断改为“生成行相对输入框上边缘是否重新可见”；用户进入浏览态后，只要生成行还没重新露出来且列表未停稳，程序就必须继续停
- 2026-03-31 底部空白 drag / fling 保护继续共用同一份 `safeDistance` 语义：远离底部、最后真实内容 item 还没进入视口时不拦；只有接近底部、最后真实内容 item 已进入视口时，才开始 clamp，避免再出现“先上再下滑不动”的回归
- 2026-03-31 生成阶段 `回到底部` 按钮主语义改为“用户已进入 `UserBrowsing` 且生成行当前不可见”；发送后的 `Idle` 锚点阶段即使生成行暂时在输入框以下，也不允许提前出现按钮
