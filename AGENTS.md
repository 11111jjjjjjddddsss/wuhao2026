...
- waiting 小球、streaming 正文、settled 完成态默认共用同一条 assistant 消息主线；但 streaming 生成期间，允许把当前 assistant 在同一个 `LazyColumn`、同一个 `LazyListState` 内持续派生成“多个稳定 block item + 至多一个 active block item”。这只是单一列表内的展示层分块，不是 overlay / active-zone / 第二滚动宿主；整条 assistant 尚无内容时可用初始 waiting item 表达小球，但生成过程中段落边界产生的空尾巴不能再作为空 active item 插入 index 0；streaming 完成后可继续保留已完成 block 快照，回到底部或下一轮发送再清掉快照并恢复完整消息 item
...
- streaming 期间 `ChatScreen.kt` 会按段落边界持续把当前 assistant 派生成多个稳定 block item 和至多一个 active block item；代码块内不切分，超长无空行内容会在句子 / 空白边界兜底切分，当前 active block 上限为 180 字，避免视觉底部 index `0` 的 active block 重新长成巨型 item。所有 streaming block 都使用稳定的 `messageId:streaming_block:<index>` key，避免 `scrollMode` 变化时同一可见块换 key 造成上下窜；段落边界如果尾部为空，不再把空 active block 插进 `chatListItems`，只有整条 assistant 还没收到任何内容时才保留初始 waiting item。AutoFollow 贴底状态下如果新 active block 产生，才在非用户接管窗口用 `requestScrollToItem(0)` 让下一次 remeasure 接住新尾巴；该贴底请求执行前会等待一帧并检查用户浏览 epoch，若用户在同帧或上一帧已触碰列表接管，则本次旧 AutoFollow 请求失效，并且 `lastAutoFollowStreamingBlockIndex` 只允许在真正发出贴底请求或新流 reset 时更新，避免被取消的旧请求污染后续跟随。不要再用 `firstVisibleItemIndex / firstVisibleItemScrollOffset` 做严格 position 复核，否则容易误杀用户手动回到底部后的正常跟随。这个方案不裁剪内容、不补 streaming 高度 `scrollBy`、不恢复 overlay；点击回到底部时清掉完成后保留的 block 快照，重新回到完整 assistant item 并恢复跟随
- `ChatStreamingRenderer.kt` 的 active streaming block 当前使用单个 soft-wrap `Text` 渲染正在吐字的段落 / 标题 / 列表正文，不再把 active 文本按物理行拆成多颗 `Text`，也不再对新字尾部做 fresh suffix 灰色高亮动画；waiting 小球只允许在整条 assistant 尚无任何内容时显示，段落切换产生的空尾巴不能进入列表数据，也不能显示小球，避免下一行黑点冒头和空 slot 重锚。active Markdown 仍实时吐字，但只有 `# ` / `- ` / `1. ` 等结构前缀后已经出现非空正文时才切成标题 / 列表 / 引用，不能把只有符号的半成品立刻结构化；跨 streaming block 的一级 / 二级标题分割线由 `ChatScreen.kt` 在全局 block 派生层计算后传给 renderer，避免每个 block 只看自己内部 previous 导致分割线时有时无。已完成 block / settled Markdown 仍可按现有静态渲染路径拆行和选择
