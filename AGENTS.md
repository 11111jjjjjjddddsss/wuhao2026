...
- terminal drain 阶段（远端 onComplete 已到达，或本地 fake stream 已经把源文本全部喂完，只剩本地 `streamingRevealBuffer` 继续按 typewriter 节奏吐尾巴）不再发新的 `requestScrollToItem` 新行锚定请求；这段尾巴仍交给现有 reveal job 逐字吐完，再进入两阶段 finalize。目的只是避免最后一拍 pending line-anchor 和 completed finalize 切树撞在一起，不能扩张成中途禁用新行锚定，也不能恢复旧 clip / renderer gate / bounds 硬等待
...
- 当前修法：移除叶子 renderer 的 `rememberGatedStreamingRenderedLines(...)` 显示门闩，改为在 `ChatScreen.kt` 的 streaming `onAdvance` 回调里，先用 `measureStreamingActiveBlockLayout(...)` 对当前 active block 与下一拍 active block 做同宽度、同样式的 pre-measure；若下一拍物理行数增加，则用当前可见 streaming item 的 top offset 和实测高度差计算 `requestScrollToItem(index, scrollOffset)`，在下一次 remeasure 里把 item 顶边上移同等高度，再同拍提交 `streamingMessageContent`。远端 / 本地源头已经结束、只剩本地 reveal buffer 尾巴时，`requestScrollToItem` 新行锚定暂停，避免 pending anchor 影响两阶段 finalize。`ChatScrollCoordinator.kt` 里的 streaming follow suppression 继续使用更严格的 `isAtStreamingWorklineStrict()`，作为 bounds 主链的后续精修
