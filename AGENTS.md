...
- Redis 生产状态：ECS `/etc/nongjiqiancha/server.env` 已配置 `REDIS_ADDR / REDIS_USERNAME / REDIS_PASSWORD / REDIS_DB`，`/healthz` 返回 `redis=ok`；Redis 当前只服务融合认证 token、手机号认证、主聊天用户级频控、App 自动日志接收、帮助与反馈用户发消息和上传短期限流，不承载聊天正文、图片、长期画像、会员资产、额度或订单真相
