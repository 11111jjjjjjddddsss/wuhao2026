...
- 第一阶段边界不是“空白面积”，而是“发送当下冻结下来的工作线底边”；正文底边碰到这条固定底线之前，往底部方向只允许回到这条线，不能越过它把初始预留空白继续放大
- 第一阶段更准确地说，应守“发送后定锚完成时界面实际停住的初始自然底线”；实现上优先取发送后稳定下来的真实内容底边，拿不到时再退回当时工作线，不要把工作线直接当成第一阶段唯一真相源
- 第二阶段从“正文底边已经碰到第一阶段固定底线”开始；进入第二阶段后，底部边界切换成当前生成行（真实 streaming 正文尾部的动态底边），继续只拦越界量；但这条第二阶段边界只应在用户已经重新回到生成尾部区域附近后才开始生效，不能在用户仍处于长消息中段或远处历史区时提前卡住回滑
- 完成态直接退出这套生成期特殊护栏，恢复普通静态滚动；短文本允许从第一阶段直接进入完成态，不要求必须经过第二阶段
...
- 当前滚动链已继续收简为“单工作线 + 单护栏 + 独立按钮脉冲”口径：
  - 不再保留单独的小球锚点 spacer / reserve 主逻辑
  - 发送后消息直接落到工作线附近
  - streaming 自动跟随也继续以这条工作线为目标
