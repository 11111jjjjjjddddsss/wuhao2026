...
- 生成期底部 drag 护栏应优先使用 `projectedBottom / overflow` 的单段几何判定，不再用“未露白按 safeDistance、已露白整段锁死”的两段式分支；停稳后二次纠偏只能作为低频保险丝，不得再形成肉眼可见的常规回弹
- 更具体地说：生成早期只要正文底边还没把小球抬起留下的保留空白吃满（`contentBottom < legalBottom`），往底部方向 drag / fling 可以直接整段拦；正文一旦吃满后，再切到 `projectedBottom / overflow` 只拦越界量
