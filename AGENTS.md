...
- Codex 本机聊天记录、归档线程和压缩摘要只按临时缓存看待，不承担项目长期记忆职责；长期规则、上线状态、风险、运维入口、自动化口径、保护提示词边界和用户明确拍板必须及时写入仓库规则、项目记忆、ADR 或 runbook。清理本机 Codex 旧聊天缓存不应影响项目真相，新窗口接手仍以仓库文档和当前代码为准
...
- 主对话锚点、今日农情提示词、记忆文档提示词是最高敏保护提示词；除非用户明确说“可以改 / 按这个改 / 落地到代码 / 部署这个提示词”，否则 Codex 只能审查、挑刺、解释风险、提出候选文案或草案，不得直接修改对应文件、内嵌 prompt 文案或等价模型指令。用户说“全盘挑刺 / 拉代理 / 联网校准 / 用户体验为主”时，也不能自动视为授权修改这三类提示词。
- 所有模型输出方向统一靠提示词、探针、人工复核和运营观察控制，不能新增后端内容过滤、关键词拦截、相似度硬拦截、摘要 / 标题 / 正文字数硬卡点、`max_tokens` 截断或按主题词卡发布 / 卡写入 / 卡展示。主聊天用户输入、AI 用户可见回复、帮助与反馈用户消息、授权客服回复和客服处理备注，都不按手机号、数字、订单号、礼品卡码、所谓敏感词或主题词做内容拦截；用户和客服可以为排障发送这些信息。工程安全兜底只允许用于请求体大小、文字长度上限、图片数量 / 大小、JSON 可解析、必要结构字段、登录鉴权、限流、日志脱敏、URL 安全、密钥保护、响应读取上限等稳定性和安全边界，不得变成限制模型表达方向或用户 / 客服表达内容的准入。
...
- 生产后端通过 [scripts/deploy-ecs-server.ps1](D:/wuhao/scripts/deploy-ecs-server.ps1) 使用单 ECS 双端口 slot 发布；部署脚本按 `server-go/go.mod` 的 Go toolchain 在 ECS 上准备工具链、运行 `go test ./...` 后重新编译二进制，并写入机器可读 revision。[scripts/check-ecs-readiness.ps1](D:/wuhao/scripts/check-ecs-readiness.ps1) 是当前线上 revision、active slot 和健康状态真相入口，Nginx 会在 `3000 / 3001` 两个本机端口间切换 API 与后台 `/admin-api/` 上游，公网流量以脚本输出的 `active_upstream_port` 和 `server_revision` 为准；不要把某次观察到的日期、commit、`3000` 或 `3001` 写成长期常量。短信资质 / 签名 / 验证码模板、Redis、OSS 上传后端、DashScope 主 / 副模型 Key 主备槽位、后台监控面板、账号ID收敛迁移、礼品卡生成 / 兑换 / 追溯 / 完整卡码加密查看、帮助反馈状态队列，以及检查更新发布配置 / 发布历史均已进入生产代码 / 环境并通过健康检查；服务端旧融合认证路由默认 410 停用，公网已验证 `/api/auth/fusion/token` 返回 `410 fusion_auth_disabled`，仅显式 `AUTH_FUSION_COMPAT_ENABLED=true` 才临时兼容历史包，新 Android 包不调用。后台静态前端通过 [scripts/deploy-ecs-admin.ps1](D:/wuhao/scripts/deploy-ecs-admin.ps1) 部署到 `https://admin.nongjiqiancha.cn/`，公网首页 200；具体线上提交和最近部署记录写入 [docs/project-state/current-status.md](D:/wuhao/docs/project-state/current-status.md) / [docs/project-state/recent-changes.md](D:/wuhao/docs/project-state/recent-changes.md)。网站 ICP 备案已通过，`api.nongjiqiancha.cn` HTTPS 已配置并公网验证通过，根域名官网 HTTPS 已部署并公网验证通过，官网已补网站公安备案 footer、警徽图标和独立用户协议 / 隐私政策页，API HTTP 80 已改为 ACME challenge + 301 HTTPS 跳转，DashScope 主 / 副模型 Key 已写入 ECS 主备槽位并显示 `bailian=ok`，App 备案已通过并取得 `京ICP备2026031728号-2A`，网站公安备案号已下发并已补官网 footer；公网 SSH 22 已撤销且 ECS 本机 ssh 服务已停用，当前公网只放行 80 / 443 和 ICMP；App 公安备案、短信验证码登录 / 主聊天真机回归和上线前 AccessKey 轮换仍缺
- 后端运行边界：Go 服务使用显式 `http.Server`，默认限制慢请求头、请求读取、空闲连接和最大 header，`WriteTimeout` 默认保持 0 以兼容 SSE；通用 JSON body 默认 64KiB，App 日志接口 8KiB，超限返回 `413 body_too_large`，并按 `user_id + IP` 做默认 10 分钟 60 次短期限流；帮助与反馈用户发消息接口按 `user_id + IP` 做默认 10 分钟 20 条短期限流；`/upload` 按 `user_id + IP` 做默认 10 分钟 120 次短期限流，仍只接受单张 `<=1MiB` JPEG；后台登录额外按“用户名 hash + IP hash”做默认 10 分钟 10 次专用限流，只保护 `admin.nongjiqiancha.cn` 后台口令入口，不影响 App 用户登录 / 聊天 / 验证码；主聊天应用层用户限流默认 `20 次 / 60 秒`，可用 `CHAT_RATE_LIMIT_MAX_HITS`、`CHAT_RATE_LIMIT_WINDOW_SECONDS`、`CHAT_RATE_LIMIT_PRUNE_INTERVAL_SECONDS` 调整，配置 Redis 时跨进程共享，未配置 Redis 时回退单进程并清理过期用户桶；2026-06-13 线上 Nginx 主聊天 IP 级限制已调宽到 `60r/m`、burst 80，并移除聊天 `limit_conn`，避免共享网络用户被旧 `6r/m` / 2 连接误伤；模型出站请求限制拨号、TLS 握手、响应头等待和空闲连接，主聊天 SSE 默认 30 分钟最长持续时间兜底；首字后可见正文空闲 watchdog 默认 90 秒，并在每次收到新的上游用户可见正文时重新计时；上游错误 / 非 SSE 预览和记忆文档摘要响应读取上限为 64KiB，今日农情 JSON 响应读取上限为 1MiB；服务启动迁移会先获取 MySQL 全局锁，迁移锁释放失败会作为启动错误暴露，早期 `004/005` 迁移里的旧 DDL 已改成 `information_schema` 条件执行但仍没有独立 `schema_migrations` 版本表
- 域名：`nongjiqiancha.cn` 已在阿里云购买，用户口头确认域名实名认证 / 模板审核已通过；当前用于 `api.nongjiqiancha.cn`、根域名官网、管理后台和下载域名。当前已创建 `api`、`@`、`www`、`admin` A 记录指向 ECS 公网 IP `39.106.1.151`，`download.nongjiqiancha.cn` CNAME 指向 OSS Bucket `nongjiqiancha-prod.oss-cn-beijing.aliyuncs.com`；已完成 `api.nongjiqiancha.cn` HTTPS / Nginx 443 反代、`nongjiqiancha.cn` / `www.nongjiqiancha.cn` 官网 HTTPS 静态站、`admin.nongjiqiancha.cn` 管理后台 HTTPS 静态托管和 `/admin-api/` 同域反代，以及 `download.nongjiqiancha.cn` OSS 自定义域名 HTTPS 证书绑定。网站 ICP 备案已通过，App 备案已通过并取得 `京ICP备2026031728号-2A`，网站公安备案号已下发并已补官网 footer，App 公安备案待补；下载域名证书使用免费 Let’s Encrypt，ECS `certbot.timer` 负责续期，续期后需通过 `scripts/sync-oss-download-certificate.ps1` 同步到 OSS CNAME 证书配置
...
- Android 内部测试包下载：2026-06-17 起，debug/internal 测试包默认通过 `download.nongjiqiancha.cn` 自有 HTTPS 下载域名访问私有 OSS 对象的限时签名 URL，不再推荐用 ECS `/test-apks/` 静态路径，也不能使用 OSS 默认公网域名。`scripts/check-android-download-domain.ps1` 检查下载域名，`scripts/sign-oss-cname-url.py` 生成 CNAME 签名链接，`scripts/sync-oss-download-certificate.ps1` 在 Let’s Encrypt 免费证书续期后同步 OSS CNAME 证书；正式 release 不能使用 72 小时测试短签名链接
...
模型：
- 主聊天正式链路当前只走阿里云百炼 `qwen3.5-plus` OpenAI 兼容流式 `chat/completions`，用于农业问诊分析、图片理解和推理判断；后端继续负责同一套上下文组装、账号、额度、归档、记忆和日志，Android 不保存、不注入、不直连任何模型 Key
...
- 主对话、记忆文档摘要和今日农情生成统一显式设置 `temperature=0.8`；主对话输出长度继续靠主聊天提示词控制，不通过 `max_tokens` 截断
- 2026-06-27 按用户真机连续断链、首字后卡死、长耗时和识图质量不稳定反馈，旧 `gpt-5.5` 第三方中转站 / Responses 优先主链已退出生产。`CHAT_PRIMARY_*` 不再是受支持的生产配置；readiness 若发现 `CHAT_PRIMARY_ENABLED=true` 应失败提醒移除。2026-06-29 后端新增独立的 `GPT_RELAY_*` 主聊天候选链路，后续生产已启用为“GPT relay 候选 + 百炼 / 千问兜底”，不复用旧变量名；该候选链路只在后端使用 Responses 流式接口，Android 不保存、不注入、不直连任何 GPT Key。2026-07-01 生产环境已因中转联盟首字不稳定收口为“只留大大科技 + 百炼 / 千问兜底”：`GPT_RELAY_PROVIDER_1_*` 中转联盟从 ECS 私密环境移除并保留本机私密备份，线上只保留 `GPT_RELAY_PROVIDER_2_LABEL=大大科技` 和 5 把私密 Key。模型全局固定 `gpt-5.5`，请求固定中等思考、低上下文联网工具、联网一次快速回答提示，不设置温度、`top_p`、输出 token 截断或图片 detail。每一波用户请求只打当前唯一 GPT provider，60 秒内只看后端是否收到该平台上游用户可见正文首字；同平台内开流前失败或可重试状态最多轮 5 把 Key，仍无可见正文就回退百炼 / 千问；一旦已吐出可见正文，不在同一条回复中途切模型。GPT 和千问一样接收主对话锚点、`【输出约束】` / 回答参考范本、时间地点、记忆、历史上下文、本轮文字和图片，GPT 只额外多一段专用联网规则。GPT relay 不再使用 Key 冷却、provider 冷却、进程内熔断、旧连接 / TLS / 响应头阶段切换限制或 7+7 同轮短探针；这些退役变量若残留在生产环境应由 readiness 报错。`/healthz` 只暴露 `disabled / ok / missing_config` 等非敏感状态；后台“模型链路”页可显示非敏感 provider label、provider 槽位、Key 槽位和模型名用于排障，但不展示真实 URL、真实 Key、账号分组、prompt 正文、AI 正文或图片 URL。日志脱敏规则可继续保留旧 `CHAT_PRIMARY_*` 和新 `GPT_RELAY_*` 变量名，避免历史环境或日志泄漏
- 主对话、今日农情、记忆整理和后续新增的任何模型输出入口，都不得用代码层内容过滤或硬截断来控制输出方向；质量、题材、边界、语气和长度目标只能由提示词、探针、后台抽查和人工运营来调。允许保留的安全 / 稳定性兜底只包括 JSON 可解析、必需字段非空、请求和响应大小上限、URL 安全、日志脱敏、密钥保护、数据库幂等和错误收口，不得扩展成主题词、关键词、相似度、字数或模型内容审核门槛。
- 后端千问 / 百炼模型 Key 池支持分组配置和旧平铺配置。当前生产按用户拍板使用 `DASHSCOPE_PRIMARY_API_KEY_1...4` 作为主账号 Key 组，健康时只在主组内按请求轮询并作为主扣费组；`DASHSCOPE_SECONDARY_API_KEY_1` 作为副账号兜底，只在主组开流前全部失败后才尝试。旧 `DASHSCOPE_API_KEY_1/2/3`、旧 `DASHSCOPE_API_KEY` 和 `DASHSCOPE_API_KEYS` 逗号 / 分号 / 换行列表继续兼容，但配置了 `DASHSCOPE_PRIMARY_* / DASHSCOPE_SECONDARY_*` 后不再混入旧平铺变量。原千问主对话链路、记忆文档摘要和今日农情共用该池。若千问模型请求打开阶段遇到 `401 / 403 / 429` 或带限流 / quota 语义的 `400`，后端会在流开始前切到同组下一把 Key；主组全失败后才切副组，并把触发限流的 Key 短暂冷却，默认 1 秒，可用 `DASHSCOPE_KEY_COOLDOWN_SECONDS` 调整。一旦 SSE 流已经成功开始，不在同一条回复中途切 Key。扩真实并发必须使用不同阿里云主账号的 Key；同一主账号下多个 API Key 共享该账号 RPM / TPM 限流，只适合轮换或应急，不算扩容
- 联网搜索另有阿里云官方 15 RPS 主账号级限制，按该主账号下所有 API Key 的联网搜索请求总和统计，不区分模型；超过时 API 不报错但搜索链路不触发。当前主聊天千问链路默认 `forced_search=false`，若本轮请求是明确实时 / 价格 / 行情等意图，后端保留 `ForceSearch=true`；今日农情强制联网生成需单独观察生成任务成功率

上下文结构：
- A 层历史滑窗：Free / Plus / Pro 全部 6 轮
- 记忆文档：Free / Plus / Pro 全部每 6 轮触发一次；后端用一份纯文字提示词、一次 `qwen-plus` 调用，覆盖更新一份自然语言用户记忆文档。物理存储暂沿用 `session_ab.b_summary` 字段，对外接口返回 `memory_document`，`b_summary` 仅作为兼容别名；旧长期记忆列不再使用并由迁移删除。记忆文档建议四块：“短期记忆 / 长期记忆 / 用户画像 / 农业事件”，一般约 1000-1400 个中文字符以内，复杂连续场景可更长一些，但这是提示词写作目标，不是后端硬截断；四块是提示词方向，不是后端硬结构。短期记忆负责接住最近正在处理的事项和下一轮要接的上下文；长期记忆记录跨多轮仍可能有用的稳定信息；用户画像记录用户明确给出的个人信息、稳定偏好和长期限制；农业事件放后续可能继续追踪的具体农事线索，并尽量保留已有时间、地点和客观图片线索。整体策略是覆盖更新而不是归档追加；最近对话、最新纠正和更可靠证据优先，旧记忆只作为融合材料，不得长期堆流水账。模型输出和最终存储都是纯文字；后端最大限度放开，不解析四块、不补旧块、不按诊断词 / 作物词 / 主题词 / 画像完整度拦截写入，只清理外层 Markdown 代码块和空白，模型返回非空文本就原样写回；只有模型失败、空回复、响应过大或写库冲突时才不写回并保留 `pending_retry_b`。记忆质量、四块是否写满、旧记忆是否保留、边界措辞和参数保真都交给提示词与后续抽查控制，避免卡太严导致新用户什么都存不了。记忆提示词要求不得把“倾向于 / 更像 / 可能 / 不能排除 / 未见明显...”升级成“确诊 / 确认为 / 已排除 / 已证实”等长期结论，除非历史对话里明确有用户复查、检测或人工确认；第三方或用户转述的判断必须写清“用户转述 / 朋友反馈 / 据用户说”等边界；关键数量和单位不得自行换算或补口径
...
- 时间 / 地点：后端每轮主对话必须注入当前时间、用户地点和地点可信度；历史轮次如果有后端 `created_at / region / region_source / region_reliability`，进入模型上下文时也要带轻量时间 / 地点前缀。时间以后端服务器时间为准，不用前端手机时间当业务真相；前端暂不显示每条消息时间戳或地点条。Android 只申请可选粗略定位权限 `ACCESS_COARSE_LOCATION`：进入聊天页时若已经授权，会静默刷新一次粗略地区；未授权时不在首屏立即弹系统权限框，而是在用户首次发送问诊时按需请求一次粗略定位权限，首轮仍可用缓存地区或后端 IP 粗定位兜底，不阻塞提问。授权后每次发送问诊前尽量短窗口刷新系统定位，并只用系统 Geocoder 反查出的省 / 市 / 区县等地区文本传给后端；地区文本、来源和可信度随 `/api/chat/stream` JSON body 的 `region / region_source / region_reliability` 字段提交，不再放进自定义 HTTP header，避免中文 header 在 OkHttp / 代理链上触发非法字符问题；2 小时内的系统定位或短窗口网络定位仍沿用后端既有 `region_source=gps` 客户端定位来源标签、`region_reliability=reliable`，该标签不代表 App 申请精确定位；只能拿到更旧系统缓存或历史缓存时仍可传地区文本但必须降级为 `region_reliability=unreliable`；不上传经纬度，不保存轨迹。未授权、定位失败或未传 body 地区时，当前 Nginx 会透传真实客户端 IP 给 Go 服务，`ResolveRegionByIP` 会优先使用 ECS 本地免费离线 `ip2region` xdb 文件（`IP2REGION_V4_XDB_PATH` / 兼容 `IP2REGION_XDB_PATH`）把公网 IP 粗定位到省 / 市级，结果只作为 `region_source=ip`、`region_reliability=unreliable` 的低可信参考，不走收费 API、不放 RDS、不把完整 IP 注入模型；库未配置、私网 IP、代理漂移或查询失败时仍兜底 `未知 / unreliable`
...
- 记忆文档摘要由后端 `qwen-plus` 异步处理，摘要请求在 OpenAI 兼容模式请求顶层显式设置 `enable_thinking=false`，单次提取请求有 60 秒超时保护；不联网、非流式，不保留其它摘要候选或分层模型灰度。记忆文档按轮数触发，Free / Plus / Pro 全部每 6 条完整 AI 回复触发一次；触发时把当时 A 层 6 轮窗口冻结成 `pending_memory_jobs_json` 队列里的一个待提取 job，每个 job 仍只包含这 6 轮窗口，不扩大到 30 轮归档输入。提取失败、超时、写库失败、旧快照写回过期或 Redis 租约暂不可用会继续保留 `pending_retry_b` 和冻结 job，后续每一条新的完整 AI 回复归档后马上按队首 job 补提取，不等下一个整除点，也不让旧 6 轮内容被后续轮次滑出丢失；如果旧 job 未成功时又到新的 6 轮触发点，会追加新的冻结 job 排队。写回必须匹配触发快照的 `round_total + updated_at + session_generation + cleared_at`，成功后只弹出队首 job，还有剩余 job 时继续 pending；删除历史后的旧异步结果不会写回新会话。同一用户先有本进程运行中保护，配置 Redis 时还会用带 TTL 的 `nj:summary:lease:*` 分布式租约避免多实例重复提取。后端不按内容拦截或四段补全，非空模型输出会直接覆盖旧记忆；提示词负责尽量保留旧稳定画像、防止不确定判断变定论、第三方转述变系统事实、单次诉求变长期画像，以及面积 / 剂量 / 倍数被模型自行换算
...
今日农情：
- 今日农情是独立的每日资讯内容，后端数据结构和接口历史命名仍叫 card，但它不是聊天消息，不写 `session_ab` / `session_round_archive`，不触发摘要，不扣用户问诊次数。主界面今日农情一旦实际展示，会按账号在 `today_agri_user_items` 保存当天一条主界面展示记录和展示用正文副本，让它重开 App 后像历史里的普通文本一样稳定留在原来的视觉位置；正文副本只保留日期、标题、摘要和短来源名，不保存外部 URL，不进记忆 / 归档 / 扣次。用户在账号管理页删除历史对话时，会一起清掉主界面今日农情展示记录；设置页“今日农情”近 30 天公开资讯历史仍来自 `daily_agri_cards`，不随聊天历史删除。远端模式下主界面只显示后端 snapshot 返回的当天保存项，不再用本地缓存先展示一条未确认农情。Android 历史包可能仍会随发送携带 `today_agri_context_day`，但后端主聊天已兼容忽略该字段，不再查询或注入今日农情系统背景，也不让该字段影响 `client_msg_id` 请求 hash；记忆文档整理、聊天归档、A/B/C、扣次和用户长期事实都不写入今日农情正文。不要改成关键词判断，也不要把今日农情伪装成远端聊天历史消息
- 后端数据真源是 `daily_agri_cards`，按 `day_cn + scope` 唯一保存；当前 scope 固定为 `CN`。2026-06-18 起支持后台人工发布同一张每日农情：人工发布会写 `source_type=manual / manual_locked=1 / manual_by / manual_at`，适合晚上由 Codex 或人工整理次日 3 条内容后发布；同一天人工锁定后，清晨 ECS 自动生成和后台补跑只复用缓存，不覆盖人工内容。没人人工发布时，原 05:35 自动生成继续兜底
...
- 内部生成接口是 `POST /internal/jobs/today-agri-card/generate`，只给定时任务 / 运维调用，必须携带 `DAILY_AGRI_JOB_SECRET`；生成前用数据库 lease 防并发。内部探针接口是 `POST /internal/jobs/today-agri-card/probe?runs=3`，只测试模型输出、来源、解析质量和 usage 成本，不写 `daily_agri_cards`，最多 5 次一组，仍只供运维内部使用。内部人工发布接口是 `POST /internal/jobs/today-agri-card/manual`，供本机脚本 [scripts/publish-today-agri-manual.ps1](D:/wuhao/scripts/publish-today-agri-manual.ps1) 和 Codex 自动化发布人工整理内容，仍使用同一张 `daily_agri_cards`，必须携带 `DAILY_AGRI_JOB_SECRET`
...
- 今日农情生成最多 2 次，模型调用失败或首轮解析失败时允许第二次换检索提示补救，仍走 `turbo`，不切 `agent`。后端只保留展示所需的最低技术兜底：模型响应能解析出 JSON、必须能取到 3 条标题 / 摘要非空的 item 才发布；超过 3 条只展示前 3 条，少于 3 条视为卡片不完整，不对用户发布。这里的“三条”是唯一硬数量要求，不要求模型必须输出 `card_name=今日农情`，也不按重复标题、主题词、发布日期、近 7 天链接、旧卡链接、养殖关键词、广告词、电商域名、社交域名或标题党拦截发布。内部 URL 只做私网 / 本机等不可追溯或危险地址清理，公开响应不下发链接。今日农情只取种植侧，养殖、水产不要；该方向以及广告软文、假新闻、重复事件和标题党主要通过提示词、内部探针、后台运营复核与 SLS 日志观察控制
- 生成时会读取过去 7 天已 ready 的今日农情内容，把标题 / 摘要 / 来源域名 / 原文短指纹喂给模型，要求今天不要重复同原文、同标题或同一事件；后端不按历史链接 / 标题或重复标题拦截发布
- 今日农情提示词版本当前为 `2026-07-01-v80`：提示词继续保持 3 条 JSON 和种植侧新闻方向，面向普通大众用户，优先近 7 天公开来源、今天或昨天的新进展更优，三条尽量分散地区 / 作物 / 主题 / 选题钩子。v80 在 v79 短段排版基础上补强“用户愿意点开的选题钩子”和“行政材料避让”：优先病虫或灾害风险窗口、作物管理关键期、价格 / 供应 / 库存变化、补贴 / 申报 / 登记节点、新品种 / 新技术 / 农机农资、可复制的地方田管案例；会议、调研、发布会、意见、通知、推进工作、总体适宜、稳中向好、加强指导这类材料本身不优先，只有能翻译成“哪个地区、哪类作物、哪类人现在该注意什么或能用什么”时才写。选题仍不限制具体作物，大田作物、经济作物、设施农业、果树、蔬菜、茶叶等都可以；天气、气象、防灾或抢收可以作为其中一个角度，但不要三条都写成天气预报，且需说明对农事安排、防灾减损或田间管理的影响。养殖、水产、畜牧、猪肉 / 生猪、禽蛋、牛羊奶、饲料、兽药、渔业、鱼虾等主体不要。摘要目标约 90-130 个中文字符，写 2-3 句正常新闻短讯，可用 JSON 转义换行拆短段。以上仍是提示词层面的方向控制，不增加后端内容过滤、关键词拦截、字数硬卡或 `max_tokens`；后端只做 JSON、3 条标题 / 摘要非空和私网 URL 安全兜底，质量继续靠提示词、探针和后台复核观察控制
...
- Android 只把今日农情作为 `ChatTimelineItem.TodayAgriCard` 插入视觉时间线；真实 `messages` 仍只包含用户 / assistant 对话。主聊天视觉上按普通 AI 文本展示“今日农情 · 日期 + 3 条资讯”，标题加粗，序号使用 `一、二、三、` 中文标识，长按复用 AI 文本区同款“复制 / 全文复制”菜单；它仍是同一个正向 `LazyColumn` 里的普通列表项，不做 overlay、sticky 尾卡、浮层、关闭动画或真实聊天消息。没有真实聊天消息时仍只显示欢迎语，今日农情不替代空态；已有完整 AI 回答历史时，它跟在当时最后一条完整回答后方，并保存当天一条主界面展示记录和展示用正文副本，后续重开 / 远端 snapshot hydrate 会按当天保存项恢复这一条，让它像历史文本一样稳定存在。用户后续发送文字 / 图片 / 失败态消息时，新消息自然追加在它后方并把它往上顶；主聊天进入正常聊天流后，用户未主动浏览时按最新视觉尾部回底 / 静态贴底，今日农情在尾部时也参与视觉贴底。主界面展示记录同一天只保留一条；删除历史对话会清掉这条主界面展示记录，同一天不再自动冒回主界面。今日农情不参与真实 `messages`、本地聊天快照、远端聊天历史、记忆文档或问诊扣次，也不能作为最新真实消息锚点；仅通过 `today_agri_context_day` 支持当天视觉项后连续 2 轮临时上下文；不提供手动关闭 / 手动隐藏入口，单条农情不可点击跳外部链接。设置页“今日农情”入口展示最近 30 天已 ready 的标题、摘要和来源名记录，不随聊天历史删除
...
- 支付 Plus 升 Pro 时，后端按 Plus 剩余有效天数折成金额抵扣：Pro 29.9 元减去剩余 Plus 时间价值，抵扣最多按一个 Plus 周期 19.9 元计算，最低补差价 10.00 元；补偿次数已退出当前业务，不再新增、不展示、不作为扣次来源。旧 `upgrade_credits` 表只作为历史遗留数据容器保留，当前支付、礼品卡和账号迁移都不再写入或消费
...
- 当前代码里的超额消耗顺序：先每日额度，再加油包；如果后续业务口径调整，再单独同步改代码和本文
...
- 当前轮输入优先，历史 / 摘要 / 联网 / 后台时间地点只作参考；参考信息与当前输入不一致时，以用户当前输入、当前图片和最新纠正为准
- 记忆摘要仅供参考，用于上下文承接和减少重复追问；除非用户明确要求回顾历史，否则不主动复述记忆摘要内容、小标题或用户画像
- 后台背景信息只作静默参考；回答应聚焦用户本轮问题，非直接相关时不主动提及、展开、串联过往内容，或追加基于背景信息的顺带建议
...
- `ChatRecyclerViewHost.kt` 当前使用：
  ...
  - 默认 `verticalArrangement = Arrangement.Bottom`，用于正向列表短内容不满一屏时也贴到底部工作线；唯一运行时例外是 `InitialWorklinePhase.TopUnreached / TopAnchoring`：清数据 / 删除历史后的首次视觉内容尚未碰到 96dp 工作线前，以及触线后等待安全切回同帧底部锚点接住的极短交接内，临时使用 `Arrangement.Top` 让首屏文本自然从顶部往下排；今日农情作为普通视觉文本项参与该触线判断
  - `messages` 仍按 oldest -> newest 存储；真实消息锚点仍通过 `ChatTimelineItem.Message` 反查 index，不能把今日农情内容写成最新真实消息；非 streaming 的视觉底部锚点可以指向最新 `ChatTimelineItem`，所以今日农情处在视觉尾部时和普通文本一样参与回底 / 静态贴底；今日农情只有在已有完整 AI 回答历史时才按主界面展示记录挂在当时最后一条完整回答后，没有真实消息时仍只显示欢迎语，后续新消息自然排在其后
