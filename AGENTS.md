...
- SLS：阿里云日志服务本体已开通，`GetSlsService` 返回 `Opened`；当前未购买 SLS 节省计划 / 资源包，未创建农技千查专用 Project / Logstore，也未接入 `server-go` 日志采集。北京区已有阿里云系统托管 Project / Logstore（例如 CMS / XTrace / 产品数据），不等于农技千查业务日志已接入
