...
- 记忆文档摘要由后端 `qwen-plus` 异步处理，摘要请求在 OpenAI 兼容模式请求顶层显式设置 `enable_thinking=false`，单次提取请求有 60 秒超时保护；不联网、非流式，不保留其它摘要候选或分层模型灰度。记忆文档使用当前 A 层 6/9 轮窗口更新；同一用户有本进程运行中保护；写回必须匹配触发时的 `round_total`，旧快照结果不能覆盖更新轮次。提取失败或超时会保持 `pending_retry_b`，后续轮次完成后继续补提取。后端不做内容过滤或四段补全，非空模型输出会直接覆盖旧记忆；提示词负责尽量保留旧稳定画像、防止不确定判断变定论、第三方转述变系统事实，以及面积 / 剂量 / 倍数被模型自行换算
...
- 生成链路固定使用 `qwen-plus` 的 DashScope `text-generation/generation` 非流式接口，显式设置 `temperature=0.8`、`enable_thinking=false`、`enable_search=true`、`search_options.search_strategy=turbo`、`search_options.enable_source=true`、`freshness=7` 和 `prompt_intervene`；不保留其它模型、multimodal 或 Responses 候选，不通过环境变量切模型。用户已明确轻量模型和即将下线模型不作为今日农情生产候选
- 今日农情默认不需要结构化 URL 来源列表下发给用户，`enable_source=true` 主要用于事实核对和辅助模型在正文 JSON 里写短来源名称；公开接口只返回标题、摘要和短来源名称，不下发 URL、source_index 或 published_date，Android 单条农情不可点击跳浏览器。默认 qwen-plus 链保留 `prompt_intervene + freshness=7`，不使用 `assigned_site_list`，继续全网宽搜。`agent / agent_max` 属于多轮检索整合且通常带来更多输入 token 和延迟，今日农情默认不使用
- 今日农情生成最多 2 次，第二次只在首轮解析失败后换检索提示补救，仍走 `turbo`，不切 `agent`。后端只保留展示所需的最低技术兜底：模型响应能解析出 JSON、能取到 3 条标题 / 摘要非空的 item，最终存储 / 展示正好 3 条；不要求模型必须输出 `card_name=今日农情`，不按重复标题、主题词、发布日期、近 7 天链接、旧卡链接、养殖关键词、广告词、电商域名、社交域名或标题党做发布硬过滤。内部 URL 只拦私网 / 本机等不可追溯或危险地址，公开响应不下发链接。今日农情只取种植侧，养殖、水产不要；该方向以及广告软文、假新闻、重复事件和标题党主要通过提示词、内部探针、后台运营复核与 SLS 日志观察控制
- 生成时会读取过去 7 天已 ready 的今日农情卡片，把标题 / 摘要 / 来源域名 / 原文短指纹喂给模型，要求今天不要重复同原文、同标题或同一事件；后端不再做历史链接 / 标题或重复标题硬过滤
- 今日农情提示词版本当前为 `2026-06-10-v29`：检索阶段最大限度全网宽搜、不锁固定网站，要求交叉使用近 7 天 / 今日 / 最新 / 地区 / 作物 / 品类 / 农资 / 农时 / 预警 / 价格等词分主题找材料，按种子 / 种苗 / 种业、植保 / 农药 / 除草剂、肥料 / 水肥 / 土壤墒情、农机 / 农时、农产品产地价格 / 批发流通 / 农资供需、补贴 / 保险、明确影响作物和农时的农业气象风险等方向分散找近 7 天具体材料；农业大类按种植和养殖理解，本卡只取种植侧，养殖、水产不要，如果结果偏养殖水产就继续换词找种植侧；普通天气只有明确影响作物、农时、农田管理或防灾减灾时才可选。标题约 10-14 个中文字符、最多不超过 16 个中文字符、一行读完，并尽量包含地区 / 作物 / 品类 / 事件中的具体对象；摘要约 38-56 个中文字符、约 3 行体量；`source_name` 只写机构 / 媒体 / 站点短名，不写文章标题、网页标题、频道页、站点口号、URL、“搜索结果”或“网络来源”；`published_date` 能确认近 7 天发布日期 / 更新日期 / 监测日期 / 复核日期时再写，不能确认就写空字符串，不因日期字段、来源层级或标题不完美而只给 2 条；必须是自然资讯口吻，禁止“值得看 / 参考意义 / 对农户有用 / 根据搜索结果 / 检索显示 / 可关注”等元表达、推荐理由和标题党话术
- 后端只发布可解析 JSON、能取到 3 条标题摘要完整 item 的结果；主题方向、广告软文、假新闻、前端元表达、推荐理由、重复和标题党类内容主要靠提示词控制，后端不再用内容硬过滤把来源卡死。公开响应只包含 `title / summary / source`，不包含 URL、source_index 或条目日期
...
- 记忆文档默认只作后台参考，用于减少重复追问和保持连续性；除非用户明确要求回顾历史，否则不主动复述记忆内容、标签或用户画像
