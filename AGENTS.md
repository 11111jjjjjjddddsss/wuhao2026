...
- 2026-06-27 按用户真机连续断链、首字后卡死、长耗时和识图质量不稳定反馈，旧 `gpt-5.5` 第三方中转站 / Responses 优先主链已退出生产。`CHAT_PRIMARY_*` 不再是受支持的生产配置；readiness 若发现 `CHAT_PRIMARY_ENABLED=true` 应失败提醒移除。2026-06-29 后端新增独立的 `GPT_RELAY_*` 主聊天候选链路，后续生产已启用为“GPT relay 候选 + 百炼 / 千问兜底”，不复用旧变量名；该候选链路只在后端使用 Responses 流式接口，Android 不保存、不注入、不直连任何 GPT Key。2026-07-01 生产环境已因中转联盟首字不稳定收口为“只留大大科技 + 百炼 / 千问兜底”：`GPT_RELAY_PROVIDER_1_*` 中转联盟从 ECS 私密环境移除并保留本机私密备份，线上只保留 `GPT_RELAY_PROVIDER_2_LABEL=大大科技` 和 5 把私密 Key。模型全局固定 `gpt-5.5`，请求固定中等思考、低上下文联网工具、联网一次快速回答提示，不设置温度、`top_p`、输出 token 截断或图片 detail。每一波用户请求只打当前唯一 GPT provider，20 秒内只看后端是否收到该平台上游用户可见正文首字；同平台内开流前失败或可重试状态最多轮 5 把 Key，仍无可见正文就回退百炼 / 千问；一旦已吐出可见正文，不在同一条回复中途切模型。GPT 和千问一样接收主对话锚点、`【输出约束】` / 回答参考范本、时间地点、记忆、历史上下文、本轮文字和图片，GPT 只额外多一段专用联网规则。GPT relay 不再使用 Key 冷却、provider 冷却、进程内熔断、旧连接 / TLS / 响应头阶段切换限制或 7+7 同轮短探针；这些退役变量若残留在生产环境应由 readiness 报错。`/healthz` 只暴露 `disabled / ok / missing_config` 等非敏感状态；后台“模型链路”页可显示非敏感 provider label、provider 槽位、Key 槽位和模型名用于排障，但不展示真实 URL、真实 Key、账号分组、prompt 正文、AI 正文或图片 URL。日志脱敏规则可继续保留旧 `CHAT_PRIMARY_*` 和新 `GPT_RELAY_*` 变量名，避免历史环境或日志泄漏
