...
- 两阶段都只允许拦“越过当前阶段底线/生成行的那一点点”，不锁死整个手势；尤其 `UserBrowsing` 下的向下 fling 不允许再整段吞掉，只保留 drag 级别的越界量限制和停稳后的自然接回
- 完成态如果正文已经吃完第一阶段底线，但最后一条消息因为列表可滚范围不足而贴不回输入框上方，不要把它当成“第一阶段空白残留”；应允许补一段仅用于完成态贴底的静态 `retainedBottomGapPx`，让 final snap 把最后一条稳在底部附近
