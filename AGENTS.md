...
- 记忆文档：Free / Plus 每 6 轮、Pro 每 9 轮触发一次；后端用一份纯文字提示词、一次 `qwen-plus` 调用，覆盖更新一份自然语言记忆文档。物理存储暂沿用 `session_ab.b_summary` 字段，对外接口返回 `memory_document`，`b_summary` 仅作为兼容别名；旧长期记忆列不再使用并由迁移删除。记忆文档固定四段：“短期承接 / 长期背景 / 用户画像 / 农业重点事件”，默认≤900字，复杂场景最多≤1200字。整体策略是覆盖更新而不是归档追加；最近对话、最新纠正和更可靠证据优先，旧记忆只作为融合材料，不得长期堆流水账。模型输出和最终存储都是纯文字，后端只用中文标签拆段；某个标签缺失时优先保留对应旧段，明确写“暂无...”时才覆盖为暂无；完全无可解析标签或模型失败时不写回并保留 `pending_retry_b`。记忆文档不得把“倾向于 / 更像 / 可能 / 不能排除 / 未见明显...”升级成“确诊 / 确认为 / 已排除 / 已证实”等长期结论，除非历史对话里明确有用户复查、检测或人工确认
...
- 记忆文档摘要由后端 `qwen-plus` 异步处理，摘要请求在 OpenAI 兼容模式请求顶层显式设置 `enable_thinking=false`，单次提取请求有 60 秒超时保护；不联网、非流式，不保留其它摘要候选或分层模型灰度。记忆文档使用当前 A 层 6/9 轮窗口更新；同一用户有本进程运行中保护；写回必须匹配触发时的 `round_total`，旧快照结果不能覆盖更新轮次。提取失败或超时会保持 `pending_retry_b`，后续轮次完成后继续补提取
...
- 今日农情生成最多 2 次，第二次只在首轮质量校验失败后换检索提示补救，仍走 `turbo`，不切 `agent`。后端只发布可解析 JSON、固定 `card_name=今日农情`、正好 3 条有效 item、标题 / 摘要非空且同批标题不重复的结果；后端不再用可信域名、主题词、发布日期、近 7 天链接、旧卡链接或养殖关键词大面积硬过滤卡死来源。广告软文、假新闻、养殖侧、重复事件和标题党主要通过提示词、内部探针、后台运营复核与 SLS 日志观察控制
- 生成时会读取过去 7 天已 ready 的今日农情卡片，把标题 / 摘要 / 来源域名 / 原文短指纹喂给模型，要求今天不要重复同原文、同标题或同一事件；后端当前只做同批重复标题兜底，不再硬过滤过去 7 天链接 / 标题
- 今日农情提示词版本当前为 `2026-06-10-v26`：检索阶段保持全网宽搜、不锁固定网站，要求交叉使用近 7 天 / 今日 / 最新 / 地区 / 作物 / 品类 / 农资 / 农时 / 预警 / 价格等词分主题找材料，按种子 / 种苗 / 种业、植保 / 农药 / 除草剂、肥料 / 水肥 / 土壤墒情、农机 / 农时、农产品产地价格 / 批发流通 / 农资供需、补贴 / 保险、明确影响作物和农时的农业气象风险等方向分散找近 7 天具体材料；农业大类按种植和养殖理解，本卡只取种植侧，普通天气只有明确影响作物、农时、农田管理或防灾减灾时才可选。标题约 10-14 个中文字符、最多不超过 16 个中文字符、一行读完，并尽量包含地区 / 作物 / 品类 / 事件中的具体对象；摘要约 38-56 个中文字符、约 3 行体量；`source_name` 只写机构 / 媒体 / 站点短名，不写 URL、频道页、“搜索结果”或“网络来源”；必须是自然资讯口吻，禁止“值得看 / 参考意义 / 对农户有用 / 根据搜索结果 / 检索显示 / 可关注”等元表达、推荐理由和标题党话术
- 后端只发布可解析 JSON、正好 3 条有效 item、标题摘要完整且同批标题不重复的结果；主题方向、广告软文、假新闻、前端元表达、推荐理由和标题党类内容主要靠提示词控制，后端不再用过严硬过滤把来源卡死。公开响应只包含 `title / summary / source`，不包含 URL、source_index 或条目日期
