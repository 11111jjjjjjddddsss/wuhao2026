...
- clean-state / 删除所有历史后的稀疏首屏是当前 96dp 工作线规则的唯一例外：当本轮发送前没有任何用户 / assistant 业务消息时，`ChatScreen.kt` 会临时启用 `cleanStateSparseLayoutActive`，让 `ChatRecyclerViewHost.kt` 用 `Arrangement.Top` 加轻量 top offset 展示早期内容，并暂停普通底部锚点；稀疏模式启用后会先把正向列表钉回顶部，再允许 `canScrollForward` 参与退出判断，避免删除历史前的残留滚动位置把首发提前拉回底部。只有最新真实消息的实测底边已经到达或超过正常 96dp 工作线，或正向列表从顶部还能继续向下滚动（`canScrollForward`）时，才退出稀疏模式并恢复默认 `Arrangement.Bottom` 和 96dp 工作线；如果退出时用户正在拖动、浏览或滚动仍未停止，不主动请求正向底部锚点，避免首轮长回答跨过工作线时把用户强行拉回底部。退出判断必须使用真实布局坐标，文字、图片、图片与文字间距、失败态 footer 等都算，不允许按第几轮、字数或有没有图片做估算；纯图片用户消息也要上报自身内容底边，不长期依赖 LazyList item fallback；`canScrollBackward` 不作为稀疏退出条件
- 发送起步当前保留的旧保护只有两样：
