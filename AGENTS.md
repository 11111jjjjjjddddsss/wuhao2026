...
- 2026-06-27 按用户真机连续断链、首字后卡死、长耗时和识图质量不稳定反馈，旧 `gpt-5.5` 第三方中转站 / Responses 优先主链已退出生产。`CHAT_PRIMARY_*` 不再是受支持的生产配置；readiness 若发现 `CHAT_PRIMARY_ENABLED=true` 应失败提醒移除。2026-06-29 后端新增独立的可选 `GPT_RELAY_*` 主聊天候选链路，默认关闭，不复用旧变量名；未显式配置并开启时，生产主聊天仍只走百炼 / 千问主链。该候选链路只在后端使用 Responses 流式接口，Android 不保存、不注入、不直连任何 GPT Key；GPT 只接收主对话锚点、时间地点、记忆、历史上下文、本轮文字和图片，不带千问专用 `【输出约束】` / 参考范本；请求固定中等思考、低上下文联网工具、联网一次快速回答提示，不设置温度、`top_p`、输出 token 截断或图片 detail。GPT 开流失败或 15 秒无用户可见正文时回退百炼 / 千问；一旦已吐出可见正文，不在同一条回复中途切模型。`/healthz` 和后台只暴露 `disabled / ok / missing_config` 等非敏感状态，不展示真实 URL、Key 数量、账号分组或供应商名。日志脱敏规则可继续保留旧 `CHAT_PRIMARY_*` 和新 `GPT_RELAY_*` 变量名，避免历史环境或日志泄漏
