...
- 生成链路使用 DashScope 原生 Generation 协议调用 `qwen-plus`，显式设置 `temperature=0.8`，显式关闭思考模式，开启强制联网搜索，`search_strategy=turbo`，`enable_source=true`，并要求输出高质量候选；`freshness=7` 和 `prompt_intervene` 只用于引导近 7 天种植侧生产经营主题搜索：种子 / 种苗 / 种业、农资、肥料、农药、植保、农机、农产品价格流通、补贴保险，以及明确影响作物或农时的农业气象风险，不再用 `assigned_site_list` 硬限定站点。农业大类按种植和养殖理解，今日农情只取种植侧，养殖侧全部排除；普通天气预报、生活天气、旅游出行天气不作为独立选题，只有明确关联作物、农时、农田管理或防灾减灾时才可选。今日农情主动排除畜牧、水产、养殖、动物疫病、生猪、猪肉、猪价、家禽、禽蛋、蛋鸡、肉鸡、牛羊、肉牛、肉羊、奶牛、奶业、饲料、兽药、渔业、水产养殖、鱼虾等非种植领域，后端也会硬过滤这类候选。`agent / agent_max` 属于多轮检索整合且会额外收费，今日农情默认不使用。2026-06-09 生产探针确认 `qwen-flash + turbo + enable_source` 可返回来源链接但执行力不足，`qwen-plus + turbo + enable_source` 可拿来源且质量更适合作为当前验证主线；`qwen3.5-flash + turbo` / `qwen3.5-flash + agent` 均返回 `400 InvalidParameter / url error`，`qwen3.5-plus` 只有走 Responses `web_search` 才能拿 sources，不适合作为当前低成本 turbo 主线。用户端已经取消外部链接点击，公开接口只返回标题和摘要；搜索来源 URL、source_index、source、published_date 只作为服务端事实核对、去重、后台排查和内部探针字段，不下发给 Android 用户卡片。后端最多展示 3 条，只有 2 条通过质量校验时也允许发布；生成最多 2 次，第二次只在首轮质量校验失败后换检索提示补救，仍走 `turbo`，不切 `agent`。解析时只信 `source_index` 对应的 DashScope 搜索来源 URL，忽略模型自拟 `link_url / url`；如果来源缺失或指向首页 / 栏目页，内容本身仍可作为候选，但 URL 只留作内部追溯，不作为用户展示条件。首页、栏目页、索引页、专题页、iframe、pdf、非可信 http 链接和非种植领域来源不得作为高质量发布依据，避免为了凑满 3 条把低质新闻塞到前端
- 生成时会读取过去 7 天已 ready 的今日农情卡片，把标题 / 摘要 / 来源域名 / 原文短指纹喂给模型，要求今天不要重复同原文、同标题或同一事件；后端也会硬过滤过去 7 天和当天候选里的重复链接 / 重复标题
- 今日农情提示词按农业实用价值排序，优先具体地区、具体作物 / 品类、明确风险 / 农时 / 价格 / 补贴 / 流通影响的信息；标题和摘要必须是自然资讯口吻，禁止“值得看 / 参考意义 / 对农户有用 / 根据搜索结果”等元表达、推荐理由和标题党话术
- 后端只发布可解析 JSON、2 到 3 条有效 item、近 7 天或来源时间合理、标题摘要完整的结果；广告、导购、软文、模型 / 提示词泄露、前端元表达、推荐理由和标题党类内容直接过滤，过滤后不足 2 条则不发布新卡片。公开响应只包含 `title / summary`，不包含 URL、来源或条目日期
