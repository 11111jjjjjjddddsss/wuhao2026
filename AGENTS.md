...
- 域名：`nongjiqiancha.cn` 已在阿里云购买，用户口头确认域名实名认证 / 模板审核已通过；当前用于 `api.nongjiqiancha.cn`、根域名官网和后续下载 / 管理后台域名。当前已创建 `api`、`@`、`www` A 记录指向 ECS 公网 IP `39.106.1.151`，并已完成 `api.nongjiqiancha.cn` HTTPS / Nginx 443 反代和 `nongjiqiancha.cn` / `www.nongjiqiancha.cn` 官网 HTTPS 静态站；网站 ICP 备案已通过，App 备案已提交阿里云初审但尚未通过，公安联网备案待提交，下载域名 / 管理后台域名仍需后续单独配置入口和证书
...
- 时间 / 地点：后端每轮主对话必须注入当前时间、用户地点和地点可信度；历史轮次如果有后端 `created_at / region / region_source / region_reliability`，进入模型上下文时也要带轻量时间 / 地点前缀。时间以后端服务器时间为准，不用前端手机时间当业务真相；前端暂不显示每条消息时间戳或地点条。当前 Nginx 会透传真实客户端 IP 给 Go 服务，`GetClientIP` 可用于限流和日志脱敏；未传 `X-User-Region` 时，`ResolveRegionByIP` 会优先使用 ECS 本地免费离线 `ip2region` xdb 文件（`IP2REGION_V4_XDB_PATH` / 兼容 `IP2REGION_XDB_PATH`）把公网 IP 粗定位到省 / 市级，结果只作为 `region_source=ip`、`region_reliability=unreliable` 的低可信参考，不走收费 API、不放 RDS、不把完整 IP 注入模型；库未配置、私网 IP、代理漂移或查询失败时仍兜底 `未知 / unreliable`。后续若要更准，只能单独评估 Android 可选定位权限，并同步隐私政策 / Manifest / 上线回归
