...
- 2026-06-06 后端最新已通过 [scripts/deploy-ecs-server.ps1](D:/wuhao/scripts/deploy-ecs-server.ps1) 部署到 ECS，并迁移为单机双端口 slot 发布；[scripts/check-ecs-readiness.ps1](D:/wuhao/scripts/check-ecs-readiness.ps1) 显示 Nginx active upstream 为 `3000`、`nongji-server-3000 active/enabled`、Host healthz 200。阿里云融合认证 Android 方案、DYPNS AccessKey / Secret、`DYPNS_FUSION_SCHEME_CODE`、包名和签名、短信资质 / 签名 / 验证码模板、Redis、OSS 上传后端、DashScope 主 / 副模型 Key 主备槽位、后台监控面板前置内部接口和最小操作审计均已进入生产代码 / 环境并通过健康检查。2026-06-05 网站 ICP 备案已通过，`api.nongjiqiancha.cn` HTTPS 已配置并公网验证通过，2026-06-06 根域名官网 HTTPS 已部署并公网验证通过，DashScope 主 / 副模型 Key 已写入 ECS 主备槽位并显示 `bailian=ok`，App 备案已提交阿里云初审但尚未通过；公网 SSH 22 已撤销且 ECS 本机 ssh 服务已停用，当前公网只放行 80 / 443 和 ICMP；公安备案、一键登录 / 主聊天真机回归和上线前 AccessKey 轮换仍缺
