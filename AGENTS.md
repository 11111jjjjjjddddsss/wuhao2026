...
- B 层短期记忆 / C 层长期农业记忆由后端 Qwen3.5-Flash 异步处理，摘要请求显式设置 `enable_thinking=false`；B 层使用当前 A 层 6/9 轮窗口更新，C 层使用 `session_round_archive` 最近 20 轮完整问答更新；同一用户同一层有本进程运行中保护；摘要写回必须匹配触发时的 `round_total`，旧快照结果不能覆盖更新轮次。提取失败会保持对应 `pending_retry_b / pending_retry_c`，后续轮次完成后继续补提取
