...
- RecyclerView 发送起步改成普通从上往下布局，由发送后一次性的 `scrollToPositionWithOffset(...)` 直接按 assistant 项顶边定位到发送起步目标，不再按 waiting 当前高度反推底边。
...
- 发送起步这次手动定位直接按 assistant 项顶边锚定，不再按 waiting 当前高度反推底边；这样 waiting 变成正文后只会继续向下长，不应再把起步上抬感明显吃掉。
