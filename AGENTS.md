...
- 第一段的判定不能直接用“当前视口里的 `contentBottom < legalBottom`”反复计算；用户上滑浏览历史时这个量也会变小，容易把浏览态误判回早期锁死态。应改成“本轮流式是否已经真正吃到锚点空白边界”的阶段标记：未吃满前，往底部方向 drag / fling 直接整段拦；一旦本轮已吃满，就永久切到第二段，不再因为当前视口位置回退
