...
部署与基础设施：
- 部署：首版线上后端路线已从“SAE 镜像托管优先”转向“ECS 传统部署优先”。当前已购买阿里云 ECS `i-2ze5nrem0jrchln4f0eh`，位于 `华北2（北京）/ cn-beijing` 可用区 L，规格 `ecs.u1-c1m2.large`（2 vCPU / 4 GiB），Ubuntu 22.04 64 位，包年包月到期 `2027-05-24`，公网 IP `39.106.1.151`，私网 IP `192.168.1.237`，生产 VPC `nongjiqiancha-prod-vpc` / `vpc-2zeax2zowza2398b9dzot`，生产交换机 `nongjiqiancha-prod-beijing-l` / `vsw-2zemsq82lj2kp8za90aky`，安全组 `sg-2ze4tilwxw1h5w77lwl1`，固定公网带宽 5 Mbps。2026-05-30 已清理北京区空闲默认 VPC 和旧 SAE 自动交换机，当前北京区只保留这一条生产 VPC / 交换机。ECS 已通过 Cloud Assistant 初始化 `nongji` 系统用户、`/opt/nongjiqiancha/server`、`/etc/nongjiqiancha/server.env`、`nongji-server.service`、Nginx 反向代理、基础限流、fail2ban 和 logrotate；`server-go` 已部署并运行在 `127.0.0.1:3000`，Nginx 按 `api.nongjiqiancha.cn` 代理，Go 进程默认监听 `127.0.0.1:${PORT:-3000}`，只有显式配置 `LISTEN_ADDR` 或 `LISTEN_HOST` 才会改为其他监听地址，避免绕过 Nginx 直接暴露业务端口；生产健康检查以 readiness 脚本为准，当前为 `ok=true / auth_strict=true / bailian=ok / dypns=ok / dypns_fusion=ok / dypns_sms=ok / dev_order_endpoints=false / redis=ok / upload_storage=oss`。阿里云 DNS 已创建 A 记录 `api.nongjiqiancha.cn -> 39.106.1.151`；2026-06-05 已通过 Let’s Encrypt / certbot 为 `api.nongjiqiancha.cn` 配置 Nginx 443 HTTPS，公网 `https://api.nongjiqiancha.cn/healthz` 和 HTTP healthz 均返回 200，证书自动续期 timer 已启用；网站 ICP 备案已于 2026-06-05 管局审核通过，主体备案号为 `京ICP备2026031728号`，网站备案号为 `京ICP备2026031728号-1`，网站名称“农技千查”，域名 `nongjiqiancha.cn`；2026-06-05 20:03 左右 App 备案订单已提交阿里云初审，订单号 `2036780517515`，页面提示预计 2026-06-07 20:00 前审核，App 备案号尚未下发；公安联网备案已生成数据码但尚未提交到全国互联网安全管理服务平台，数据码不进入仓库；2026-06-05 已通过 Cloud Assistant 将 DashScope 主 / 副模型 Key 写入 ECS `/etc/nongjiqiancha/server.env` 主备槽位并重启服务，健康检查显示 `bailian=ok`，真实 Key 值不进入仓库、文档或聊天记忆；公网完整上线仍等 App 备案通过、公安备案和真机登录 / 主聊天 / 图片问诊回归。此前曾在 `华北2（北京）/ cn-beijing` 创建标准版 SAE 应用 `nongjiqiancha`，AppId `366147d5-3760-4548-bd68-f38debbc5f23`，规格 `0.5 核 / 1GB / 单实例`，自动弹性未开启；该 SAE 应用只是默认 demo 镜像，未部署 `server-go` 真实后端，已于 2026-05-24 21:50 左右先停止，并于 2026-05-24 21:51 左右通过阿里云 CLI 删除，删除变更单 `14a360d3-e2b4-4b93-9701-b76dfcc7bfd9` 已提交成功；删除后 `ListApplications` 返回空列表，`TotalSize=0`。运维侧本机已安装阿里云 CLI，默认 Region 为 `cn-beijing`；真实 AccessKey 只允许保存在本机 CLI 配置或云端密钥管理中，不允许写入仓库、聊天记忆或文档
- 2026-06-05 后端最新已通过 [scripts/deploy-ecs-server.ps1](D:/wuhao/scripts/deploy-ecs-server.ps1) 部署到 ECS，并复查 readiness 通过；阿里云融合认证 Android 方案、DYPNS AccessKey / Secret、`DYPNS_FUSION_SCHEME_CODE`、包名和签名、短信资质 / 签名 / 验证码模板、Redis、OSS 上传后端、DashScope 主 / 副模型 Key 主备槽位、后台监控面板前置内部接口和最小操作审计均已进入生产代码 / 环境并通过健康检查。2026-06-05 网站 ICP 备案已通过，`api.nongjiqiancha.cn` HTTPS 已配置并公网验证通过，DashScope 主 / 副模型 Key 已写入 ECS 主备槽位并显示 `bailian=ok`，App 备案已提交阿里云初审但尚未通过；公安备案、一键登录 / 主聊天真机回归和上线前 AccessKey 轮换仍缺
...
- 域名：`nongjiqiancha.cn` 已在阿里云购买，用户口头确认域名实名认证 / 模板审核已通过；后续用于 `api.nongjiqiancha.cn`、下载域名和管理后台域名。当前已创建 `api` A 记录指向 ECS 公网 IP `39.106.1.151`，并已完成 `api.nongjiqiancha.cn` HTTPS / Nginx 443 反代；网站 ICP 备案已通过，App 备案已提交阿里云初审但尚未通过，公安联网备案待提交，根域名官网 / 下载域名 / 管理后台域名仍需后续单独配置入口和证书
...
- SLS：阿里云日志服务本体已开通，`GetSlsService` 返回 `Opened`；当前未购买 SLS 节省计划 / 资源包，未创建农技千查专用 Project / Logstore，也未接入 `server-go` 日志采集。北京区已有阿里云系统托管 Project / Logstore（例如 CMS / XTrace / 产品数据），不等于农技千查业务日志已接入。App 侧当前只新增最小自动日志接收骨架：Android 在关键失败点自动调用后端 `POST /api/app/logs`，后端写入 `client_app_logs` 表并打结构化服务日志；后端另有 `GET /internal/app/logs` 只读内部查询，暂复用 `SUPPORT_ADMIN_SECRET`，用于后续网页管理后台 / 监控面板接入；`admin_audit_logs` 和 `GET /internal/admin/audit-logs` 已作为最小内部操作审计地基落地，现有内部日志查询、帮助与反馈内部会话列表 / 读取 / 回复、今日农情内部生成会写审计元信息，但仍不是完整后台账号、角色权限、SLS 采集或告警系统；不做用户手动上传日志，不上传聊天正文、图片内容、图片 URL、token、手机号或模型 Key
...
如果后续长期让 Codex 辅助发版、回滚、查日志、查库，应把入口固化进仓库，例如部署脚本、回滚脚本、日志脚本、数据库只读脚本。整体 App / 后端 / 管理后台的长期协助运维蓝图以 [docs/runbooks/operations-blueprint.md](D:/wuhao/docs/runbooks/operations-blueprint.md) 为入口；ECS / RDS / OSS / Redis、真实后端部署、`api` DNS A 记录、`api` HTTPS、网站 ICP 备案和 App 备案提交已落地，未落地的是 App 备案通过 / 备案号、公安联网备案、农技千查专用 SLS、后台账号和权限；未落地项不伪造服务地址、密钥或后台入口，只记录待回填项。
