...
- 2026-04-05 生成阶段底部护栏统一为“只按真实正文尾部与当前合法底线消费越界量”；drag / fling 同口径，不整段锁死手势
- 2026-04-05 生成阶段 `回到底部` 按钮只在 `UserBrowsing` 且已离开工作线/返回线区域时允许显示；`Idle / AutoFollow` 必须隐藏；静态完成态按钮只按“普通列表当前仍可继续向下滚动”派生，不再依赖旧的 overflow 阈值；按钮脉冲与滚动链写状态解耦
