...
- 开机历史态 / 完成态的底部 reserve 在“输入为空、无 focus、IME 收起、composer 非 settling、非发送锁”的折叠稳定窗口中，必须优先使用同拍实测的 `measuredComposerHeightPx + STREAM_VISIBLE_BOTTOM_GAP`，不能只吃启动估值或旧观察值；否则真机输入框真实高度大于估值时，工作线以下空白露不全，用户还能继续往上扒
