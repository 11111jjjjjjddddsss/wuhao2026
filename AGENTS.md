...
- 发送起步不再依赖整表快照或本轮消息对隐藏来遮坏帧
- waiting / streaming / settled 继续共用同一个 assistant 宿主，waiting 阶段优先保持与首行 streaming 接近的真实占位，减少首字出现时的几何突变
- 发送起步时序以“先提交新数据，再在 observer / pre-draw 后对齐”为准，不再先在旧列表上预滚
