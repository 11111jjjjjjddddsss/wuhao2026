...
- 流式阶段底部工作线 reserve 不应继续作为 LazyColumn 尾部可滚动 item 存在；应优先作为消息视口自身的非滚动底部预留承载
- 流式阶段用户从历史位置向下滑回底部工作线时，恢复自动跟随应直接滚到去掉锚点 spacer 的底线，不要继续等待尾部 spacer 全部滚完
- 流式阶段如果用户继续向下拖并且已经碰到底部工作线，应在拖动阶段直接拦住，不能等空白先被拖出来后再恢复跟随
- 不要再乱调无关参数
- 不要顺手改输入区
