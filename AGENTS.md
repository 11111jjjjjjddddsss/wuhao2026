...
- 所有模型输出方向统一靠提示词、探针、人工复核和运营观察控制，不能新增后端内容过滤、关键词拦截、相似度硬拦截、摘要 / 标题 / 正文字数硬卡点、`max_tokens` 截断或按主题词卡发布 / 卡写入 / 卡展示。工程安全兜底只允许用于请求体大小、JSON 可解析、必要结构字段、日志脱敏、URL 安全、密钥保护、响应读取上限等稳定性和安全边界，不得变成限制模型表达方向的内容准入。
