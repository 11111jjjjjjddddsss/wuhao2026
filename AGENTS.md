...
- Android 首次隐私同意当前合并在登录页协议勾选中：未同意时可以看到登录页 UI，但不会初始化 `IdManager`、不会补报待发送崩溃日志，不会请求后端、拉起一键登录 SDK 或申请电话状态权限；用户勾选后才记录同意并进入正常登录链路，遗留后台待发送图文 Worker 也仍然等同意后才发起业务请求。
