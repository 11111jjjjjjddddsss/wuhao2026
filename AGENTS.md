...
- 摘要模型：固定 `qwen-plus`，用于一次性提取一份记忆文档；摘要请求走 OpenAI 兼容模式 HTTP 原始接口，`enable_thinking=false` 必须放在请求顶层，不联网、非流式。后端不再保留轻量摘要候选，不再保留短期 / 长期分层灰度；旧长期记忆列和旧重试列会由迁移合并进现有记忆文档 / pending 标记后删除。`qwen-plus` 资源包只适合非思考实时推理且单次输入不超过 128K 的请求；摘要通常输入大、输出短，资源包是否省钱取决于输出占比，不能因为有资源包就认为一定省钱。2026-06-10 远端实测发现放在 `extra_body` 内不会真正关闭思考，会产生大量 `reasoning_tokens` 和明显更高延迟；顶层关闭后同类摘要请求回到正常 token 量级。用户已拍板质量优先，记忆文档先统一用 `qwen-plus`。记忆提示词要在“能存住”和“别瞎说”之间折中：旧长期背景 / 用户画像未被否定时继续保留；第三方转述保留来源和不确定边界；剂量、面积、倍数、浓度、兑水量、时间等关键参数照抄不换算；没有复查、检测或人工确认时，不把倾向判断写成诊断定论；单次回答方式要求先放短期承接，多轮反复或用户明确长期偏好时再写成稳定画像
...
- 今日农情生成最多 2 次，第二次只在首轮解析失败后换检索提示补救，仍走 `turbo`，不切 `agent`。后端只保留展示所需的最低技术兜底：模型响应能解析出 JSON、至少能取到 2 条标题 / 摘要非空的 item 即可发布；提示词目标仍是 3 条，超过 3 条只展示前 3 条，不再把“必须正好 3 条”作为发布或 Android 展示硬闸门；1 条视为卡片不完整，不对用户发布。不要求模型必须输出 `card_name=今日农情`，不按重复标题、主题词、发布日期、近 7 天链接、旧卡链接、养殖关键词、广告词、电商域名、社交域名或标题党拦截发布。内部 URL 只做私网 / 本机等不可追溯或危险地址清理，公开响应不下发链接。今日农情只取种植侧，养殖、水产不要；该方向以及广告软文、假新闻、重复事件和标题党主要通过提示词、内部探针、后台运营复核与 SLS 日志观察控制
- 生成时会读取过去 7 天已 ready 的今日农情卡片，把标题 / 摘要 / 来源域名 / 原文短指纹喂给模型，要求今天不要重复同原文、同标题或同一事件；后端不按历史链接 / 标题或重复标题拦截发布
- 今日农情提示词版本当前为 `2026-06-11-v59`：检索阶段最大限度全网宽搜、不锁固定网站，质量优先级是“近 7 天真实具体材料 > 种植侧大类正确 > 信息对生产或流通有直接意义 > 三条尽量不雷同 > 手机卡片好读”。大类边界要清楚：本卡只取种植侧，覆盖粮油、蔬菜、果品、棉花、茶桑、种子种业、植保病虫草害、农药肥料农资、农机农时、农田水利、明确影响作物生产的农业气象 / 灾害预警 / 田管措施、种植侧产地 / 批发 / 农资价格、补贴保险和技术推广；养殖、水产、畜牧、动物疫病、生猪、猪肉、猪价、家禽、禽蛋、牛羊、奶业、饲料、饲用原料、兽药、渔业、鱼虾等不要。即使来源把猪肉、生猪、禽蛋、水产、饲料说成影响农户收入、消费预期、农资购买力或农业整体行情，也不作为本卡条目；不要用“间接影响种植户”作为保留理由。大类边界清楚，小类不要抠太细：天气、农时进度、价格、政策、补贴、平台建设、技术推广都可以选，只要近 7 天、真实具体、明确服务种植生产或种植侧流通即可；同等质量下尽量分散主题，如果某类材料明显更真实、更新、更有直接影响，可以出现两条，但不能把同一事件、同一材料或同一组综合行情拆条。标题约 10-14 个中文字符、最多不超过 16 个中文字符、一行读完；摘要约 90-130 个中文字符、尽量不低于 85 个中文字符、手机卡片约 3-4 行体量；至少两句，补清事实后的影响、农时窗口、风险点、供应变化、农资 / 流通影响或基层可留意的事项，宁可写到 100 字左右也不要收成 70 多字的薄摘要，第二句不要只剩“需关注”“可留意”这类很短的收尾；若标题 / 摘要以猪肉、生猪、禽蛋、水产、饲料等为主体也先重写，要让普通人知道这条新闻和种植生产、农资、农时、防灾或流通有什么关系；价格类标题和摘要都要写出具体作物、品类、产区、市场或农资对象，不用“农产品价格”“批发价微降”“市场价格变化”这类泛泛标题独立成条；`source_name` 只写机构 / 媒体 / 站点短名，不写文章标题、网页标题、频道页、站点口号、URL、“搜索结果”或“网络来源”；`published_date` 能确认才写，不能确认就写空字符串，不因日期字段、来源层级或标题不完美而只给 2 条。提示词继续压住旧闻、过期材料、综合指数 / 综合行情凑数、养殖 / 饲料行情混入、广告软文、谣言假新闻、空泛会议 / 成果 / AI 工具新闻、标题党和模型编造数字；数字、指数、价格、比例、面积、补贴金额和进度必须来自来源材料，不确定就省略，尤其 7.82% 不能写成“近八成”。必须是自然资讯口吻，禁止“值得看 / 参考意义 / 对农户有用 / 根据搜索结果 / 检索显示 / 可关注”等元表达、推荐理由和标题党话术；这仍是提示词层面的方向控制，不增加后端内容过滤或字数拦截；养殖水产边界也继续靠提示词和探针观察控制
- 后端只发布可解析 JSON、至少 2 条标题摘要完整 item 的结果；主题方向、广告软文、假新闻、前端元表达、推荐理由、重复和标题党类内容主要靠提示词控制、探针抽查和后台复核，不靠后端按内容拦截。公开响应只包含 `title / summary / source`，不包含 URL、source_index 或条目日期
