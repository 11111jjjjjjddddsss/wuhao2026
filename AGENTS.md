...
- 2026-06-26 起，主对话支持可选的后端优先中转站链路：配置 `CHAT_PRIMARY_ENABLED=true`、`CHAT_PRIMARY_BASE_URL`、`CHAT_PRIMARY_API_KEY` 后，主聊天会先把同一套后端组装好的 messages 发给中转站流式模型，默认模型名 `gpt-5.5`，显式 `temperature=0.8`、`enable_thinking=false`、`enable_search=true`，不发送 `top_p`；生产可用 `CHAT_PRIMARY_FORCE_SEARCH=true` 强制该优先链路联网。中转站只在开流前使用，连接失败、超时、非 2xx 或非 SSE 时快速回落原 `qwen3.5-plus` 主备 Key 池；一旦任一上游 SSE 已成功开始，不在同一条回复中途切模型，避免半条回答和重复成本。Android 仍不保存、不注入、不直连任何模型 Key；中转站密钥只能在服务器环境变量 / 本机私密配置里保存，不进仓库、文档、日志或聊天复述。该链路会把用户文本和普通问诊图片一起发给第三方中转服务，属于数据处理和可用性风险，正式长期使用前需继续观察质量、首字延迟、图片能力、联网稳定性、成本和隐私责任
