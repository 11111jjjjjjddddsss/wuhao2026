...
- 摘要模型：Qwen3.5-Flash，用于 B 层通用短期记忆、C 层长期通用记忆；摘要请求走 OpenAI 兼容模式 HTTP 原始接口，`enable_thinking=false` 必须放在请求顶层。2026-06-10 远端实测发现放在 `extra_body` 内不会真正关闭思考，会产生大量 `reasoning_tokens` 和明显更高延迟；顶层关闭后同类摘要请求回到正常 token 量级
...
- B 层通用短期记忆 / C 层长期通用记忆由后端 Qwen3.5-Flash 异步处理，摘要请求在 OpenAI 兼容模式请求顶层显式设置 `enable_thinking=false`，单次提取请求有 60 秒超时保护；B 层使用当前 A 层 6/9 轮窗口更新，C 层使用 `session_round_archive` 最近 20 轮完整问答更新且仍为每 20 轮一次；同一用户同一层有本进程运行中保护；摘要写回必须匹配触发时的 `round_total`，旧快照结果不能覆盖更新轮次。提取失败或超时会保持对应 `pending_retry_b / pending_retry_c`，后续轮次完成后继续补提取
...
- 生成链路使用 DashScope 原生 text-generation 协议调用 `qwen-plus`，显式设置 `temperature=0.8`，显式关闭思考模式，开启强制联网搜索，`search_strategy=turbo`，`enable_source=true`，并要求输出高质量候选；`freshness=7` 和 `prompt_intervene` 只用于引导近 7 天种植侧生产经营主题搜索：种子 / 种苗 / 种业、农资、肥料、农药、植保、农机、农产品价格流通、补贴保险，以及明确影响作物或农时的农业气象风险，不再用 `assigned_site_list` 硬限定站点。农业大类按种植和养殖理解，今日农情只取种植侧；普通天气预报、生活天气、旅游出行天气不作为独立选题，只有明确关联作物、农时、农田管理或防灾减灾时才可选。畜牧、水产、养殖、动物疫病、生猪、猪肉、猪价、家禽、禽蛋、蛋鸡、肉鸡、牛羊、肉牛、肉羊、奶牛、奶业、饲料、兽药、渔业、水产养殖、鱼虾等非种植领域主要靠提示词排除，不再用后端主题词硬过滤卡死来源。`agent / agent_max` 属于多轮检索整合且会额外收费，今日农情默认不使用。2026-06-09 生产探针确认 `qwen-flash + turbo + enable_source` 可返回来源链接但执行力不足，`qwen-plus + turbo + enable_source` 可拿来源且质量更适合作为当前生产主线；2026-06-10 按客服口径确认 `qwen3.5-flash` 走 text-generation 直接换模型会返回 `400 InvalidParameter / url error`，但走 `multimodal-generation/generation + stream=true + enable_thinking=true + search_strategy=turbo + enable_source=true` 可通，作为后续降本备用链单独适配。用户端已经取消外部链接点击，公开接口只返回标题、摘要和短来源名称；搜索来源 URL、source_index、published_date 只作为服务端事实核对、后台排查和内部探针字段，不下发给 Android 用户卡片。后端只发布正好 3 条完整内容，不再把 2 条作为正常展示结果；生成最多 2 次，第二次只在首轮质量校验失败后换检索提示补救，仍走 `turbo`，不切 `agent`。解析时优先使用 `source_index` 对应的 DashScope 搜索来源 URL，忽略模型自拟 `link_url / url`；如果来源缺失或指向首页 / 栏目页，内容本身仍可作为候选，URL 只留作内部追溯，不作为用户展示条件，用户侧来源名优先来自模型 / 搜索来源名称，缺失时用安全域名短标签兜底。后端解析器只保留 JSON 结构、固定标题、正好 3 条、标题 / 摘要非空、同批重复标题和私网 / 明显电商 URL 清洗等低风险兜底；广告软文、假新闻、养殖侧、重复事件和标题党主要通过提示词、探针、后台运营复核与日志观察控制
- 今日农情降本候选口径：`qwen3.5-flash` 不能直接替换当前 text-generation 生产链，直接换模型会复现 `400 InvalidParameter / url error`；2026-06-10 按客服口径用 `multimodal-generation/generation + stream=true + enable_thinking=true + search_strategy=turbo + enable_source=true` 生产探针返回 200，约 4.6 秒、753 tokens，其中 reasoning 约 363 tokens，能生成 3 条内容，质量初看可用。它是真实可行的降本备用方向，但不是当前生产接口链，缺口是没有像 `qwen-plus + text-generation + enable_source` 一样返回稳定 `search_results` 来源列表，且输出可能包代码块。若后续要降本，只能单独做 3.5flash multimodal 流式备用链、加强 JSON 清洗 / 来源名兜底 / usage 采集 / 灰度探针后再切；不要直接把 `dailyAgriCardModel` 改成 3.5flash。用户已明确 `qwen-flash` 质量不行、`qwen-turbo` 即将下线，二者都不作为今日农情生产候选
