...
- 2026-06-27 按用户真机连续断链、首字后卡死、长耗时和识图质量不稳定反馈，`gpt-5.5` 第三方中转站 / Responses 优先主链已退出生产和当前代码。生产 ECS 已热清 `CHAT_PRIMARY_*` 并回到千问主链；后端代码已删除 `PrimaryChatClient`、`/v1/responses` 转换器、`chat_primary` health/admin 口径和主聊天中转回落路径。`CHAT_PRIMARY_*` 不再是受支持的生产配置；readiness 若发现 `CHAT_PRIMARY_ENABLED=true` 应失败提醒移除。日志脱敏规则可保留旧变量名，避免历史环境或日志泄漏
