...
- Cloud Assistant 上传远端脚本统一走 [cloud-assistant-safe.ps1](D:/wuhao/scripts/cloud-assistant-safe.ps1)。2026-07-01 起，较长脚本会自动分片 `SendFile` 并在 ECS 本机拼回目标脚本，避免 Windows 本机命令行长度限制导致 readiness、部署、日志查询或巡检脚本报“文件名或扩展名太长”；该 helper 只改变运维脚本上传方式，不改变生产服务进程、Nginx 或环境变量。
...
- 已配置基础 IP 级限流：
  ...
  - 主聊天流：`60r/m`，burst 80
  - 上传提交 `/upload`：`20r/m`，burst 8，不设置 `limit_conn`
  - 单 IP 连接数：普通 API 20；2026-06-13 已移除主聊天流 `limit_conn`，2026-06-28 已移除上传提交 `/upload` 的 `limit_conn`，并为图片读取 `/uploads/` 同时去掉 `limit_req` 和 `limit_conn`，避免正常 4 图问诊、模型供应商共享出口 IP、同轮多图和模型重试拉图被误伤
