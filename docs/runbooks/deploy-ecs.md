...
- 2026-06-01 08:52（北京时间）已通过 Cloud Assistant 将后端源码包 `9be4cc76` 部署到 ECS：分片上传源码包、ECS 上校验 SHA-256、运行 `go test ./...`、编译、备份旧二进制、替换并重启 `nongji-server`；重启瞬间 Nginx healthz 曾短暂 502，随后 readiness 复查显示 systemd active、Nginx 配置 OK、Host healthz 200。
...
3. 配置阿里云手机号认证 `DYPNS_*`、短信签名模板，并导入 Android 一键登录 SDK。
