...
- 2026-06-01 08:29（北京时间）已通过 Cloud Assistant 将后端源码包 `5b86941b` 部署到 ECS：分片上传源码包、ECS 上校验 SHA-256、运行 `go test ./...`、编译、备份旧二进制、替换并重启 `nongji-server`；延迟复查直接 Go 端和 Nginx Host healthz 均正常。
