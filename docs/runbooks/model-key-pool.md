...
- 仓库最新待部署代码中的 GPT relay 首字预算为 15 秒：从后端收到用户请求开始算，包含 provider 起手选择、同平台 key 切换、连接、TLS、响应头等待和模型首个可见正文前的等待。15 秒内没有上游可见正文首字，就回退 Bailian / Qwen，不在同一轮再试另一家 GPT provider。
