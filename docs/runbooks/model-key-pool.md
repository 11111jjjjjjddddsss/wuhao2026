...
- 当前后端会对主对话 `qwen3.5-plus`、记忆文档摘要 `qwen-plus`、今日农情 `qwen-plus` 共用同一个 Key 池，按配置顺序做主备使用。记忆文档摘要和今日农情当前都固定 `qwen-plus`，不再保留轻量模型候选或环境变量切换入口。
