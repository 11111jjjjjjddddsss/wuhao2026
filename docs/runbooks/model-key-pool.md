...
- GPT relay 请求固定 `reasoning.effort=medium`、`web_search.search_context_size=low`、`tool_choice=auto` 和“用户明确要求查 / 实时信息才联网，必须只搜索一次、快速回答”的联网规则；当前代码不会读取环境变量把它改成 `high / auto / large`，避免误开高成本路径。
