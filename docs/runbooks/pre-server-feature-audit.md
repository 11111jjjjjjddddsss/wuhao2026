...
结论：支付宝 APP 支付代码已进入仓库联调阶段，但生产收费仍不能直接放量。当前主链已经从“安全占位”推进为“服务端创建支付宝订单 + Android 调起支付宝 SDK + 支付宝异步通知验签 + 幂等发会员 / 加油包权益 + 后台只读核查”；真正对外收费前仍必须完成生产密钥配置、小额真机实付、异步通知重复到达、权益发放、异常补偿、对账和退款验收。
...
- 后台订单页只读核查支付宝订单和权益发放状态，不提供手工伪造支付成功、直接改权益、退款或对账写入口。
