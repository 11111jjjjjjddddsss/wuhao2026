...
- Android 只把 ready 内容作为 `ChatTimelineItem.TodayAgriCard` 插入视觉时间线；真实 `messages` 仍只包含用户 / assistant，不写本地聊天快照、不写远端聊天历史、不写记忆、不写归档、不扣次，也不能作为最新真实消息锚点。视觉上按普通 AI 文本展示“今日农情 · 日期 + 3 条资讯”，标题加粗，序号使用 `一、二、三、`，长按复用 AI 文本区同款“复制 / 全文复制”菜单。当天内容加载时没有真实消息，则作为首条视觉内容从顶部自然向下排；内容底边到达或超过 96dp 工作线后，按同一套首屏文档流交接进入底部锚定。已有真实消息则锚在当时最后一条真实消息后方。用户发送新消息后，内容不隐藏、不退出，新消息自然排在它后方并把它往上顶。Android 只在当天视觉项后方的连续三轮用户发送中携带 `today_agri_context_day`，后端校验为服务器上海当天后临时注入当天农情作为系统背景；第四轮起自动不带，重试 / pending 复用同一轮固定 day。
