...
- Android 的开通 Plus、开通 Pro、升级 Pro、购买加油包按钮当前已进入支付宝 APP 支付正式链路：先调用后端创建支付宝订单，再调起支付宝 SDK；2026-06-28 生产门禁已切到 public，后续重点观察真实订单回调、对账、退款和异常补偿闭环。
...
- 礼品卡页输入框显示正式占位提示，前端只做空白归一化和 64 字符长度上限；输入为空时“兑换”按钮禁用，卡码有效性、档位、过期和重复兑换全部以后端返回为准。
...
- 后台 `app_release_configs` 或兜底 `APP_ANDROID_*` 配置完整物料后，用旧包真机跑完整链路：检查更新 -> 发现新版本 -> 授权安装未知应用 -> 下载 -> 校验 -> 系统安装页 -> 覆盖安装成功。
...
- 设置页只有一个“隐私与协议”入口，进入本地内置目录页。
- 目录页包含 6 个二级页面：服务协议、隐私政策、第三方信息共享清单、个人信息收集清单、应用权限、风险提示。
- 6 个二级页面都走设置页右进左出的页面栈；左上角返回和系统返回键会先回到隐私与协议目录，再回到设置菜单。
...
结论：当前记忆链路已经收敛为一份自然语言记忆文档，可以进入首版内测观察。这份文档同时承接短期记忆、长期记忆、用户画像和农业事件，不再保留独立 C 层，也不再保留轻量摘要模型候选；没有发现 Android 端直连摘要模型或旧摘要接口仍在主链生效。买服务器前不需要继续盲改提示词，买服务器后重点补多实例下的摘要 claim / lease、日志告警和真实效果抽查。

当前代码真相：

- 主对话完成并成功归档后，后端在 goroutine 中调用 `SummaryService.ProcessSessionSummaries`，不会阻塞当前 SSE 完成态。
- 记忆文档提示词由 `summary_extraction_prompt.txt` 控制，提示词建议输出“短期记忆 / 长期记忆 / 用户画像 / 农业事件”四块。它不是通用知识库，也不是病例流水账；没有用户复查、检测结果或人工确认时，不得把“倾向于 / 更像 / 可能 / 不能排除 / 未见明显...”升级成“确诊 / 确认为 / 已排除 / 已证实”等长期结论。摘要输入会带每轮 time / region，提示词要求关键农业事件尽量保留已有时间和地点；主对话模型对用户图片的客观描述可作为图片线索沉淀，但不保存图片 URL 或文件名。
- 触发频率：Free / Plus 每 6 条完整 AI 回复，Pro 每 9 条完整 AI 回复；触发时把当时 A 层 6 / 9 轮窗口冻结成 `session_ab.pending_memory_jobs_json` 队列里的一个待提取 job，输入为该冻结窗口 + 旧记忆文档，整体覆盖更新一份记忆文档。每个 job 只包含当次 6 / 9 轮窗口，不扩大到 30 轮归档输入；这样即使前一次摘要模型失败，下一轮重试仍处理原冻结窗口，不会把早几轮内容从滑窗里挤掉。
...
- 单次摘要提取有 60 秒超时；模型失败、超时、写回失败、旧快照写回过期或 Redis 租约暂不可用都会保持 `pending_retry_b` 和队首冻结 job。只要 `pending_retry_b=1`，后续每一条新的完整 AI 回复归档后都会马上再次尝试，不等下一个 6 / 9 轮整除点；如果旧 job 未成功时又到新的 6 / 9 轮触发点，会追加新的冻结 job 排队。
- 写回摘要时必须匹配触发快照的 `round_total + updated_at + session_generation + cleared_at`；如果处理期间已有新轮次完成、用户删除历史，或旧快照已经过期，旧结果不会覆盖新轮次或新会话。
...
- `pending_retry_b` 只会在后续完整 AI 回复归档后继续被处理；它不是定时任务，如果用户长期不再发问，失败摘要不会自己定时补账。
...
- Android 只把 ready 内容作为 `ChatTimelineItem.TodayAgriCard` 插入视觉时间线；真实 `messages` 仍只包含用户 / assistant，不写远端聊天历史轮次、不写记忆、不写归档、不扣次，也不能作为最新真实消息。视觉上按普通 AI 文本展示“今日农情 · 日期 + 3 条资讯”，标题加粗，序号使用 `一、二、三、`，长按复用 AI 文本区同款“复制 / 全文复制”菜单。没有真实聊天消息时只显示欢迎语，今日农情不替代空态；已有完整 AI 回答历史时，今日农情跟在当时最后一条完整回答后方，并保存一条主界面展示记录和展示用正文副本到 `today_agri_user_items`，正文副本只保留日期、标题、摘要和短来源名，不保存外部 URL。重开 App 或远端 snapshot hydrate 后按当天保存项恢复这一条内容；远端模式下主聊天不再用本地缓存先展示未确认农情。用户发送新消息后，内容不隐藏、不退出，新消息自然排在它后方并把它往上顶。删除历史对话会清掉主界面展示记录；设置页“今日农情”近 30 天公开历史仍来自 `daily_agri_cards`，不随聊天历史删除。Android 只在当天视觉项后方的连续 2 轮用户发送中携带 `today_agri_context_day`，后端校验为服务器上海当天后临时注入当天农情作为系统背景；第三轮起自动不带，重试 / pending 复用同一轮固定 day。
- 当前补充：2026-06-30 起，后端主聊天已忽略 `today_agri_context_day`，不再把今日农情作为模型 system 背景注入，也不再让它影响请求 hash；上面“两轮携带”只保留为旧 Android 发送行为和历史排查线索。
...
- 检索阶段不要再限制到固定网站；如果某天外部搜索结果仍多是广告软文、普通天气或养殖水产内容，提示词和探针质量可能导致当天不展示，这是刻意选择，不要为了“每天必有”把广告、软文或标题党下发给用户。首页 / 栏目页 / URL 低价值本身不再作为发布硬阻断，避免外链门槛把事实清楚的 3 条内容卡死。
- “同一事件换标题”和近 7 天重复主要靠提示词和人工抽查约束，后续如果重复感强，再增加更强的事件指纹或人工审核，不先把今日农情塞进聊天上下文。
...
结论：当前账号链路已经进入手机号登录骨架阶段，但还不是公开生产账号体系。后端已有严格鉴权开关、v2 session token、手机号账号表、旧本机身份迁移桥和普通短信验证码接口；Redis 认证短期限流已部署到 ECS 并健康。阿里云短信签名模板、普通短信套餐包和 Android 自有短信登录页已落地；`api.nongjiqiancha.cn` HTTPS 已补齐，真正上线前缺的是 HTTPS 公网入口下的真机短信登录回归、AccessKey 轮换和旧 `X-User-Id` 兜底隔离，不是 Go 语言性能问题。
...
- 阿里云短信登录收口：短信签名模板、普通短信发送统计、后端 `/api/auth/sms/*`、Android 自有短信登录页和真机登录回归；Redis 分布式认证限流已接入，后续只需继续观察生产健康。
...
结论：本章节原本是买服务器前的后台巡检，旧结论已被当前实现替代。当前统一管理后台首版已经上线到 `https://admin.nongjiqiancha.cn/`，形态是 Vite `admin` 前端 + `server-go` `/admin-api/v1/*`，具备后台账号 / session / CSRF / 角色 / 审计，覆盖登录、总览、监控面板、用户、会员额度、订单占位、礼品卡、帮助反馈、App 日志、今日农情、检查更新、审计和服务健康。SLS 已接入 5 条 AlertHub 最小告警；仍未完成的是完整客服工单、第一封 SLS 告警邮件送达确认、发布 / 回滚写操作、支付正式订单链路、礼品卡批量发放和更细运营报表。
...
- 检查更新用户侧接口 `GET /api/app/update` 现在优先读取后台 `app_release_configs`，无记录时才回退 `APP_ANDROID_*` 环境变量；后台发布历史已由 `app_release_events` 承接，当前不再新增一套并行 `app_releases` 主表。当前默认普通更新，强更字段保留兼容但默认不对用户生效，除非未来显式开启强更能力。
...
- 检查更新发布页：当前版本、APK 链接、文件大小、SHA-256、是否启用、停更和发布审计；当前主链继续使用 `app_release_configs + app_release_events`，后续重点是 APK 上传、完整回滚入口和旧包真机覆盖安装验收，不再新增一套并行 `app_releases` 主表。
...
结论：支付宝 APP 支付代码已进入仓库联调阶段，但生产收费仍不能直接放量。当前主链已经从“安全占位”推进为“服务端创建支付宝订单 + Android 调起支付宝 SDK + 支付宝异步通知验签 + 幂等发会员 / 加油包权益 + 后台订单核查 + 按已付款异常订单人工补发权益”；真正对外收费前仍必须完成生产密钥配置、小额真机实付、异步通知重复到达、权益发放、异常补偿、对账和退款验收。
...
- 不能把开发期订单接口打开当正式收费。
- 不能信任 Android 同步返回的“支付成功”；会员 / 加油包权益只能由服务端已验签支付结果触发。
- 正式支付必须校验订单号、支付渠道订单号、金额、商品类型、用户、支付状态和幂等键。
- 支付回调可能重复到达，权益发放必须幂等；已支付订单不能被后续 `TRADE_CLOSED` 通知降级。
- 自动续费当前不存在；如果未来接入，必须单独补签约、解约、续费通知、扣款失败、协议文案和隐私清单。

买服务器后必须补：

- 在生产 ECS 配置支付宝 AppID、商户卖家 ID、应用私钥、支付宝公钥和回调地址，不把密钥写入仓库。
- 用小额真机实付验收下单、支付宝同步返回、异步通知验签、重复通知、订单查询和权益发放。
- 补自动对账、退款、异常补偿和完整审计流程。
- 按真实接入状态继续更新协议、隐私政策、风险提示、第三方服务清单、支付 / 退款 / 自动续费说明和应用商店材料。
