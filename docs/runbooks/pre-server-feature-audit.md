...
- 检查更新用户侧接口 `GET /api/app/update` 现在优先读取后台 `app_release_configs`，无记录时才回退 `APP_ANDROID_*` 环境变量；后台已提供发布、强更和停更写入口，但完整发布历史仍待后续补 `app_releases`。
...
- 检查更新发布页：当前版本、APK 链接、文件大小、SHA-256、是否启用、停更和发布审计；后续建议补 `app_releases` 表记录多版本发布 / 回滚历史。
