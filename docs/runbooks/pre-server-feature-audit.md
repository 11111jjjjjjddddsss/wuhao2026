...
- 历史完整卡码不导出；完整卡码只在创建成功当次展示，数据库只保存 hash / 掩码 / 尾号。
...
- 礼品卡后端表、用户侧 `POST /api/gift-cards/redeem`、后台批次生成、卡状态查询、未兑换卡作废、兑换尝试查询、失败原因聚合和 Android 礼品卡兑换入口均已接入；完整卡码只在创建成功当次返回，历史不导出。
...
- 帮助与反馈后台：现有会话列表、详情和回复，后续补处理状态、处理人、标签和搜索。
- 用户详情页：现有账号ID / 脱敏手机号查询、会员、额度、扣次流水、反馈、订单、礼品卡和 App 日志，后续补更多筛选和导出审批。
