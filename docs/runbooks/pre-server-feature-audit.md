...
- 输入内容后点“兑换”或键盘 Done，只提示“礼品卡兑换暂不可用”，不调用后端，不修改会员、额度、加油包、订单或 `quota_ledger`。
...
结论：当前 B/C 记忆链路与现行口径一致，可以进入首版内测观察。B 层是短期工作记忆，负责接住最近对话主线；C 层是用户长期记忆，不是通用知识库或病例流水账；没有发现 Android 端直连摘要模型或旧摘要接口仍在主链生效。买服务器前不需要继续盲改提示词，买服务器后重点补多实例下的摘要 claim / lease、日志告警和真实效果抽查。

当前代码真相：

- 主对话完成并成功归档后，后端在 goroutine 中调用 `SummaryService.ProcessSessionSummaries`，不会阻塞当前 SSE 完成态。
- B 层短期工作记忆由 `b_extraction_prompt.txt` 控制，默认 `<=350` 字，复杂最多 `<=500` 字，定位是全场景当前主线 / 当前事务短期承接。
- C 层用户长期记忆由 `c_extraction_prompt.txt` 控制，默认 `<=260` 字，复杂最多 `<=320` 字，定位是农业长期档案、通用用户画像、稳定偏好和历史经验，不保存通用知识、短期病例或具体剂量配方。
- B 层触发频率：Free / Plus 每 6 轮，Pro 每 9 轮；输入使用当前 A 层窗口。
- C 层触发频率：每 20 轮；输入使用旧 C 层用户长期记忆 + `session_round_archive` 最近 20 轮完整问答，不再用 A 层 6/9 轮窗口冒充 20 轮归档。
...
- 礼品卡和支付接入后再补生成、发放、兑换、作废、订单、退款、对账和人工补偿入口。
