...
- Android 静态 `SESSION_API_TOKEN` 运行时绕过已移除；正式 release APK 只应使用后端按真实手机号账号签发的 per-user session token。严格模式下后端优先使用 bearer token 里的 `userID`，不得用共享静态 token 把不同设备合并成同一用户
