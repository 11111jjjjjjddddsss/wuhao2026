...
记录农技千查后端部署到阿里云 SAE 的历史入口。当前首版部署路线已转向优先评估 / 准备 ECS，旧 SAE demo 应用已经删除；如果后续重新启用 SAE，必须重新创建资源并同步更新本文，不得按旧 AppId 继续操作。

## 当前现状

- 主规则已明确 ECS / SAE 运维优先走脚本、CLI、OpenAPI 等可审计入口；当前 SAE 已删除，首版优先 ECS
