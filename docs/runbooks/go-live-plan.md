...
- SLS 最小日志集已接入服务端 JSON 日志和 Nginx error log，并已有 5 条 AlertHub 最小告警；后续补外部通知 / 仪表盘，继续覆盖主对话、上传、帮助与反馈、今日农情、检查更新、资源水位和模型调用失败。
...
- 后台第一阶段代码已覆盖按用户查看反馈 / 回复、用户额度查询、礼品卡、检查更新状态、今日农情状态、App 日志和审计；上线后继续补发布记录、SLS 外部通知 / 仪表盘、数据库只读脚本和高风险动作二次确认。
