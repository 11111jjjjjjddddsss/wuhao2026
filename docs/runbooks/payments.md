...
- Plus / Pro 会员仍按 30 天周期处理；未到期同档续费按当前到期时间顺延，不覆盖掉剩余天数。加油包次数和 Plus 升 Pro 生成的升级补偿次数都按永久有效处理，不随会员到期清零；加油包同一时刻只允许 1 个有剩余的 active 包，未用完时不能再创建加油包支付订单，用完后可再买。每日额度是自然日额度，不跨天结转。后续扣次顺序仍是每日额度 -> 升级补偿 -> 加油包。
