...
- 管理后台订单页已接订单核查和窄口径人工补发：可查看支付宝订单、渠道交易号、订单状态、金额、权益发放状态和会员变更记录；`owner / finance_ops` 可对已 `paid` 但 `grant_status` 不是 `success` 的支付订单执行“补发权益”，必须填写原因并二次确认。补发复用服务端同一套 `pay_<out_trade_no>` 幂等发权益逻辑，不允许凭空给账号加会员或次数；未付款订单、已成功发放订单、开发期订单和普通只读角色不能补发。当前仍不提供退款、对账自动化或随意改权益按钮。
