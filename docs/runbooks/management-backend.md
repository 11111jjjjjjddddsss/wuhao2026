...
- 订单：`GET /admin-api/v1/orders`，授权角色可按账号ID筛选或留空查看最近支付订单 / 会员变更记录；页面只做只读核查和粗略统计，不提供补发、退款、对账自动化或手动改权益。支付正式放量前，金额汇总只能作为联调 / 核查信息，不得当成完整财务报表；0.01 联调订单保留为测试订单但不计入正式支付金额；已付款但权益未到账的运营口径是先客服核实、优先人工补权益，正式收费前还要补受控补权益入口或同等 SOP。
...
- 订单 / 支付：后台当前只读核查支付订单和权益发放状态，可按账号ID或最近记录查看权益变更来源；生产放量前仍需完成回调验收、对账、受控人工补权益、退款和异常补偿流程。
...
| 订单 / 订购 | 已接只读核查，不能当正式收费完成 | `payment_orders` / `payment_notifications` 记录支付订单与通知，历史 `orders` 仍仅开发期权益变更记录，`/admin-api/v1/orders` 只读查询 | 生产小额实付验收、受控人工补权益、退款、对账、异常补偿和运营 SOP |
...
- 不在生产支付验收前用后台伪造真实支付收入或随手直接改权益；现有订单页只做支付订单和权益变更只读核查。正式收费前应补 owner / finance_ops 二次确认并写审计的人工补权益入口，承接“已付款但权益未到账优先补权益”的客服口径。
