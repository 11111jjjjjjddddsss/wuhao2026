...
- 订单：`GET /admin-api/v1/orders`，授权角色可按账号ID筛选或留空查看最近支付订单 / 会员变更记录；`POST /admin-api/v1/orders/grant` 仅允许 `owner / finance_ops` 对已付款但权益未成功的支付订单人工补发权益。页面不提供伪造支付成功、退款、对账自动化或随意改权益。支付正式放量前，金额汇总只能作为联调 / 核查信息，不得当成完整财务报表；0.01 联调订单保留为测试订单但不计入正式支付金额。
