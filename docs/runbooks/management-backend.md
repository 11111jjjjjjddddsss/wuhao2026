...
- 性能边界：总览、监控面板和产品洞察这三类聚合接口在服务端共享 4 秒查询超时；`033_monitoring_query_indexes.sql` 已给 App 日志等级 / 事件时间窗、有效 session 统计和待回复反馈队列补索引。后台慢查询会按内部错误收口和记录日志，不应长时间占住请求或影响主聊天 SSE。
