...
- 模型策略显示口径：主聊天默认仍是百炼 / 千问；旧 `CHAT_PRIMARY_*` 已退役且 readiness 继续拒绝。2026-06-29 起后台健康可显示可选 `GPT_RELAY_*` 状态，`disabled` 属于正常状态，不代表模型问诊故障；只有显式配置并开启后，它才作为主聊天开流前候选链路，失败或当前首字预算无可见正文仍回退千问。
- 模型链路记录：后台“模型链路”页读取 `/admin-api/v1/model-calls`，用于看每次主聊天走 GPT relay 还是千问 / 百炼、模型名、非敏感 provider label、provider 槽位、Key 槽位、首字 / 开流 / 总耗时、输入 / 输出 / 缓存 / 思考 token、图片数、联网次数、状态和 fallback 原因。该记录默认 14 天留存，随日常维护任务分批清理；记录里不保存真实 Key、真实 URL、账号分组、prompt 正文、AI 正文或图片 URL。该页只开放给 `ops_readonly` 和 `auditor`，不开放给客服角色。
...
- 用户列表排序：`GET /admin-api/v1/users` 支持固定白名单排序，后台可按最近活跃、最近问诊、最早问诊、会员级别、问诊轮次和新注册切换；排序参数只接受服务端白名单，不拼接用户输入 SQL。默认“最近活跃”偏账号登录 / 更新时间，若要看问诊时间应选“最近问诊”。
...
- 模型链路：`GET /admin-api/v1/model-calls`，可按时间窗、provider、记录类型、状态、账号ID和 `client_msg_id` 查询最近模型调用记录；用于排查中转站 0 token、首字慢、Key 切换、联网 token 和千问兜底，不替代供应商控制台账单。
