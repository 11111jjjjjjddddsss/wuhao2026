...
- 后端默认同一手机号 10 分钟最多 10 次验证码登录校验，同一 IP 10 分钟最多 20 次验证码登录校验。
- 验证码默认 5 分钟有效。
...
- `AUTH_SMS_*`：短信发送 / 登录校验限流；`AUTH_SMS_IP_*` 同时覆盖发送和登录校验的 IP 总量限流，二者使用不同 scope。
...
- 短信供应商发送失败或本地发送前失败时，后端会退回已占用的发送限流桶；如果验证码实际到达但登录失败，仍按登录校验限流处理，避免暴力撞码。
