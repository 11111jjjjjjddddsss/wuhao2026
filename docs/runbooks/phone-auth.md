...
- `DYPNS_SMS_TEMPLATE_PARAM`：默认 `{"code":"##code##"}`；只有短信模板实际包含其他变量时才额外配置，避免阿里云 `SendSmsVerifyCode` 因模板变量不匹配返回“非法参数”
...
2. 阿里云融合认证 Android SDK 已接入；后续真机回归时重点确认运营商网络、双卡 / 无 SIM、Wi-Fi-only 和 SDK 取消态都会回落验证码登录，并确认去掉登录前电话状态权限审批窗后，一键登录在主流 ROM 上仍可稳定拉起。
