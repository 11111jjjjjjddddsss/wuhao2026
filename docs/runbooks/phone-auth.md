...
- 明显会回落验证码的环境（VPN / 系统代理 / 纯 WiFi / 无可用移动数据 / 无 SIM）不应先弹 `READ_PHONE_STATE` 权限；只有环境看起来可能走一键取号时，用户点击“一键登录”才按需申请电话状态权限。
