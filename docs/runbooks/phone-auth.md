...
- VPN / 代理或当前活动网络不是蜂窝数据时仍继续硬拉阿里云融合认证 SDK
...
- App 一键登录失败提示已按这个顺序收口：关闭代理 / VPN、打开移动数据、确认默认数据卡；用户不需要反复尝试一键登录，验证码登录就是正式兜底路径。
- VPN / 国外代理、纯 Wi-Fi、移动数据关闭、SIM 卡欠费 / 未激活、运营商网络切换中、部分 5G 兼容性问题，都可能导致 SDK 环境检测、授权页拉起或取号失败。App 当前已把 VPN / 无蜂窝也纳入前置拦截，优先保住“至少能进验证码登录”，不再为了少数 ROM 组合去冒授权页直接闪退的风险。
