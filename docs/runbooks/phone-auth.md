...
- `DYPNS_SMS_TEMPLATE_PARAM`：默认留空；只有短信模板实际包含控制台定义的额外变量时才显式配置，并按模板变量名原样传 JSON。若模板只是平台验证码模板（例如正文里只有 `${code}`），服务端不应强行传 `TemplateParam`，否则可能触发阿里云 `SendSmsVerifyCode` “非法参数”
- `DYPNS_SMS_SCHEME_NAME`：可选短信认证方案名；不配置时后端不向阿里云请求传 `SchemeName`，让接口使用默认方案，避免方案名与模板 / 控制台配置不匹配导致“非法参数”
