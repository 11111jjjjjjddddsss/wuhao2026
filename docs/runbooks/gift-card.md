...
- 同一账号重复提交自己已经兑换成功的同一卡码时，用户侧接口按幂等成功返回既有兑换结果，响应带 `replay=true`，并额外记录一次成功尝试，避免 App 丢失首次 200 后再次输入同一卡码被误判为失败；其他账号重复兑换、已作废、未生效或已过期仍按失败处理。
...
- 已兑换、作废、未到有效期或已过期的卡不会再次发权益；同一账号重复提交自己已兑换成功的卡码只返回既有成功结果，并在 App 里提示“权益已生效”。
