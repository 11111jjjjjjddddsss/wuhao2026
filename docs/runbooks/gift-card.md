...
- 同一账号重复提交自己已经兑换成功的同一卡码时，用户侧接口按幂等成功返回既有兑换结果，并额外记录一次成功尝试，避免 App 丢失首次 200 后再次输入同一卡码被误判为失败；其他账号重复兑换、已作废、未生效或已过期仍按失败处理。
...
- 已兑换、作废、未到有效期或已过期的卡不会再次发权益；同一账号重复提交自己已兑换成功的卡码只返回既有成功结果。
...
- 同一用户 / IP 连续兑换失败。
- 同一礼品卡重复兑换：同一账号重复提交应表现为幂等成功，其他账号重复兑换仍应失败。
- 兑换成功但权益发放失败。
- 已作废 / 已过期礼品卡被大量尝试。
- 礼品卡生成、导出和作废操作。
