...
当前正在下发的正式包为 `1.0.11(12)`，APK URL 是 `https://download.nongjiqiancha.cn/android/releases/12/nongjiqiancha-1.0.11-v12-7858a079.apk`，SHA-256 为 `42ac4cce8e2c548079334ba0668f14d15e47395fd9516a783d7cd5818434c03f`，文件大小 `14,426,056` 字节，`force_update=false`。后台检查更新、官网正式下载按钮和 `download.nongjiqiancha.cn` 公网下载都已对齐该物料；旧正式包 `1.0.10(11)` / `1.0.9(10)` / `1.0.8(9)` / `1.0.7(8)` / `1.0.6(7)` / `1.0.5(6)` / `1.0.4(5)` / `1.0.3(4)` / `1.0.2(3)` / `1.0.1(2)` 分别保留在 `android/releases/11/` / `android/releases/10/` / `android/releases/9/` / `android/releases/8/` / `android/releases/7/` / `android/releases/6/` / `android/releases/5/` / `android/releases/4/` / `android/releases/3/` / `android/releases/2/` 和发布历史里，不再作为当前下发目标。
