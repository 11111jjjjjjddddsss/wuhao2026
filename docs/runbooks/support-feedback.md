...
- `POST /api/support/messages`
  - 请求体：`{"body":"...","images":["https://.../uploads/support/xxx.jpg"]}`
  - 当前限制正文最多 2000 字
  - 支持纯文字、纯图片或图文混合；单次最多 4 张图片
  - Android 先复用主聊天图片链生成 App 私有稳定 JPEG 副本；帮助反馈上传时如果该副本已经是 JPEG、`<=1MB`、可解码、长边不超过 1024 且 EXIF 方向正常 / 未定义，则直接上传，避免重复压缩；否则再走现有压缩链转成 `<=1MB` JPEG，并额外传 `purpose=support`。后端把图片写入 OSS `support/` 并返回 `/uploads/support/<file>.jpg`，同时记录该对象归属账号；提交反馈时后端会再次校验图片确属当前账号后才写入帮助与反馈消息
...
- `GET /internal/support/conversations`
  ...
  - `query` 非空且没有显式传 `since_ms` 时，后端会查全部历史，避免按账号ID、手机号或历史消息搜索时被默认 30 天窗口漏掉；浏览器管理后台在输入搜索词后会自动切到“全部队列 + 全部历史”，无搜索词时仍默认待回复 / 近 30 天
...
- `GET /admin-api/v1/support/conversations`
  ...
  - 支持按 `status=open|replied|closed`、时间范围和 `query` 筛选；时间范围可选近 30 天、近 90 天和全部历史。`query` 对所有可进入帮助反馈页面的后台角色都可查账号ID和脱敏手机号线索，owner / support 这类授权客服处理角色还能查最近消息正文，并可按完整手机号在服务端用 `phone_hash` 精确匹配；只读 / 审计角色不能借搜索拿到正文或完整手机号。浏览器后台输入搜索词后会自动切到“全部队列 + 全部历史”，避免老的已处理 / 已关闭反馈被当前队列过滤漏掉
  - 列表排序为待回复优先，其次按最新消息时间倒序，避免大量消息时把未处理会话压到下面
  - 生产日常按一个 owner 主账号使用时，owner / support 可在会话列表和会话详情直接查看、复制完整手机号并回复客户消息，方便回访；页面还会提供从用户列表跳到反馈会话、清空筛选等简单操作。只读 / 审计角色只看脱敏号。`finance_ops` 可在用户、订单和礼品卡等财务 / 权益页面查看必要完整手机号，但默认不作为帮助反馈客服入口。审计只记录是否展示完整号，不记录手机号值
...
- 当前仍缺正式坐席分配、标签、站外通知、客服绩效、SLA、自动归档和更细的聊天记录保存 / 删除规则。
- 没有系统通知 / 推送
- 当前后台仍只适合早期内测 / 小规模运营使用；公开大规模运营前至少要补坐席分配、标签和消息保存 / 删除规则，并继续完善后台审计
- OSS 上传后端已接入，帮助与反馈图片已通过 `/upload + purpose=support` 切到 `support/` 30 天生命周期；普通问诊图仍走 `uploads/` 3 天生命周期。图片过期不等于客服聊天记录删除，聊天记录正文和状态仍以 MySQL 为真源
