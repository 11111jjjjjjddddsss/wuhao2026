...
- 生产 ECS 已配置 `REDIS_ADDR / REDIS_USERNAME / REDIS_PASSWORD / REDIS_DB`，并随 `4bedc613` 部署验证融合认证 token、短信发送、短信登录、App 自动日志接收和帮助与反馈用户发消息限流；后续 `/upload` 限流同样会复用该 Redis；`/healthz` 当前返回 `redis=ok`
