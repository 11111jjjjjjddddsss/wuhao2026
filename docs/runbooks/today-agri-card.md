...
Android 展示口径：今日农情不是聊天消息，只作为 `ChatTimelineItem.TodayAgriCard` 插入视觉时间线；真实 `messages` 仍只包含用户 / assistant 对话。主聊天视觉上按普通 AI 文本展示“今日农情 · 日期 + 3 条资讯”，标题加粗，序号使用 `一、二、三、` 中文标识，长按复用 AI 文本区同款“复制 / 全文复制”菜单；从用户视觉和滚动体验看，它仍必须像一条普通列表项一样上下滑动，不能做成 overlay、sticky 尾卡、浮层、边框卡片或关闭动画。当天内容加载时如果没有真实消息，文本作为第一条视觉内容排在列表顶部；此时允许只通过现有 `LazyColumn` top padding 给一点额外安全距，避免顶边被顶部标题栏遮住，并且不走“历史消息启动贴底”那条逻辑。如果已有真实消息，文本锚在当时最后一条真实消息后方；这时今日农情若处在视觉尾部，会参与“跳到底部”和静态底部锚点，回底目标按最新视觉尾部处理。用户发送文字 / 图片 / 失败态消息后，不再隐藏，新消息自然追加在其后方并把它往上顶。普通发送 / 工作线仍以真实消息为主，只有“纯今日农情、无真实消息”场景按顶部内容展示；“有真实消息 + 今日农情在尾部”场景可以贴到底。聊天页会按上海日期定期检查跨天，跨天后先清掉旧日期内容并只接受 `date_cn` 等于当前日期的内容，避免 App 长时间前台或后台恢复时继续挂着昨天内容。聊天页不提供手动关闭 / 手动隐藏入口，单条农情不可点击跳外部链接。设置页“今日农情”入口展示近 30 天已 ready 的标题、摘要和来源名记录。
