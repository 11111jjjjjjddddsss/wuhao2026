...
模型提示词版本当前为 `2026-06-10-v29`，要求必须输出 3 条成稿内容；如果已经找到 3 条高质量且主题 / 地区不重复的材料，可以停止继续深挖，不再把 2 条作为正常发布结果。检索阶段最大限度全网宽搜，不把来源卡死在农业农村部、中国农业信息网、农民日报等少数网站；同等质量下优先官方、农业农村部门、农技推广、气象、主流媒体、农业专业媒体、地方农业信息、市场流通、农资、种业、植保等正式来源。今日农情按“农业大类分种植和养殖，当前只取种植侧，养殖、水产不要”的口径由提示词控制：种子 / 种苗 / 种业、植保农药、肥料水肥、农机农时、农产品价格流通、农资供需、补贴保险，以及明确影响作物和农时的农业气象风险可以选；畜牧、水产、养殖、动物疫病、生猪、猪肉、猪价、家禽、禽蛋、牛羊、奶业、饲料、兽药、渔业、鱼虾等养殖水产内容由提示词排除，若搜索结果偏养殖水产就继续换词找种植侧。普通天气预报、生活天气、旅游出行天气不作为独立选题。v29 额外强调 `source_name` 必须是机构 / 媒体 / 站点短名，不拿文章标题、网页标题、频道页或站点口号当来源；`published_date` 能确认近 7 天发布日期 / 更新日期 / 监测日期 / 复核日期时再写，不能确认就写空字符串，不因日期字段、来源层级或标题不完美而只给 2 条。后端解析器不再做大面积主题、可信域名、近 7 天链接、旧卡链接、同批重复标题、发布日期、养殖关键词或广告词等硬过滤，只保留 JSON 结构、能取到 3 条标题 / 摘要非空 item 和内部 URL 安全兜底；广告软文、假新闻、养殖水产和重复事件主要靠提示词、生成探针、后台运营复核与后续日志观察控制。
...
探针用于验证当前固定 `qwen-plus + text-generation + turbo` 的 JSON 执行力、来源名、解析通过率、usage 和新闻质量。探针不会写入 `daily_agri_cards`，也不会改变用户当天看到的卡片。该入口同样必须带 `DAILY_AGRI_JOB_SECRET`，`runs` 默认 1，最多 5。
...
- `runs[].candidate_items / valid_items / reject_reasons`：模型候选数量、通过数量和解析原因
- `runs[].valid_source_hosts`：最终采用来源域名或来源名
...
- JSON 可解析
- 能取到 3 条标题和摘要都非空的 item，最终公开展示正好 3 条
...
- 后端不再硬过滤重复标题、过去 7 天链接 / 标题，也不按可信域名、主题词、发布日期、首页 / 栏目页形态做发布阻断
...
最小解析后不足 3 条时，不发布新卡片。不要为了当天一定有卡片而放宽到用户打开 App 时临时多次调模型；需要排查时走内部探针或后台补跑。
...
- 如果连续失败，先查模型 Key、联网搜索权限、搜索来源返回、模型是否没有按 3 条 JSON 输出、来源名是否异常、正文是否被代码块或非 JSON 包住；内容质量问题优先调提示词和探针，不要直接放宽到广告、软文、养殖水产或任意来源。
