...
模型提示词版本当前为 `2026-06-12-v63`，提示词必须输出 3 条成稿内容；这是今日农情唯一硬数量要求。质量仍然优先，但不能把数量降到 2 条；如果材料不够，就扩大检索词继续找真实种植侧材料，不用养殖、水产、旧闻、广告软文、弱材料或泛泛动态凑数。发布、Android 展示和后台预览都要求 3 条标题 / 摘要完整 item，超过 3 条只取前 3 条，少于 3 条视为卡片不完整。检索阶段最大限度全网宽搜，不把来源卡死在农业农村部、中国农业信息网、农民日报等少数网站；同等质量下优先近 7 天、今天或昨天发布 / 更新 / 监测 / 复核 / 公示 / 兑现或形成明确新进度的公开材料。v63 延续“核心目标 / 选稿原则 / 事实原则 / 写作要求 / 输出前自检”结构和普通用户口径，质量优先级是：近 7 天真实具体材料 > 种植侧大类正确 > 信息对生产或流通有直接意义 > 内容尽量不重复 > 手机卡片好读；同时要求面向普通种植户、农资门店和基层农技人员，用普通话写清事实，让普通人知道这条新闻和种植生产、农资、农时、防灾或流通有什么关系，不写官样话、行业口号或模型自我说明。摘要目标维持手机卡片约 3 到 4 行，宁可接近 100 字，也不要收成 70 多字的薄通知。v63 额外把过细约束收成通用表达：摘要尽量包含具体对象和两三个事实要素，信息不足时补来源事实；调研、会议、平台上线和成果展示类材料必须有具体种植事实和直接影响，否则换更具体材料；标题不要用“调研推动”“场景上新”“即将投用”“成果展示”这类过程词掩盖空泛材料。

今日农情按“农业大类分种植和养殖，当前只取种植侧，养殖、水产不要”的口径由提示词控制，但不做后端关键词过滤：种子 / 种苗 / 种业、植保农药、肥料水肥、农机农时、种植类价格流通、补贴保险，以及明确影响作物和农时的农业气象风险可以选；畜牧、水产、养殖、动物疫病、生猪、猪肉、猪价、家禽、禽蛋、鸡肉、鸡蛋、牛羊、奶业、奶价、饲料、饲用原料、兽药、渔业、鱼虾和动物类价格等通常属于养殖或水产主体。v63 继续要求模型按材料主体判断，不按单个词机械判断：材料只是顺带提到养殖水产词，但主体仍是作物、种植、农资或农时，可以只摘种植侧事实；材料主体是养殖、水产、饲料、饲用原料或动物类价格，则换成种植侧材料。普通生活天气、旅游出行天气不作为独立选题，只有明确影响作物、农时、田间管理或防灾减灾时才可选。

v63 继续压住几类历史质量问题：旧闻和过期材料不能凑今日新闻，网页原文较早时，只有它明确包含近 7 天的新通知、新名单、新进度、新兑现、新监测、新复核或新影响，才作为今日新闻使用；“全国农产品批发价格200指数”“菜篮子指数”“综合行情快讯”“综合价格指数”“批发价格指数”“市场综述”不能作为独立条目，价格类必须聚焦明确种植侧单品、品类、产区、批发市场或农资变化，标题和摘要都要写出具体对象，不用“农产品价格”“批发价微降”“市场价格变化”这类泛泛标题独立成条；不优先选饲料行业站、养殖行情站或饲用原料站，摘要不要夹带猪肉、生猪、鸡肉、鸡蛋、禽蛋、奶价、水产、饲料等养殖、水产或饲用主体内容；如果材料主体是养殖、水产、饲料、饲用原料或动物类价格，就整条换掉，不要只把摘要改成“间接影响种植”；数字、指数、价格、比例、面积、补贴金额和进度必须来自来源材料，不确定就省略，尤其不能把 7.82% 写成“近八成”。
...
2026-06-12 生产环境 `qwen3.5-plus + turbo + v63` 多轮探针均得到 `ok_count=2/2`，每次均为 3 条完整 item，未返回 `reasoning_tokens`；较好样本摘要长度约 91-108 字，最新样本约 67-98 字，说明摘要厚度仍有波动。期间曾短暂尝试 v64 / v65 强化“约 100 字 / 不单薄”表达，但线上探针反而更薄，已回退 v63；后续不恢复后端字数硬过滤，继续用探针和后台复核小步校准。样本未见代码块、外链下发或养殖水产主体。2026-06-11 生产环境曾用 `qwen3.5-plus + turbo + v52` 跑过探针：`runs=3`，`ok_count=3/3`，每次均得到 3 条可解析 item，未见 `reasoning_tokens`，单次 total tokens 约 6.3k-6.6k。样本没有链接下发、代码块、养殖水产主体或明显广告软文；有一组出现两条农业天气 / 农事风险相关内容，但都明确服务种植生产，不是生活天气。v53 线上 `runs=2` 仍为 `ok_count=2/2`，但有一组摘要只有 76-79 字，偏薄；v54 小幅加强摘要厚度提示后，线上 `runs=2` 继续 `ok_count=2/2`，6 条摘要长度约 94-112 字，未返回 `reasoning_tokens`，单次 total tokens 约 7.16k-7.18k。2026-06-11 后续 v54 复测 `runs=2` 继续 `ok_count=2/2`，单次 total tokens 约 7.13k-7.17k，标题和来源可展示，但其中一条“农产品批发价微降”偏综合价格标题，因此 v55 只在提示词里补“价格类标题和摘要要写具体对象”，不增加后端内容拦截。兼容 Chat 链路返回的结构化 `sources[]` 可能为空，质量判断仍要看 `source_name`、正文事实和后台抽查。
