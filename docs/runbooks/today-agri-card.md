...
本文记录“今日农情”每日资讯内容的当前运维入口。后端表、接口和部分代码历史命名仍叫 card，但主聊天用户侧现在按普通 AI 文本块展示，不再按带边框卡片展示。
...
Android 展示口径：今日农情不是聊天消息，只作为 `ChatTimelineItem.TodayAgriCard` 插入视觉时间线；真实 `messages` 仍只包含用户 / assistant 对话。主聊天视觉上用普通 AI 文本块展示“今日农情 · 日期 + 3 条编号资讯”，不再使用带边框资讯卡片。但从用户视觉和滚动体验看，它仍必须像一条普通列表项一样上下滑动，不能做成 overlay、sticky 尾卡、浮层或关闭动画。当天内容加载时如果没有真实消息，文本块作为第一条视觉内容排在列表顶部；此时允许只通过现有 `LazyColumn` top padding 给一点额外安全距，避免顶边被顶部标题栏遮住。如果已有真实消息，文本块锚在当时最后一条真实消息后方。用户发送文字 / 图片 / 失败态消息后，不再隐藏，新消息自然追加在其后方并把它往上顶。普通发送 / 工作线仍以真实消息为主，但“跳到底部”和静态底部锚点会按最新视觉尾部处理，避免只有今日农情或今日农情在尾部时回底位置错乱。聊天页会按上海日期定期检查跨天，跨天后先清掉旧日期内容并只接受 `date_cn` 等于当前日期的内容，避免 App 长时间前台或后台恢复时继续挂着昨天内容。聊天页不提供手动关闭 / 手动隐藏入口，单条农情不可点击跳外部链接。设置页新增“今日农情”入口，展示近 30 天已 ready 的标题、摘要和来源名记录。

文本排版口径：主聊天里的今日农情复用 `ChatStreamingRenderer` 的完成态文本渲染，正文由 `TodayAgriCardUi.kt` 转为纯文本。调这个块时优先改 `TodayAgriCardUi.kt` 的 `toTodayAgriPlainText()` 和 `ChatScreen.kt` 里传给 `TodayAgriNewsText` 的 padding，不要重写聊天滚动链，也不要把它塞进真实 `messages`。
