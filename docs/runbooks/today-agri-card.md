...
模型提示词版本当前为 `2026-07-01-v80`。v80 是在 v79 短段排版基础上的选题质量微调：必须输出 3 条仍是今日农情唯一硬数量要求；面向普通大众用户，写成手机资讯卡片，先说清发生了什么，再说明对农事安排、防灾减损、田间管理、农资选择、产地销售或政策申报有什么参考。新增重点是把今日农情当成给农户、合作社和农资店老板挑三件“今天值得知道的事”，每条最好有清楚选题钩子，例如病虫或灾害风险窗口、作物管理关键期、价格 / 供应 / 库存变化、补贴 / 申报 / 登记等政策节点、新品种 / 新技术 / 农机农资、可复制的地方田管案例。会议、调研、发布会、意见、通知、推进工作、总体适宜、稳中向好、加强指导这类行政材料本身不优先；只有能翻译成“哪个地区、哪类作物、哪类人现在该注意什么或能用什么”时才写，否则换题。标题要像新闻标题，不像公文小标题；摘要第一句讲清真实新进展，第二句给用户能用的判断、提醒或参考，避免“指导各地做好”“加强推进”“持续关注”这类空泛句。继续优先近 7 天公开来源，今天或昨天的新进展更优；三条尽量分散地区、作物、主题和选题钩子，近 7 天已推送过的同地区、同作物、同一事件尽量避开。选题仍以种植方面新闻为主，不限制具体作物；大田作物、经济作物、设施农业、果树、蔬菜、茶叶、特色作物等都可以，但不要总集中在水稻、蔬菜、夏收夏种和天气提醒。摘要目标约 90-130 个中文字符，写 2-3 句完整短讯，信息量要够，一般别低于 80 字；`summary` 字段允许用 JSON 转义换行 `\n` 拆成 2-3 个短段，避免手机端显示成一整块长段。这里的字数、分段、选题钩子和行政材料避让都是提示词目标，不是后端内容过滤，不恢复 `max_tokens`、截断、关键词拦截、相似度拦截、摘要字数硬卡或发布卡点。
...
上下文口径：今日农情不再进入主聊天模型上下文。远端模式下，主聊天仍必须等后端 snapshot 已有当天保存项后才展示今日农情；但用户后续发送文字 / 图片时，后端不再读取当天 ready 农情作为临时系统背景，也不根据 `today_agri_context_day` 影响 `client_msg_id` 请求 hash。今日农情继续不写入归档、记忆文档、A/B/C、扣次或用户长期事实。
...
2026-07-01 提示词已升到 v80：在保持 `qwen3.5-plus + turbo`、3 条 JSON、提示词控制质量和不增加后端内容过滤的前提下，补强“选题钩子”和“行政材料避让”，避免自动生成继续写成文件通知流水账。v80 仍保留 v79 的 `summary` JSON 转义换行短段排版、v78 的近 7 天避重和生产经营参考口径，不按主题词、摘要字数、近 7 天链接、可信域名或养殖水产词做发布阻断，不恢复模型输出截断、`max_tokens` 或字数过滤；内容质量继续靠提示词、内部探针、后台抽查和人工发布观察。2026-06-28 v79 生产探针曾得到 `ok_count=1/1`，2026-06-23 v78 生产探针曾得到 `ok_count=1/1`，2026-06-15 v77 生产探针曾得到 `ok_count=3/3`，每次均为 3 条完整 item；更早 v76 / v74 / v70 / v67 / v52-v55 的探针记录只作历史排障参考。兼容 Chat 链路返回的结构化 `sources[]` 可能为空，质量判断仍要看 `source_name`、正文事实和后台抽查。
