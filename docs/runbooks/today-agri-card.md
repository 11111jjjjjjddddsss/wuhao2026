...
模型提示词版本当前为 `2026-06-13-v74`。v74 已从旧长清单压缩为短任务说明：必须输出 3 条，这是今日农情唯一硬数量要求；面向普通大众用户，写成手机资讯卡片；优先近 7 天公开来源，今天或昨天的新进展更优；三条尽量分散地区、作物或主题；摘要目标约 90-130 个中文字符，写 2-3 句正常新闻短讯，一般别低于 80 字。这里的字数是提示词目标，不是后端字数硬卡，不恢复 `max_tokens`、截断或字数过滤。
