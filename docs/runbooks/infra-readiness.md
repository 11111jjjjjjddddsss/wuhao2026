...
- 阿里云 ECS：当前首版优先的 `server-go` 运行载体。实例 `i-2ze5nrem0jrchln4f0eh` 已购买并运行；系统用户、部署目录、systemd、反向代理、`api` HTTPS、日志、最小告警和回滚入口已部分固化，后续继续补发布记录、黑盒 healthz 监控和后台入口
...
4. 下一步：跟进 App 备案审核 / App 公安备案、真机登录 / 主聊天 / 图片问诊回归、上线前 AccessKey 轮换、第一封 SLS 告警邮件送达确认和黑盒 healthz 监控；网站公安备案号已下发并补到官网 footer，SLS 最小日志集和 5 条 AlertHub 最小告警已先接入用于排障
