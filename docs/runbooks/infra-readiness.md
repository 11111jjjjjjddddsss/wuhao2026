...
记录“农技千查”上线所需的基础设施、依赖顺序、关键配置和补文档动作。当前 ECS / RDS / OSS 资源包已部分落地，本文继续作为后续配置和部署前检查入口。
...
- OSS：图片上传与持久化。当前已购买华北2 100GB 标准-本地冗余存储包，尚未创建 Bucket 或接入后端上传链
- SLS：后端日志检索与告警
...
4. 下一步：补 DashScope 模型 Key、创建 OSS Bucket、补 HTTPS、真实后端部署脚本、日志方案；SLS 是否首版接入再按成本和排障需求确认
