...
- 阿里云 RDS MySQL：首版主业务数据。PolarDB 暂作为后续高并发 / 更高规格升级选项，不再作为个人创业首版默认采购项
...
2. 再买 RDS MySQL 和 SAE，先让后端有一条最小可运行链
...
- 数据库首版倾向直接使用 RDS MySQL；只需在采购前确认规格、Region、备份策略、连接白名单和后续是否需要平滑升级 PolarDB
