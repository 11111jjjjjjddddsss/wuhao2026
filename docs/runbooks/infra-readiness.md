...
- 当前尚未购买 / 接入：SLS 节省计划 / 资源包、SLS 告警 / 仪表盘、CDN / OSS 下行流量包。当前尚未完成：App 备案通过、公安备案、一键登录 / 验证码 / 主聊天 / 图片问诊真机回归、上线前 AccessKey 轮换；网站 ICP、`api.nongjiqiancha.cn` HTTPS、根域名官网 HTTPS、DashScope 主 / 副模型 Key 和 SLS 最小日志集已于 2026-06-05 至 2026-06-06 通过 / 配置；默认 7 天 RDS 备份策略是否调整仍待确认
...
4. 下一步：跟进 App 备案审核 / 公安备案、真机登录 / 主聊天 / 图片问诊回归、上线前 AccessKey 轮换和 SLS 告警 / 仪表盘是否启用；SLS 最小日志集已先接入用于排障
