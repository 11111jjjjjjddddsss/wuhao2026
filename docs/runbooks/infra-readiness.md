...
4. 下一步：跟进 App 备案审核 / 网站公安备案审核 / 公安备案号 / App 公安备案、真机登录 / 主聊天 / 图片问诊回归、上线前 AccessKey 轮换和 SLS 外部通知 / 仪表盘；SLS 最小日志集和 5 条 AlertHub 最小告警已先接入用于排障
...
- SLS 外部通知 / 仪表盘是否首版启用；当前专用 SLS 最小日志集和 5 条 AlertHub 最小告警已接入，仍保留 `journalctl` / Nginx 本地日志作为兜底
