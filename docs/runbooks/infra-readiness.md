...
4. 下一步：补 DashScope 模型 Key、HTTPS、融合认证 SDK / SchemeCode / 短信模板、Redis 业务接入和日志方案；SLS 是否首版接入再按成本和排障需求确认
