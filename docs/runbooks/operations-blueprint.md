...
第一版网页后台代码已落地并部署到 `https://admin.nongjiqiancha.cn/`；`api.nongjiqiancha.cn` HTTPS、根域名官网 HTTPS、管理后台 HTTPS 和 SLS 最小日志集已落地，下载域名仍需后续单独配置。本文记录当前真实环境和后续生产运维里 Codex 应该协助你的范围、入口和安全边界。
...
- 当前监控面板已接真实业务表、App 自动日志、后台审计、健康检查和地区聚合；红黄绿状态用于早期排障，不伪造未接入能力。礼品卡“可用”按当前可兑换口径统计，App 错误 Top 尊重 Top10 limit，检查更新把配置合法性和正式下载物料齐全度分开展示，监控入口会按后台角色裁剪，避免无权限账号点进 403。
- 继续补齐：SLS 告警 / 仪表盘、数据库只读脚本、工单状态 / 搜索 / 分配、发布记录表、礼品卡批量发放 / 发放对象管理、高风险操作二次确认和支付正式订单链路。
