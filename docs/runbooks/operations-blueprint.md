...
- 已回填：阿里云 Region、ECS 实例、RDS MySQL、Redis、OSS bucket、DNS `api/@/www` A 记录、`api.nongjiqiancha.cn` HTTPS / Nginx 443、根域名官网 HTTPS / 静态站、ECS 后端双端口部署 / 回滚脚本、官网部署脚本、生产 readiness 检查、帮助与反馈内部会话列表 / 详情 / 回复接口、App 自动日志内部查询、今日农情内部生成入口。
- 仍需回填：App 备案通过和公安备案后的正式公网入口、农技千查专用 SLS Project / Logstore、关键日志查询和告警规则、数据库只读排查脚本、网页管理后台账号 / 权限、App release 发布记录表或后台入口；下载域名和管理后台域名若启用，需要分别补入口和证书。网站 ICP 备案已于 2026-06-05 通过，App 备案已提交阿里云初审，最小内部操作审计表和查询接口已落地但还不是正式后台账号 / 权限系统，公安备案数据码不写入仓库或聊天交接。
