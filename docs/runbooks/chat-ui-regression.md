...
- 清数据 / 删除历史后的首次真实业务内容未触到工作线前，运行时是 `InitialWorklinePhase.TopUnreached`：同一个 `LazyColumn` 临时 `Arrangement.Top`，真实消息、图片 pending / 失败、assistant placeholder、失败 footer 和小球从顶部自然向下排；首屏文档流底边触到 96dp 工作线后，如果用户没在浏览，先进入极短 `TopAnchoring` 并继续保持 Top 布局，等列表可正向滚动时复用现有强制底部锚点接一次，下一帧再回到默认 `Arrangement.Bottom`
