...
- 轻分割线只跟随结构标题：`# / ## / ###` 标题、`一、标题` 这类中文章节标题、`**二、标题**` / `**二、标题**（说明）` 这类加粗中文章节标题，以及 `1. 营养需求重点` 这类紧凑编号章节。`**处理建议**`、`**注意事项：**` 这类普通独立加粗小标题只保留小标题样式和上下呼吸，不加分割线；正文里的行内加粗词不应触发分割线，也不应在连续标题之间叠加多条线
