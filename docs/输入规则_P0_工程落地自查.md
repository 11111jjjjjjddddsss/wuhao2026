...
| A 层历史对话 | 必须标记为【中等参考性·A层历史对话】 | `ABLayerManager.kt`：`getARoundsTextForMainDialogue()` 返回 `"[中等参考性·A层历史对话]\n" + buildDialogueText(snapshot)`（约 47–54 行）；`QwenClient.kt`：`callApi` 内 `aText = ABLayerManager.getARoundsTextForMainDialogue()`，拼入 systemContent，且 `ensureSystemRole` 后通过 `buildSystemContentWithLayers(base, aText, bSum)` 重写 system（约 113–116、168–174、318–324 行） | ✅ 已落地 |
| B 层累计摘要 | 必须标记为【低参考性·B层累计摘要】 | `QwenClient.kt`：`callApi` 内 `systemContent` 拼接 `"[低参考性·B层累计摘要]\n$bSum"`，以及 `buildSystemContentWithLayers` 中同样拼接（约 117–119、170、320–322 行） | ✅ 已落地 |
| 联网搜索结果 | 必须标记为【极低参考性·工具信息】；仅本轮有效、不写 A/B、不参与后续轮 | `QwenClient.kt`：`callApi` 内 `toolText = "[极低参考性·工具信息]\n$it"`（约 146 行）；`toolInfo` 仅在本轮 user 消息中传入，不写入 A/B（A 仅 `onRoundComplete` 写入，B 仅 B 提取成功写入）；`gpt-demo.html`：`pendingSendByStreamId` 仅当前轮使用，下一轮不携带（约 2456–2476、2726–2730 行） | ✅ 已落地 |
| **2️⃣ 输入形态约束** | | | |
...
| 最长边 ≤1280px | 预处理真实生效 | `ImageUploader.kt`：`MAX_LONG_EDGE = 1280`，`compressImage()` 内按长边缩放至 ≤1280（约 27、92–106 行） | ✅ 已落地 |
| 单张 ≤800KB | 预处理真实生效 | `ImageUploader.kt`：`MAX_SIZE_BYTES = 800 * 1024`，先质量 80 再 75，仍超则按比例缩放直至 ≤800KB（约 28–29、112–127 行） | ✅ 已落地 |
| JPEG 质量 75–80（优先 80，超限再 75） | 预处理真实生效 | `ImageUploader.kt`：`JPEG_QUALITY_HIGH = 80`、`JPEG_QUALITY_LOW = 75`，`compressToJpeg(bitmap, quality)`，先 80 后 75（约 31–33、112–116、163–167 行） | ✅ 已落地 |
| 仅允许 JPEG/PNG；HEIC/WEBP 转 JPEG | 解码后统一输出 JPEG | `ImageUploader.kt`：`compressImage()` 使用 `BitmapFactory.decodeStream` 解码（支持 JPEG/PNG/WEBP；Android 10+ 支持 HEIC），输出统一 `compressToJpeg`（约 63–64、110–127、163–167 行） | ✅ 已落地（解码依赖系统能力，输出统一 JPEG） |
| **4️⃣ 图片与扣费/轮次** | | | |
...
| **压缩与缩放** | `ImageUploader.kt` → `compressImage(imageBytes)`（约 39–155 行） | 1）EXIF 方向矫正；2）长边缩放到 **MAX_LONG_EDGE = 1280**；3）转 JPEG：先 **JPEG_QUALITY_HIGH = 80**，若 size > **MAX_SIZE_BYTES = 800*1024** 则用 **JPEG_QUALITY_LOW = 75**；4）若仍 >800KB，按 **scaleFactor *= 0.85** 迭代缩放后再压 75，直至 ≤800KB |
| **输出格式** | `ImageUploader.kt` → `compressToJpeg(bitmap, quality)`（约 163–167 行） | 统一 `Bitmap.CompressFormat.JPEG`，质量 75 或 80 |
...
| 冒烟项 | 预期 | 验证方式与结论 |
|--------|------|----------------|
| **大图压缩** | 长边 ≤1280px、单张 ≤800KB | `ImageUploader.compressImage()`：先按 1280 长边缩放，再 JPEG 80→75，再循环缩放 0.85 倍直至 size≤800KB；日志输出压缩后尺寸与字节数。**结论**：代码路径与参数已按 P0 落地，需真机/模拟器选大图发送做一次端到端验证。 |
...
| 项 | 要求 | 实现位置与结论 |
|----|------|----------------|
| 成功时 | 结果进入主对话模型（【极低参考性·工具信息】）且 UI 灰块展示 | 模型：`toolInfo` 经 `QwenClient.callApi(..., toolInfo)` 拼入 user 文本（见上第三节）。UI：`onSearchResult(streamId, resultText)` → `renderToolResult(streamId, 'bocha', trimmed)`（`gpt-demo.html` 约 2726–2729 行）。**结论**：满足。 |
...
|------|------------------------|--------------|------------|
| **成功且有结果** | 是：`toolInfo` 为 Bocha 返回的「标题 - URL - 摘要」文本，拼入 user【极低参考性·工具信息】 | 是：`onSearchResult(streamId, resultText)` → `renderToolResult(streamId, 'bocha', trimmed)`，灰块内 URL 可点击 | 正常回复 + 灰块 |
...
**说明**：本自查表与代码一一对应，P0 条款均在工程中落地；冒烟项中“大图压缩”“HEIC/WEBP”建议在真机/模拟器上各做一次端到端验证以闭环确认。
