...
- **触发规则不变（口径统一）**：  
  - A 是累计队列，不固定轮数；B 提取输入 = 触发时刻 A 的**全量快照**（此刻有多少轮就用多少轮）。  
  - B 提取：A≥24 后，每轮 onComplete 尝试；**B 成功写入后才清 A**；B 失败/超时/写入失败 → 禁止清 A。  
  - interrupted/stop/timeout：不写 A、不扣费、不影响 A/B。
...
2. 后端 A 追加一轮（累计轮数随会话变化）；触发 B 提取（输入为当前累计 A 的全量快照，可能>24）。  
