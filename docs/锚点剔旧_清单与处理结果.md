...
## 7) P0 口令「锚点逻辑收敛唯一入口」验收

**口令要求**：`rg "SystemAnchor\.getText\(|systemAnchorText|role\s*[:=]\s*\"system\""` 在 app 里只允许命中 SystemAnchor.kt 实现与 QwenClient.callApi 唯一调用点。

**落实**：
- `getText()` 已 private，无外部调用；`systemAnchorText` 已删。
- `addProperty("role", "system")` 仅保留两处：**SystemAnchor.kt:155**（applySystemFix 内）、**QwenClient.kt:116**（callApi 内占位，由 ensureSystemRole 覆盖）。
- **QwenClient.extractBSummary** 已删除手写 system message，改为将 B 层指令并入单条 user 消息，不再出现 role="system"。
