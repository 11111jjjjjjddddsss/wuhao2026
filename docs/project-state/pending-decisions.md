...
- 待定原因：模型 Key 归属与生产充值告警、SLS 是否首版接入、默认 7 天 RDS 备份是否调整、数据库迁移是否长期改成独立发布步骤或加锁、域名解析 / HTTPS / ICP / App 备案、App 生产 API 地址仍需继续落地；Android 正式登录 token 链已落首版账号 / session / 迁移骨架，融合认证 SchemeCode / 包名 / 签名和 DYPNS 基础凭证已写入生产环境，但阿里云融合认证 SDK、短信签名模板、AccessKey 轮换和真机登录联调仍待闭环
