...
- 待定原因：仍需结合真实发版、回滚、查日志、查库、客服回复、礼品卡和会员运营频率，决定哪些能力继续做网页后台，哪些沉淀成脚本 / CLI 封装；SLS 已有最小 AlertHub 告警、邮件行动策略、最小仪表盘和低成本公网黑盒 timer 告警，但首封告警邮件真实送达、登录后后台 smoke、完整 APK 上传 / 回滚入口仍需继续闭环
...
- 现状：`scripts/check_project_memory.py` 和 CI 已接入，2026-06-12 已从“至少更新一份 memory 文件”收紧为文件级规则：关键代码 / runbook / 运维脚本变更默认要求 `recent-changes.md`，当前真相类变更要求 `current-status.md`，风险敏感区域要求 `open-risks.md`，项目记忆校验策略变化要求 `pending-decisions.md`。2026-06-15 起，SLS 低成本巡检脚本 `scripts/check-sls-cost-guard.ps1`、数据留存 / 文字表成本巡检脚本 `scripts/check-data-retention-cost.ps1` 和费用中心总账巡检脚本 `scripts/check-aliyun-costs.ps1` 也纳入项目记忆护栏；2026-06-16 起，服务器性能快检脚本 `scripts/check-server-performance.ps1` 也纳入护栏，避免后续修改日志成本、聊天归档、App 日志、客服留存、云资源账单、模型成本或服务器性能巡检口径却不更新项目状态 / 风险记录。2026-06-17 起，`admin/**`、`site/**`、`.github/workflows/**` 也进入项目记忆护栏，新增独立 `project-memory.yml` 在 GitHub 上专门跑记忆检查；`check-admin-surface.mjs`、`publish-android-test-apk.ps1`、`check-android-download-domain.ps1`、`sign-oss-cname-url.py`、`sync-oss-download-certificate.ps1`、`check-android-release-artifact.ps1`、`check-app-update-release-match.ps1` 和 `check-launch-readiness.ps1` 等后台 surface、测试包发布 / 下载域名 / 证书同步 / 正式包 / 上线门禁脚本已纳入 watched/current-status 集合，避免后台、官网、测试包发布策略、下载分发、证书续期、正式发布校验、上线门禁或后台静态巡检变化时只改代码 / 脚本不改项目真相；2026-06-19 起，已删除的测试包 / APK 清理脚本从 watched/current-status 清单移除，测试包清理改为用户明确提出后单次人工处理。同日项目记忆脚本也把未跟踪文件纳入变更集合，避免新增测试或脚本还没 stage 时漏过记忆检查。2026-06-18 起，根 Gradle / wrapper 文件也纳入本地项目记忆 watch，正式自更新和测试包下载脚本的门禁变化也要求同步项目记忆；本轮继续把整个 `app/**` 纳入 watch，避免 Android manifest、资源、Gradle、ProGuard、测试或 UI 预览变化漏过记忆检查。GitHub PR 环境下脚本会优先用 PR merge-base 取变更范围，本地裸跑则检查当前工作树 staged / unstaged / untracked 变更，减少“只看最后一个 commit”或“本地未 stage 新文件漏掉”的假绿。`web-ci.yml` 已补 `admin` / `site` build 覆盖，前端后台或官网源码改动不再只靠本地构建发现错误
...
- 当前事实：`server-go/internal/app/chat.go` 每轮主对话已经注入当前上海时间（精确到秒）和用户地点；也就是说模型知道“当前是什么时间 / 用户大概在哪里”。Android 已接入可选定位权限，用户授权后每次发送问诊前尽量短窗口刷新系统定位，并只把系统 Geocoder 反查出的省 / 市 / 区县等地区文本通过 `/api/chat/stream` JSON body 的 `region / region_source / region_reliability` 传给后端；App 不向农技千查服务器上传经纬度，不保存轨迹，系统定位和地理编码能力按设备系统规则处理，也不再把中文地区放进自定义 HTTP header。未授权、定位失败或 body 未传地区时，后端会兼容读取旧 header 或用服务器拿到的公网 IP 走免费离线 `ip2region` xdb 粗定位，结果只是省 / 市级 `unreliable` 参考；库未配置、私网 IP、代理漂移或查询失败时仍兜底 `未知 / unreliable`
- 已落地部分：`SessionRound.created_at / region / region_source / region_reliability` 已接入后端快照和前端兼容解析，历史轮次进入模型上下文时会增加轻量前缀“后台背景时间：...（Asia/Shanghai，仅供参考）”和“后台背景地点：...；地点可信度：...（仅供参考）”；前端暂不显示每条消息时间戳或地点条
...
- 当前事实：代码已接入 `daily_agri_cards`、`GET /api/today-agri-card`、`GET /api/today-agri-cards`、内部 `POST /internal/jobs/today-agri-card/generate` / `probe` / `manual`、后台 `POST /admin-api/v1/today-agri/generate` 和后台人工发布 `POST /admin-api/v1/today-agri/manual`；ECS systemd timer 已作为生产兜底主线落地，Android 只展示已 ready 的“今日农情”标题 + 摘要，缺失时静默不展示。后台 / 内部脚本人工发布仍写同一张 `daily_agri_cards`，会标记 `source_type=manual / manual_locked=1 / manual_by / manual_at`，同一天自动生成和补跑不覆盖人工内容；没人人工发布时原自动生成继续兜底。当前唯一硬数量要求是 3 条，后端发布、Android 展示和后台预览都要求 3 条标题 / 摘要完整 item，少于 3 条视为不完整；主题方向仍靠提示词、探针和后台复核，不做后端内容过滤。生成模型固定 `qwen3.5-plus`，联网入口为 OpenAI 兼容 `chat/completions + enable_search=true + search_strategy=turbo + forced_search=true + enable_source=true + enable_thinking=false`；不保留其它模型候选、其它接口候选或环境变量切换入口。`agent / agent_max` 会带来更多检索和 token 成本，今日农情默认不使用；生成最多 2 次，第二次仍走 `turbo`；只做种植侧，养殖侧主要靠提示词排除；来源 URL 只做内部追溯和后台排查，不作为用户可点击入口；兼容 Chat 链路通常没有结构化搜索来源列表，来源名主要来自模型 JSON 的 `source_name`。
- 当前倾向：继续保持“后台人工发布优先 + 云端定时生成兜底 + 后台补跑异常兜底”的主线，不让用户打开 App 时触发生成风暴
- 已定边界：失败告警首版走 SLS AlertHub `nongji-daily-agri-failed`，后台支持手工补跑 / 人工发布，本机 Codex 自动化可在 22:00 / 23:00 发布次日人工卡片，ECS systemd timer 每天约 05:35 兜底生成；服务端内部最多 2 次模型尝试用于解析失败重试。以上都写同一张 `daily_agri_cards`，人工锁定不被自动生成覆盖。
- 仍待决策：是否允许后台下架单条或重发当天卡片、未来是否做地区 / 作物个性化、是否给本机 Codex 自动化增加“开机后补跑错过时段”的外部机制。当前首版只做全国 `CN` 卡片，不做按用户画像推送
