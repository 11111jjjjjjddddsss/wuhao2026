...
- 现状：`scripts/check_project_memory.py` 和 CI 已接入，2026-06-12 已从“至少更新一份 memory 文件”收紧为文件级规则：关键代码 / runbook / 运维脚本变更默认要求 `recent-changes.md`，当前真相类变更要求 `current-status.md`，风险敏感区域要求 `open-risks.md`，项目记忆校验策略变化要求 `pending-decisions.md`。2026-06-15 起，SLS 低成本巡检脚本 `scripts/check-sls-cost-guard.ps1`、数据留存 / 文字表成本巡检脚本 `scripts/check-data-retention-cost.ps1` 和费用中心总账巡检脚本 `scripts/check-aliyun-costs.ps1` 也纳入项目记忆护栏；2026-06-16 起，服务器性能快检脚本 `scripts/check-server-performance.ps1` 也纳入护栏，避免后续修改日志成本、聊天归档、App 日志、客服留存、云资源账单、模型成本或服务器性能巡检口径却不更新项目状态 / 风险记录。2026-06-17 起，`admin/**`、`site/**`、`.github/workflows/**` 也进入项目记忆护栏，新增独立 `project-memory.yml` 在 GitHub 上专门跑记忆检查；`check-admin-surface.mjs`、`publish-android-test-apk.ps1`、`clean-local-android-apks.ps1`、`clean-oss-test-apks.ps1`、`check-android-download-domain.ps1`、`sign-oss-cname-url.py`、`sync-oss-download-certificate.ps1`、`check-android-release-artifact.ps1`、`check-app-update-release-match.ps1` 和 `check-launch-readiness.ps1` 等后台 surface、测试包 / 下载域名 / 证书同步 / 正式包 / 上线门禁脚本也已纳入 watched/current-status 集合，避免后台、官网、测试包保留策略、下载分发、证书续期、正式发布校验、上线门禁或后台静态巡检变化时只改代码 / 脚本不改项目真相。同日项目记忆脚本也把未跟踪文件纳入变更集合，避免新增测试或脚本还没 stage 时漏过记忆检查。2026-06-18 起，根 Gradle / wrapper 文件也纳入本地项目记忆 watch，正式自更新和测试包下载脚本的门禁变化也要求同步项目记忆。`web-ci.yml` 已补 `admin` / `site` build 覆盖，前端后台或官网源码改动不再只靠本地构建发现错误
