...
- 现状：主规则已明确“脚本 / CLI / OpenAPI 优先”；`docs/runbooks/operations-blueprint.md` 已补整体 App / 后端 / 管理后台的 Codex 协助运维总蓝图。ECS、RDS、Redis、OSS、DNS、部署 / 回滚脚本和少量内部接口已经落地；仍待拍板的是第一版网页管理后台的形态、后台账号 / 角色权限 / 审计模型、SLS 或数据库查询作为日志面板真源的优先级
