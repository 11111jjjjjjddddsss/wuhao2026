...
- 补充：2026-06-13 Key 池 auto 模式已从单纯请求数触发改成“请求数 + 实际 token 用量”双信号：低中流量继续主 Key 优先，高峰或开流前限流 / 鉴权 / 额度错误才拉副 Key 兜底；当前生产阈值是 10 秒 200 次请求或 10 秒 600000 token，触发后临时轮询 120 秒。600000 token / 10 秒按主账号 500 万 TPM 折算约为 72% 容量，用来给长上下文、图片和长回复留缓冲，但上线后仍要观察真实 `model_total_tokens`、429 / quota 日志和副 Key 消耗比例，再决定是否微调，不能把当前阈值当永久容量规划。
