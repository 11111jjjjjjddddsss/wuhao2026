...
- 后续动作：下个窗口若继续 Android UI，直接读取 ADR-0002 并按“三刀”推进：第一刀做底部 AutoFollow 态 streaming 正文进 Overlay + 基础完成态交接；第二刀做用户上滑交回 LazyColumn；第三刀做完成态交接精修。若第一刀前需要实施计划，应只围绕 Overlay 写计划，不再重新会诊局部补丁。回归仍需覆盖：发送瞬间小球是否稳定贴工作线、发送后输入框是否稳定回缩、用户不滑时下一行是否还会冒头、每行上推是否还轻抖、完成态尾部是否更稳、切后台 / 杀进程后 completed assistant 是否落盘恢复
