...
- 状态：未关闭
- 说明：聊天底座已收敛到 `LazyColumn(reverseLayout = true)` 单主链；按最新真机反馈，streaming 中途下掉、生成完成瞬间轻微重排、完成后偶发底部留白这几条主问题都已收口，当前只剩“发送瞬间轻微上下抖一下”这一条未关闭体感问题
...
- 后续动作：后续每次继续排查聊天抖动，都先把问题收敛到 `commitSendMessage()`、发送期几何口径和 `requestScrollToItem(0)` 这一拍，不再把已关闭的 streaming/finalize 问题重新并列拉回来
