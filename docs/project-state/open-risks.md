...
- 状态：未关闭
- 说明：当前后端业务接口和 `/upload` 都要求身份头 / token。服务端已新增手机号账号表、登录 session 表、旧本机 `user_id` 迁移表和 v2 bearer token；Android 已新增登录门、账号 token 保存、验证码登录 UI 和调后端短信 / 融合认证接口的客户端代码。登录成功后使用账号ID `acct_...`，底层字段仍可能叫 `user_id`，生产语义统一按账号ID理解；手机号只是注册 / 登录凭证和脱敏展示信息，不作为业务主键。Android 一键登录和短信登录 payload 会提交 `legacy_user_id` 作为旧本机身份线索；2026-06-13 起服务端默认只接受可由旧 bearer token 证明的 legacy ID，不再凭裸本机 UUID 合并资产，且不接受 `acct_...` 作为 legacy bridge，避免账号之间互相合并。`user_id_migrations` 既有映射已经收紧为不可改写，同一旧 ID 若已映射到另一个账号，本次登录不会重映射或合并资产；跨 Plus / Pro 合并时，被 Pro 覆盖的 Plus 剩余价值会折成 `upgrade_credits`，避免会员价值静默丢失。2026-06-11 已清理生产库旧测试本机 ID 残留，当前没有正式 `app_accounts` / `auth_sessions`，也没有非 `acct_...` 用户资产归属；后端正式 token、严格鉴权 session 和 Android 本地登录态都已加 `acct_...` 保险丝
...
- 风险：这还不是完整可上线登录：阿里云 `CreateSchemeConfig` 已返回 SchemeCode，且 DYPNS AccessKey / Secret、`DYPNS_FUSION_SCHEME_CODE`、包名、签名、短信签名和验证码模板已写入本机密钥文件与 ECS 环境；Android 一键登录 SDK / AAR 已导入并接到登录页，源码 manifest 仍显式声明 `READ_PHONE_STATE` 和设备网络 / Wi-Fi 状态权限；2026-06-08 已修复一键登录 SDK 初始化顺序导致的“一点就退”NPE 崩溃，并把本机 debug 测试包改为有 release 签名时同签名开启一键登录、缺签名才退验证码；2026-06-13 前置环境策略按“能一键就尽量一键、明显不满足就验证码”再校准：点击一键登录时按需申请 `READ_PHONE_STATE`，拒绝就直接切验证码；无网络、无 SIM、SIM 未就绪、纯 WiFi / 无可用移动数据直接走验证码；4G+WiFi、VPN、系统代理或活动网络非蜂窝但仍检测到移动数据能力时只记录 `auth.fusion_env_warning` 并允许一键尝试，失败后仍回 App 验证码页。manifest 覆盖 `com.mobile.auth.gatewayauth.LoginAuthActivity` / `PrivacyDialogActivity` 主题为本地 translucent 兼容主题，尽量降低荣耀 / 华为 / Android 15+ 授权页拉起闪退风险。SDK 授权页已接自定义 UI，隐藏 SDK 自带“其他手机号 / 更多登录方式”，让短信兜底回到 App 自己的验证码页；App 初始化 SDK 前已调用 `useSDKSupplyCaptchaModule(false)` 关闭内置图形认证模块，一键登录失败路径也不再 `continueSceneWithTemplateId(..., false)` 继续 SDK 下一节点，而是结束场景回 App 验证码页；但仍需在阿里云融合认证 100001 的全局策略和当前方案 Code 的方案策略里确认没有图形验证节点，且不能关闭“融合认证解决方案”本身；短信登录主链已改成普通 Dysms `SendSms` + 后端 Redis 验证码摘要，账号登录成功后才清验证码，短信供应商返回超时 / 异常时也不再主动清验证码，避免短信实际送达但后端验证码被提前删除；Android 只允许提交 6 位验证码，减少无意义失败请求；同时已修复旧本机 ID 迁移时 `daily_usage` 同表 SQL 歧义导致 `/api/auth/sms/login` 500 的问题，并补齐注销申请归属迁移。但完整真机运营商网络回归仍未闭环，尤其需要确认荣耀 / 华为 / Android 15+ 在重新加回按需电话权限审批后授权页能稳定拉起并完成最终登录，且不再弹图形验证码。当前 Android 100001 一键登录主链不再调用半程 verify-only，只在最终 `onVerifySuccess` 后由 `/api/auth/fusion/login` 消费一次 token；仍需用真机确认授权页可稳定拉起、最终登录成功、失败 / 取消会回落验证码。验证码登录需要用户用新包重新发送新验证码回归，旧验证码不要复用；已暴露过的主账号 AccessKey 需要在上线前轮换；Redis 认证限流已部署到 ECS 并健康；旧 `X-User-Id` 兜底在公开生产仍需严格关闭，旧无 `session_id` bearer token 也不应在公开生产兼容。旧本机资产迁移现在默认只接受旧 token 证明，不再接受未证明 UUID；若未来为极短迁移窗口设置 `AUTH_ALLOW_UNPROVEN_LEGACY_UUID=true`，必须有审计复查并且 readiness 会失败，不能作为公开生产常态。所有会员、额度、加油包、订单、礼品卡、帮助反馈、App 日志、记忆文档、聊天归档和注销申请都必须继续收敛到同一个账号ID，后续接支付不能再按手机号或本机 ID 分叉
