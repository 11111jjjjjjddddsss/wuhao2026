...
- 补充：当前仍兼容 Android 早期阶段的裸 `X-User-Id` 本机身份兜底，方便本地 / 迁移期联调；`SESSION_API_TOKEN` 仍只是 Gradle 静态注入字段，不是正式登录 token，release 构建已禁止非空 `SESSION_API_TOKEN`
- 风险：这还不是完整可上线登录：阿里云 `CreateSchemeConfig` 已能返回 `OK`，但 CLI 未返回 `SchemeCode`，需要从控制台核对并配置；Android 一键登录 SDK / AAR 尚未导入，当前一键登录按钮不会消耗融合认证试用次数，只提示先用验证码；短信登录依赖 `DYPNS_SMS_SIGN_NAME` 和 `DYPNS_SMS_TEMPLATE_CODE`，ECS 环境变量尚未配置；Redis 已购买但代码未接入，当前短信限流只是单 ECS 进程内限流；旧 `X-User-Id` 兜底在公开生产仍需严格关闭
- 后续动作：正式公开上线前必须完成阿里云融合认证 Android SDK 接入、`DYPNS_FUSION_SCHEME_CODE`、包名 / 签名配置、短信签名 / 模板、ECS 环境变量、Redis 验证码 / 限流接入和真机登录回归；生产环境保持 `APP_SECRET` 与 `AUTH_STRICT=true`，并逐步移除或隔离裸 `X-User-Id` 兜底
