...
- 状态：未关闭
- 说明：客服反馈当前已具备后端消息表、用户侧历史 / 发送 / 已读接口，以及 `SUPPORT_ADMIN_SECRET` 保护的内部读取会话和发送客服回复接口；Android 也能展示历史、发送反馈，并在设置页“客服反馈”行用红点提示未读客服 / 系统消息
- 风险：当前还不是完整客服系统，没有网页管理后台、客服账号 / 权限、工单分配、附件上传、站外推送、消息搜索或未回复队列。后台想回复用户时需要先通过内部接口或未来管理后台调用，同步依赖真实服务器、数据库和密钥配置
