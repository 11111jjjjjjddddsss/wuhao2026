...
- 状态：未关闭
- 说明：今日农情首版已接入独立后端链路和 Android UI-only 卡片；它不影响聊天主链、不扣用户问诊次数、不进入 A/B/C 或归档。当前生成已改为 `qwen-plus + turbo` 原生 Generation 强制联网链，显式关闭思考、`enable_source=true`、`freshness=7`，不再用 `assigned_site_list` 硬限定站点，只用 `prompt_intervene` 引导种植侧生产经营主题搜索；`prompt_intervene` 当前按种子 / 种苗 / 种业、病虫害 / 植保 / 农药、影响作物和农时的农业气象风险、农时农事 / 农机、肥料 / 化肥 / 水肥管理、农资价格 / 农产品流通 / 补贴保险等种植农资方向分散检索，避免只搜“农业新闻 / 农情”导致来源卡死；农业大类按种植和养殖理解，今日农情只取种植侧，普通天气预报不单独入选，畜牧、水产、养殖等非种植领域主要靠提示词排除，不再由后端主题词硬过滤。`agent / agent_max` 属于多轮检索整合且会额外收费，今日农情默认不使用。2026-06-09 生产探针确认 `qwen-flash + turbo + enable_source` 可返回来源链接但执行力偏弱，当前按用户要求切到 `qwen-plus + turbo + enable_source` 测质量；`qwen3.5-flash + turbo/agent` 会返回 `400 InvalidParameter / url error`。生成前会把过去 7 天已 ready 的今日农情喂给模型要求去重，服务端当前只保留 JSON 结构、正好 3 条、标题 / 摘要非空、同批重复标题和私网 / 明显电商 URL 清洗等低风险兜底；广告软文、假新闻、养殖侧、重复事件和标题党主要靠提示词、探针、后台运营复核与日志观察控制。提示词已放宽种植侧农业新闻覆盖面，并要求正好 3 条成稿内容；生成最多 2 次，第二次只在首轮质量校验失败后换检索提示补救，仍走 `turbo`，不切 `agent`；来源 URL 只作为内部追溯和后台排查，不下发给 Android 用户卡片。ECS systemd timer 和后台补跑都已落地
