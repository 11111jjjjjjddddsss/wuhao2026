...
- 风险：这还不是完整可上线登录：阿里云 `CreateSchemeConfig` 已返回 SchemeCode，且 `DYPNS_FUSION_SCHEME_CODE`、包名和签名已写入本机密钥文件与 ECS 环境；但 Android 一键登录 SDK / AAR 尚未导入，当前一键登录按钮不会消耗融合认证试用次数，只提示先用验证码；DYPNS AccessKey / Secret 尚未安全配置，已暴露过的主账号 AccessKey 需要轮换；短信登录依赖 `DYPNS_SMS_SIGN_NAME` 和 `DYPNS_SMS_TEMPLATE_CODE`，ECS 环境变量尚未配置；Redis 认证限流已部署到 ECS 并健康；旧 `X-User-Id` 兜底在公开生产仍需严格关闭
- 后续动作：正式公开上线前必须完成阿里云融合认证 Android SDK 接入、DYPNS AccessKey / Secret 安全配置、短信签名 / 模板和真机登录回归；SchemeCode、包名和签名已配置；Redis 验证码 / 限流已接入；生产环境保持 `APP_SECRET` 与 `AUTH_STRICT=true`，并逐步移除或隔离裸 `X-User-Id` 兜底
