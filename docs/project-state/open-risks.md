...
- 风险：这还不是完整可上线登录：阿里云 `CreateSchemeConfig` 已返回 SchemeCode，且 DYPNS AccessKey / Secret、`DYPNS_FUSION_SCHEME_CODE`、包名、签名、短信签名和验证码模板已写入本机密钥文件与 ECS 环境；Android 一键登录 SDK / AAR 已导入并接到登录页，但还没做用户真机运营商网络回归；验证码登录已具备后端配置但仍需真机回归；已暴露过的主账号 AccessKey 需要在上线前轮换；Redis 认证限流已部署到 ECS 并健康；旧 `X-User-Id` 兜底在公开生产仍需严格关闭
- 后续动作：正式公开上线前必须完成 AccessKey 轮换、HTTPS / 备案后的真机登录回归和旧 `X-User-Id` 兜底隔离；DYPNS 基础凭证、SchemeCode、包名、签名、短信签名、验证码模板和 Android SDK 客户端链路已配置；Redis 验证码 / 限流已接入；生产环境保持 `APP_SECRET` 与 `AUTH_STRICT=true`
