...
- 补充：2026-06-15 总门禁继续拆细为业务负责人更容易看懂的步骤：短信发送统计已从资源巡检中拆成独立 `sms usage`，后端数据边界巡检也从“非 `acct_...` 资产归属”扩展到 `acct_...` 孤儿记录和缺加密手机号账号检查。本轮线上只读复查这些新增检查均为 0；但这仍不替代 App 备案 / App 公安备案、后台登录后 smoke 实跑、短信套餐包余额确认、首封告警邮件送达和真机完整回归。
