...
- 状态：未关闭
- 说明：今日农情首版已接入独立后端链路和 Android UI-only 卡片；它不影响聊天主链、不扣用户问诊次数、不进入记忆文档或归档。当前生成固定 `qwen-plus + DashScope text-generation/generation + enable_search=true + search_strategy=turbo + enable_source=true + freshness=7 + prompt_intervene`，不保留其它模型候选或环境变量切换入口。生成前会把过去 7 天已 ready 的今日农情喂给模型要求去重；提示词版本 `2026-06-10-v26` 已放宽种植侧农业新闻覆盖面，并要求正好 3 条成稿内容、标题一行、摘要约 3 行、短来源名、不带链接。服务端当前只保留 JSON 结构、正好 3 条、标题 / 摘要非空、同批重复标题和私网 / 明显电商 URL 清洗等低风险兜底；广告软文、假新闻、养殖侧、重复事件和标题党主要靠提示词、探针、后台运营复核与日志观察控制。生成最多 2 次，第二次只在首轮质量校验失败后换检索提示补救，仍走 `turbo`，不切 `agent`；来源 URL 只作为内部追溯和后台排查，不下发给 Android 用户卡片。ECS systemd timer 和后台补跑都已落地。
...
- 补充：买服务器前今日农情巡检已记录到 [pre-server-feature-audit.md](D:/wuhao/docs/runbooks/pre-server-feature-audit.md)。当前没有发现用户打开 App 临时生成、写记忆文档、写归档、扣次或伪装成 `ChatMessage` 的旧链路；残余风险主要是质量运营，比如同一事件换标题、养殖侧漏入、广告软文和假新闻需要靠提示词、探针、后台人工抽查与后续告警约束。当前检索阶段已改为全网宽搜、不限定固定网站，发布阶段也不再用可信域名 / 近 7 天链接 / 主题词大面积硬过滤；如果模型没有给满 3 条结构完整内容，用户侧会静默不展示，需要通过后台补跑或内部探针排查。
