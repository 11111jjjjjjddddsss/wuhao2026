...
- 风险：这次反向列表单主人重构已经完成编译和静态自查，但还没有完整用户真机回归。当前最需要确认的是：1) 首屏进入有历史时是否仍稳定贴底；2) 发送瞬间的小球 / 历史文本是否仍抖动；3) streaming 过程中上下拖动是否不再乱窜、重叠、抢手；4) streaming 连续 block item 化后，用户小幅上滑是否只需滑过当前短 active block 即可稳定，下面内容是否继续正常生成且不被裁剪；5) 回到底部是否清掉完成后保留的 block 快照并恢复 streaming 跟随；6) finalize 收口是否不再出现多 block 合回巨型 item 的重排闪动；7) 输入框上方和静态文本底部是否不再出现额外白块
