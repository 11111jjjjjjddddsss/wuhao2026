...
- 补充：2026-06-12 发布 / 回滚脚本已同步验证 API 与后台 `/admin-api/` 上游跟随同一个 slot，并在回滚失败时恢复 Nginx 配置；这里剩余的“发布 / 回滚写操作”主要指尚未做成管理后台页面的一键发版 / 回滚入口。
