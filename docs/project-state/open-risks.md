...
- 风险：这次反向列表单主人重构已经完成编译和静态自查，但还没有完整用户真机回归。当前最需要确认的是：1) 首屏进入有历史时是否仍稳定贴底；2) 发送瞬间的小球 / 历史文本是否仍抖动；3) streaming 过程中上下拖动是否不再乱窜、重叠、抢手；4) streaming 连续 block item 化后，用户小幅上滑是否只需滑过当前短 active block 即可稳定，下面内容是否继续正常生成且不被裁剪；5) 用户手动回到真实底部后是否能恢复 AutoFollow，且半路不会自动吸回；6) finalize 收口是否不再出现 pending/finish 阶段合回巨型 item 的重排闪动；7) active streaming 改成单个 soft-wrap Text 且收口空 active block / 半截 Markdown / 跨 block 分割线后，吐字行跳动、下一行黑点冒头和分割线时有时无是否明显减轻；8) 段落边界不再插入空 active item、AutoFollow 新 block 贴底请求增加用户浏览 epoch 软取消且只在真正发出请求后更新 last index 后，刚上滑时被上一拍贴底请求带回一小截又回弹的感觉是否消失，且手动回到底部后的跟随是否仍正常；9) 输入框上方和静态文本底部是否不再出现额外白块；10) active / committed 列表、标题等 Markdown 渲染宽度模型尚未统一，若仍看到某一行先长后短，需要下一刀单独处理
