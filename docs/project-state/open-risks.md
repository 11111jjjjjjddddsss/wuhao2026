...
- 风险补充：当前渲染所有权已经切到 `BottomActiveZoneSlice`，但部分几何/follow/finalize 判断仍然继续沿用“单条 assistant bottom”口径，例如 `streamingContentBottomPx` 仍主要由 active assistant bounds 驱动。只要底部活跃区的真实视觉重心和这条单 assistant 几何口径继续存在偏差，就可能留下连接处残影、回底判断偏差或 finalize 收尾感知不稳。这是下一刀最值得继续盯的代码矛盾，但本轮先不扩大手术面
