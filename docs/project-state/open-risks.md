...
- 风险补充：2026-06-14 主界面短提示从输入框上方改为屏幕中部全局浮层，额度、限流和活动流进行中这类明确不可立即重试的中断不再生成 `回复未完成 · 点击重试` assistant 尾巴，避免和额度耗尽提示重叠；跳到底部 / 静态底部锚点会把今日农情等视觉尾部纳入目标，减少只有卡片或卡片在尾部时的回底错位。该改动只调整状态展示和视觉锚点，不恢复 overlay、反向列表或 raw delta；仍需真机继续观察输入法、今日农情卡片、长回复、额度耗尽和复制菜单同时出现时是否还有遮挡。
