...
- 风险：这次反向列表单主人重构已经完成编译和静态自查，但还没有完整用户真机回归。当前最需要确认的是：1) 首屏进入有历史时是否仍稳定贴底；2) 发送瞬间的小球 / 历史文本是否仍抖动；3) streaming 过程中上下拖动是否不再乱窜、重叠、抢手；4) 小分割 / block item 化撤掉后，文本上滑时“重新找位置 / 行先变长再缩短”的感觉是否消失；5) 撤掉小分割后，原先“小幅上滑仍在本条 assistant 内时被 index 0 长高带一下”的抢手是否回归，以及回归程度是否可接受；6) 用户手动回到真实底部后是否能恢复 AutoFollow，且半路不会自动吸回；7) streaming -> settled 已共用 soft-wrap block renderer 后，finalize 收口是否不再出现行高 / 行宽微动；8) 输入框上方和静态文本底部是否不再出现额外白块
