...
- 状态：未关闭
- 说明：Android 设置页已接入本地内置“服务协议 / 隐私政策 / 风险提示”三份正文，当前按用户提供公司名和邮箱、当前真实权限清单、农业 AI 工具定位编写
- 风险：这三份文案不是律师审定稿；公司主体、ICP备案、支付规则、礼品卡规则、账号注销、隐私同意弹窗、第三方 SDK 清单、图片 OSS 保存周期和真实权限一旦变化，当前文案可能需要同步调整。当前也尚未接首次启动隐私弹窗 / 勾选同意流程，上架或合规审核前可能需要补
- 后续动作：正式上架 / 公开推广前，按真实公司主体、备案、权限、SDK、数据保存周期、支付和账号注销流程复核三份文案；若新增定位、通知、相机、相册或存储权限，必须同步修改隐私政策、Manifest 和项目记忆
