...
- 风险：Overlay 第一刀已经编译通过，但尚未真机回归。需要重点确认：底部观看时下一行残影是否彻底消失、每行上推是否不再像列表补滚、用户上滑是否不抢手、回到底部是否恢复 Overlay、完成态收口是否不比上一基线更抖、复制/输入选择期间是否不会切层。当前代码已经补上五层关键护栏：一是列表对 Overlay 正文的占位改成同拍测得的 Overlay 实际高度，不再只留一行假占位；二是 Overlay 每次继续长高时，不再靠 `LaunchedEffect + scrollBy` 晚一拍追滚，而是通过 `onSizeChanged` 累计真实高度差，再在 `SideEffect` 里用 `LazyListState.dispatchRawDelta(...)` 直接推底层列表；三是如果某一拍 `dispatchRawDelta(...)` 没完全消费掉本次增量，剩余 delta 会保留到下一拍继续追，不再悄悄丢失；四是 `pending finalize` 期间会主动清空 overlay follow 待消费 delta，并禁止 finalize 收口阶段继续按 Overlay 尺寸变化补滚，避免收尾时自己再推自己一下；五是 Overlay pending finalize 的外壳现在会补齐与列表 item 相同的 `CHAT_MESSAGE_ITEM_VERTICAL_PADDING`，placement 也会用同值做底边补偿，同时把 `selectionEnabled` 切到和列表那棵 `alpha(0f)` completed 树一致，继续减少尾部交接那一拍因为壳不同构带来的微抖。Overlay 收口同时仍会切到更接近列表 completed 的 `Settled + disclaimer` 口径；若真机仍剩轻微尾抖，下一刀也只继续精修 finalize 交接，不要再回去重开 `dispatchRawDelta` / bounds release / requestScrollToItem 行锚定 / renderer gate / clip/mask / 32ms hold 这些已排除路径。这些旧链路当前只允许作为 `LAZY_COLUMN` fallback 存在
