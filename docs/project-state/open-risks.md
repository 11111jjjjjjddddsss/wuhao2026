...
- 状态：未关闭
- 说明：今日农情首版已接入独立后端链路和 Android UI-only 卡片；它不影响聊天主链、不扣用户问诊次数、不进入记忆文档或归档。当前生成固定 `qwen-plus + DashScope text-generation/generation + enable_search=true + search_strategy=turbo + enable_source=true + freshness=7 + prompt_intervene`，不保留其它模型候选或环境变量切换入口。生成前会把过去 7 天已 ready 的今日农情喂给模型要求去重；提示词版本 `2026-06-10-v29` 已按“今日农情最大限度放开，但养殖水产不要”收口，要求正好 3 条成稿内容、标题一行、摘要约 3 行、短来源名、不带链接；`source_name` 必须是机构 / 媒体 / 站点短名，`published_date` 能确认近 7 天发布日期 / 更新日期 / 监测日期 / 复核日期时再写，不能确认就写空字符串。服务端当前只保留 JSON 结构、能取到 3 条标题 / 摘要非空 item 和私网 / 本机 URL 安全兜底；广告软文、假新闻、养殖水产、重复事件和标题党主要靠提示词、探针、后台运营复核与日志观察控制。生成最多 2 次，第二次只在首轮解析失败后换检索提示补救，仍走 `turbo`，不切 `agent`；来源 URL 只作为内部追溯和后台排查，不下发给 Android 用户卡片。ECS systemd timer 和后台补跑都已落地。
- 风险：如果 `turbo` 返回的材料质量差、主题偏养殖水产、广告软文多或信息过期，模型仍可能产出不理想；后端当前不靠内容硬过滤拦截这些问题，只在不足 3 条可展示 item 时不发布，前端静默不展示。2026-06-10 生产探针 5 次均通过，未发现养殖水产词、广告词、URL 下发或元表达，且无 `reasoning_tokens`，但样本里仍有标题偏短和同一地区小麦主题跨探针重复较多的问题，属于后续提示词和运营抽查要继续看的质量点。联网搜索会增加输入 token，并且搜索策略本身也有调用费用，所以仍要坚持“云端每天定时一次 + 后台补跑兜底”，不能改成用户打开 App 时临时生成。当前仅有 `scope=CN` 全国卡片，尚未做地区 / 作物个性化；`nongji-daily-agri-failed` AlertHub 告警已落地，但外部通知、自动重试策略和运营抽查节奏还需要继续收口。
