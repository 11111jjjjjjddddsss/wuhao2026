...
- 补充：额度扣减在轮次归档成功后执行；若 `ConsumeOnDone` 临时失败，服务端会按同一 `client_msg_id` 短重试，重复扣由 `quota_ledger` 唯一键防住。2026-05-05 起，当日已完成轮次 replay 时还会按该轮原完成日期异步尝试补 `quota_ledger`，避免“已归档但短重试失败”后长期漏记；该补偿仍是幂等的，不会重复扣同一 `client_msg_id`。隔天历史轮次不在用户 replay 时硬补，避免会员档位变化后按错误权益补旧账
- 补充：主模型自动开流重试已从 2 次收紧为 1 次，避免一次传输 / 协议异常后服务端自动再调一次 Qwen3.5-Plus；如果上游开流前就失败，客户端会看到失败 / 可重试，而不是服务端静默二次开流。`chat_stream_inflight` 获取结果改为校验 lease token，降低 MySQL `RowsAffected` 配置差异导致的误判风险；旧 `/api/session/round_complete` 在同一消息主流式进行中时会拒绝抢写
- 风险：WorkManager 不是实时任务，系统可能延后执行；后端进行中锁当前用 30 分钟租约防死锁，若服务进程极端卡死且租约过期，后续请求才会重新接管。这个设计优先保护成本，不承诺像前台直播一样立刻可见；如果模型已经吐完但归档写库本身失败，客户端不会收到完成态，该轮仍可能需要用户重试或人工排障。若服务进程在归档后、扣减前崩溃且当天未触发 replay，或已跨日不再自动补旧账，仍可能出现单轮成本漏记；后续若真实上线，应补后台对账任务或按业务日志做周期性巡检
