...
- 风险补充：streaming 行级 reveal 当前已经在 `ChatStreamingRenderer.kt` 增加了只落在显示层的“整组结果延迟放行”门闩，用来压“工作线下面下一行提前冒头、一闪一消失”。这刀不动 `revealMode = Free`、`onTick = {}` 和 `streamingLineAdvanceTick` 接线，风险比全链接回 reveal 参数低，但仍需要真机确认两类边界：一是大 chunk 一次跨过多个换行点时，是否还会残留极短暂的提前冒头；二是 finalize 后 `freshTick` 回落口径是否始终能让整条消息完整显示，不会把最后几行误卡在门闩里
