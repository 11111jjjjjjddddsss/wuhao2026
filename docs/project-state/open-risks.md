...
- 风险补充 2026-04-23 深夜：上一轮最明显的“输入框上方多出一整节白色空白、静态态底部文本拖不动、开机进入不贴底”更像是底部活跃区的所有权没有及时退场，而不是输入框自己变高了。当前代码已继续收口：发送起步先保持 `LAZY_COLUMN`，等首个 `scrollToBottom(false)` 把整条消息列表压稳后再切 `OVERLAY`；`finalizeStreamingStop()`、`resetStreamingUiState(...)` 和 `uiRuntimeResetKey` 进入时也都会把 `streamingLocation` 收回 `LAZY_COLUMN` 并清掉 `anchoredUserMessageId`。这刀已编译通过，但还没有新的真机验证，需要重点确认空白、静态拖动和开机贴底是否一起恢复
