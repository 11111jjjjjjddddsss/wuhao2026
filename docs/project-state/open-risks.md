...
- 状态：未关闭
- 说明：当前后端业务接口和 `/upload` 都要求身份头 / token。服务端已新增手机号账号表、登录 session 表、旧本机 `user_id` 迁移表和 v2 bearer token；Android 已新增登录门、账号 token 保存和融合认证短信登录 UI。2026-06-13 起新包登录页只保留一个可见入口：调用阿里云融合认证 SDK 的短信登录；不再展示本机号码一键登录，不再展示 App 自己的普通验证码表单，也不再申请 `READ_PHONE_STATE`。服务端普通 Dysms 短信发送 / 登录接口仍保留给历史包和应急兼容，新包主 UI 不调用。登录成功后使用账号ID `acct_...`，底层字段仍可能叫 `user_id`，生产语义统一按账号ID理解；手机号只是注册 / 登录凭证和脱敏展示信息，不作为业务主键。Android 登录 payload 会提交 `legacy_user_id` 作为旧本机身份线索；2026-06-13 起服务端默认只接受可由旧 bearer token 证明的 legacy ID，不再凭裸本机 UUID 合并资产，且不接受 `acct_...` 作为 legacy bridge，避免账号之间互相合并。`user_id_migrations` 既有映射已经收紧为不可改写，同一旧 ID 若已映射到另一个账号，本次登录不会重映射或合并资产；跨 Plus / Pro 合并时，被 Pro 覆盖的 Plus 剩余价值会折成 `upgrade_credits`，避免会员价值静默丢失。2026-06-11 已清理生产库旧测试本机 ID 残留，当前没有正式 `app_accounts` / `auth_sessions`，也没有非 `acct_...` 用户资产归属；后端正式 token、严格鉴权 session 和 Android 本地登录态都已加 `acct_...` 保险丝
...
- 补充：Redis 认证限流已覆盖融合认证 token 获取、融合认证登录校验、普通短信发送和普通短信登录校验；fusion token / fusion login 按 IP hash 限流，普通短信发送按手机号 hash + IP hash 和 IP hash 两层限流，普通短信登录按手机号 hash + IP hash 限流。新包主 UI 只走融合认证短信，普通短信限流主要用于历史包 / 应急接口保护
- 风险：这还不是完整可上线登录：阿里云 `CreateSchemeConfig` 已返回 SchemeCode，且 DYPNS AccessKey / Secret、`DYPNS_FUSION_SCHEME_CODE`、包名、签名、短信签名和验证码模板已写入本机密钥文件与 ECS 环境；Android 融合认证 SDK / AAR 已导入并接到登录页。2026-06-13 最新收口后，新包不再提供本机号码一键登录入口，Manifest 显式移除 `READ_PHONE_STATE`，登录页只调用 `FusionOneLoginClient.startSmsLogin(...)`，SDK 初始化前继续调用 `useSDKSupplyCaptchaModule(false)`，半程 verify-only 回调不会消费 token，最终 `onVerifySuccess` 才由 `/api/auth/fusion/login` 换手机号并签账号 token。仍需在阿里云融合认证 100001 的全局策略和当前方案 Code 的方案策略里保持“短信认证”为主路径，并关闭 / 删除号码认证和图形验证节点；如果新包仍弹一键登录授权页或图形验证，优先按云端策略未生效 / 方案选错 / SDK 策略缓存排查，而不是恢复 App 端双路径。短信认证需要用户用新包真机重新回归，旧验证码不要复用；已暴露过的主账号 AccessKey 需要在上线前轮换；旧 `X-User-Id` 兜底在公开生产仍需严格关闭，旧无 `session_id` bearer token 也不应在公开生产兼容。旧本机资产迁移现在默认只接受旧 token 证明，不再接受未证明 UUID；若未来为极短迁移窗口设置 `AUTH_ALLOW_UNPROVEN_LEGACY_UUID=true`，必须有审计复查并且 readiness 会失败，不能作为公开生产常态。所有会员、额度、加油包、订单、礼品卡、帮助反馈、App 日志、记忆文档、聊天归档和注销申请都必须继续收敛到同一个账号ID，后续接支付不能再按手机号或本机 ID 分叉
- 后续动作：正式公开上线前必须完成 AccessKey 轮换、HTTPS 公网入口下的融合短信真机登录回归和旧 `X-User-Id` / 旧 bearer token 兜底隔离；DYPNS 基础凭证、SchemeCode、包名、签名、短信签名、验证码模板和 Android SDK 客户端链路已配置；Redis 验证码 / 限流已接入；`scripts/check-android-build-parity.ps1` 和 GitHub Android CI 已负责拦截 debug / release 后端、签名、融合短信主链、电话状态权限、明文网络配置和 debug-only 预览面板再次分叉；生产环境保持 `APP_SECRET` 与 `AUTH_STRICT=true`，不要设置 `AUTH_ALLOW_LEGACY_TOKEN` 或 `AUTH_ALLOW_UNPROVEN_LEGACY_UUID`。`scripts/check-ecs-readiness.ps1` 现在会硬拦生产误开这两个兼容开关，让这条不只停留在文档规则
