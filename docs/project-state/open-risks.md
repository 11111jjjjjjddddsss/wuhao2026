...
- 补充：2026-06-13 继续巡检监控面板和服务健康页时，已把后台可见 SLS 文案统一到当前状态：5 条 AlertHub 最小告警已绑定邮件行动策略和最小仪表盘，ECS / RDS / Redis 资源水位另走云监控邮件；旧的“外部通知、资源水位和仪表盘仍需继续补”口径已从页面和监控 notes 中移除，并新增单测防回潮。剩余仍是首封 SLS 告警邮件送达确认、更细趋势和完整 Nginx access 聚合。
- 补充：管理后台总方案见 [management-backend.md](D:/wuhao/docs/runbooks/management-backend.md)，详细页面设计见 [admin-dashboard-design.md](D:/wuhao/docs/runbooks/admin-dashboard-design.md)，架构决策见 [ADR-0004-admin-backend-architecture.md](D:/wuhao/docs/adr/ADR-0004-admin-backend-architecture.md)。后台当前覆盖登录、账号安全、总览、监控面板、产品洞察、用户、会员额度、订单、礼品卡、帮助反馈、App 日志、今日农情、检查更新、审计和服务健康；账号安全已支持登录后自助改密和 `must_change_password` 强制改密硬兜底；监控面板已聚合真实业务表、App 自动日志、审计、健康状态和地区分布，并补了“当前结论 / 就绪-需处理-阻塞 / 上线人工确认项 / 登录与账号ID / 礼品卡与权益 / 客服反馈 / App质量”决策卡，以及登录排障、登录阶段漏斗、检查更新排障卡和上线前真机回归清单；产品洞察首版已补脱敏聚合趋势、反馈主题命中、App 事件分类、Top App 事件和礼品卡失败原因，不返回聊天全文、反馈正文、图片 URL、手机号、token、模型 Key 或礼品卡完整码；礼品卡后台已补汇总、完整卡码加密保存后页面查看复制、尾号 / 批次 / 账号ID追溯和失败原因聚合，owner / finance_ops 可看完整卡码，owner / support / finance_ops 可看完整手机号；帮助反馈已补 open / replied / closed 队列、状态筛选、搜索、关闭和重开，客服图片展示收紧到同源 support 图片，状态备注会拒绝敏感全文；今日农情补跑、检查更新发布 / 停更、礼品卡作废、反馈状态更新等写操作已有二次确认或按钮忙碌态和失败弹窗，长字段会换行，降低管理层试用时的误操作和表格撑破风险。新增 `scripts/check-admin-surface.mjs` 只读巡检前端路由、按钮动作、后台 API 调用和服务端授权矩阵是否漂移；新增 `scripts/check-admin-authenticated-smoke.ps1`，可用 owner 凭据做登录后只读核心 API smoke，但本机当前没有后台密码，尚未真实跑通这项。真机回归清单只把登录、聊天、图片、礼品卡、今日农情、检查更新和反馈入口串起来辅助测试，不代表这些 Android 真机链路已经全部验收完成。SLS 已有 `nongji-server-5xx`、`nongji-server-slow`、`nongji-nginx-upstream`、`nongji-daily-agri-failed`、`nongji-model-auth-config` 5 条 AlertHub 最小告警，已绑定邮件行动策略和最小仪表盘，其中正常 200 的 `/api/chat/stream` 长连接已从慢请求事件分流为 `http_sse_stream`，避免污染慢请求告警；但第一封 SLS 告警邮件送达确认、登录后 smoke 实跑、更细按版本 / 设备 / 网络环境的登录转化趋势、发布 / 回滚写操作、产品洞察日报 / 人工标签 / 处理状态仍未接入；内部共享密钥接口仍保留给脚本兼容，但浏览器后台不应持有内部 secret。
...
- 后续动作：保留 `check-sls-alert-readiness.ps1 -RequireExternalNotification -RequireDashboard -FailOnWarning` 作为巡检门槛，并在真实或测试告警触发时确认邮件送达；用 owner 凭据执行 `check-admin-authenticated-smoke.ps1 -RequireOwner` 完成一次登录后核心 API smoke；继续补数据库只读脚本、后台账号受控重置 / 禁用 / 角色管理，把真实发版、回滚、查日志、查库、客服回复、礼品卡或会员运营入口沉淀到 runbook，并按最小权限继续拆角色。
...
- 补充：2026-06-12 readiness 脚本继续收紧，远端 `nginx -t` 配置检测失败会直接让检查失败，不再只打印后继续跑后续项；这能防住反代配置坏掉时的“假绿”。同日新增 `scripts/check-resource-capacity.ps1` 和 `scripts/check-backend-data-boundaries.ps1`：资源巡检显示 ECS / RDS / Redis / OSS 容量宽裕，OSS 已开启默认服务端 AES256 加密；ECS / RDS 是包年包月，删除保护接口不适用，Redis 释放保护已开启；ECS 系统盘已绑定普通低频自动快照策略，每周二、周六北京时间 04:00 创建并保留 7 天；资源巡检现在会深度校验云监控资源、阈值和周期，严格模式会把 SLS 外部通知 / 仪表盘缺失当失败，当前已通过；数据边界巡检显示当前没有正式用户、没有会员 / 订单 / 礼品卡资产，且所有需要账号归属的表非 `acct_...` 计数均为 0，并会输出 24 小时 App warn / error Top 事件辅助登录排障。剩余风险仍是真机登录 / 主聊天 / 图片问诊回归、App 备案 / App 公安备案、AccessKey 轮换、首封 SLS 告警邮件送达确认、独立公网黑盒 healthz 监控和帮助反馈图片生命周期。
- 补充：同日发布 / 回滚脚本补齐后台 `/admin-api/` 上游同步切换和 401 验证，且生产 health 断言加入 `sms=ok`；严格资源巡检会把 SLS warning 漂移也当失败。剩余风险仍是真机登录 / 主聊天 / 图片问诊回归、App 备案 / App 公安备案、AccessKey 轮换、首封 SLS 告警邮件送达确认、独立公网黑盒 healthz 监控和帮助反馈图片生命周期。
