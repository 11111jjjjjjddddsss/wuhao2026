...
- 补充：额度扣减在轮次归档成功后执行；若 `ConsumeOnDone` 临时失败，服务端会按同一 `client_msg_id` 短重试，重复扣由 `quota_ledger` 唯一键防住。这个口径优先保证用户收到已归档答案并能通过 snapshot 恢复，不再因为一次扣减临时错误卡住完成态
- 风险：WorkManager 不是实时任务，系统可能延后执行；后端进行中锁当前用 30 分钟租约防死锁，若服务进程极端卡死且租约过期，后续请求才会重新接管。这个设计优先保护成本，不承诺像前台直播一样立刻可见；如果模型已经吐完但归档写库本身失败，客户端不会收到完成态，该轮仍可能需要用户重试或人工排障；如果服务进程在归档后、扣减前崩溃，或扣减数据库连续异常且短重试也失败，可能出现单轮成本漏记，需要后续结合日志 / 对账任务补偿
