...
- 风险补充：2026-06-14 主界面短提示从输入框上方改为屏幕中部全局浮层，额度耗尽仍在消息流里保留 assistant 重试尾巴，状态文案为“今日额度已用完”、按钮仍叫“重试”，今天点击只提示额度，第二天额度恢复后可沿用原重试链继续发同一条；限流和活动流进行中这类明确不可立即重试的中断只保留中部提示，避免和输入框、消息尾部或其它黑色提示重叠。跳到底部 / 静态底部锚点会把今日农情等视觉尾部纳入目标，减少只有卡片或卡片在尾部时的回底错位；回到底部按钮当前只在用户滚离底部后短暂显示约 2.2 秒，避免长期压住内容。若今日农情原锚点消息被 30 轮 UI 窗口裁剪，卡片会插在历史保留提示之后，降低首屏提示和农情卡顺序错乱风险。该改动只调整状态展示和视觉锚点，不恢复 overlay、反向列表或 raw delta；仍需真机继续观察输入法、今日农情卡片、长回复、额度耗尽和复制菜单同时出现时是否还有遮挡。
