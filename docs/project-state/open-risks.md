...
- 后续动作：下一轮真机回归优先只看 4 件事：发送起步小球是否回到工作线、streaming 过程中是否继续贴底、完成态是否仍无底部空白、回到底部按钮/首次进入贴底是否仍工作；若继续改发送抖动，只围绕 `commitSendMessage()`、`pendingStartAnchorScrollOffsetPx`、`scrollToBottom(false)` 和 `alignChatListBottom()` 这一条正向主链排查
