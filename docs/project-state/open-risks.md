...
- 风险：这还不是完整可上线登录：阿里云 `CreateSchemeConfig` 已能返回 `OK`，但 CLI 未返回 `SchemeCode`，需要从控制台核对并配置；Android 一键登录 SDK / AAR 尚未导入，当前一键登录按钮不会消耗融合认证试用次数，只提示先用验证码；短信登录依赖 `DYPNS_SMS_SIGN_NAME` 和 `DYPNS_SMS_TEMPLATE_CODE`，ECS 环境变量尚未配置；Redis 已购买且代码已接认证限流，但生产环境变量 / 部署验证仍需收口；旧 `X-User-Id` 兜底在公开生产仍需严格关闭
