...
- 补充：买服务器前“账号 / 手机号登录与生产鉴权”巡检已记录到 [pre-server-feature-audit.md](D:/wuhao/docs/runbooks/pre-server-feature-audit.md)。当前 Android 的 `SESSION_API_TOKEN` 只是 Gradle 静态注入字段，不是正式登录 token；release 构建已禁止非空 `SESSION_API_TOKEN`，避免把一个共享静态 token 打进正式 APK 后被后端严格模式解析成同一个 `userID`
