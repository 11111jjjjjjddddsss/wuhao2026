...
- 后端独立“【诊断约束】”按用户最新拍板继续去模板化并已部署生产，最终线上 revision 为 `3a28380f`：先删除“候选判断要说明支持点 / 反证 / 验证方法、处理建议要说明适用前提 / 主要风险 / 避免唯一答案”这类固定格式尾句，又继续删除“带图时先详细描述图片关键信息、证据不足明确不确定、不要把单张图片或单轮描述写成确诊”这一段，因为主对话锚点已有相关问诊边界。当前独立诊断约束只保留禁止表格、移动端简洁排版、不要刻意套模板 / 不限固定格式、默认中文表达、必要英文例外、本轮图片和客观信息优先，以及时间地点天气历史记忆只作风险背景。该改动只影响后端每轮主聊天额外 system 约束，不改主对话锚点、今日农情提示词、记忆文档提示词，不新增内容过滤、关键词拦截、表格硬拦截、字数硬卡或 `max_tokens`；旧版和新版 Android 都会通过线上后端吃到。验证通过 `go test ./...`、ECS 远端测试 / 编译 / 部署、`check-ecs-readiness.ps1 -ExpectedRevision 3a28380f` 和 `check-public-blackbox.ps1 -ExpectedAndroidUpdateVersionCode 4 -PreviousAndroidVersionCode 3`，公网黑盒 `warnings=0 / errors=0 / status=ready`。

- 按用户明确口令发布 Android 普通更新版 `1.0.3(4)`，并同步后端、后台、官网和检查更新配置：正式 APK 已用固定 release 签名构建并上传 OSS `android/releases/4/nongjiqiancha-1.0.3-v4-849e8283.apk`，公网 URL 为 `https://download.nongjiqiancha.cn/android/releases/4/nongjiqiancha-1.0.3-v4-849e8283.apk`，大小 `14,193,276` 字节，SHA-256 为 `cd26be1da35b339019f86d1c7d33322b98f4840f8d42127840ddda0e7b0d1f9d`；生产检查更新已启用普通更新，`force_update=false`，旧 `versionCode=3` 会收到 `has_update=true`，当前 `versionCode=4` 返回无更新。官网 `nongjiqiancha.cn` / `www.nongjiqiancha.cn` 下载按钮已部署到同一新版 APK。生产后端部署到 `849e8283`，ECS readiness 显示 `/healthz revision=849e8283`；后台静态包 SHA-256 为 `ac2e8d3f1fbcb2036cf278a5517136c5a47273a1938aa3dcaf1895381f7d51aa`，官网静态包 SHA-256 为 `744ff7b98e223782f68b5d85178c909f925cf138606af81fb841319c874ba4ff`。本轮小修包含中部浮层收起动画随正式包带出、图片不可用文案更直白、用量免责声明识别补 `g/亩` 等写法、后台 / 官网 / 检查更新 APK URL 继续拒绝显式非 443 端口、后台检查更新页强调普通更新不强制，以及后端独立“诊断约束”新增“回答不要刻意套固定模板；不限固定格式，根据本轮问题自然组织，避免呆板机械”。验证通过：Android debug 编译 / 单测、release 构建和物料校验、下载域名和公网 APK SHA 回验、`check-app-update-release-match.ps1 -RequireEnabled -VerifyDownload -PreviousVersionCode 3 -ProbePreviousVersionUpdate`、`check-ecs-readiness.ps1 -ExpectedRevision 849e8283`、`check-public-blackbox.ps1 -ExpectedAndroidUpdateVersionCode 4 -PreviousAndroidVersionCode 3`、后台登录后只读 smoke。`check-launch-readiness.ps1 -AppUpdateReleaseGate` 剩余失败来自 owner 全量 smoke 凭据、安装未知应用权限和备案 / AccessKey / 首封告警 / 短信余额 / 最终真机回归等人工确认项，不是本次普通更新下发链路失败。本轮不改主对话锚点、今日农情提示词、记忆文档提示词，不新增内容过滤、关键词拦截、字数硬卡或 `max_tokens`。

- 后端 / 管理后台按只读巡检结果做低风险小修并部署生产：后台 App 日志“图片上传失败”快捷筛选从错误的 `upload.*` 改为精确 `image.upload_failed`；用户详情 / 用户列表跳帮助反馈时拆分“会话搜索词”和“消息正文搜索词”，避免账号ID被当作客服正文关键词；用户列表待回复标记改为同时看 `support_conversations` 状态和最后非系统消息，已处理 / 已关闭会话不再误标待回复；帮助反馈后台回复和状态更新的早期失败分支补审计，检查更新后台写接口 body 超限改按 `413 body_too_large` 返回；正式 APK URL 校验在后端、公网黑盒和 release-match 脚本里都补非 443 端口拒绝；后台登录后 smoke 覆盖检查更新发布历史接口；监控行动脚本默认排除 `launch_only` 上线准备项，公网黑盒在指定期望最新版时提醒同时传真实旧包 `versionCode`。生产后端已部署到 `d75f4bcf`，后台静态包 SHA-256 为 `3407ca5501dd19ff77d02c3e6d620792064fec2d962442e99882f0f0dee2347d`；ECS readiness、公网黑盒、后台登录后 smoke 和监控行动项复验均通过。同步修正检查更新和管理后台 runbook 旧口径；本轮不改 Android 正式包、检查更新下发配置、官网首页或三份保护提示词。

- Codex 三个本机全局自动化已重新核对并同步 runbook 口径：`今日农情人工发布` 仍为每天北京时间 `22:00 / 23:00`、本地 `D:\wuhao`、`gpt-5.5 + xhigh`，提示词已包含放开选题池、先找 6 到 10 个候选、稍微接地气但保持正式准确、禁止改后端今日农情提示词 / 主对话锚点 / 记忆文档提示词 / 业务代码；`运维自动化` 实际时间为每天 `23:20`，避开今日农情 23:00 补跑窗口；`农技千查续费与证书巡检` 实际时间为每周一 `09:30`，只读报告续费、余额、成本和证书问题，不自动同步证书或修改云资源。2026-06-23 今日农情人工发布状态只读检查为 `missing / should_publish=true`，今晚自动化应正常尝试发布。

- Android 手机中部浮层提示做展示小修并已随 `1.0.3(4)` 正式包带出：`GlobalStatusHint` 不再使用 `AnimatedVisibility` 默认尺寸收缩动画，改为淡入淡出加轻微中心缩放，避免提示消失最后一小段圆角形状被挤压得不规则。该浮层不只限主聊天界面，设置、帮助反馈、礼品卡、复制成功和链接失败等短提示也会走同一层；debug-only 预览面板分组同步改名为“手机中部浮层”。该改动只影响 Android 展示层和预览 / 文档命名，不改提示文案、主聊天逻辑或后端。

- 按用户明确口令发布 Android 修复版 `1.0.2(3)` 并同步后端、后台和官网：本轮 Android 小修包含登录页手机号 / 验证码 / 倒计时 / 错误提示等可保存状态，降低 Activity 重建后验证码状态丢失；帮助反馈图片若已经是合规 JPEG、`<=1MiB`、可解码、长边不超过 1024 且 EXIF 方向正常，则直接上传，避免从私有缓存图再重复压缩；未知来源安装权限页返回后若已授权会继续同一更新下载 / 安装流程，减少提前报“安装未完成”；检查更新 APK URL 和远端图片 URL 继续拒绝显式非 443 端口。后台 / 后端小修包含：用户列表 / 用户详情跳“看反馈”时自动切到全部历史范围，避免只看默认 30 天漏掉旧反馈；无反馈权限时不显示跳转入口；检查更新监控只把 `app_update.install_intent_failed` 算作安装页失败，不再把用户取消或未完成安装混入失败计数。正式 APK 已用固定 release 签名构建并上传 OSS `android/releases/3/nongjiqiancha-1.0.2-v3-87b0963d.apk`，公网 URL 为 `https://download.nongjiqiancha.cn/android/releases/3/nongjiqiancha-1.0.2-v3-87b0963d.apk`，大小 `14,193,280` 字节，SHA-256 为 `706c07765a52bd5aa76ab2b077bd3e63bc7340835ec8fa7a490d2740e3aae1e9`；生产后台检查更新已通过 owner 正规接口启用普通更新，`force_update=false`，旧 `versionCode=2` 会收到 `has_update=true`，当前 `versionCode=3` 返回无更新。官网 `nongjiqiancha.cn` / `www.nongjiqiancha.cn` 下载按钮已部署到同一新版 APK。生产后端部署到 `87b0963d`，ECS readiness 显示 `/healthz revision=87b0963d`；后台静态包 SHA-256 为 `b0ec661fc79911c2edf8b373eb247f40f7ee21fa30ef6738dac20332a60dbe59`，官网静态包 SHA-256 为 `275250fc5d498ff65aca5e786af8b38e3b4cf983ced1a283753ac958d71e5f83`。验证通过：`go test ./...`、`go build ./...`、admin build、`check-admin-surface.mjs`、Android `compileDebugKotlin` / unit test、`check-android-build-parity.ps1`、release artifact 校验、下载域名校验、公网 APK 大小 / SHA 回验、`check-app-update-release-match.ps1 -RequireEnabled -VerifyDownload -PreviousVersionCode 2 -ProbePreviousVersionUpdate`、`check-ecs-readiness.ps1 -ExpectedRevision 87b0963d`、`check-public-blackbox.ps1 -ExpectedAndroidUpdateVersionCode 3`、后台登录后只读 smoke 和官网脚本地址探针。监控动作巡检现在无 bad，仅剩安装未知应用权限需要用户确认的 warning，属于旧包真机覆盖安装回归项。本轮不改主对话锚点、今日农情提示词、记忆文档提示词、真实支付或主聊天滚动主方案。

- 本机 Codex 全局自动化 `今日农情人工发布` 按用户反馈继续放开选题池并同步 runbook：自动化提示词和 `docs/runbooks/codex-automations.md` 现在要求先找 6 到 10 个中国种植侧公开新闻候选，再选 3 条分散地区 / 作物 / 主题的内容；除栽培管理、植保病虫、苗情墒情外，也可纳入政策补贴、农资农机、种业种苗、节水灌溉、绿色防控、技术培训、产地流通、批发价格、仓储保鲜、特色作物、地方农技推广和区域生产动态。最近 3 天已写过的同地区 / 同作物 / 同进展，只有出现明显新进展才继续写，否则优先换方向。该改动只影响本机 Codex 自动化和仓库 runbook，不改后端今日农情提示词、主对话锚点、记忆文档提示词、业务代码、发布脚本或后端内容过滤。

- 后台管理页按“功能保留、默认简洁”继续降复杂度并已部署到 `admin.nongjiqiancha.cn`：联网参考渐进披露、后台导航层级、手机端少入口和“一次只聚焦一个任务”的成熟设计原则后，默认切到“简单后台”模式；侧栏优先展示 `首页 · 今天先看 / 用户管理 / 帮助反馈 / App日志 / 礼品卡`，其它页面直接显示在左侧“更多工具”可见分组中，不再折叠隐藏。首页默认只显示当前结论、今天先处理、常用操作和核心决策卡；智能问诊链路、手机实测清单、上线前检查、趋势、地区和功能接入情况折叠到“更多监控和上线检查”。顶栏保留搜索、手机常用入口、简单 / 全部功能切换、截图模式、刷新和退出，手机端隐藏当前页小字、角色、账号名和重复账号安全按钮。真实路由、后台 API、后端权限、CSRF、审计、高风险二次确认、owner 完整手机号可见和低权限脱敏均不变；不改 Android、官网、检查更新、正式包或三份保护提示词。后台静态包 SHA-256 为 `9c2f6152df903c13d0339397f5a274fe749c9e54705c32c48e91b7cec0beed13`；`npm run build`、`check-admin-surface.mjs`、`git diff --check`、部署脚本 HTTPS / JS / 未登录 401 验证、公网黑盒和登录后只读 smoke 均通过。监控行动脚本仍因最近 24 小时检查更新 / 安装权限类 App 日志报失败，按 App 更新回归提醒处理，不是后台静态页部署失败。

## 2026-06-21

- 后台管理页继续做手机适配和傻瓜化操作并已部署到 `admin.nongjiqiancha.cn`：导航改成“首页 · 今天先看 / 数据总览”，监控页“先处理事项”前移，顶部补账号ID / 手机号全局搜索、截图模式和手机常用入口；帮助反馈详情补返回队列、下一条待回复、打开用户详情和常用回复模板；App 日志补常用筛选和清空筛选，审计筛选也可清空；多张宽表在 720px 以下转字段卡片，按钮 / 输入框 / 链接触摸面积加大，详情跳转和返回在手机上会自动滚到对应区域。截图模式只是前端临时遮罩完整手机号、完整礼品卡码、反馈正文、问诊全文和图片，不改变后端权限或审计。后台静态包 SHA-256 为 `b8175e995f71cab73d87b9fcbc7253cb4ac39982ae4f75cb8be26f8ff0bf41fe`；`npm run build`、`check-admin-surface.mjs`、`git diff --check`、手机卡片字段名扫描、部署脚本 HTTPS / JS / 未登录 401 验证、公网黑盒和登录后只读 smoke 均通过。监控行动脚本仍因最近 24 小时检查更新 / 安装权限类 App 日志报失败，按 App 更新回归提醒处理，不是后台静态页部署失败。

- 后台管理页继续按“一个主账号、简单好用”补易用操作并已部署到 `admin.nongjiqiancha.cn`：用户列表的反馈列直接提供“看反馈”跳转，用户详情的最近反馈区提供“打开反馈会话”，账号ID和手机号支持一键复制，完整手机号在详情里提供拨打链接；用户筛选和帮助反馈筛选增加“清空”，复制成功改为底部轻提示，不再弹窗打断页面。手机端跳到用户详情 / 反馈详情时保留顶部留白，减少被顶栏遮住；帮助反馈 runbook 同步删除“会话列表默认只展示脱敏号”的旧口径。后台静态包 SHA-256 为 `e14b7e35c46b0545aabab8a4d0990638a4919a1fb1af78074d129e2fdd688a9e`，部署脚本验证 HTTPS 首页 / 首个 JS / 未登录 auth 401 通过，部署后公网黑盒 `warnings=0 / errors=0 / status=ready`。该改动不改 Android、官网、检查更新、正式包或三份保护提示词。
...
- 后端主聊天尾段和后台列表隐私做最小稳态修复：模型上游 `[DONE]` 后的 session 归档、扣次、outbox 标记改用短超时上下文，inflight 释放也加独立短超时，避免数据库极端卡顿时请求长期挂住、后续重发 / 新发被占用；失败仍沿用现有 `STREAM_ARCHIVE_FAILED`、扣次后台重试和远端 snapshot 恢复链，不改 Android。当时后台列表曾收紧为不批量返回完整手机号；同日后续已按“一个 owner 主账号后台”重新改为授权主账号列表直显完整手机号，低权限巡检仍脱敏，以本节上方后台收口记录和 `current-status.md` 为当前真相。Android pending / remote completion 单测继续通过。
...
- 后端主聊天诊断约束继续收紧中文表达：原“避免夹杂不必要英文术语”改为“默认必须使用中文表达，禁止中英混写；普通英文词和英文短语必须改成中文说法，不得直接夹在中文句子里”，保留农药 / 肥料成分、品种名、病原拉丁学名、登记标签、品牌商品名和用户原文确需保留英文的例外，并要求尽量先给中文解释。该改动只调整每轮主聊天独立 system 约束，不改 Android、不改主对话锚点、今日农情提示词或记忆文档提示词，不新增英文硬过滤、关键词拦截、字数硬卡或 `max_tokens`。

- 后端主聊天补上游流式 watchdog：百炼 SSE 已连接但 3 分钟没有任何可见正文时返回 `UPSTREAM_STREAM_TIMEOUT`，开始出字后若 4 分钟没有新的可见正文返回 `UPSTREAM_IDLE_TIMEOUT`；这类超时不归档、不扣次，只记录请求 ID、账号、是否强制搜索、是否开启思考、预算和可见字数等脱敏字段。SSE `data:` 如果不是合法 JSON，也不再原样透传给 Android，避免极端上游异常绕过 `reasoning_content` 过滤。线上 Nginx 图片读取边界同步收口：上传提交 `/upload` 仍保留限流和连接数限制，普通 / support 图片读取 `/uploads/` 单独去掉 `limit_conn`，避免 DashScope 等模型供应商共享出口 IP 拉图偶发 429；readiness 和流量 runbook 已同步锁住该口径。

- 后端主聊天显式联网触发词按用户最新口径再放宽：`查一下`、`查一查`、`查查`、`帮我查` 现在直接强制 `forced_search=true`，不再要求同时出现产品 / 渠道 / 网购等目标词。这样会让部分普通病虫害 / 用肥问诊也多走联网搜索，成本和延迟略增，但可降低模型回复“我去查 / 正在查找”后直接结束的风险。该改动不改 Android、不改主对话锚点、今日农情提示词或记忆文档提示词。

- 后端主聊天显式联网触发词继续按“够用但不误伤普通问诊”收口：补充 `查价`、`查报价`、`查行情`、`市场价`、`网购`、`淘宝`、`京东`、`拼多多`、`1688` 等明确交易 / 联网线索；`查一下`、`查一查`、`查查`、`帮我查` 只有同时出现产品、牌子、品牌、厂家、渠道、网购、电商等目标词时才强制 `forced_search=true`，避免“查一下这张叶片是什么病”这类普通图文问诊误触发联网搜索。该改动不改 Android、不改主对话锚点、今日农情提示词或记忆文档提示词。
...
- 后端独立“【诊断约束】”第一段按用户最新拍板更新为“所有回答禁止表格。排版尽量简洁，多用短段落和适当空行；不要使用多级列表和项目符号列表，避免长段堆叠。”；其余短版诊断边界保持不变：以本轮图片和客观信息为准，历史 / 天气 / 记忆只作风险背景，带图先描述关键信息，证据不足说“不确定”，不把单张图片或单轮描述写成确诊，候选判断和处理建议写支持点 / 反证 / 验证方法 / 适用前提 / 主要风险。该改动只调整每轮主聊天独立 system 约束，不改主对话锚点、今日农情提示词、记忆文档提示词，不新增内容过滤、关键词拦截、表格硬拦截、字数硬卡、`max_tokens` 或 Android 渲染逻辑。

- 生产后端已部署提交 `18570a8e`，`scripts/check-ecs-readiness.ps1 -ExpectedRevision 18570a8e` 通过，线上 healthz 返回 `revision=18570a8e` 且关键依赖 ready；每轮主聊天独立“【诊断约束】”第一段已从“少用密集项目符号”继续收紧为“不使用多级列表和项目符号列表”并在线上生效。Android 测试包不需要重装即可吃到后端组装词；本轮未发布正式包、未配置检查更新。
...
- 已为当前 `a1b36221002f` 生成最新内部 debug 测试包 `test-apks/debug/20260621/nongjiqiancha-debug-internal-20260621-105725-a1b36221002f.apk`，SHA-256 为 `a6bdcc1b8c3b82b8591e597ed58a1aa3cdc2865b3bf382c976ec3d5e84ac89d1`，大小 `20,958,018` 字节；脚本确认包名、签名、debug 属性、下载域名 200 / 206 探针均 ready，并清理上一版内部测试 APK `test-apks/debug/20260621/nongjiqiancha-debug-internal-20260621-104433-fb272a946a15.apk`。该包包含恢复 Markdown 无序列表可见小点、保留冒号原样展示、继续不改普通段落正文；未发布正式包、未写官网正式下载、未配置检查更新。

- Android 主聊天 Markdown 无序列表恢复可见小点：撤销“无序列表无圆点左齐”的展示口径，`- / * / +` 列表重新在 UI 里显示 `•`，复制纯文本也保留可见小点和嵌套缩进，避免“影响明显：”这类引出句下面的要点看起来像普通散文或漏写。该改动只恢复列表层级展示，不改模型原文、不把冒号改句号、不改段落正文、不改表格、不改滚动主链、不改后端诊断约束或三份保护提示词；debug-only 预览面板和 Android parity 已同步。

- 已撤回 Android 主聊天“孤冒号统一改句号”的展示层试验，恢复 Markdown 原始层级和引出句冒号展示：`2. 对农业的影响` 这类一级标题下方可以用一句带冒号的引出句，再接 `病害风险：...`、`根系缺氧：...` 等二级标签正文。已为撤回后的 `fb272a946a15` 生成最新内部 debug 测试包 `test-apks/debug/20260621/nongjiqiancha-debug-internal-20260621-104433-fb272a946a15.apk`，SHA-256 为 `af17a9be236c258a454fc925a0ef6c353f218180f12d28178ca8722fd46388aa`，大小 `20,956,696` 字节；脚本确认包名、签名、debug 属性、下载域名 200 / 206 探针均 ready，并清理上一版内部测试 APK `test-apks/debug/20260621/nongjiqiancha-debug-internal-20260621-104033-e155d8b8e838.apk`。本轮未发布正式包、未写官网正式下载、未配置检查更新。

- 生产后端已部署提交 `7c632f37`，`scripts/check-ecs-readiness.ps1 -ExpectedRevision 7c632f37` 通过，线上 healthz 返回 `revision=7c632f37` 且关键依赖 ready；这版只让每轮主聊天独立“【诊断约束】”使用用户最新短版口径。Android 测试包不需要重装即可吃到新后端组装词；本轮未打新 Android 包、未发布正式包、未配置检查更新。

- 后端独立“【诊断约束】”按用户最新拍板继续去咬嘴：保留“所有回答不得使用表格”、本轮图片和客观信息优先、历史 / 天气 / 记忆只作背景、带图先描述关键现象、证据不足说不确定、不把单张图片或单轮描述写成确诊、候选判断写支持点 / 反证 / 验证方法、处理建议写适用前提和主要风险；去掉“不使用百分比、分值、几成等精确概率”和“需现场验证”的加强表述。该改动只调整每轮主聊天独立 system 约束，不改主对话锚点、今日农情提示词、记忆文档提示词，不新增内容过滤、关键词拦截、字数硬卡、`max_tokens` 或后端表格硬拦截。
...
- 已为当前 `5817cd0d2a88` 生成最新内部 debug 测试包 `test-apks/debug/20260621/nongjiqiancha-debug-internal-20260621-050206-5817cd0d2a88.apk`，SHA-256 为 `c5e0d2486bee44f4486da62fa6cd8c6099adc3e5880670fe9ef08df97047bd24`，大小 `20,954,159` 字节；脚本确认包名 `com.nongjiqiancha`、debuggable、签名匹配、OSS `test-apks/` 3 天生命周期、下载域名 200 / 206 探针均 ready，并清理上一个内部测试 APK `test-apks/debug/20260621/nongjiqiancha-debug-internal-20260621-033032-98b2054f00ad.apk`。该包只撤掉流式正文变更前预锚点，保留 SideEffect 同帧锚点、程序滚动标记、低频滚动抖动日志和现有渲染 / 表格 / 分割线 / 提示词口径；未发布正式包、未写官网正式下载、未配置检查更新。

- 后端独立“【诊断约束】”继续按用户拍板收口：新增一句“所有回答不得使用表格。”，降低模型老吐表格导致移动端阅读负担的概率。该句只放在每轮主聊天组装的诊断约束 system 消息里，不改主对话锚点、今日农情提示词、记忆文档提示词，不新增后端内容过滤、关键词拦截、表格硬拦截、字数硬卡或 `max_tokens`。

- 后端独立“【诊断约束】”按用户最终拍板继续简化：去掉“用户明确描述的现场现象和已发生操作是主要依据”“用户猜测病名只作待核对线索”“反推诊断”等拗口表述，收成“以本轮图片和客观信息为准，时间 / 地点 / 天气 / 历史 / 记忆只作风险背景，不能盖过本轮证据；带图先详细描述图片关键信息；证据不足明确不确定 / 需现场验证；候选判断写支持点、反证和验证方法；处理建议写适用前提和主要风险”。该约束仍作为每轮主聊天独立 system 消息，不改主对话锚点、今日农情提示词、记忆文档提示词，不新增内容过滤、关键词拦截、表格硬拦截、字数硬卡或 `max_tokens`。

- 生产后端已部署提交 `712afa75`，`scripts/check-ecs-readiness.ps1 -ExpectedRevision 712afa75` 通过，线上 healthz 返回 `revision=712afa75` 且关键依赖 ready；这版只让每轮主聊天独立“【诊断约束】”使用上述简洁口径。现有 Android 测试包不需要重装即可吃到新后端组装词；本轮未打新测试包、未发布正式包、未配置检查更新。
...
- 已为当前 `98b2054f00ad` 生成最新内部 debug 测试包 `test-apks/debug/20260621/nongjiqiancha-debug-internal-20260621-033032-98b2054f00ad.apk`，SHA-256 为 `dc43bf2ce4d6e152cc37e88a7db83a512c9ecb054325b5a707b9f3c41c9118df`，大小 `20,955,479` 字节；脚本确认包名 `com.nongjiqiancha`、debuggable、签名匹配、OSS `test-apks/` 3 天生命周期、下载域名 200 / 206 探针均 ready，并清理上一个内部测试 APK。该包包含 `ui.streaming_scroll_jitter` 低频滚动抖动诊断日志、撤掉布局后补锚、保留预锚点和同帧锚点，以及“正在思考...”约 2.6 秒后出现；只用于内部真机回归，未发布正式包、未写官网正式下载、未配置检查更新。
...
- 后端主聊天后台背景边界按总组装继续收口：当前轮时间 / 地点仍单独注入并仅供参考；独立“【诊断约束】”会作为 system 消息放在历史 / 记忆 / 今日农情之后、当前用户图文之前。当前口径是：本轮图片中可见症状、用户明确描述的现场现象和已发生操作是主要依据；用户猜测的病名、原因或结论只作待核对线索；时间、地点、天气、历史对话和记忆摘要只作风险背景，不能替代症状证据，也不能反推诊断。作答时先详细描述本轮关键可见症状和现场现象，再给出优先排查方向；不使用百分比、分值、几成等精确概率，不把单张图片或单轮描述写成确诊；候选判断应说明支持点、不支持点 / 反证和现场验证方法；涉及用药、用肥或其它处理建议时，应说明适用前提和主要风险，避免把方案说成唯一答案。系统锚点已按用户明确确认移除短版重复诊断约束，只保留地点可信度边界；记忆整理提示词继续防止把时间地点沉淀成诊断依据。该改动不新增内容过滤、关键词拦截、字数硬卡或 `max_tokens`，也不改今日农情提示词。

- 生产后端已部署提交 `33c69a80`，`scripts/check-ecs-readiness.ps1 -ExpectedRevision 33c69a80` 通过，线上 healthz 返回 `revision=33c69a80` 且关键依赖 ready；这版只调整主聊天提示组装，未打新 Android 测试包、未发布正式包、未开启检查更新下发。

## 2026-06-20

- 今日农情临时上下文窗口从视觉项后方连续三轮收窄为连续两轮：只有今日农情已实际进入当前主界面可见范围、并且后端确认是服务器上海当天 ready 后，后方第 1、2 次用户发送才携带 `today_agri_context_day`，第 3 次起自动不带，失败本地用户消息不占轮数。Android 发送侧、后端 anchor 轮数兜底、debug-only 预览、Android parity 和单测已同步到两轮。该改动不修改今日农情提示词、不改今日农情生成、不把农情写入记忆或归档。

- Android 主聊天等待态按后端思考策略加轻量“正在思考”过渡：普通文字问诊的空回复等待仍只显示原有呼吸小球；只有当前 active 轮用户消息带普通问诊图片、对应后端会开启图片诊断思考时，等待态才会先显示小球，约 1.8 秒后淡切成“正在思考...”，省略号循环闪动。debug-only 预览面板保留“清数据首次发送”为普通小球样例，并在“图片与预览”新增“图片问诊思考”专门看动画；Android parity 锁住图片问诊才显示思考文字。该改动只影响等待视觉，不改变主聊天滚动链、输入框、发送 / 重发 / 重试逻辑、模型输出或后端接口。

- 后端主聊天模型请求策略当时按图片诊断收口：纯文字关闭思考，带普通问诊图片的本轮请求才显式开启思考，并限制 `thinking_budget=1024`，不按会员区分，也不让文字问诊因关键词自动开思考。供应商返回的思考内容只在内部字段中出现，农技千查 SSE 仍只转最终正文给 Android，不归档、不展示思考过程，也不向用户、协议、官网或后台公开模型内部名称。上游非 200 错误会额外记录脱敏响应摘要，遮蔽图片 URL、token、密钥等敏感片段，方便继续定位图片问诊 400；该改动不新增内容过滤、关键词拦截、字数硬卡、`max_tokens` 或客户端模型 Key。
...
- 生产后端已部署当前代码提交 `2a4b9aa0`，`scripts/check-ecs-readiness.ps1 -ExpectedRevision 2a4b9aa0` 通过；这版包含图片问诊思考开启、`thinking_budget=1024`、`reasoning_content` 不下发、上游错误脱敏摘要、今日农情后方两轮上下文和诊断约束总组装。ECS 生产环境已用公开示例图片做一次不打印密钥 / 模型名 / 正文内容的供应商探针，返回 `200 text/event-stream`，拿到最终正文，且确认供应商会返回思考字段；服务端仍只下发最终 `content` 给 Android。已为同一提交生成最新内部 debug 测试包 `test-apks/debug/20260620/nongjiqiancha-debug-internal-20260620-231758-2a4b9aa0684e.apk`，SHA-256 为 `adc5434c6cdb7b86e54fe8bd32cf0f597c88b87ea1ea73e3ad65d484bbfa4e99`，大小 `20,950,012` 字节；脚本确认包名 `com.nongjiqiancha`、debuggable、签名匹配、OSS `test-apks/` 3 天生命周期、下载域名 200 / 206 探针均 ready，并清理上一个内部测试 APK，OSS `test-apks/debug/` 当前只剩 1 个对象。该包包含图片问诊“正在思考...”动画、图片问诊思考预览面板、AI Markdown `--- / *** / ___` 控制符删除、装饰 emoji 清洗和预览面板同步；该包只用于内部真机回归，未发布正式包、未写官网正式下载、未配置检查更新。
...
- Android 短业务提示统一收敛到主屏中部浮层：主聊天图片数上限、图片读取 / 上传失败、相机打开失败、额度 / 网络 / 服务忙等原有 `showComposerStatusHint` 继续走中部浮层；设置页 / 帮助反馈 / 礼品卡 / 今日农情历史等菜单内 `showNotice/onPendingAction` 短提示也上抛到同一中部浮层，不再保留菜单底部小黑条。中部浮层不再因附件面板、菜单、会员中心或 debug 预览打开而隐藏，只在文本选择菜单打开时避让；渲染顺序在附件面板之后，zIndex 提高到 120。已选满 4 张时，在附件面板里点“相机”或“照片”会立即看到“最多4张图片”；debug 预览面板“主界面中部浮层”新增菜单短提示样例，Android parity 新增对应层级、菜单提示上抛和预览样例护栏。
...
- 为当前 `7b57bfbcc184` 生成最新内部 debug 测试包 `test-apks/debug/20260620/nongjiqiancha-debug-internal-20260620-211653-7b57bfbcc184.apk`，SHA-256 为 `b4b05eca5a2c4e5c70b724970b8b4bee4c7c935cba906afbe8586c320610105b`，大小 `20,942,639` 字节；脚本确认包名 `com.nongjiqiancha`、debuggable、签名匹配、OSS `test-apks/` 3 天生命周期、下载域名 200 / 206 探针均 ready，并清理上一个内部测试 APK，OSS `test-apks/debug/` 当前只剩 1 个对象。该包包含设置页“隐私与协议”新顺序、预览面板同步、短业务提示统一到主屏中部浮层、Markdown 列表无圆点左齐、表格图标只复制当前可见表格内容和主动发送 / 重发 / 重试链路修复，只用于内部真机回归，未发布正式包、未写官网正式下载、未配置检查更新；本轮没有部署生产后端。

- Android 图片问诊继续按真机复现收口：后端 `Content-Length` 修复部署后，上游已能正常拉取同一 `/uploads/*.jpg` 并返回 200；用户安装新测试包后仍看到“正在重试...”时，线上日志没有新的 `/upload` 或 `/api/chat/stream`，说明不是模型看不到图片，而是 Android 本地发送 / 恢复链被旧 pending、远端启动保护窗或系统网络预检卡住。主聊天用户主动发送 / 重发 / 重试现在不再被 `hasActiveNetworkConnection()` 本地预检 fail-closed，直接尝试真实上传 / SSE，请求失败再走现有失败态；帮助反馈发送也不再做本地网络预检，真实上传 / 提交失败后再提示。带图用户主动发送 / 重试会用 `REPLACE + 0ms` 重新挂同一条 WorkManager 兜底，避免安装覆盖后旧唯一 work 挡住新动作；前台流可恢复中断时会释放 active 并重置 pending 的 `remoteStartedAtMs`，已有可恢复失败的 pending 不再等完整 10 分钟远端启动保护窗，前台恢复循环会在 60 秒后转通用失败 / 可重发。表格单元格灰色字段标签字号同步到正文 15sp / 23sp，仍保持正常字重、灰色、字距 0；Android parity 和单测已同步锁住这些边界。本轮不改后端、不改主聊天滚动主方案、不改三份保护提示词、不新增模型输出过滤或 `max_tokens`，仍需新包真机验证新拍 / 新选图会实际产生 `/upload` 和 `/api/chat/stream`。
...
- 为无摘要表头 / 首行内置 GPT 式复制图标提交 `ff0deb6d` 生成最新内部 debug 测试包 `test-apks/debug/20260620/nongjiqiancha-debug-internal-20260620-182724-ff0deb6d86be.apk`，SHA-256 为 `03729883ef107af6c65366a77a9581306280b1a9b342aea45595ecd26f1e8df7`；脚本确认包名、签名匹配、debug 属性、OSS 生命周期、下载域名 200 / 206 探针均 ready。该包只用于内部真机回归，未发布正式包、未写官网正式下载、未配置检查更新，也未部署生产后端 / 后台。

- Android 主聊天表格按用户截图继续收口为一个完整轻量表格框：外层统一 8dp 圆角细边框，不再额外显示“成品含... / 普通...”这类原始表头摘要条；复制动作并入首个维度标题栏右侧，使用 44dp 隐形触摸区 + 28dp 画布的 GPT 式方块复制图标，前方方块会遮住后方方块内侧线，只露出后方右上边角，toast 只显示“已复制”；每个维度在同一表格框内用浅灰行标题和细分割线分层，不再使用外置按钮、黑色大表头、圆框小按钮、多张独立卡片或重复表头摘要。表格只保留每个维度标题加粗，单元格灰色字段标签和正文均为正常字重；单元格正文仍会吃掉 Markdown `**` 标记，但不再应用加粗，避免表格内部到处黑块。嵌套列表视觉缩进从每层 18dp 收窄到 10dp，保留层级但避免二级圆点正文被推到过右。消息尾部已复核仍只在 AI 回复未完成 / 用户发送失败及重试进行中显示可操作状态，过程态不挂尾巴；主界面中部浮层仍为短暂黑底白字提示，菜单、预览、文本选择时隐藏，本轮不改位置。debug-only 预览面板“AI 表格”项同步标注“无摘要表头、正文不加粗、GPT 式复制图标”，“AI Markdown 兜底”项同步标注嵌套列表收窄；Android parity 脚本同步锁住该表格框 / 方块复制图标 / 表格正文不加粗 / 列表缩进 / 预览口径，并防止恢复冗余表头摘要条；`app/AGENTS.md` 已同步补强“用户可见 UI 改动若已有对应预览项，必须同次更新预览面板”的规则。该改动只调整表格复制提示、表格视觉、列表缩进、预览面板说明和对应护栏，不改变 Markdown 表格解析、复制内容、主聊天滚动链、模型输出或图片问诊链路；流式时不会再提前显示只有表头和分隔线的空壳，等首行表格内容到来后展示完整表格。

- 为表格顶部去首列 / 无框复制图标提交 `a455ba60` 生成最新内部 debug 测试包 `test-apks/debug/20260620/nongjiqiancha-debug-internal-20260620-180818-a455ba60893d.apk`，SHA-256 为 `bffb35d64dac5569acb43914b6b40ccdc38e15ab975043a92d0d02aba52eac0e`；脚本确认包名、签名匹配、debug 属性、OSS 生命周期、下载域名 200 / 206 探针均 ready。该包只用于内部真机回归，未发布正式包、未写官网正式下载、未配置检查更新，也未部署生产后端 / 后台。

- 为完整轻量表格框提交 `40f771bf` 生成最新内部 debug 测试包 `test-apks/debug/20260620/nongjiqiancha-debug-internal-20260620-175816-40f771bf3f7b.apk`，SHA-256 为 `37b41df0710e35825c2dc10bee20c8760156bff2d6dc038ad3077116dee330ad`；脚本确认包名、签名匹配、debug 属性、OSS 生命周期、下载域名 200 / 206 探针均 ready。该包只用于内部真机回归，未发布正式包、未写官网正式下载、未配置检查更新，也未部署生产后端 / 后台。

- 为表格表头 / 内置复制按钮提交 `9772256a` 生成最新内部 debug 测试包 `test-apks/debug/20260620/nongjiqiancha-debug-internal-20260620-174324-9772256adc6d.apk`，SHA-256 为 `b466737a700bfda46589e1ab4358b6ceaf3ec5f7e7a1ddd4abe3d50aac5c1e28`；脚本确认包名、签名匹配、debug 属性、OSS 生命周期、下载域名 200 / 206 探针均 ready。该包只用于内部真机回归，未发布正式包、未写官网正式下载、未配置检查更新，也未部署生产后端 / 后台。

- Android 主聊天 Markdown 渲染按联网校准后的 CommonMark / GFM 高频形态继续补齐：嵌套列表保留缩进层级，支持 `+` 列表和 `1)` 编号，任务列表降级为视觉 checkbox 文本，删除线正常隐藏 `~~` 标记并加线，图片 Markdown 只显示说明 / URL 文本且不加载外部图片或生成跳转链接，数学公式 `$...$` / `$$...$$` 原样可见不被吞，普通 Markdown 链接和裸 URL 继续可点击可复制；表格测试补 GFM 对齐分隔行和转义管道。debug-only 预览面板新增“AI Markdown 兜底”样例，方便真机检查嵌套列表、任务项、删除线、普通链接、图片语法和公式原文。该改动不改变主聊天滚动链、分割线、模型输出规则、图片问诊上传 / 入模链路、今日农情或三份保护提示词。

- 为 Markdown 渲染兜底提交 `db53d5d7` 生成最新内部 debug 测试包 `test-apks/debug/20260620/nongjiqiancha-debug-internal-20260620-171831-db53d5d7d875.apk`，SHA-256 为 `d582b5a59869216a9f8c6301b21358365dde1bcd1c3bf5a6617735b6cf9f2dbc`；脚本确认包名、签名匹配、debug 属性、OSS 生命周期、下载域名 200 / 206 探针均 ready。该包只用于内部真机回归，未发布正式包、未写官网正式下载、未配置检查更新，也未部署生产后端 / 后台。

- 为正文密度半步回弹提交 `cfb3dc87` 生成最新内部 debug 测试包 `test-apks/debug/20260620/nongjiqiancha-debug-internal-20260620-163451-cfb3dc877ae5.apk`，SHA-256 为 `3ddd77c1a03fe601dc046ec9a38e158fd6070b7f729ffe3150547da7692a4710`；脚本确认包名、签名匹配、debug 属性、OSS 生命周期、下载域名 200 / 206 探针均 ready。该包只用于内部真机回归，未发布正式包、未写官网正式下载、未配置检查更新，也未部署生产后端 / 后台。

- Android 主聊天 Markdown 表格视觉继续按用户截图反馈收口：仍不恢复横向宽表，避免三列内容在手机上挤压；移动端表格卡片改成更像正规表格的“顶部表头加重 + 每个维度块内置浅底标题栏 + 行分割”结构，卡片字段标签加深并使用 Medium，提升表头 / 字段 / 正文层级。该改动只调整表格视觉层级，不改变 Markdown 表格解析、复制表格内容、流式出壳、主聊天滚动链或模型输出。

- Android 主聊天正文按用户最新截图反馈继续微调阅读密度：AI 正文在上一版偏小后回调到 16.5sp / 27.5sp，streaming 行高为 29sp，章节标题为 19.5/17.5sp，列表圆点同步为 17.5sp；中文正文字距归零，减少截图压缩、粗体词和长行换行叠加造成的“文字有点斜 / 漂”的视觉错觉。今日农情主聊天渲染继续复用普通 AI 标题 / 正文 / 灰色辅助文本样式，避免农情和正常正文出现两套字号。该改动只调整文字视觉密度，不改变 Markdown 解析、表格卡片、分割线、滚动链、输入框或模型输出。
...
- Android 主聊天“回到底部”悬浮按钮按用户真机滑动反馈收紧：用户上滑 / 下滑、LazyList 正在滚动、RecyclerView 报滚动或 runtime 标记用户交互时，按钮立即不渲染，避免手指滑动路过按钮时被它吃掉触摸；停止滑动后仍按原有远离底部 + 短暂 pulse 逻辑显示。带图输入缩略图右上角删除按钮保持原 20dp 视觉圆点，但触摸热区放大到 36dp，降低真机点不准概率。该改动不改变底部锚点、滚动算法、回到底部动作、图片发送链路或主聊天列表结构。

- Android 用户可见文案和主聊天排版继续按“清晰、简洁、少过度提示”收口：礼品卡兑换页删掉解释性正文，只保留兑换卡码输入和动作；检查更新默认说明改为“优化使用体验。”，未知来源安装权限和安装确认提示改成短句；会员中心“套餐”下方的升级补偿次数 / 加油包次数改为横向自动换行标签，正常手机宽度下并排展示；主聊天有序列表的 `1. 2. 3. 4.` 标记和右侧小标题改为按第一行文本基线对齐，修掉数字视觉上比标题文字偏高的问题；AI 正文内 Markdown 粗体从 `SemiBold` 调轻到 `Medium`，标题和表格标题仍保留更强层级，降低整段文字过重的观感。该改动只影响文案和排版，不改变 Markdown 解析、表格、分割线、滚动链或模型输出，不开启真实支付、不发布正式 Android 包、不配置检查更新下发。

- Android 主聊天消息尾部按“少提示，只保留可操作异常态”收口：删除带图发送专属的“后台发送中 / 正在同步回复”尾部展示和 debug-only 预览项，后台带图 WorkManager、remote-completion marker 和 snapshot 恢复仍静默运行；图片还在本地 URI 阶段、已上传成远端图片 URL 但仍等待 AI 回复快照、以及终态失败标记都会进入同一静默恢复链；最终失败才复用“发送失败 · 点击重发”，AI 中断仍用“回复未完成 · 点击重试”。同时修掉 remote-completion 恢复窗口结束后 marker 不清理的问题，避免后台已完成但前台长期拿不到完整回复时留下过程状态。该改动不改变图片上传、扣次、后端幂等、主聊天滚动链或模型输出边界。

- Android 主聊天 Markdown 表格卡片按截图体验微调：保持移动端“每行一张卡片”的表格渲染，不恢复横向宽表；在同一张表格卡片内，从第二个可见字段块开始增加浅灰细分割线，降低多个对比项挤在一个框里的阅读压力。该改动不改变表格解析、流式表格出壳、复制表格内容、主聊天滚动链或模型输出。

- 为表格卡片字段分割线提交 `6bd08804` 生成最新内部 debug 测试包 `test-apks/debug/20260620/nongjiqiancha-debug-internal-20260620-130057-6bd08804ca7e.apk`，SHA-256 为 `39109cab5f503ce8dcf29f0c1a18c85ea00c5470449f687c4cedf2c8d825683f`；脚本确认包名、签名匹配、debug 属性、OSS 生命周期、下载域名 200 / 206 探针均 ready。该包只用于内部真机回归，未发布正式包、未写官网正式下载、未配置检查更新。
...
- 为帮助与反馈占位文案提交 `9d952b45` 生成最新内部 debug 测试包 `test-apks/debug/20260620/nongjiqiancha-debug-internal-20260620-125550-9d952b45ce0a.apk`，SHA-256 为 `77eda8859ce5b295d5a421e59c430cd06cf746eee39d1cb5f42b2fb032de39a2`；脚本确认包名、签名匹配、debug 属性、OSS 生命周期、下载域名 200 / 206 探针均 ready。该包只用于内部真机回归，未发布正式包、未写官网正式下载、未配置检查更新。
...
- 继续把 debug-only 网络预览面板文案收成“无网 / 门户 Wi-Fi；可联网先放行”，并为提交 `99e762aa` 重新生成最新内部 debug 测试包 `test-apks/debug/20260620/nongjiqiancha-debug-internal-20260620-123640-99e762aa8118.apk`，SHA-256 为 `b43d1cc26992764ee556028cfd3c92a1ef51392c29afeffd7c0ea6cb37114cae`；脚本确认包名、签名匹配、debug 属性、OSS 生命周期、下载域名 200 / 206 探针均 ready。该包仍只用于内部真机回归，未发布正式包、未写官网正式下载、未配置检查更新。
...
- 按正式应用市场标准继续校准隐私与协议材料：Android “隐私与协议”本地 6 页保留服务协议、隐私政策、第三方信息共享清单、个人信息收集清单、应用权限和风险提示；官网新增第三方信息共享清单、个人信息收集清单、应用权限、风险提示四个独立静态页，并在官网 footer / 协议页互链，公网黑盒脚本和官网部署脚本同步增加这些页面的 marker 探针。官网静态包已部署到生产，包 SHA-256 为 `11d3f93ea16a90043b59091f53243cef8c955044aeef25028b82a483f235f699`，公网黑盒确认根域名和 www 下 6 个法律页面均 200。清单字段按应用市场常见口径补齐信息类型 / 使用目的 / 使用场景 / 收集方式 / 处理方式 / 拒绝影响、共享类型 / 服务提供方 / 涉及信息 / 共享方式 / 第三方处理规则、权限调用场景 / 调用时机 / 拒绝影响 / 边界；第三方共享清单已按真实接入列明阿里云计算有限公司及其关联服务平台、短信服务平台和公开隐私政策链接，支付未接入真实渠道前只写“购买入口开放后由页面展示的支付服务商”。用户可见文案不展示具体模型名称、模型平台名、云资源 ID、Key 名称、供应商账号或内部环境变量。定位文案收成粗略定位，明确 App 不向农技千查服务器上传经纬度、不保存轨迹，系统定位和地理编码能力按设备系统规则处理；Android 系统粗略定位权限在首次问诊发送时才按需触发，触发前提示“可选粗略定位，生成地区文本；拒绝也可问诊”。本轮不改三份保护提示词、不改官网首页定稿文案、不新增模型输出过滤、不发布正式 Android 包；严格上架前仍需法务 / 目标市场材料复核，若真实服务商、SDK、支付渠道或权限变化必须同步更新，但不得公开密钥、供应商账号、资源 ID 或具体模型名。

- 继续按“全盘挑刺、收口收尾、用户体验优先”落地帮助反馈图片强归属、分割线和正式协议措辞：后端新增 `support_upload_ownership` 记录 support 图片上传归属，迁移会从既有 support 消息回填历史归属，旧本机身份并入手机号账号时 support 图片归属也跟随迁移；用户提交反馈、后台回复携带 support 图和普通用户读取 `/uploads/support/*.jpg` 都必须证明对象属于目标账号，避免拿到别人完整 support URL 后二次提交认领。后台 owner / support 等可看客服正文角色仍可通过后台 session 查看；普通问诊 `/uploads/*.jpg` 不受影响。Android 帮助反馈缩略图和全屏预览仅对可信生产域名 `/uploads/support/*.jpg` 附带 bearer，并按账号隔离 support 图缓存、禁用 support 图 Coil 内存 / 磁盘缓存、退出登录清 support 图缓存。主聊天分割线保持“正文仍是正文、分割线不撤回”：独立标题稳定识别时线在标题上方同帧出现，`**重点** 后面还有正文` 仍按段落显示，但前方已有正文块时会保留轻分割线，避免 streaming 中一会儿出现一会儿消失；记忆摘要空 pending 自动清掉，写回成功会删除实际提取的 pending job，账号合并 pending job 测试补锁。App 内和官网协议 / 隐私把支付改为“购买入口开放后”才由支付服务商处理必要订单和支付状态，定位统一为“粗略定位”，客服说明收成“客服提供的农业技术说明仅供参考”。已验证后端 support / 记忆专项 Go 测试和 Android `ChatStreamingRendererTest`；本轮不改三份保护提示词、官网首页文案，不新增模型输出过滤 / 关键词拦截 / 字数硬卡 / `max_tokens`，不发布正式 Android 包。

- 后续代理复核继续收口上线门禁、下载边界和 Android 交互：ECS 公网黑盒 timer 的最近运行检查改为优先使用 systemd monotonic 时间，再从 `ExecMainExitTimestamp`、`InactiveEnterTimestamp`、`LastTriggerUSec` 和 `list-timers` PASSED 相对时间多字段兜底计算年龄，`check-ecs-readiness.ps1` 和 `check-resource-capacity.ps1` 都会在无法证明 30 分钟内成功时失败，避免 `ExecMainExitTimestamp=n/a` 误报、`CST` 字符串误解析或长期不跑假绿。官网正式 APK URL 校验补拒 userinfo、query string 和 fragment，只接受 `download.nongjiqiancha.cn/android/releases/...apk` 稳定裸地址；测试包下载 runbook 的发包前检查不再使用 `-AllowAttentionExitZero`。Android 冷启动远端 hydrate 晚到且用户已抢先发送时，会在生成结束后把远端历史与本地新增尾部合并，不再丢 pending hydrate，也不覆盖刚发的新轮；定位权限收窄为粗略定位，协议页同步“粗略定位”口径。正式 `check-launch-readiness.ps1 -ReleaseGate` 会让后台监控动作纳入人工上线项和 launch readiness，日常巡检仍低噪音。本轮不改三份保护提示词、官网首页文案，不新增模型输出过滤或 `max_tokens`，不发布正式 Android 包。

- 多代理全盘挑刺后继续收口用户体验、后端频控和低成本监控：Android 已上传图片的失败用户消息点“重发”会重新挂 WorkManager 后台兜底，服务端已完成但 snapshot 未恢复时继续保留“已发送 · 正在同步回复 / 刷新”语义，不再误报“发送失败”诱导重复发送；WorkManager 明确 `server_failure / retry_exhausted` 终态失败会更早露出失败入口。短信发送拆成 IP 防刷桶和手机号发送桶，手机号桶不再被 IP 拆开，供应商未确认发出、验证码缓存失败或内部生成失败会退回手机号桶；短信登录按手机号桶限制验证码尝试。礼品卡空码和今日农情未 ready / 锚点未归档 / session 过期等正常失败不再烧用户频控；今日农情主界面展示副本剥离人工发布后台字段，只给 App 公开卡片字段。ECS 公网黑盒 timer 的 `/healthz` 判据从固定 `ok=true` 收紧到 `auth_strict / bailian / sms / redis / oss / dev_order_endpoints` 等生产关键字段，并把 timer 自身 enabled / active / service success / 最近运行年龄纳入 readiness 和资源巡检；最近运行年龄会从 `ExecMainExitTimestamp`、`InactiveEnterTimestamp`、`LastTriggerUSec` 多字段兜底计算，无法证明 30 分钟内成功则失败，避免单字段缺失或 timer 长期不跑假绿；后台监控文案同步补“公网黑盒探测”。验证通过 `go test ./...`、Android `ChatTimelineItemsTest`、后台 build、admin surface、Android parity、公网黑盒、ECS readiness 和资源容量巡检；已通过 Cloud Assistant 更新并手动运行线上黑盒 timer。本轮不修改三份保护提示词、官网首页文案，不新增模型输出过滤、关键词拦截、字数硬卡或 `max_tokens`，不发布正式 Android 包。

- 继续按“正式上线前收口、少人工盯后台、用户体验优先”落地低风险修复：Android 带图后台发送在服务端已完成但前台 10 分钟仍拉不回远端 AI 回复时，会把尾部从“已发送 · 正在同步回复”转成失败 / 可重发，并新增“刷新”入口继续拉 snapshot，避免状态挂 24 小时；正常成功仍自动回填，不多扣次数。后端记忆摘要新增低频后台 drain worker，默认每 10 分钟小批量扫描仍 pending 的记忆 job 继续提取，失败不丢冻结轮次、不灌主聊天上下文、不等下一个 6/9 轮才补，成功后仍按原子写入弹出一个 job。帮助反馈 `client_msg_id` 幂等增加正文 / 图片一致性校验，重用同一发送 ID 但 payload 不一致会返回冲突；礼品卡兑换先解析 JSON / 归一卡码再消耗兑换频控，坏请求不白烧用户频控。后台上线检查不再返回前端类型不支持的 `partial` launch 状态；SLS 仓库期望扩为 9 条并新增公网黑盒失败告警，配合 ECS `nongji-public-blackbox.timer` 每 5 分钟探测 API / 官网 / 后台 HTTPS，失败只写脱敏 `blackbox_probe_failed` 日志并走现有 SLS 邮件告警。提交 `08a0259b` 已部署到生产后端 active slot `3001`，ECS readiness、公网黑盒、SLS 9 条告警严格巡检均 ready；上线门禁日常模式无失败，attention 仍只剩支付占位、短信套餐余额 / 费用确认、App 公安备案、AccessKey 轮换、最终真机回归、首封 SLS 邮件和最终 release 物料等人工项。本轮不修改三份保护提示词、官网首页文案，不新增模型输出过滤、关键词拦截、字数硬卡或 `max_tokens`，不发布正式 Android 包。
...
- 按用户“把清理脚本删除，需要清理时再叫 Codex”的口径，曾删除 `scripts/clean-local-android-apks.ps1` 和 `scripts/clean-oss-test-apks.ps1`，并把当时 `scripts/publish-android-test-apk.ps1` 的发布后自动清 OSS 旧测试包、清 ECS `/test-apks/` 旧镜像、写 ECS cron 清理文件逻辑全部撤掉；当时测试包发布只负责生成 / 上传 debug/internal 包、输出并探测 `download.nongjiqiancha.cn` 限时签名链接，清理状态输出为 `test_apk_cleanup=manual_only`。该历史口径先被 2026-06-20 的 `test_apk_cleanup=oss_lifecycle_3d` 取代，又按用户“测试包服务器只留一个”收敛为 `test_apk_cleanup=keep_latest_only`：发布脚本在新包下载探针通过后清理同前缀旧内部测试 APK，只保留最新 1 个，OSS `test-apks/` 3 天生命周期作为兜底；仓库仍不保留单独清理脚本，也不写 ECS 清理 cron。已通过 Cloud Assistant 删除 ECS 旧 `/etc/cron.d/nongjiqiancha-test-apks-clean`，返回 `ecs_test_apk_cleanup_cron=removed`。该轮不生成正式 release APK、不配置检查更新、不改官网正式下载、不改三份保护提示词、不新增模型输出过滤。
...
- 继续按代理 / 管理层试用前的普通用户视角收口 Android 可见细节：设置首页“服务协议”入口改为“隐私与协议”，让隐私政策、第三方信息共享清单、个人信息收集清单和应用权限入口更直观；账号管理页手机号行改为静态展示，不再带点击感和右箭头，避免被误判为失效换绑入口；会员中心规则说明补充“升级补偿和加油包未用完不随会员到期清零”。debug-only 预览面板同步更新设置入口、协议目录和会员规则说明；本轮不修改三份提示词、官网首页文案、真实支付、主聊天滚动主方案或今日农情三轮临时上下文规则。
