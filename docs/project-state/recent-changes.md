...
- Android 主聊天 Markdown 展示层补“孤冒号”归一：AI 已完成段落、标题、列表项和对应复制文本如果只以 `：` / `:` 结尾且后面没有正文，会显示为句号，避免“影响明显：”这类引出句看起来像冒号后空内容；冒号后已有正文的标签句不改，当前 streaming 最后一行也先不改，避免后续继续吐正文时标点闪动。debug-only 预览面板“AI Markdown 兜底”、`ChatStreamingRendererTest` 和 Android parity 已同步锁住该口径。
