...
- Android 主界面中部浮层提示做未发布小修：`GlobalStatusHint` 不再使用 `AnimatedVisibility` 默认尺寸收缩动画，改为淡入淡出加轻微中心缩放，避免提示消失最后一小段圆角形状被挤压得不规则。debug-only 预览面板已有“主界面中部浮层”分组，本轮无需新增样例；该改动只影响 Android 展示层，不改提示文案、主聊天逻辑、后端、官网、检查更新或正式 APK，等待下次用户明确发布新版时一起带出。
