...
- 后台管理页继续按“一个主账号、简单好用”补易用操作：用户列表的反馈列直接提供“看反馈”跳转，用户详情的最近反馈区提供“打开反馈会话”，账号ID和手机号支持一键复制，完整手机号在详情里提供拨打链接；用户筛选和帮助反馈筛选增加“清空”，复制成功改为底部轻提示，不再弹窗打断页面。手机端跳到用户详情 / 反馈详情时保留顶部留白，减少被顶栏遮住；帮助反馈 runbook 同步删除“会话列表默认只展示脱敏号”的旧口径。该改动只改后台前端和文档，不改 Android、官网、检查更新、正式包或三份保护提示词。
