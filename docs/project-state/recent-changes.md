...
- 针对用户真机发现“清数据 / 删除历史后无网发图片失败，随后再发消息又从底部工作线出来”的边角问题，拉 3 路代理复查 clean-state 稀疏首屏。两路工程审查确认根因不是滚动主链漏锚，而是稀疏态把失败图片用户消息当成普通历史，同时 `canScrollForward` 会被 UI-only `SparseBottomSpacer` 自己误触发，导致稀疏态提前退出；一路产品审查建议若仍复杂可整条撤掉首发靠上。当前先保留首发靠上但按最小改动收窄规则：只有出现成功落地的非失败 assistant 回复后才算已有普通历史，本地失败用户消息、图片上传 pending / 失败、assistant 失败态和重试态仍可继续处于稀疏首屏；稀疏退出只认真实内容底边到达 96dp 工作线且 spacer 已缩回容差内，不再用 `canScrollForward`；spacer bottom reserve 只吃折叠输入框稳定高度，避免发送前图片预览展开高度把 spacer 算小。继续不恢复 `Arrangement.Top`、反向列表、active-zone、overlay、raw delta 或 contentPadding 方案。
