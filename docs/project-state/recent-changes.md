...
- 按用户确认合并主对话锚点 C(2)/C(3)：会员和 APP 使用问题统一成一条边界，不再让模型自行理解“通用产品规则 / 已知入口”，而是明确只回答本 APP 已知固定入口和公开规则：会员中心、帮助与反馈、检查更新、服务协议、Plus/Pro 每日问答次数和加油包；涉及个人账户、订单、扣费、权益异常、持续故障或个人数据时仍不编造账户信息。礼品卡继续不写入锚点，B 层和 C 层提示词不变。
