...
- `ChatScreen.kt` 继续对 Overlay 残留的“擦边重影”补了最小视觉拦截：当前不仅保留 `onSizeChanged -> pending delta -> SideEffect + dispatchRawDelta(...)` 这条真实列表让位链，还在 `SubcomposeLayout` 的 `layout {}` 里根据同一份尚未被列表结算掉的 pending delta，对 `LazyColumn` 和欢迎语同帧临时做 `placeRelative(0, -offset)` 视觉上抬。这样 Overlay 长高的那一帧，底层列表先在画面上被一起提起来；等 `dispatchRawDelta(...)` 真正把列表状态吃掉后，这层视觉 offset 自然归零，不再双吃 delta。同时保留“`dispatchRawDelta(...)` 未完全消费的剩余 delta 继续保留到下一拍”的机制；`pending finalize` 期间则显式关闭 Overlay follow 与这层视觉 offset，避免收尾时自己再推自己一下。`./gradlew.bat :app:compileDebugKotlin` 已通过。
