...
- 后台管理页继续做手机适配和傻瓜化操作并已部署到 `admin.nongjiqiancha.cn`：导航改成“首页 · 今天先看 / 数据总览”，监控页“先处理事项”前移，顶部补账号ID / 手机号全局搜索、截图模式和手机常用入口；帮助反馈详情补返回队列、下一条待回复、打开用户详情和常用回复模板；App 日志补常用筛选和清空筛选，审计筛选也可清空；多张宽表在 720px 以下转字段卡片，按钮 / 输入框 / 链接触摸面积加大，详情跳转和返回在手机上会自动滚到对应区域。截图模式只是前端临时遮罩完整手机号、完整礼品卡码、反馈正文、问诊全文和图片，不改变后端权限或审计。后台静态包 SHA-256 为 `6e80e7bc07f2777086d9bb2dd3add44fce941beac8d7ead39b97338e2454cdea`；`npm run build`、`check-admin-surface.mjs`、`git diff --check`、手机卡片字段名扫描、部署脚本 HTTPS / JS / 未登录 401 验证、公网黑盒和登录后只读 smoke 均通过。监控行动脚本仍因最近 24 小时检查更新 / 安装权限类 App 日志报失败，按 App 更新回归提醒处理，不是后台静态页部署失败。
