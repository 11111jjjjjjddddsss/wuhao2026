...
- `ChatStreamingRenderer.kt` 把 283f118 那层“整组结果延迟放行”门闩收成了单一的“隐藏行高度预留”方案：当换行首拍测出新的 `stableLines + activeLine` 时，渲染层继续保留上一拍旧可见行，但会先预留隐藏新行对应的真实高度，让 `onStreamingContentBoundsChanged` / follow 仍能及时感知到高度变化；等后续再次观察到 activeLine 真实继续吐字时，再放行真实新行文字。目的不是再加第二层显示补丁，而是把“先露字再被 follow 推回去”的现象收成“先保留旧行、只让高度先变”，继续只改 renderer 单文件，不新增第二条滚动链
...
- `ChatStreamingRenderer.kt` 当天较早还试过一版“整组结果延迟放行”门闩，只保留旧可见行、延后放行新 `stableLines + activeLine`；这一版随后已经被上面的“隐藏行高度预留”单方案收掉，不再作为独立运行时真相保留。后续如果继续排这条问题，应以“旧可见行 + 隐藏行预留高度”的当前方案为准，不要再把它理解成两层门闩并存
