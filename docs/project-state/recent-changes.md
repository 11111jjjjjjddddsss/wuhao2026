...
- 修正设置抽屉“今日农情”历史页的时间分组感：有历史卡片时不再把多天简报塞进同一个大灰框里连续显示，改为每个日期一组，日期行带浅分割线，下面是当天独立白底内容块，块内保留“今日农情”和正文之间的轻分割线；加载、失败、空态仍沿用原灰底稳定容器。debug-only 预览面板同步更新“农情历史页”说明。

- 按用户要求给“删除历史对话”确认卡补一条简短记忆提示：现在会明确说明会删除历史对话并清除用于承接对话的记忆，同时强调会员、礼品卡和反馈记录不受影响；debug-only 预览面板同步更新该项说明。该改动只改用户可见文案，不改变 `/api/session/clear` 的真实删除范围。
