...
- 后端独立“【诊断约束】”第一段按用户最新拍板更新为“所有回答禁止表格。排版尽量简洁，多用短段落和适当空行，少用多级列表、密集项目符号和长段堆叠。”；其余短版诊断边界保持不变：以本轮图片和客观信息为准，历史 / 天气 / 记忆只作风险背景，带图先描述关键信息，证据不足说“不确定”，不把单张图片或单轮描述写成确诊，候选判断和处理建议写支持点 / 反证 / 验证方法 / 适用前提 / 主要风险。该改动只调整每轮主聊天独立 system 约束，不改主对话锚点、今日农情提示词、记忆文档提示词，不新增内容过滤、关键词拦截、表格硬拦截、字数硬卡、`max_tokens` 或 Android 渲染逻辑。
