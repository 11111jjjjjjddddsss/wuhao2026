...
- 后台用户详情补完整问诊查看和返回列表：`/admin-api/v1/users/detail` 在现有后台角色权限下为最近问诊返回完整 `user_text / assistant_text`，用户详情页最多展示最近 12 轮并支持展开查看完整用户问题和 AI 回复；用户管理页内点“详情”不再覆盖当前查询筛选，详情卡增加“返回列表”。该改动只改后台和 admin API，不影响 Android App、不新增聊天正文导出，也不把完整聊天放进总览、产品洞察或日志。
