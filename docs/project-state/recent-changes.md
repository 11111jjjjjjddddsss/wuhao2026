...
- 今日农情线上 v53 探针 `runs=2` 得到 `ok_count=2/2`，每次 3 条可展示 item，未返回 reasoning tokens，单次 total tokens 约 7.1k-7.3k；来源名能写出，兼容 Chat 链路结构化 `source_count=0` 仍属预期。样本质量总体能跑通，但第一组摘要只有 76-79 个中文字符，偏薄。已把今日农情提示词小幅升到 `2026-06-11-v54`：仍不加后端字数过滤、不按内容拦截，只在提示词里要求摘要 90-130 字左右、尽量不低于 85 字，材料足够时补清楚进度、影响范围、农时窗口、风险点、供应变化或农资 / 流通影响，不能用套话凑字。
...
- 记忆文档和今日农情提示词继续按“不要坑用户、不要过度压模型、适合普通群众”微调：今日农情版本先升到 `2026-06-11-v53`，继续固定 `qwen3.5-plus + OpenAI兼容 chat/completions + turbo + forced_search=true + enable_thinking=false`，后端仍不做内容关键词过滤，只保留 JSON / 至少 2 条可展示 item / 内网 URL 安全兜底；提示词把“普通人要知道新闻和种植生产、农资、农时、防灾或流通有什么关系”写明，继续强调“大类边界清楚，小类不要抠太细”，摘要目标从 80-110 字放宽到 90-130 字，避免新闻卡片太薄。记忆文档提示词把“下一轮主对话”改成“下一轮回答”，普通账号、验证码、费用、页面和功能体验问题可以短期承接，但必须写成人能看懂的话，不写成内部配置、接口、模型、排障清单或开发任务；用户自己提到模型名或输出要求不等于泄露，只要不由系统主动吐露即可。
...
- 今日农情和记忆文档提示词按“不要坑用户、不要过度压模型、适合普通群众”再次收口并部署到 ECS：今日农情版本升到 `2026-06-11-v52`，继续固定 `qwen3.5-plus + OpenAI兼容 chat/completions + turbo + forced_search=true + enable_thinking=false`，后端不做内容关键词过滤，只保留 JSON / 至少 2 条可展示 item / 内网 URL 安全兜底；提示词改为按材料主体判断种植侧与养殖水产，不按单个词机械判断，小类只给天气、农时、价格、政策、补贴、平台、技术推广等方向，不设硬配额，也不为了凑类别牺牲真实新材料。生产探针 `runs=3` 得到 `ok_count=3/3`，每次 3 条、无 reasoning tokens，单次约 6.3k-6.6k tokens，样本未见链接下发、代码块、养殖水产主体或明显广告软文。记忆文档提示词改成普通用户口径：账号登录、验证码、会员权益、支付订单、资料填写、页面设置、功能使用等临时事务只放短期承接，不写成长画像；农事参数继续按原话原单位保留，禁止自行换算、合计或括号估算。生产不写库样本复测确认：登录问题未写成长画像，辣椒卷叶第三方转述未确诊，`一桶水30斤兑芸苔素10毫升，打两桶` 未再被改成“约60斤药液”，无 reasoning tokens，单次约 1.5k-1.7k tokens。
