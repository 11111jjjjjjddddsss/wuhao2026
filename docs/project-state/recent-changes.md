...
- 新增可复用 ECS 发版脚本 `scripts/deploy-ecs-server.ps1`：本地打包 `server-go`、按 Cloud Assistant `SendFile` 32KB 限制分片、ECS 上校验 SHA-256、跑 `go test ./...`、编译、备份旧二进制、替换、重启、`nginx -t` 和 Host healthz；脚本支持 `-PackageOnly` 只验证打包，不读取或打印生产环境变量。后续仍需补独立回滚脚本 / 回滚步骤。
