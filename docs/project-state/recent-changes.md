...
- 按用户最终确认继续校准今日农情提示词到 `2026-06-15-v77`：不再限制具体作物，也不单独点名排斥某个大田作物；选题以种植方面新闻为主，更优先找有生产价值或技术含量的内容，如栽培管理、植保病虫、种子种苗、农资农机、技术推广、苗情墒情、产地流通 / 价格和政策补贴。天气、气象、防灾或抢收仍可作为其中一个角度，但不要三条都写成天气预报，并要求写清对农事安排、防灾减损或田间管理的影响。该改动只调整今日农情提示词和防回归测试，不增加后端主题词 / 字数 / 来源硬过滤，不改变 Android 今日农情卡片作为普通列表项展示的主链。
