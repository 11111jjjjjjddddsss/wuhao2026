...
- 对比“预热系统”落地时的代码后，确认尾部收口函数本身与当前基线差异不大，真正新增变量是 streaming 中途的 `requestScrollToItem(index, scrollOffset)` 行增长锚定。为避免它在最后一拍和两阶段 finalize 切树撞车，`ChatScreen.kt` 现在新增 terminal drain 保护：远端 `onComplete` 到达，或本地 fake stream 源文本已经喂完、只剩 `streamingRevealBuffer` 按 typewriter 节奏吐尾巴时，暂停新的 `requestScrollToItem` 行锚定请求，先让尾巴自然 drain 完，再进入原有两阶段 finalize。中途 active block 涨行仍保留 `requestScrollToItem` remeasure 锚定；旧 renderer 门闩、clip、bounds 硬等待、32ms time-based hold 都不恢复
