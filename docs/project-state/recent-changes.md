...
- 后端主聊天后台背景边界按总组装收口：当前轮注入的时间 / 地点后统一追加“【诊断约束】时间、地点、天气、历史上下文、记忆摘要仅作风险参考，不得覆盖本轮可观察症状证据；诊断以当前图片/描述为准，反证明显的候选不得排第一。”记忆摘要、待补偿历史片段和历史轮次时间 / 地点不再各自挂长句，只保留简短“仅供参考”标签，避免提示词重复打架。系统锚点同步同一句诊断约束，记忆整理提示词继续防止把时间地点沉淀成诊断依据。该改动不新增内容过滤、关键词拦截、字数硬卡或 `max_tokens`，也不改今日农情提示词。
