...
- 继续收口“工作线以下 64dp breathing gap 被 settled 态误吃成底部 padding”的问题：`ChatScreen.kt` 的 `conversationBottomPaddingPx` 当前已不再在非 streaming 场景无条件叠加 `STREAM_VISIBLE_BOTTOM_GAP`。现在这 64dp 只在 `isStreaming || hasStreamingItem` 时进入 `contentPadding.bottom`，避免首屏历史态和 finalize 完成态仍被一段空白 padding 顶在工作线附近、贴不到 composer 上方真实底部
