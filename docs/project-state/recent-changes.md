...
- 后台管理页按“功能保留、默认简洁”继续降复杂度并已部署到 `admin.nongjiqiancha.cn`：联网参考渐进披露、后台导航层级、手机端少入口和“一次只聚焦一个任务”的成熟设计原则后，默认切到“简单后台”模式；侧栏常驻只保留 `首页 · 今天先看 / 用户管理 / 帮助反馈 / App日志 / 礼品卡`，其它页面进入“更多工具”，当前如果在高级页会自动展开更多区。首页默认只显示当前结论、今天先处理、常用操作和核心决策卡；智能问诊链路、手机实测清单、上线前检查、趋势、地区和功能接入情况折叠到“更多监控和上线检查”。顶栏保留搜索、手机常用入口、简单 / 全部功能切换、截图模式、刷新和退出，手机端隐藏当前页小字、角色、账号名和重复账号安全按钮。真实路由、后台 API、后端权限、CSRF、审计、高风险二次确认、owner 完整手机号可见和低权限脱敏均不变；不改 Android、官网、检查更新、正式包或三份保护提示词。后台静态包 SHA-256 为 `b5e883b5aac34b58969b004df3df69553c3b62da18249214e668f8e49bee36b7`；`npm run build`、`check-admin-surface.mjs`、`git diff --check`、部署脚本 HTTPS / JS / 未登录 401 验证、公网黑盒和登录后只读 smoke 均通过。监控行动脚本仍因最近 24 小时检查更新 / 安装权限类 App 日志报失败，按 App 更新回归提醒处理，不是后台静态页部署失败。
