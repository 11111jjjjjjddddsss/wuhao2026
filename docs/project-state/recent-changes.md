...
- 后端主聊天后台背景边界加硬：当前轮注入的时间 / 地点、历史轮次里的时间 / 地点、记忆摘要和待补偿历史片段都明确标为“低可信承接参考”，只用于理解对话顺序和用户表达，不得直接作为病害诊断、用药用肥、农技指导或产品判断依据；结论必须以本轮用户明确描述、当前图片可见信息和用户确认的当前条件为准。系统锚点和记忆整理提示词同步强调新问题 / 新地块 / 换作物时降低旧农业事件权重，不把旧判断套到新问题上。该改动不新增内容过滤、关键词拦截、字数硬卡或 `max_tokens`，也不改今日农情提示词。
