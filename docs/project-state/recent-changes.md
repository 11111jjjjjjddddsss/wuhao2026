...
- 后端主聊天模型请求策略按图片诊断收口：纯文字继续关闭思考，带普通问诊图片的本轮请求才显式开启思考，并默认限制 `thinking_budget=1024`；后续只允许用 `CHAT_THINKING_MODE=off` 关闭或用 `CHAT_THINKING_BUDGET` 调整预算，不按会员区分，也不让文字问诊因关键词自动开思考。供应商返回的思考内容只在内部字段中出现，农技千查 SSE 仍只转最终正文给 Android，不归档、不展示思考过程，也不向用户、协议、官网或后台公开模型内部名称。上游非 200 错误会额外记录脱敏响应摘要，遮蔽图片 URL、token、密钥等敏感片段，方便继续定位图片问诊 400；该改动不新增内容过滤、关键词拦截、字数硬卡、`max_tokens` 或客户端模型 Key。
