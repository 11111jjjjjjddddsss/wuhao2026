...
- 用户真机反馈清数据后首发消息仍落在大屏手机中上位置，不够靠近顶部。确认原因是稀疏首屏在顶部栏保留高度外又叠加固定 `128dp` 呼吸区，大屏也会被推到偏中上；本轮只把 `CLEAN_STATE_SPARSE_TOP_OFFSET` 收到 `48dp`，让 clean-state 首条内容更接近顶部栏下方，同时不改“未到工作线不拉底、到工作线恢复 AutoFollow / 小球工作线”的主规则。
