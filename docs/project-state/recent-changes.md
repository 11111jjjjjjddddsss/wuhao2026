...
- `ChatScreen.kt` 再次收口主聊天顶部按钮与正文的横向视觉轨道：普通手机宽度下顶部左右按钮外边界从 16dp 轻放到 14dp，小屏从 12dp 放到 10dp；消息正文左右 padding 不再单独使用更大的 18/24dp，而是复用同一套顶部 chrome padding，让顶部两个按钮和正文边界对齐。只改主聊天页横向留白，不改输入框、工作线、滚动状态机、消息气泡最大宽度或模型 / 后端逻辑。
