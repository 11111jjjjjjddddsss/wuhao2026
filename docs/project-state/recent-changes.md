...
- `ChatScreen.kt` 继续收紧 streaming 新行防闪的 release gate：原来的 wrap guard 只记一个目标 `lineCount`，上一版又只要求旧内容底边高于工作线就放行，仍可能在只推进 1px 时把下一批跨行字符写进 Compose tree。现在状态升级为 `StreamingWrapGuardPending`，同时记录目标行数、当次预滚高度和 hold 当拍的旧内容底边；只有观察到旧内容底边真实上移了本次预滚高度（4px 容差）后才允许 commit。若 bounds 不可用或列表没有可用预滚空间，则直接放弃 hold，避免卡死；旧 renderer 门闩、clip、freshTick / lineAdvanceTick 和 reveal 空转参数仍不恢复
