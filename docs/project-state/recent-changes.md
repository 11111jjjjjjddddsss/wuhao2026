...
- 后端主聊天后台背景边界按总组装继续收口：当前轮时间 / 地点仍单独注入并仅供参考；“【本轮诊断约束】本轮图片和用户描述中的可观察症状是诊断主证据；时间、地点、天气、历史上下文、记忆摘要和用户预设病名仅作风险参考，不得覆盖、替代或反推诊断。证据充分时可以给出“倾向判断/优先排查”，证据不足时必须明确“不确定/待排查”；可以按可能性高低排序，但不要使用百分比、分值、几成等看似精确的概率表达，改用“优先排查、倾向、可能性较高/较低”等自然语言。候选结论应说明支持证据、不支持点/反证和下一步验证方法；与本轮症状不吻合或缺少关键证据的候选，不得写成确诊。”会作为独立 system 消息放在历史 / 记忆 / 今日农情之后、当前用户图文之前，以提高本轮注意力。系统锚点已按用户明确确认移除短版重复诊断约束，只保留地点可信度边界；记忆整理提示词继续防止把时间地点沉淀成诊断依据。该改动不新增内容过滤、关键词拦截、字数硬卡或 `max_tokens`，也不改今日农情提示词。
