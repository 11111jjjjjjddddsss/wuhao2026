...
- 生产后端已部署提交 `376c5f60`，`scripts/check-ecs-readiness.ps1 -ExpectedRevision 376c5f60` 通过，线上 healthz 返回 `revision=376c5f60` 且关键依赖 ready；当前最新代码已将每轮主聊天独立“【诊断约束】”第一段从“少用密集项目符号”继续收紧为“不使用多级列表和项目符号列表”。Android 测试包不需要重装即可吃到后端组装词；本轮未发布正式包、未配置检查更新。
