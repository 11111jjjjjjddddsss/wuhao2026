...
- 发送链当前也已随底部活跃区重构收口：`commitSendMessage()` 仍然在同一发送事务里完成输入框收口、`upsertUserMessage(...)`、assistant placeholder、`prepareScrollRuntimeForStreamingStart(...)` 和 `streamingLocation = OVERLAY`，但不再对 assistant placeholder 发 `requestScrollToItem(index, offset)`；现在改为在切到底部活跃区后，复用现有 `scrollToBottom(false)` 给“仅剩历史消息的 `LazyColumn`”补一发新的发送起步回底，让历史列表先压回底部，而当前轮正文/小球由底部活跃区自己贴底承接
...
- 焦点 1：底部统一活跃区下的发送起步与历史尾部空间归属
  ...
  - 当前真实顺序：发送时先即时 `prepareComposerCollapse(...)`、`input.value = TextFieldValue("")`、`clearFocus/hide keyboard`，再 `upsertUserMessage(...)`、`upsertAssistantMessagePlaceholder(...)`、切 `streamingLocation = OVERLAY`，最后补一发 `scrollToBottom(false)` 把“只剩历史消息的列表”压回底部；当前轮 user / assistant 由底部活跃区自己贴底承接
