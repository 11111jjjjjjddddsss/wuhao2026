...
- Android 端 `+` 入口当前已接入首版图片输入：点击后从底部弹出附件卡片，只开放“相机 / 照片”两项；照片入口优先使用 Android 官方 Photo Picker 按剩余槽位请求选择上限，支持的系统会在选图阶段限制到本轮最多 4 张，不支持 / 兜底选择器仍由 App 回来后截断并提示。图片先进入输入框壳体内的缩略图预览，最多 4 张；缩略图可点开预览，带图时 composer 会增加自身高度和文本输入保底空间，不把图片塞进 `BasicTextField` 光标流。发送时客户端读取本地 URI，统一转 JPEG 并压缩到单张 <=1MB 后上传。当前压缩序列为 `1024@Q85 -> 1024@Q80 -> 896@Q80 -> 896@Q70 -> 768@Q70 -> 640@Q60 -> 512@Q60`，极端可解码图片仍超限时继续保持 Q60 等比缩小，直到 <=1MB；全程不裁剪、不拉伸。上传成功后按用户选择顺序把远端 URL 写入 `SessionApi.StreamOptions.images`。用户消息会优先展示本地 `imageUris`，冷启动 / 远端 hydrate 后用 `imageUrls` 兜底显示；聊天区用户消息图片缩略图当前为 112dp，可点开全屏预览。图片预览高度只影响 composer / 消息内容内部，不进入列表 reserve / 96dp 工作线
