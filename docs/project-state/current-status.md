...
- `ChatStreamingRenderer.kt` 当前又把这层显示门闩收紧了一刀：当 `buildStableStreamingLineBuffer(...)` 首次测出“上一行可升格为 stable、下一行开始成为 activeLine”时，渲染层仍先继续保留上一拍的旧可见行；但现在只有等新 `activeLine` 至少形成 2 个可见字符，并且已经跨过后续一次新的 `freshTick`，才放行新的整组 `stableLines + activeLine`。这样不会再像更早的版本那样一有新行就马上见光，也不再使用“预留隐藏高度”的方案，同时继续避免恢复旧 fresh-line preview 的锁空串/预览串方案
...
- streaming 长段落换行时，工作线下面的新一行不要在上一行尚未上推完成前提前冒头；当前门闩已经收紧到“至少 2 个可见字符 + 下一次 freshTick 才放行”，重点观察是否还会残留“下一行一闪一消失”，以及是否出现明显的延迟见字感
