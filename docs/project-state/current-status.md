...
- 焦点 1：发送起步锚点稳定贴工作线后的回归观察
  ...
  - 当前最新验证刀法：发送事务发出 `requestScrollToItem(index, offset)` 前，普通发送会先把 `LazyColumn` 的 `bottomPaddingPx` 锁到“稳定底栏高度 + `STREAM_VISIBLE_BOTTOM_GAP`”这份终态值，重试/不收口发送才退回当前 `conversationBottomPaddingPx` 快照；发送瞬间的小球继续贴工作线，不再单独抬到中部偏上。目的不是重新把两类问题绑死，而是在已经证明“底部空白”和“小球位置”可拆之后，继续维持当前更稳的产品口径
