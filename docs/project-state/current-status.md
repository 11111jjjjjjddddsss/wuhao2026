...
- `bottomContentReservedHeightPx` 当前又增加了一层更窄的 waiting 窗口短路：当 `isStreaming && pendingStreamingFinalizeMessageId == null && streamingMessageContent.isBlank()` 时，列表底部保留高度直接落到 `fallbackReservedHeightPx`，只服务“小球等待首个可见字”这一个极短窗口，继续验证发送瞬间“先踩旧椅子再换新椅子”的竞态是否就是主因
