...
- `ChatScrollCoordinator.kt` 当前也已回到单主人口径：
  ...
  - `UserBrowsing` 期间当前有一条 reverse-list 视口锚点补偿：仅当最新 streaming item 仍是可见锚点 `firstVisibleItemIndex == 0`，且用户已经离开真实底部 `firstVisibleItemScrollOffset > 0` 时，若该 item 的 bounds 高度继续增长，会按增长量做反向 `scrollBy(-growthPx)`，抵消它在反向列表里把历史内容顶走的位移。底部仍继续吐字，不暂停 SSE / buffer / 可见 streaming 渲染
