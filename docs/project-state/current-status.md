...
- Android 左上角汉堡按钮当前已接入首版简洁设置页：使用独立 `HamburgerMenuSheet.kt`，不做头像、不清空聊天、不接多会话历史。页面顶部不再展示 App 名或短 ID，只保留左上角返回按钮和设置入口组；入口包括“会员中心 / 账号管理 / 客服反馈 / 检查更新 / 礼品卡 / 服务协议 / 隐私政策 / 风险提示 / 退出登录”。其中主界面右上角“会员中心”仍打开现有底部面板；设置页内的“会员中心”不再关闭汉堡页或弹底部面板，而是进入右进左出的会员中心子页，复用同一套会员内容和 `/api/me` 数据，短 ID 收进“会员中心”标题行展示，左上角返回和系统返回键都会先回到汉堡菜单页；“礼品卡”放在“检查更新”下方，避免和会员中心挨得太近，可进入右进左出的轻量子页，输入框保留浅色边框，但不展示占位提示或说明文案，前端不限制字符类型、大小写或长度；礼品卡入口图标按设置页统一视觉放大到同一 24dp 图标槽内约 74% 宽的线性礼物盒；当前只给“礼品卡功能后续接入”提示，不调用后端、不改会员权益；礼品卡内容为空时兑换按钮是禁用态，输入完成后支持键盘 Done 触发同一占位提示；“账号管理”现在进入右进左出的轻量子页，先放“手机号 / 退出设备 / 注销账号”三项，手机号显示未绑定，各项只给后续接入轻提示；这些二级子页的左上角返回和系统返回键都会先回到汉堡菜单页，只有菜单首页的返回才关闭汉堡页；其余入口先显示“功能后续接入”或“登录功能后续接入”的轻提示，等待后续真实账号、一键登录、礼品卡后端、协议页、隐私页、客服反馈链路和检查更新接入；当前不调用新后端接口，不修改 `IdManager.resetUserId()`，不影响聊天滚动链
