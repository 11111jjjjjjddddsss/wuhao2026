...
- 最新一刀继续把发送起步共同基准收窄：普通发送且会收口 composer 时，`conversationBottomPaddingLockPx` 已不再吃当前多行输入框量出来的 `stableComposerBottomBarHeightPx`，而是单独吃“启动单行 chrome 行高估算 + safe bottom inset”这份 collapsed 基准，专门压“长文本发送时工作线被算高、小球更靠上”的现象；`stableComposerBottomBarHeightPx` 仍保留给普通运行时几何和 streaming 主链使用，不和发送期 lock 混用
