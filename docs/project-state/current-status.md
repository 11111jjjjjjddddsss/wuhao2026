...
- 买服务器前功能巡检已开始按功能记录到 `docs/runbooks/pre-server-feature-audit.md`；当前已完成会员中心 / 额度体系、Go 后端高并发 / 性能边界、帮助与反馈、礼品卡、检查更新五块。会员/额度当前可作为“占位展示 + 后端真相”继续推进，但真实收费前必须先接账号 token、生产 `AUTH_STRICT=true`、真实支付回调验签和对账；买服务器本身不等于会员可以直接开卖。帮助与反馈当前主链可过早期内测，但公开运营前还要补后台账号 / 权限 / 审计 / 未处理列表，并明确消息和图片保存 / 删除规则。礼品卡当前只是前端占位，不调用后端、不发权益、不假弹真实成功；后续真实接入以 `docs/runbooks/gift-card.md` 为入口。检查更新当前主链是自有服务器 APK 分发，后端支持可选 `APP_ANDROID_APK_SHA256`，Android 下载后会先校验最终 https、文件大小、SHA-256、包名和 `versionCode`，再调起系统安装页
...
- “服务协议”当前已从设置页平铺的“服务协议 / 隐私政策 / 风险提示”三行改为一个二级目录，点设置页“服务协议”后进入目录页，再进入“用户协议 / 隐私政策 / 第三方信息共享清单 / 个人信息收集清单 / 应用权限 / 风险提示”；这些页面都是 Android 本地内置正文，不需要联网加载，左上角返回和系统返回键会先从正文回到服务协议目录，再回到设置菜单。协议主体暂按用户提供的“北京农技千问科技有限公司”和 `465989879@qq.com` 填写。文案已统一面向用户使用“您”，并按会诊结论不在 App 正文里暴露具体模型品牌 / 模型平台名；隐私政策只保留“第三方大模型和云服务”这类必要委托处理告知，正式上线前再按真实接入情况补第三方服务清单。隐私政策和应用权限页按当前真实权限写明：App 只声明网络访问、网络状态、安装更新 APK 相关权限；照片走系统 Photo Picker，拍照走外部相机 + FileProvider 临时 URI，当前不申请定位权限、相机权限、相册 / 存储读写权限、录音、通讯录、短信或通知权限，且当前不做 App 外通知 / 推送。用户协议已从长合同式 19 节压成 10 节，风险提示从 9 条压成 6 条，继续覆盖农业 AI 建议仅供参考、图片局限、药肥农资标签、官方与时效信息、会员 / 加油包 / 礼品卡、帮助与反馈、禁止行为、个人信息处理、第三方 / 系统能力、用户权利、未成年人和紧急情况。后续若公司主体、ICP备案、隐私政策、支付规则、权限清单、第三方服务清单或律师审核意见变化，必须同步改 App 内文案和项目记忆
