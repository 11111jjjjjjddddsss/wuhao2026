...
- streaming 新行防闪当前又加了一层 draw-phase workline clip 保险：只在 active streaming assistant 上，`ChatScreen.kt` 会基于该消息外层宿主的当前 window top 和 `streamingWorklineBottomPx` 计算出本拍可见下边界，再通过 `Modifier.drawWithContent { clipRect(...) }` 把工作线以下的像素裁掉。前馈预滚与 strict follow gate 继续保留；这层 clip 只服务最后那 1 帧左右的视觉保险，不改 selection、bounds 上报、finalize 或静态态渲染
