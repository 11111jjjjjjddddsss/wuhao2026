...
- 基础设施首版采购口径已进入真实落地阶段：当前已在阿里云 `华北2（北京）/ cn-beijing` 创建标准版 SAE 应用 `nongjiqiancha`，AppId `366147d5-3760-4548-bd68-f38debbc5f23`，规格 `0.5 核 / 1GB / 单实例`，自动弹性未开启；当前仍运行 SAE 默认 demo 镜像，尚未部署 `server-go` 真实后端镜像。域名 `nongjiqiancha.cn` 已购买，仍需完成实名认证 / 模板审核、DNS 解析、ICP / App 备案、HTTPS 证书和 SAE 域名绑定后才能作为正式对外入口。数据库首版仍倾向阿里云 RDS MySQL；PolarDB 暂作为后续高并发 / 更高规格升级选项，不再作为个人创业首版默认采购项。当前 RDS / OSS / SLS 尚未购买或接入
