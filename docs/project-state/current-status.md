...
- 最新一刀继续把发送起步共同基准收窄：普通发送且会收口 composer 时，`conversationBottomPaddingLockPx` 已不再吃当前多行输入框量出来的 `stableComposerBottomBarHeightPx`，而是单独吃现有 `STARTUP_BOTTOM_BAR_HEIGHT_ESTIMATE` 这份固定 collapsed 总高度基准，专门压“长文本发送时工作线被算高、小球更靠上”的现象；`stableComposerBottomBarHeightPx` 仍保留给普通运行时几何和 streaming 主链使用，不和发送期 lock 混用
...
- 焦点 1：发送起步锚点稳定贴工作线后的回归观察
  ...
  - 当前最新验证刀法：发送事务发出 `requestScrollToItem(index, offset)` 前，普通发送会先把 `LazyColumn` 的 `bottomPaddingPx` 锁到“固定 collapsed 总高度估算 + `STREAM_VISIBLE_BOTTOM_GAP`”这份终态值，重试/不收口发送才退回当前 `conversationBottomPaddingPx` 快照；同时本次发送真正发出的 `requestScrollToItem(index, offset)` 也会直接复用这份锁值反推工作线，不再让“列表锁定 padding”和“发送起步 offset”各吃一套基准。发送瞬间的小球继续贴工作线，不再单独抬到中部偏上。目的不是重新把两类问题绑死，而是在已经证明“底部空白”和“小球位置”可拆之后，继续维持当前更稳的产品口径
