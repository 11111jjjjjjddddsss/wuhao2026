...
- Android 左上角汉堡按钮当前已接入首版简洁设置页：使用独立 `HamburgerMenuSheet.kt`，不做头像、不清空聊天、不接多会话历史。页面顶部不再展示 App 名或短 ID，只保留左上角返回按钮和设置入口组；入口包括“会员中心 / 账号管理 / 数据管理 / 帮助与反馈 / 检查更新 / 礼品卡 / 服务协议 / 退出登录”。其中主界面右上角“会员中心”仍打开现有底部面板；设置页内的“会员中心”不再关闭汉堡页或弹底部面板，而是进入右进左出的会员中心子页，复用同一套会员内容和 `/api/me` 数据，短 ID 收进“会员中心”标题行展示，左上角返回和系统返回键都会先回到汉堡菜单页；“数据管理”当前是右进左出的轻量页，展示“数据用于优化体验：未开启”、我的数据说明、申请查询个人信息、申请删除个人信息和删除所有历史对话等入口，其中查询 / 删除 / 删除历史仍走帮助与反馈或后续接入提示，不调用后端删除接口、不清空当前聊天 UI、不修改本机身份；“帮助与反馈”现在进入右进左出的站内消息子页，会从后端拉取同一用户的帮助与反馈历史，用户可发送反馈，后台 / 系统新消息会让设置页“帮助与反馈”行右侧显示小红点，进入帮助与反馈页并成功同步后调用已读接口让红点消失；“检查更新”按自有服务器 APK 分发接入 `GET /api/app/update`，无更新提示“已是最新版本”，有新版本弹“发现新版本”卡片，按钮为“稍后 / 立即更新”，立即更新会下载后端返回的 https APK 并调起 Android 系统安装确认，debug-only 预览面板已补“检查更新”卡片预览；“礼品卡”放在“检查更新”下方，主设置页入口只显示“礼品卡”三个字，不再带副标题，可进入右进左出的轻量子页，输入框边框为纯黑，输入框和兑换按钮放在页面中部形成上下两行结构，兑换按钮字体加大，但不展示占位提示或说明文案，前端不限制字符类型、大小写或长度；礼品卡入口图标按设置页统一视觉放大到同一 24dp 图标槽内约 74% 宽的线性礼物盒；当前只给“礼品卡功能后续接入”提示，不调用后端、不改会员权益，也不在没有后端成功结果时假弹“兑换成功”卡片；debug-only 预览面板已提供“礼品卡兑换成功”卡片样式，卡片下方有“确定”，点击后关闭；礼品卡内容为空时兑换按钮是禁用态，输入完成后支持键盘 Done 触发同一占位提示；“账号管理”现在进入右进左出的轻量子页，先放“手机号 / 退出设备 / 注销账号”三项，手机号显示未绑定，各项只给后续接入轻提示；这些二级子页的左上角返回和系统返回键都会先回到汉堡菜单页，只有菜单首页的返回才关闭汉堡页；其余入口先显示“功能后续接入”或“登录功能后续接入”的轻提示，等待后续真实账号、一键登录、礼品卡后端、正式隐私同意弹窗和账号注销等接入；当前不修改 `IdManager.resetUserId()`，不影响聊天滚动链
