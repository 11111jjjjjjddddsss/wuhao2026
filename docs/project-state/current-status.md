...
- Go 后端数据库连接池当前可通过环境变量配置：`MYSQL_MAX_OPEN_CONNS` 默认 10，`MYSQL_MAX_IDLE_CONNS` 默认 10 且不会超过 max open，`MYSQL_CONN_MAX_IDLE_SECONDS` 默认 300，`MYSQL_CONN_MAX_LIFETIME_SECONDS` 默认 1800。买服务器前不做盲目性能调参；首版按 RDS 规格、后端实例数和真实监控再调整。当前高并发巡检结论已沉淀到 `docs/runbooks/pre-server-feature-audit.md`：Go 语言本身不是当前瓶颈，首版单实例可跑早期；多后端实例前必须先解决本机图片存储、迁移抢跑、进程内限流 / 摘要锁等边界
...
- 基础设施首版采购口径已进入真实落地阶段，但后端部署路线已从“SAE 镜像托管优先”转向“优先评估 / 准备 ECS 传统部署”。真实 ECS 尚未购买或部署。此前曾在阿里云 `华北2（北京）/ cn-beijing` 创建标准版 SAE 应用 `nongjiqiancha`，AppId `366147d5-3760-4548-bd68-f38debbc5f23`，规格 `0.5 核 / 1GB / 单实例`，自动弹性未开启；该 SAE 应用只是默认 demo 镜像，未部署 `server-go`，已于 2026-05-24 21:50 左右先停止，并于 2026-05-24 21:51 左右通过阿里云 CLI 删除，删除变更单 `14a360d3-e2b4-4b93-9701-b76dfcc7bfd9` 已提交成功；删除后 `ListApplications` 返回空列表，`TotalSize=0`。当前 VPC 为 `vpc-2zeax2zowza2398b9dzot`，原 SAE 默认交换机为北京可用区 F `vsw-2ze3elcd2iad6n1madi5g`；RDS MySQL 使用同一 VPC 下北京可用区 L 交换机 `nongjiqiancha-rds-beijing-l` / `vsw-2zemsq82lj2kp8za90aky` / `192.168.1.0/24`。RDS MySQL 实例 `rm-2zes3vmj76p85n8g1` 已创建并运行，MySQL 8.0、基础版、1 核 2GB、50GB、内网地址 `rm-2zes3vmj76p85n8g1.mysql.rds.aliyuncs.com:3306`、到期时间 2027-05-24；当前自动备份保留 7 天，默认每周二 / 四 / 六 17:00-18:00 北京时间左右执行；当前白名单仍是默认 `127.0.0.1`，数据库账号 / 库名 / 后端运行环境变量尚未配置。域名 `nongjiqiancha.cn` 已购买，仍需完成实名认证 / 模板审核、DNS 解析、ICP / App 备案、HTTPS 证书和正式后端入口绑定（优先按 ECS + 反向代理规划，若后续回到 SAE 再同步修改）后才能作为正式对外入口。PolarDB 暂作为后续高并发 / 更高规格升级选项，不再作为个人创业首版默认采购项。当前 OSS / SLS 尚未购买或接入
- 后期整体运维协助入口已沉淀到 `docs/runbooks/operations-blueprint.md`：范围覆盖 Android 整体 App、Go 后端 `server-go`、RDS / 日志 / 成本 / 发布回滚、帮助与反馈、礼品卡、会员和未来统一管理后台。当前本机已安装阿里云 CLI，默认 Region 为 `cn-beijing`，可通过 OpenAPI 读取 RDS 等云资源；旧 SAE demo 应用已删除。真实 AccessKey 只允许保存在本机 CLI 配置或云端密钥管理中，不写入仓库、聊天记忆或文档。ECS、RDS、OSS、SLS、真实域名绑定、数据库地址和密钥落地后，必须把可执行入口回填到 deploy / rollback / logs / db / app-update 等 runbook
...
- 买服务器前功能巡检已开始按功能记录到 `docs/runbooks/pre-server-feature-audit.md`；当前已完成会员中心 / 额度体系、Go 后端高并发 / 性能边界、帮助与反馈、礼品卡、检查更新、服务协议 / 隐私政策 / 风险提示、主聊天与图片发送、B/C 记忆与模型调用、今日农情、账号 / 手机号登录与生产鉴权、统一管理后台、支付真实接入十二块。会员/额度当前可作为“占位展示 + 后端真相”继续推进，但真实收费前必须先接账号 token、生产 `AUTH_STRICT=true`、真实支付回调验签和对账；买服务器本身不等于会员可以直接开卖。帮助与反馈当前主链可过早期内测，但公开运营前还要补后台账号 / 权限 / 审计 / 未处理列表，并明确消息和图片保存 / 删除规则。礼品卡当前只是前端占位，不调用后端、不发权益、不假弹真实成功；后续真实接入以 `docs/runbooks/gift-card.md` 为入口。检查更新当前主链是自有服务器 APK 分发，后端支持可选 `APP_ANDROID_APK_SHA256`，Android 下载后会先校验最终 https、文件大小、SHA-256、包名和 `versionCode`，再调起系统安装页。协议与隐私当前口径见 `docs/runbooks/legal-privacy.md`，买服务器和真实服务商落地后必须按实际权限、SDK、数据保存和账号权利入口复核。主聊天与图片发送当前没有发现旧直连模型、旧完成接口、旧 active-zone、旧图片手势或旧上传通道并存；买服务器后重点验证公网 https 图片链、WorkManager 恢复、弱网多图、删除历史活跃流 409，以及首版单实例 / OSS 取舍。B/C 记忆当前没有发现旧摘要接口或 Android 直连摘要模型参与主链，失败会保留 `pending_retry_b/c`，写回按 `round_total` 防旧快照覆盖；多实例前需要补摘要数据库 lease 或明确单实例观察。今日农情当前是后台定时生成 + 用户只读缓存的独立卡片，不进入 A/B/C、不扣问诊次数、不在用户打开 App 时临时调模型；买服务器后需要配置 `DAILY_AGRI_JOB_SECRET`、定时任务、SLS 告警和后台状态页。账号 / 手机号登录当前仍是本机 UUID 内测身份 + 可选签名 token，设置页手机号 / 退出 / 注销仍是占位；公开生产必须补 per-user token、动态登录和本机 `user_id` 迁移，不能把共享静态 `SESSION_API_TOKEN` 打进正式 APK 当登录方案。统一管理后台当前还不存在，真实入口只有少量内部接口和 runbook 蓝图；买服务器后第一版后台应先做客服、用户查询、检查更新、今日农情、审计日志这些最小运营入口。支付当前没有真实渠道、SDK、自动续费、退款或对账；真实接入必须走服务端创建订单、支付渠道回调验签、幂等发权益和对账，不允许打开开发期直改接口当正式收费
