...
- streaming 完成态最后收口现在有一条很窄的免打扰容差：`restoreBottomAnchorIfNeededAfterStreamingStop` 只在 completed bounds 离底超过 `STREAMING_FINALIZE_BOTTOM_TOLERANCE`（当前 2dp）时才补 `scrollToBottom(false)`；静态态首屏贴底 / 回到底部按钮仍继续使用 `STATIC_BOTTOM_POSITION_TOLERANCE = 0.dp`，不把这 2dp 扩张成全局静态容差
- 远端 streaming 完成时，`finishStreaming()` 当前不会再把尚未逐字 reveal 的 `streamingRevealBuffer` 尾巴一次性 flush 到可见正文再进入 finalize；如果 onComplete 到达时 reveal buffer 仍非空，代码会等待现有 reveal job 把尾巴按正常 typewriter 节奏吐完，再走两阶段 finalize。这样最后几字 / 半行仍能经过 `onAdvance` 的 active block pre-measure 与 `requestScrollToItem` 锚定，不绕过新行防闪主链，专门避免“全部文本完成后尾部突然往上弹一下”
...
- streaming 新行防闪当前已撤掉上一版“`dispatchRawDelta + 32ms time-based hold`”的 reveal 提交口 wrap guard，改成“active block pre-measure + `requestScrollToItem` 下一次 remeasure 锚定”这一条唯一主链：`ChatScreen.kt` 的 streaming `onAdvance` 会先用 `measureStreamingActiveBlockLayout(...)` 比较当前 active block 与 `advance.content` 的物理行数；若下一拍会涨行，则用当前可见 streaming item 的 top offset 和实测高度差计算 `requestScrollToItem(index, scrollOffset)`，请求下一次 remeasure 时 item 顶边上移同等高度，然后同拍写入 `streamingMessageContent`。旧的 `streamingWrapGuardTargetLineCount / streamingWrapGuardHoldStartMs`、`dispatchRawDelta` 前馈和 32ms hold 均已删除，避免新老方案并存。原有 bounds -> follow 主链继续保留做后续精修
- 为避免旧 reveal 方案继续误导，`ChatStreamingRenderer.kt` / `ChatScreen.kt` 已把不再接线的 `StreamingRevealMode.Conservative`、`strictLineReveal`、`lineRevealLocked`、`streamingLineAdvanceTick` 这条空转 plumbing 一并移除；当前 streaming 新行问题只再围绕 `onAdvance` 提交口、active block pre-measure、`requestScrollToItem` offset 语义和 bounds refine 这条链排查
...
- 当前唯一仍开放的主要体感问题，仍然是 streaming 长段落换行时“工作线下面下一行提前冒头 / 一闪一消失”；但当前主修法已经切到 `onAdvance` 的 active block pre-measure + `requestScrollToItem` remeasure 锚定，不再继续靠 renderer 叶子门闩、preview lock、整行延迟显示或 draw-phase clip 去藏问题。这条问题后续只再按 active block pre-measure / requestScrollToItem offset / bounds refine 这条链处理，不要重新把整个滚动链翻回未收口状态
- 当前冻结基线：继续保留 `ff4480f` 的 strict follow gate；streaming 新行主修法已改成 `onAdvance` 的 active block pre-measure + `requestScrollToItem(index, scrollOffset)` remeasure 锚定请求，旧叶子显示门闩、preview lock、draw-phase clip、32ms time-based hold 和 reveal 空转参数都不再存在。后续如果继续会诊或回归，就从“requestScrollToItem remeasure anchor + strict follow gate 精修”这条基线出发，不再顺手多撤其他滚动链修复
...
- streaming 长段落换行时，工作线下面的新一行不要在上一行尚未上推完成前提前冒头；重点观察 `onAdvance` 的 `requestScrollToItem` remeasure 锚定后是否还会残留“下一行一闪一消失”，或带来新的上下跳 / 过补偿
- 下个窗口入口：
  - 聊天滚动链默认先视为已收口；新窗口如果继续 Android UI，只盯“下一行提前冒头 / 一闪一消失”这一条，并先检查 `measureStreamingActiveBlockLayout(...)` 的宽度/样式是否和真实 active block 一致
- 会诊时默认直接带上当前冻结基线：保留 `ff4480f`，主修法是 `onAdvance` 的 active block pre-measure + `requestScrollToItem` remeasure 锚定，旧叶子门闩 / clip / reveal 空转参数 / 32ms time-based hold 都已移除；不要把已收住的发送微抖 / 首屏贴底 / finalize 归位重新打散
