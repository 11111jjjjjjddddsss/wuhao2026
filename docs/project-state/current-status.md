...
- Android 端 `+` 入口当前已接入首版图片输入：面板只开放“相机 / 照片”两项，图片先进入输入框缩略图预览，最多 4 张；发送时客户端读取本地 URI、压缩到单张 <=1MB 后上传，按用户选择顺序把远端 URL 写入 `SessionApi.StreamOptions.images`。用户消息会优先展示本地 `imageUris`，冷启动 / 远端 hydrate 后用 `imageUrls` 兜底显示；图片预览高度只影响 composer 内部，不进入列表 reserve / 96dp 工作线
