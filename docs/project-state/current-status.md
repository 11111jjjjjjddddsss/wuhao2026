...
- 买服务器前功能巡检已开始按功能记录到 `docs/runbooks/pre-server-feature-audit.md`；当前已完成会员中心 / 额度体系、Go 后端高并发 / 性能边界、帮助与反馈三块。会员/额度当前可作为“占位展示 + 后端真相”继续推进，但真实收费前必须先接账号 token、生产 `AUTH_STRICT=true`、真实支付回调验签和对账；买服务器本身不等于会员可以直接开卖。帮助与反馈当前主链可过早期内测，但公开运营前还要补后台账号 / 权限 / 审计 / 未处理列表，并明确消息和图片保存 / 删除规则
