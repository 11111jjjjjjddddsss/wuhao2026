...
- 发送链当前也已随底部活跃区重构继续收口：`commitSendMessage()` 仍然在同一发送事务里完成输入框收口、`upsertUserMessage(...)`、assistant placeholder、`prepareScrollRuntimeForStreamingStart(...)`，但不会再在插入消息的同一拍立刻切 `StreamingLocation.OVERLAY`。当前做法是先保持 `LAZY_COLUMN`，让首个 `requestSendStartBottomSnap()` 用整条消息列表完成一次真实 `scrollToBottom(false)`；回底做完后，如果仍在 streaming 且用户未进入 `UserBrowsing`，才切进底部活跃区。这样发送瞬间先稳住历史列表和 waiting 小球，再把当前轮 user / assistant 交给底部活跃区承接
...
- 焦点 1：底部统一活跃区下的发送起步与历史尾部空间归属
  ...
  - 当前真实顺序：发送时先即时 `prepareComposerCollapse(...)`、`input.value = TextFieldValue("")`、`clearFocus/hide keyboard`，再 `upsertUserMessage(...)`、`upsertAssistantMessagePlaceholder(...)`，先以 `LAZY_COLUMN` 口径补一发 `scrollToBottom(false)` 把整条消息列表压到底；这次首发回底结束后，若仍在 streaming 且未进入 `UserBrowsing`，才切 `streamingLocation = OVERLAY`，把当前轮 user / assistant 交给底部活跃区继续贴底承接
  - 当前退场顺序：`finalizeStreamingStop()`、`resetStreamingUiState(...)` 和 `uiRuntimeResetKey` 进入时都会显式把 `streamingLocation` 收回 `LAZY_COLUMN`，并清掉 `anchoredUserMessageId`。这条口径是为了避免上一轮 settled 尾巴继续残留在底部活跃区里，表现成“输入框上方像多了一截白色空白 / 静态底部文本拖不动 / 重进聊天页不贴底”
