...
- 当前已决策下一轮不再继续沿 `dispatchRawDelta` / bounds release / renderer gate / clip / requestScrollToItem 行锚定这些局部补丁路线压残影。根据本地排查和外部评审，streaming 下一行残影、每行上推轻微发抖、完成态尾部轻微抖动共同指向“生成态 assistant 正文在正向 `LazyColumn` item 内动态长高”这一结构根因。下一轮根治方向已落为 [ADR-0002 Bottom-Anchored Streaming Overlay](D:/wuhao/docs/adr/ADR-0002-streaming-overlay-for-active-assistant.md)：用户停留底部观看时，把正在生成的 assistant 正文临时移到工作线上的 Overlay 层，底边固定、向上生长；小球锚点、发送起步、历史列表和输入框 reserve 主链不重写
