...
- Android 左上角汉堡按钮当前已接入首版简洁设置页：使用独立 `HamburgerMenuSheet.kt`，不做头像、不清空聊天、不接多会话历史。页面顶部不再展示 App 名或短 ID，只保留左上角返回按钮和设置入口组；入口包括“会员中心 / 账号管理 / 客服反馈 / 检查更新 / 兑换码 / 服务协议 / 隐私政策 / 风险提示 / 退出登录”。其中“会员中心”复用现有会员底部面板并继续展示短 ID，方便后续支付 / 客服核对；从设置页打开会员中心时会先关闭设置页，会员中心关闭后回到聊天主界面，不再露回汉堡菜单；“兑换码”放在“检查更新”下方，避免和会员中心挨得太近，可进入右进左出的轻量子页，输入英文 / 数字 / 短横线兑换码并自动转大写，当前只给“兑换码功能后续接入”提示，不调用后端、不改会员权益；兑换码为空时兑换按钮是禁用态，输入完成后支持键盘 Done 触发同一占位提示；“账号管理”现在进入右进左出的轻量子页，先放“手机号 / 退出设备 / 注销账号”三项，手机号显示未绑定，各项只给后续接入轻提示；这些二级子页的左上角返回和系统返回键都会先回到汉堡菜单页，只有菜单首页的返回才关闭汉堡页；其余入口先显示“功能后续接入”或“登录功能后续接入”的轻提示，等待后续真实账号、一键登录、兑换码后端、协议页、隐私页、客服反馈链路和检查更新接入；当前不调用新后端接口，不修改 `IdManager.resetUserId()`，不影响聊天滚动链
