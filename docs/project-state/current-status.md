...
- `ChatScrollCoordinator.kt` 当前也已回到单主人口径：
  ...
  - streaming 期间用户拖动优先级高于程序滚动；一旦检测到用户拖动，会先结束程序滚动标记并立即进入 `UserBrowsing`，不再让 `programmaticScroll` 分支吞掉用户手势。进入 `UserBrowsing` 后本轮不再靠工作线 bounds 自动恢复 `AutoFollow`，避免轻微上滑被吸回；恢复只通过用户主动点“回到底部”或下一轮发送起步
  - streaming 中用户主动点开输入框 / IME 时，会立即释放发送起步锚点锁、结束 `sendUiSettling` 并清掉 locked bottom padding，让键盘外部几何接管工作线；IME 外部高度当前直接读 `WindowInsets.ime` 进入列表 bottom reserve，不再通过 composer 实测高度反推。输入框内部文字或图片内容高度仍不允许顶起聊天列表
