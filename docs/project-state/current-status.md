...
- Android 左上角汉堡按钮当前已接入首版简洁设置页：使用独立 `HamburgerMenuSheet.kt`，不做头像、不清空聊天、不接多会话历史。页面顶部不再展示 App 名或短 ID，只保留左上角返回按钮和设置入口组；入口包括“会员中心 / 账号管理 / 客服反馈 / 检查更新 / 礼品卡 / 服务协议 / 隐私政策 / 风险提示 / 退出登录”。其中主界面右上角“会员中心”仍打开现有底部面板；设置页内的“会员中心”不再关闭汉堡页或弹底部面板，而是进入右进左出的会员中心子页，复用同一套会员内容和 `/api/me` 数据，短 ID 收进“会员中心”标题行展示，左上角返回和系统返回键都会先回到汉堡菜单页；“客服反馈”现在进入右进左出的站内消息子页，会从后端拉取同一用户的客服历史，用户可发送反馈，客服 / 系统新消息会让设置页“客服反馈”行右侧显示小红点，进入客服页并成功同步后调用已读接口让红点消失；“礼品卡”放在“检查更新”下方，主设置页入口只显示“礼品卡”三个字，不再带副标题，可进入右进左出的轻量子页，输入框保留浅色边框，输入框和兑换按钮放在页面中部形成上下两行结构，兑换按钮字体加大，但不展示占位提示或说明文案，前端不限制字符类型、大小写或长度；礼品卡入口图标按设置页统一视觉放大到同一 24dp 图标槽内约 74% 宽的线性礼物盒；当前只给“礼品卡功能后续接入”提示，不调用后端、不改会员权益，也不在没有后端成功结果时假弹“兑换成功”卡片；debug-only 预览面板已提供“礼品卡兑换成功”卡片样式，卡片下方有“确定”，点击后关闭；礼品卡内容为空时兑换按钮是禁用态，输入完成后支持键盘 Done 触发同一占位提示；“账号管理”现在进入右进左出的轻量子页，先放“手机号 / 退出设备 / 注销账号”三项，手机号显示未绑定，各项只给后续接入轻提示；这些二级子页的左上角返回和系统返回键都会先回到汉堡菜单页，只有菜单首页的返回才关闭汉堡页；其余入口先显示“功能后续接入”或“登录功能后续接入”的轻提示，等待后续真实账号、一键登录、礼品卡后端、协议页、隐私页和检查更新接入；当前不修改 `IdManager.resetUserId()`，不影响聊天滚动链
- 客服反馈首版以后端 `support_messages` 为真源：用户侧接口包括 `GET /api/support/summary`、`GET /api/support/messages`、`POST /api/support/messages`、`POST /api/support/read`，均走当前用户鉴权；后台 / 运维侧先提供 `SUPPORT_ADMIN_SECRET` 保护的 `GET /internal/support/messages?user_id=...` 和 `POST /internal/support/messages`，用于后续管理后台读取历史和推送客服回复。客服消息支持文字和图片附件，图片继续复用主聊天的相机 / Photo Picker / 私有 JPEG 副本 / 压缩上传链，单次最多 4 张，每张上传前按现有图片规则压到 JPEG 且不超过 1MB；客服页输入区复用主聊天的底部输入框壳、加号、发送按钮、附件面板和图片预览条，并通过 IME padding 避免键盘遮挡输入框；客服页附件面板只复用结构，不显示“建议拍清病斑、异常部位、叶背或果实”等农业问诊提示词，只保留中性的图片数量限制，附件面板可见时系统返回键和汉堡页左上角返回按钮都会优先收起该面板，不再直接退出客服页；客服反馈发送必须连通后端，若未配置后端或发送接口失败，前端提示“发送失败，请检查网络后重试”。当前还没有完整网页管理后台、工单分配、客服坐席登录、站外推送通知或客服未处理队列；首版只保证单用户一条站内会话线、历史可拉取、后台可回复、App 内红点可提示未读
