...
- “服务协议 / 隐私政策 / 风险提示”当前已从占位提示改为 Android 本地内置二级页，点设置页对应入口直接右进左出打开正文，不需要联网加载；协议主体暂按用户提供的“北京农技千问科技有限公司”和 `465989879@qq.com` 填写。隐私政策按当前真实权限写明：App 只声明网络访问、网络状态、安装更新 APK 相关权限；照片走系统 Photo Picker，拍照走外部相机 + FileProvider 临时 URI，当前不申请定位权限、相机权限、相册 / 存储读写权限、录音、通讯录、短信或通知权限，且当前不做 App 外通知 / 推送。三份文案覆盖农业 AI 建议仅供参考、账号与身份、用户内容授权、图片规则、AI 生成内容标识、会员 / 加油包 / 礼品卡、帮助与反馈、禁止行为、知识产权、个人信息处理场景、模型处理、本地缓存、第三方 / 系统能力、用户权利、未成年人、农业操作风险、时效风险和紧急情况。后续若公司主体、ICP备案、隐私政策、支付规则、权限清单或律师审核意见变化，必须同步改 App 内文案和项目记忆
