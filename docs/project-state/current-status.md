...
- 当前唯一仍开放的主要体感问题，仍然是 streaming 长段落换行时“工作线下面下一行提前冒头 / 一闪一消失”；但当前主修法已经切到 `onAdvance` 的 active block pre-measure + `requestScrollToItem` remeasure 锚定，不再继续靠 renderer 叶子门闩、preview lock、整行延迟显示或 draw-phase clip 去藏问题。这条问题后续只再按 active block pre-measure / requestScrollToItem offset / bounds refine 这条链处理；尾部收口则按“源头结束后先 drain reveal buffer、drain 期间不再发新行锚定、drain 完再 finalize”的单一路径处理，不要重新把整个滚动链翻回未收口状态
- 当前冻结基线：继续保留 `ff4480f` 的 strict follow gate；streaming 新行主修法已改成 `onAdvance` 的 active block pre-measure + `requestScrollToItem(index, scrollOffset)` remeasure 锚定请求，但 terminal drain 阶段会暂停新的 `requestScrollToItem` 行锚定，避免最后一拍 pending anchor 撞上两阶段 finalize。旧叶子显示门闩、preview lock、draw-phase clip、32ms time-based hold 和 reveal 空转参数都不再存在。后续如果继续会诊或回归，就从“中途 requestScrollToItem remeasure anchor + terminal drain 禁新锚定 + strict follow gate 精修”这条基线出发，不再顺手多撤其他滚动链修复
...
- streaming 长段落换行时，工作线下面的新一行不要在上一行尚未上推完成前提前冒头；重点观察 `onAdvance` 的 `requestScrollToItem` remeasure 锚定后是否还会残留“下一行一闪一消失”，以及 terminal drain 禁新锚定后生成完成尾部是否恢复到预热系统时期那种稳定收口
- 下个窗口入口：
  - 聊天滚动链默认先视为已收口；新窗口如果继续 Android UI，只盯“下一行提前冒头 / 一闪一消失”这一条，并先检查 `measureStreamingActiveBlockLayout(...)` 的宽度/样式是否和真实 active block 一致
- 会诊时默认直接带上当前冻结基线：保留 `ff4480f`，主修法是 `onAdvance` 的 active block pre-measure + `requestScrollToItem` remeasure 锚定；远端/本地源头结束后只剩本地 reveal buffer 尾巴时暂停新的行锚定，尾巴 drain 完再 finalize。旧叶子门闩 / clip / reveal 空转参数 / 32ms time-based hold 都已移除；不要把已收住的发送微抖 / 首屏贴底 / finalize 归位重新打散
