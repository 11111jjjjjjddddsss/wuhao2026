...
- `ChatScrollCoordinator.kt` 当前也已回到单主人口径：
  ...
  - streaming 用户接管后，若仍停留在最新 assistant item 内，`ChatScreen.kt` 会冻结该 active streaming item 的测量高度，防止 index `0` item 继续长高时把小幅上滑重新锚回工作线；这不恢复旧 `scrollBy(...)` / `dispatchRawDelta(...)` 高度追滚
