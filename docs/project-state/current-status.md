...
- `ChatStreamingRenderer.kt` 当前又把 streaming 行级门闩收成了“隐藏行高度预留”方案：当 `buildStableStreamingLineBuffer(...)` 首次测出“上一行可升格为 stable、下一行开始成为 activeLine”时，渲染层会继续保留上一拍旧可见行，但同时先预留隐藏新行对应的真实高度；只有后续再次观察到 activeLine 真实继续吐字，才放行新的整组 `stableLines + activeLine`。这样 follow 仍能及时感知到高度变化，但工作线下面不会先露出真实新行文字，也不恢复旧 fresh-line preview 的锁空串/预览串方案
...
- streaming 长段落换行时，工作线下面的新一行不要在上一行尚未上推完成前提前冒头；当前门闩已经改成“先预留隐藏行高度、后放行真实新行”，重点观察是否还会残留“下一行一闪一消失”或短暂的空白占位感
