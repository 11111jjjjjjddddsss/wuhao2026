...
- 回到底部按钮当前改为用户滚动触发，不再用消息 bounds / 工作线 `atBottom` 判断按钮资格。静态态离底资格看反向列表真实位置：`firstVisibleItemIndex != 0 || firstVisibleItemScrollOffset > 0`；streaming 态用户真实拖动后进入 `UserBrowsing` 即可触发按钮 pulse，因为生成中反向列表的 `index/offset` 可能还没及时变化但用户已经表达离底意图。开机 / 程序贴底 / bounds 初次上报不会点亮按钮；继续滚动会续亮，停止滚动后自动隐藏；点击按钮直接 `scrollToItem(0)` 回到反向列表真实底部并恢复对应滚动模式
