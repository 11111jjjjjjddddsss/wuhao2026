...
- `ChatScrollCoordinator.kt` 当前也已回到单主人口径：
  ...
  - streaming 期间用户拖动优先级高于程序滚动；一旦检测到用户拖动，会先结束程序滚动标记并立即进入 `UserBrowsing`，不再让 `programmaticScroll` 分支吞掉用户手势。`scrollToBottom(...)` / `alignVisibleChatListBottom(...)` 这类程序对齐循环也会逐帧检查用户是否已接管，接管后立即停止。只有列表停稳且最新内容底边进入 4dp 级工作线命中带后，才允许恢复 `AutoFollow`
  - streaming 中用户主动点开输入框 / IME 时，会释放发送起步锚点锁并允许键盘外部几何接管工作线；正文跟随键盘上抬依赖 reverse-list + `contentPadding.bottom` 的同一主人自然重排，不再额外发起 `alignVisibleChatListBottom(...)` 主动滚动，避免 IME / 手势边界抢手。输入框内部文字或图片内容高度仍不允许顶起聊天列表
