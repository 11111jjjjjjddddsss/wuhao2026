...
- streaming 新行防闪当前已继续从“单纯前馈预滚”收口到“reveal 提交口 wrap guard + 前馈预滚”这一条唯一主链：`ChatScreen.kt` 的 streaming `onAdvance` 现在不再无条件把 `advance.content` 整批写进 `streamingMessageContent`。若 `measureStreamingActiveBlockLayout(...)` 检测到这批字符会让 active block 物理涨行，就先按实测高度差 `dispatchRawDelta(deltaPx)`，并把这次目标行数记进 `streamingWrapGuardTargetLineCount`，当前拍暂不消费该 batch；后续不再按“同一目标 lineCount 再来一次”直接放行，而是要先观察到当前已渲染内容底边真实高于工作线（`currentStreamingContentBottomPx() < currentStreamingLegalBottomPx()`）后才允许 commit，避免 wrap guard 只挡一拍就提前放行。若当前列表根本没有可用的向上预滚空间（`LazyListState.canScrollBackward == false`），则直接走原路径，不让 guard 卡死。原有 bounds -> follow 主链继续保留做后续精修
- 当前已决策下一轮不再继续沿 `dispatchRawDelta` / bounds release / renderer gate / clip / requestScrollToItem 行锚定这些局部补丁路线压残影。根据本地排查和外部评审，streaming 下一行残影、每行上推轻微发抖、完成态尾部轻微抖动共同指向“生成态 assistant 正文在正向 `LazyColumn` item 内动态长高”这一结构根因。下一轮根治方向已落为 [ADR-0002 Bottom-Anchored Streaming Overlay](D:/wuhao/docs/adr/ADR-0002-streaming-overlay-for-active-assistant.md)：用户停留底部观看时，把正在生成的 assistant 正文临时移到工作线上的 Overlay 层，底边固定、向上生长；用户上滑时可交回 `LazyColumn` 避免遮挡历史，但只要用户回到底部且仍在 streaming，就必须恢复 Overlay。小球锚点、发送起步、历史列表和输入框 reserve 主链不重写
