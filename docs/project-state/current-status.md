...
- 当前所有真实模型调用统一显式设置 `temperature=0.8`：主对话 `qwen3.5-plus`、B 层短期记忆 / C 层长期农业记忆 `qwen3.5-flash` 和今日农情 `qwen3.5-plus` 生成都引用后端同一个 `unifiedModelTemperature` 常量；`top_p / max_tokens / frequency_penalty / presence_penalty` 等其他采样参数暂不显式设置，继续走模型服务默认值。主对话仍是流式、关闭思考、可联网但不强制搜索；B 层短期记忆 / C 层长期农业记忆仍是非流式、关闭思考、不联网；今日农情仍是原生 Generation、关闭思考、强制联网搜索
...
- 主对话锚点真源仍是 `server-go/assets/system_anchor.txt`，当前版本已按“农技千查”产品名收口：强调当前轮优先，历史 / 摘要 / 长期记忆 / 联网只作参考且不是定论；同一作物 / 同一地块可承接仍有效信息，新问题不能直接套旧判断；信息不足必须列 2 到 3 种可能性并追问 1 到 2 个关键问题；图片先客观详细描述再分析判断；混配、浓度、倍数、亩用量、兑水量、面积换算应先核对关键参数，再给出计算或使用建议；会员和 App 使用问题只回答已明确的固定入口和公开规则，会员中心可查看档位 / 剩余次数 / 权益，设置里有“帮助与反馈”“检查更新”“服务协议”，但不编造个人账户、订单、扣费或权益异常信息；商业相关内容不做推广、导购、背书或诋毁，但允许按用户要求做相关信息整理、标签解读、技术分析或中立比较；联网同轮最多一次，证件 / 登记 / 备案 / 审定类只给疑点和查询方法，不做真伪裁决。B 层提示词已改为短期工作记忆口径，C 层提示词已改为用户长期农业记忆口径
- 当前产品策略倾向已记录为待决策：C 层当前已先用 `c_summary` 文本承载“用户长期农业记忆”，后续仍可能升级为结构化 `C+ = 长期摘要 + 用户农业画像 + 用户农业档案`；在结构化代码落地前，当前真实实现仍是现有 `session_ab.c_summary`
