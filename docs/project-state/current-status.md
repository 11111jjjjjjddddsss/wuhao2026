...
- B 层短期记忆 / C 层长期农业记忆当前由后端 `SummaryService` 异步触发 Qwen3.5-Flash，并显式设置 `temperature=0.8`、显式关闭思考模式：B 层仍按 Free / Plus 每 6 轮、Pro 每 9 轮触发并使用当前 A 层 6/9 轮窗口更新；C 层改为每 20 轮触发并读取 `session_round_archive` 最近 20 轮完整问答 + 旧 C 层记忆重写，不再用 A 层 6/9 轮窗口替代 20 轮归档；如果归档不足 20 轮，则保持 `pending_retry_c`，后续轮次完成后继续补提取。同一用户同一层有本进程内运行中保护，避免并发重复抽取；写回时会校验 `session_ab.round_total` 必须仍等于触发时快照轮次，旧快照结果不会覆盖更新轮次。若处理过程中又有新轮次完成，旧结果写入会被跳过，pending 状态保留到后续触发继续处理；模型请求失败、写回失败或归档读取失败会保持对应 `pending_retry_b / pending_retry_c`，后续轮次完成后继续补提取
