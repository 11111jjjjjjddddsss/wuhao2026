...
- Go 后端数据库连接池当前可通过环境变量配置：`MYSQL_MAX_OPEN_CONNS` 默认 10，`MYSQL_MAX_IDLE_CONNS` 默认 10 且不会超过 max open，`MYSQL_CONN_MAX_IDLE_SECONDS` 默认 300，`MYSQL_CONN_MAX_LIFETIME_SECONDS` 默认 1800。买服务器前不做盲目性能调参；首版按 RDS 规格、SAE 实例数和真实监控再调整。当前高并发巡检结论已沉淀到 `docs/runbooks/pre-server-feature-audit.md`：Go 语言本身不是当前瓶颈，首版单实例可跑早期；多 SAE 实例前必须先解决本机图片存储、迁移抢跑、进程内限流 / 摘要锁等边界
...
- 基础设施首版采购口径已进入真实落地阶段：当前已在阿里云 `华北2（北京）/ cn-beijing` 创建标准版 SAE 应用 `nongjiqiancha`，AppId `366147d5-3760-4548-bd68-f38debbc5f23`，规格 `0.5 核 / 1GB / 单实例`，自动弹性未开启；当前仍运行 SAE 默认 demo 镜像，尚未部署 `server-go` 真实后端镜像。当前 VPC 为 `vpc-2zeax2zowza2398b9dzot`，SAE 默认交换机为北京可用区 F `vsw-2ze3elcd2iad6n1madi5g`；RDS MySQL 使用同一 VPC 下北京可用区 L 交换机 `nongjiqiancha-rds-beijing-l` / `vsw-2zemsq82lj2kp8za90aky` / `192.168.1.0/24`。RDS MySQL 实例 `rm-2zes3vmj76p85n8g1` 已创建并运行，MySQL 8.0、基础版、1 核 2GB、50GB、内网地址 `rm-2zes3vmj76p85n8g1.mysql.rds.aliyuncs.com:3306`、到期时间 2027-05-24；当前自动备份保留 7 天，默认每周二 / 四 / 六 17:00-18:00 北京时间左右执行；当前白名单仍是默认 `127.0.0.1`，数据库账号 / 库名 / SAE 环境变量尚未配置。域名 `nongjiqiancha.cn` 已购买，仍需完成实名认证 / 模板审核、DNS 解析、ICP / App 备案、HTTPS 证书和 SAE 域名绑定后才能作为正式对外入口。PolarDB 暂作为后续高并发 / 更高规格升级选项，不再作为个人创业首版默认采购项。当前 OSS / SLS 尚未购买或接入
- 后期整体运维协助入口已沉淀到 `docs/runbooks/operations-blueprint.md`：范围覆盖 Android 整体 App、Go 后端 `server-go`、RDS / 日志 / 成本 / 发布回滚、帮助与反馈、礼品卡、会员和未来统一管理后台。当前本机已安装阿里云 CLI，默认 Region 为 `cn-beijing`，可通过 OpenAPI 读取 SAE 应用；真实 AccessKey 只允许保存在本机 CLI 配置或云端密钥管理中，不写入仓库、聊天记忆或文档。RDS、OSS、SLS、真实域名绑定、数据库地址和密钥落地后，必须把可执行入口回填到 deploy / rollback / logs / db / app-update 等 runbook
- 下一阶段上线推进计划已沉淀到 `docs/runbooks/go-live-plan.md`：当前顺序是先定死 App 名称 / 图标 / 包名 / 签名 / 协议 / 软著材料，买域名和中国内地云资源后立即启动 ICP / App 备案；手机号登录、SAE、RDS、OSS、SLS、帮助与反馈、检查更新、模型 Key 池和真机联调在备案等待期间并行推进；备案通过后补备案号、正式域名 / HTTPS、公安联网备案和应用商店物料；不等手机号登录全部完成才开始备案，也不提前硬做完整管理后台
