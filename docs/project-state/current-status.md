...
- 静态态贴底当前又收紧了一刀：首屏历史贴底、完成态归位和静态回到底部按钮，已经不再和 streaming 共用同一个“16dp 算到底”口径，而是单独走更紧的静态容差；同时 `ChatRecyclerViewHost` 已移除列表尾部那颗额外的 1dp footer spacer，静态态对“最后一条消息底边”的判定也优先改成 `LazyColumn` 里最后一条可见 item 的真实测量底边，不再只盯 assistant / user 的内层 content bounds。这样 item 外层 `CHAT_MESSAGE_ITEM_VERTICAL_PADDING`、assistant / user 失败 footer 都会一起算进底边，继续收“开机进入 / 生成完成后还能再扒出一丁点空白”的剩余体感；streaming 工作线命中带保持原样
