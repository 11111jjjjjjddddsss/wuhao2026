...
- 基础设施首版采购口径已进入真实落地阶段，后端部署路线已从“SAE 镜像托管优先”转向“ECS 传统部署优先”。ECS `i-2ze5nrem0jrchln4f0eh` 已购买并运行，位于 `华北2（北京）/ cn-beijing` 可用区 L，规格 `ecs.u1-c1m2.large`（2 vCPU / 4 GiB），Ubuntu 22.04 64 位，公网 IP `39.106.1.151`，私网 IP `192.168.1.237`，VPC `vpc-2zeax2zowza2398b9dzot`，交换机 `vsw-2zemsq82lj2kp8za90aky`，安全组 `sg-2ze4tilwxw1h5w77lwl1`，固定公网带宽 5 Mbps，到期时间 2027-05-24；当前只是空 Ubuntu 主机，尚未部署 `server-go`。此前曾在阿里云 `华北2（北京）/ cn-beijing` 创建标准版 SAE 应用 `nongjiqiancha`，AppId `366147d5-3760-4548-bd68-f38debbc5f23`，规格 `0.5 核 / 1GB / 单实例`，自动弹性未开启；该 SAE 应用只是默认 demo 镜像，未部署 `server-go`，已于 2026-05-24 21:50 左右先停止，并于 2026-05-24 21:51 左右通过阿里云 CLI 删除，删除变更单 `14a360d3-e2b4-4b93-9701-b76dfcc7bfd9` 已提交成功；删除后 `ListApplications` 返回空列表，`TotalSize=0`。RDS MySQL 实例 `rm-2zes3vmj76p85n8g1` 已创建并运行，MySQL 8.0、基础版、1 核 2GB、50GB、内网地址 `rm-2zes3vmj76p85n8g1.mysql.rds.aliyuncs.com:3306`、同一 VPC / 同一可用区 L 交换机、到期时间 2027-05-24；当前自动备份保留 7 天，默认每周二 / 四 / 六 17:00-18:00 北京时间左右执行；当前白名单仍是默认 `127.0.0.1`，数据库账号 / 库名 / 后端运行环境变量尚未配置。域名 `nongjiqiancha.cn` 已购买，用户口头确认域名实名认证 / 模板审核已通过；仍需完成 DNS 解析、ICP / App 备案、HTTPS 证书和正式后端入口绑定。OSS 标准-本地冗余存储包（华北2）100GB 已购买并生效，资源包实例 `OSSBAG-cn-mqq4sqfvr001`，有效期至 2027-05-24；当前尚未创建 Bucket，后端图片上传链也尚未接 OSS。PolarDB 暂作为后续高并发 / 更高规格升级选项，不再作为个人创业首版默认采购项。SLS / Redis 尚未购买或接入
- 后期整体运维协助入口已沉淀到 `docs/runbooks/operations-blueprint.md`：范围覆盖 Android 整体 App、Go 后端 `server-go`、RDS / 日志 / 成本 / 发布回滚、帮助与反馈、礼品卡、会员和未来统一管理后台。当前本机已安装阿里云 CLI，默认 Region 为 `cn-beijing`，可通过 OpenAPI 读取 ECS、RDS 和 OSS 资源包等云资源；旧 SAE demo 应用已删除。真实 AccessKey 只允许保存在本机 CLI 配置或云端密钥管理中，不写入仓库、聊天记忆或文档。真实后端部署、RDS 账号 / 白名单、OSS Bucket、SLS、真实域名绑定、数据库地址和密钥落地后，必须把可执行入口回填到 deploy / rollback / logs / db / app-update 等 runbook
...
- Android 端右上角黑色线性“圆角方框 + 加号”当前作为会员中心入口并已接入首版会员中心底部面板：打开时读取 `/api/me` 的 `tier / tier_expire_at / daily_remaining / topup_remaining / upgrade_remaining`，并用本地 request epoch 防止重叠刷新时旧慢请求覆盖最新会员 UI 状态。后端 Plus / Pro 会员周期当前为 30 天。面板标题下不再显示“次数、追问记忆和加油包”副文案；顶部只用一条轻量状态条展示今日剩余额度和当前档位，Free 只显示“基础额度”，不显示到期日期；Plus / Pro 显示到期日期；未同步或读取中时只显示“未同步 / 读取中”，不重复显示“未连接”，且套餐开通按钮置灰显示“同步后开通”，不允许在会员真相未同步时启动开通。若存在升级补偿次数或加油包次数，会在“套餐”标题下方同一行以胶囊展示“升级补偿次数 x次 / 加油包 x次”；Free 用户如果是 Plus / Pro 到期后仍剩升级补偿或加油包，也照样展示并可按后端消耗顺序继续使用。套餐区不再单列免费版大卡，直接展示 Plus / Pro 付费套餐；当前同档套餐按钮置灰显示“当前套餐”，Pro 用户看到 Plus 卡片时显示“当前为 Pro”，Plus 用户可点“升级 Pro”，Pro 用户不能降买 Plus。套餐卡片用户可见文案不展示具体 A 层轮数，只展示 Plus 25 次 / Pro 40 次、图文问题、记忆与上下文、复杂问题推理和多作物多地块复盘等产品化权益表达。Plus 升 Pro 规则在底部卡片里展示“升级 Pro 后，Plus 剩余权益会自动折成补偿次数。”；加油包文案明确 `6元/80次`、仅 Plus / Pro 可订购、永久有效、用完再续；若 `/api/me.topup_remaining > 0`，Plus / Pro 按“用完再续”置灰，Free 到期后仍有加油包余额则显示“剩余次数可用”，避免暗示 Free 可直接续订。当前支付功能尚未接入，主界面会员底部面板和设置会员子页里的开通 / 升级 / 订购加油包按钮都只提示“支付暂未接入”，不会调用后端订单接口；后端开发期订单变更接口默认返回 `PAYMENT_NOT_CONFIGURED`，只有显式设置 `ALLOW_DEV_ORDER_ENDPOINTS=true` 且当前环境不是 `APP_ENV / ENV / GO_ENV = prod / production` 时才允许本地 / 内测调试；真实支付成功后的统一收口 UI 仍只预留在主界面会员底部面板，显示中间黑底白字卡片“订购成功 / 确定”，确认后会刷新 `/api/me` 同步会员中心数据，并在刷新到可用额度 / 补偿 / 加油包后清掉当天“次数用完”的本地发送锁。套餐到期暂不做主动提醒，到期后由后端自动按 Free 权益计算
...
- Android 左上角汉堡按钮当前已接入首版简洁设置页：使用独立 `HamburgerMenuSheet.kt`，不做头像、不清空聊天、不接多会话历史。页面顶部展示居中“设置”标题，并保留左上角返回按钮和设置入口组；入口包括“会员中心 / 账号管理 / 帮助与反馈 / 检查更新 / 礼品卡 / 服务协议 / 退出登录”，设置首页入口行、图标、文字、分割线和卡片圆角已调到适中一档：主设置卡片使用白底、浅边框、内缩分割线和右侧轻箭头，首页外侧边距收窄以放大卡片本体，同时保留足够的行高和图标 / 文字重量，避免灰块过重或整体过小。其中主界面右上角“会员中心”仍打开现有底部面板；设置页内的“会员中心”不再关闭汉堡页或弹底部面板，而是进入右进左出的会员中心子页，复用同一套会员内容和 `/api/me` 数据，短 ID 收进“会员中心”标题行展示，左上角返回和系统返回键都会先回到汉堡菜单页；“帮助与反馈”现在进入右进左出的站内消息子页，会从后端拉取同一用户的帮助与反馈历史，用户可发送反馈，后台 / 系统新消息会让设置页“帮助与反馈”行右侧显示小红点，进入帮助与反馈页并成功同步后只把本次已拉取到的最后一条后台 / 系统消息及更早消息标为已读，避免加载和已读之间新到的消息被误清红点；“检查更新”按自有服务器 APK 分发接入 `GET /api/app/update`，无更新提示“已是最新版本”，有新版本弹“发现新版本”卡片，按钮为“稍后 / 立即更新”，立即更新会下载后端返回的 https APK 并调起 Android 系统安装确认，debug-only 预览面板已补“检查更新”卡片预览；“礼品卡”放在“检查更新”下方，主设置页入口只显示“礼品卡”三个字；“服务协议”和帮助与反馈、检查更新、礼品卡合并在同一个设置卡片组里，不再单独占一个卡片；礼品卡不再带副标题，可进入右进左出的轻量子页，输入框边框为纯黑，输入框和兑换按钮放在页面中部形成上下两行结构，兑换按钮字体加大，但不展示占位提示或说明文案，前端不限制字符类型、大小写或长度；礼品卡入口图标按设置页统一视觉放大到同一 24dp 图标槽内约 74% 宽的线性礼物盒；当前只给“礼品卡功能后续接入”提示，不调用后端、不改会员权益，也不在没有后端成功结果时假弹“兑换成功”卡片；debug-only 预览面板已提供“礼品卡兑换成功”卡片样式，卡片下方有“确定”，点击后关闭；礼品卡内容为空时兑换按钮是禁用态，输入完成后支持键盘 Done 触发同一占位提示；“账号管理”现在进入右进左出的轻量子页，放“手机号 / 删除所有历史对话 / 退出设备 / 注销账号”四项，手机号显示未绑定；删除所有历史对话会先弹“取消 / 确定”二次确认，确认后调用 `POST /api/session/clear`，后端删除当前用户 `session_ab` 里的 A 层滑窗、B 层短期记忆、C 层用户长期记忆和 `session_round_archive` 30 天 UI 归档，前端成功后清当前聊天 UI、本地聊天快照、草稿、streaming draft、待发送 WorkManager 任务和私有 composer 图片；该操作不删除会员、额度、加油包、礼品卡、帮助与反馈、`quota_ledger` 或本机 `user_id`，若当前有活跃生成流则提示稍后再删除；退出设备和注销账号只给后续接入轻提示；这些二级子页的左上角返回和系统返回键都会先回到汉堡菜单页，只有菜单首页的返回才关闭汉堡页；其余入口先显示“功能后续接入”或“登录功能后续接入”的轻提示，等待后续真实账号、一键登录、礼品卡后端、正式隐私同意弹窗和账号注销等接入；当前不修改 `IdManager.resetUserId()`，不影响聊天滚动链
- “服务协议”当前已从设置页平铺的“服务协议 / 隐私政策 / 风险提示”三行改为一个二级目录，点设置页“服务协议”后进入目录页，再进入“用户协议 / 隐私政策 / 第三方信息共享清单 / 个人信息收集清单 / 应用权限 / 风险提示”；这些页面都是 Android 本地内置正文，不需要联网加载，左上角返回和系统返回键会先从正文回到服务协议目录，再回到设置菜单。协议主体暂按用户提供的“北京农技千问科技有限公司”和 `465989879@qq.com` 填写。文案已统一面向用户使用“您”，并按会诊结论不在 App 正文里暴露具体模型品牌 / 模型平台名；隐私政策只保留“第三方大模型和云服务”这类必要委托处理告知，正式上线前再按真实接入情况补第三方服务清单。隐私政策和应用权限页按当前真实权限写明：App 只声明网络访问、网络状态、安装更新 APK 相关权限；照片走系统 Photo Picker，拍照走外部相机 + FileProvider 临时 URI，Android Q+ 拍照成功后会把原始照片另存到系统相册 `Pictures/农技千查`，当前不申请定位权限、相机权限、相册 / 存储读写权限、录音、通讯录、短信或通知权限，且当前不做 App 外通知 / 推送。用户协议已从长合同式 19 节压成 10 节，风险提示从 9 条压成 6 条，继续覆盖农业 AI 建议仅供参考、图片局限、药肥农资标签、官方与时效信息、会员 / 加油包 / 礼品卡、帮助与反馈、禁止行为、个人信息处理、第三方 / 系统能力、用户权利、未成年人和紧急情况。后续若公司主体、ICP备案、隐私政策、支付规则、权限清单、第三方服务清单或律师审核意见变化，必须同步改 App 内文案和项目记忆
