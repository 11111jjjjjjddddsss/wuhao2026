...
- 首屏启动门槛当前已拆成两层：`startupListReady` 只负责“列表/欢迎语显示、splash 放行、首屏首次贴底补一发”，依赖 hydration barrier + message viewport 已测量；`startupLayoutReady` 继续只服务 jump button、部分启动辅助几何与 composer 真值相关逻辑，不再把 `composerMeasured` 直接当成首屏显示硬门槛
- 首屏显示与首次贴底当前已经彻底解耦：只要 hydration barrier 通过且 `messages` 非空，历史列表就直接 reveal，不再等待 `initialBottomSnapDone`；首次贴底保留为独立补一发 `scrollToBottom(false)` 的辅助动作，即使首帧没严丝合缝贴到底，也不允许因为等待 strict snap 把整页列表继续挂成 `alpha = 0f`
