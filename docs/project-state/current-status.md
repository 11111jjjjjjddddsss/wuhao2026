...
- streaming 新行防闪当前已从“继续调列表补滚”推进到 [ADR-0002 Bottom-Anchored Streaming Overlay](D:/wuhao/docs/adr/ADR-0002-streaming-overlay-for-active-assistant.md) 第一刀：`ChatScreen.kt` 新增 `StreamingLocation.OVERLAY / LAZY_COLUMN`，普通发送起步仍保留原 `LazyColumn` 小球锚点链；当 `streamingMessageContent` 已有正文且用户停留底部观看时，正文由 `SubcomposeLayout` 同层的 Overlay 承接，底边锚在 composer 上方工作线，`LazyColumn` 内 active assistant item 不再同时渲染同一份 streaming 正文。Overlay 模式下 `onAdvance` 直接提交 content/fresh，不再跑 `resolveStreamingWrapGuardDecision(...)`、`dispatchRawDelta(...)` 或 wrap guard hold；`BindChatListScrollEffects(...)` 也不再按 streaming 正文做 follow delta。用户进入 `UserBrowsing` 时，正文交回 `LazyColumn`，旧 reveal-layer wrap guard + strict follow 只作为 `LAZY_COLUMN` fallback 继续存在；如果用户回到底部且仍在 streaming、没有 message/input selection、没有拖动/fling/程序滚动、发送锚点和 composer settling 都已结束，则恢复 Overlay。当前列表对 Overlay 正文的占位已从“一行假高度”改成同拍测得的 Overlay 实际高度；Overlay 自身继续长高时，也不再通过 `LaunchedEffect + scrollBy` 晚一拍补滚，而是改成在 `onSizeChanged` 里累计真实增量，再由 `SideEffect + LazyListState.dispatchRawDelta(...)` 直接按“本次测得的高度差”把底层列表往上推；如果某一拍只消费了部分 delta，剩余量会继续保留到下一拍，不再丢失。与此同时，`SubcomposeLayout` 的 `layout {}` 现在也会在同一帧先按这份“尚未被真实列表吃掉的 pending delta”对 `LazyColumn` 做临时视觉上抬，等 `dispatchRawDelta(...)` 结算后再自然归零，专门拦截 Overlay 长高与底层列表让位之间那一帧的擦边重影。同时，`pending finalize` 期间会主动清空这条 overlay follow 待消费 delta，并禁止 finalize 收口阶段继续按 Overlay 尺寸变化补滚，避免收尾时自己再推自己一下。Overlay 在 `pending finalize` 期间还会切到和列表 completed 更接近的 `Settled` 渲染口径，并同步打开 disclaimer，专门减少收尾交接那一拍的高度差与轻微尾抖。小球锚点、发送起步、历史列表、输入框 reserve 和复制链主结构不重写
...
- 当前唯一仍开放的主要体感问题，已经从单一“streaming 下一行提前冒头 / 一闪一消失”扩展为三个同源问题：下一行残影、每行上推轻微发抖、完成态尾部轻微抖动。当前判断是局部补丁路线已经到头，下一轮若继续 Android UI，应直接按 [ADR-0002](D:/wuhao/docs/adr/ADR-0002-streaming-overlay-for-active-assistant.md) 做 Bottom-Anchored Streaming Overlay，而不是继续在 wrap guard / bounds release / `dispatchRawDelta` 上调参
- 当前冻结基线：继续保留 `ff4480f` 的 strict follow gate；现有 `onAdvance` reveal-layer wrap guard + active block pre-measure 仍作为 Overlay 未实现前的基线和 `LAZY_COLUMN` 回退态兜底。旧叶子显示门闩、preview lock、draw-phase clip、32ms hold、requestScrollToItem 行锚定和 reveal 空转参数都不再作为候选方向。下个窗口如果开干，先写 Overlay 第一刀实施计划，再下刀
