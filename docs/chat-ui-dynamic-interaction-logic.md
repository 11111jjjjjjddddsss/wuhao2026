...
- 黑卡片位置可以按内容视口做边界夹取，但不要再按顶部白遮罩或底部输入区边界把卡片“吸住”；进入遮罩时应由遮罩自己盖住。
- 顶部白遮罩和底部输入区的层级必须高于黑卡片；黑卡片不能透出遮罩。
...
- 工具卡片锚点跟当前选区手柄/选区中心走，但只能按整个内容视口做夹取；不要再按顶部白遮罩或底部输入区边界吸附。
- 顶部白遮罩和底部输入区层级必须高于黑卡片；卡片进入遮罩区时只能被遮住，不能透出遮罩。
