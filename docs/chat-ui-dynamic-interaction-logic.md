...
- 发送后输入文本要立刻清空；底部高度链只认真实输入区高度，不再额外挂一层发送收口冻结，否则首发阶段容易放大回缩感。
...
- 回到前台后，这个静默贴底只在静态场景执行；执行前先判断当前是否真的已经偏离底部，不能无条件补滚。
- 执行恢复期间需要压住回到底部按钮，避免因为底部差一丢丢就误冒。
...
- 黑卡片挂在聊天页根层，由自定义 `TextToolbar` 驱动；不再以系统白色工具条为主链。
...
- 黑卡片和选区手柄按同一组锚点一起移动；只允许在内容视口内做横向夹取，不再按顶部白遮罩或底部输入区边界把卡片“吸住”。
- 顶部白遮罩和底部输入区的层级必须高于黑卡片；卡片进入遮罩区时只能被遮住，不能被遮罩边界挂住，也不能透出遮罩。
- 首次进入 app 后，在底部消息附近长按进入选区时，不允许出现系统 `bringIntoView` 触发的整页上窜；当前通过 `StaticMessageSelectionBringIntoViewSpec` 强制返回 `0f` 来压住这条系统滚动链。
- 用户消息右对齐 owner 必须是最外层固定 `Row(... Arrangement.End)`，不要再让 `SelectionContainer` 直接决定用户气泡位置。

结论：
- 当前目标是“原位选区 + 自定义黑卡片 + 最小退出规则”，不再叠第二套分流卡片或系统白工具条。
- 当前实现优先保证：选区不乱丢、黑卡片不乱跳、用户消息和 AI 消息同一条链、首次底部长按不触发整页上窜。
...
- final bottom snap 只允许在 `StreamAnchorFollow` 真正接管期间触发；如果结束前已经不在自动跟随态，就不要再补这一脚。
...
- 不再允许切回 `BasicTextField` / plain-text 选区模式；不允许为了复制把渲染好的 Markdown 文本切成原始文本。
- 自定义黑色 `复制 / 全文复制` 工具卡片必须挂在聊天页根层，不挂在 `Scaffold` 内容层，也不挂在消息项内部，避免底部长按时被输入框区域盖住。
- 工具卡片锚点跟当前选区手柄/选区中心走；只允许做横向夹取，不再按顶部白遮罩或底部输入区边界吸附或挂住。
- 顶部白遮罩和底部输入区层级必须高于黑卡片；卡片进入遮罩区时只能被遮住，不能被遮罩挂住，也不能透出遮罩。
- 消息区当前仍然是系统 `SelectionContainer` 手柄 + 自定义黑卡片，不是自绘手柄；系统白色消息工具条也不是主链。
- 消息区手柄当前固定黑色常显，不再因为靠近顶部白遮罩或底部输入框就切透明；优先保证拖选操作性。
- 工具卡片优先挂在当前选区上侧；上侧放不下再切下侧。切换必须带侧别记忆和阈值，不能上下双闪。
- 如果上下两侧都放不下“完整卡片 + 手柄安全间距”，消息卡片不再消失，改为退到当前可见安全区中间兜底。
- 中间兜底只用于极限边界场景；正常场景仍然优先贴近手柄锚点，不把中间兜底当主定位模式。
- 任何时候都优先避免“卡片压到手柄上”；临时居中兜底也比和手柄重叠更安全。
...
- 首次进入 app 后，在底部消息附近长按进入选区时，不允许出现整页上窜；当前靠 `StaticMessageSelectionBringIntoViewSpec` 返回 `0f` 把系统自动 bring-into-view 滚动压掉。
- 消息长按相关改动只允许影响静态消息选择链，不能顺手碰流式生成、滚动 owner、输入框、启动恢复、回到底部按钮。

## 用户消息与 AI 消息当前要求（2026-03-24）

- 用户消息和 AI 消息共用同一套消息复制状态链、退出规则、黑卡片定位规则。
- 用户消息只保留右侧气泡壳与纯文本正文，不复用 assistant 的满宽 Markdown 宿主。
- 用户短文本气泡必须按内容包裹宽度显示；clean-state 下也不能被重新拉满。
## Clean-State 启动基线补充（2026-03-24）

- 清除应用数据后的聊天页启动，不允许依赖旧历史、旧底部视口、旧草稿或旧布局值才正常。
- 聊天页内容 reveal、欢迎占位、回到底部按钮和 splash 放行，都必须等首帧关键布局稳定后再进入正常态。
- 首帧关键布局至少包括：消息视口尺寸、底部输入区整块高度、输入框在视口内的真实顶部位置。
- 远端历史 hydrate 只允许补数据，不允许把聊天页重新打回“启动中”状态。

## 输入框文本选择工具条基线补充（2026-03-24）

- 底部输入框的文本选择菜单使用自定义黑底白字卡片，不再允许系统白色工具条飘到顶部作为主链。
- 输入框继续保留系统选区和系统手柄，只替换菜单层，不自绘输入框手柄。
- 输入框卡片固定贴在输入框上方，和输入框保持小间距，不允许和输入框重叠。
- 输入框卡片必须跟随输入框和键盘位置一起移动；键盘弹起或收起时，不允许卡片滞留在顶部标题区。
- 输入框卡片保留系统文本动作语义：`复制 / 粘贴 / 剪切 / 全选`。
- 输入框卡片退出规则固定为三条：点输入框外退出、按返回键退出、执行动作后按系统预期退出。
- 输入框卡片纵向目标位固定为“输入框上方小间距”，只允许做横向夹取；不再按标题区剩余空间把卡片翻到顶部。

## 复制/剪切链当前验收补充（2026-03-24）

- 消息区长按后，必须先出现系统选区高亮和手柄，再出现自定义黑色 `复制 / 全文复制` 卡片；不允许只出卡片、不出手柄、正文像卡死。
- 输入框长按或全选后，必须出现黑底白字 `复制 / 粘贴 / 剪切 / 全选` 卡片；不允许回退到顶部白色系统工具条。
- 消息区和输入区的选择菜单都必须支持“点外部退出”和“返回键退出”；不能只支持其中一条。
