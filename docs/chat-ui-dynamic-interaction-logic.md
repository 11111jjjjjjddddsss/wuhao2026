...
- 静态消息长按选择现在只保留一条链：`渲染后的消息文本本身可选 + 自定义黑色复制/全选工具条`。
- 不再允许切回 `BasicTextField` / plain-text 选区模式；不允许为了复制或全选把渲染好的 Markdown 文本切成原始文本。
- 自定义黑色 `复制 / 全选` 工具条必须挂在聊天页根层，不挂在 `Scaffold` 内容层，也不挂在消息项内部，避免底部长按时被输入框区域盖住。
- 工具条锚点优先跟长按手指位置走，默认出现在手指正上方附近；实在放不下时再做边界夹取。
- 消息区默认关闭系统白色文本工具条；如果真机上重新冒出白色 `复制 / 全选`，优先视为链路回退或 owner 冲突，不要继续叠第二套菜单。
