...
- 只要用户开始手动滚动列表，当前消息选区必须立刻清掉；不要让 `复制/全选` 菜单跟着列表滚到顶部或输入框附近。
- 用户消息右对齐 owner 必须是最外层固定 `Row(... Arrangement.End)`，不要再让 `SelectionContainer` 直接决定用户气泡位置。

结论：
- 当前目标是“按哪起选尽量贴哪、滚动即取消选区、菜单不乱飞”，不是“首次自动帮用户多选固定 8/10/20 个字”。
- 消息区首次长按自动多选几个字，原生链路下不可精确指定；如果以后还要强做，只能走自定义选区模型，风险明显更高。
