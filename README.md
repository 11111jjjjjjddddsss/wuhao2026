...
- 官网本地开发：`cd site && npm install && npm run dev`
- 官网构建：`cd site && npm run build`
- 乱码扫描：`python3 scripts/check_mojibake.py`
- 项目记忆检查：`python3 scripts/check_project_memory.py`
...
当关键真相文件变更但未同步更新 `AGENTS.md` 或 `docs/project-state/*` 时，CI 会执行 `scripts/check_project_memory.py` 直接报警，防止代码和仓库记忆再次脱节。
