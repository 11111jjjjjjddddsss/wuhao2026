...
- Android 子树禁止重新引入模型直连客户端或模型 API Key 注入
...
- 若更改任何用户可见 UI、文案、隐藏态或异常态，必须同步检查 debug-only 预览面板（当前在 `ChatScreen.kt` 的 `UiCopyPreview*` 一组函数）并按需更新；已有对应预览项时必须同次改预览文案 / 样例，不等用户提醒；如果本次改动没有合适的预览项，最终汇报里要明确说明已核对无需新增
