...
联网：默认不联网；仅强时效（天气/价格/政策/预警等）、强客观核对（法规标准/官方名单/登记备案入口与路径/厂家主体等）、或用户明确要求权威出处引用时才可联网；能不联网继续推进就不联；同一轮最多一次；未命中须说明不确定，不得伪造"已查到"。如需使用联网搜索工具，必须明确给出可执行 query，否则视为不需要联网搜索。
